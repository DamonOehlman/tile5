(function(){function Q(n){n=n.split(",");for(var x=[],y=0;y<n.length;y++){var B=n[y].trim().replace(".","/");x[y]=oa[B]}return x}function La(n,x){for(var y=n.split(","),B=true,K=typeof $LAB!=="undefined"?$LAB:null,Y=0;Y<y.length;Y++){var ga=y[Y].trim().replace(".","/");if(!oa[ga]){B=false;Ma&&La("./plugins/"+pluginFile)}}if(x)if(Ma||B)x.apply(Ba,Q(n));else K&&$LAB.wait(function(){x.apply(Ba,Q(n))});return Ba}function Z(n,x,y){if(n&&n.split){var B=n.split(fb);if(B.length>1){n=B[0];x=B[1]}}else if(n&&
n.lat){x=n.lon;n=n.lat}this.lat=parseFloat(n||0);this.lon=parseFloat(x||0);this.radius=y||gb}function Ua(n){this.positions=[];for(var x=n.length;x--;)this.positions[x]=typeof n[x]=="string"?new Z(n[x]):n[x]}function aa(n,x){if(n&&n.splice){for(var y=x,B=new Z(ra,Ga),K=new Z(hb,$),Y=n.length;Y--;){var ga=typeof n[Y]=="string"?new Z(n[Y]):n[Y];if(ga.lat<B.lat)B.lat=ga.lat;if(ga.lat>K.lat)K.lat=ga.lat;if(ga.lon<B.lon)B.lon=ga.lon;if(ga.lon>K.lon)K.lon=ga.lon}this.min=B;this.max=K;if(typeof y=="undefined"){y=
this.size();y=Math.max(y.x,y.y)*0.3}this.min=new Z(B.lat-y,(B.lon-y)%360);this.max=new Z(K.lat+y,(K.lon+y)%360)}else if(n&&n.min){this.min=new Z(n.min);this.max=new Z(n.max)}else{this.min=n;this.max=x}}function ua(n){if(typeof n=="string"){var x=(n.replace(/\d|\.|\s/g,"")||"m").toLowerCase();n=parseFloat(n)*({km:1E3}[x]||1)}this.meters=n||0}function na(n,x){if(typeof n=="number"){this.days=n||0;this.seconds=x||0}else if(typeof n!="undefined"){this.days=n.days||0;this.seconds=n.seconds||0}}var oa=
...
return false},offset:function(n,x){var y=this.lat*ha,B=y+n/this.radius;y=this.lon*ha+Math.asin(Math.sin(x/this.radius)/Math.cos(y));B=(B+Ha)%Math.PI-Ha;y%=Ca;return new Z(B*va,y*va)},to:function(n,x){if(typeof n=="object")n=this.bearing(n);var y=x/this.radius,B=n*ha,K=this.lat*ha,Y=Math.asin(Math.sin(K)*Math.cos(y)+Math.cos(K)*Math.sin(y)*Math.cos(B));y=this.lon*ha+Math.atan2(Math.sin(B)*Math.sin(y)*Math.cos(K),Math.cos(y)-Math.sin(K)*Math.sin(Y));y=(y+3*Math.PI)%(2*Math.PI)-Math.PI;return new Z(Y*
va,y*va)},toBounds:function(n){var x=n.radians(),y=this.lat*ha,B=this.lon*ha;n=y-x;var K=y+x;if(n>X&&K<$a){y=Math.asin(Math.sin(x)/Math.cos(y));x=B-y;if(x<qb)x+=Ca;B=B+y;if(B>Na)B-=Ca}else{n=Math.max(n,X);K=Math.min(K,$a);x=$;B=Ga}return new aa(new Z(n*va,x*va),new Z(K*va,B*va))},toString:function(n){return this.lat+(n||" ")+this.lon},valid:function(){return!(isNaN(this.lat)||isNaN(this.lon))}};Ua.prototype={constructor:Ua,distance:function(){for(var n=0,x=[],y,B=this.positions.length-1;B--;){y=this.positions[B].distanceTo(this.positions[B+
1]);n+=x[B]=y}return{total:n,segments:x}},traverse:function(n,x){var y=0,B=0;if(!x||!x.segments)x=this.distance();if(n>x.total)return this.positions[this.positions.length-1];else if(n<=0)return this.positions[0];else{for(;B<x.segments.length;){y+=x.segments[B];if(y>n){y-=x.segments[B];break}B++}if(B<this.positions.length-1){var K=this.positions[B];B=K.bearing(this.positions[B+1]);return K.to(B,n-y)}else return this.positions[B]}}};aa.prototype={constructor:aa,bestZoomLevel:function(n,x){var y=this.center(),
B=Va[Math.min(Math.round(Math.abs(y.lat)*0.05),Va.length)],K=this.size();y=Math.ceil(Math.log(Va[3]*x/K.y)/Math.LN2);B=Math.ceil(Math.log(B*n/K.x)/Math.LN2);return Math.min(isNaN(y)?1E3:y,isNaN(B)?1E3:B)},center:function(){var n=this.size();return new Z(this.min.lat+n.y/2,this.min.lon+n.x/2)},expand:function(n){return new aa(new Z(this.min.lat-n,(this.min.lon-n)%360),new Z(this.max.lat+n,(this.max.lon+n)%360))},size:function(n){var x={x:0,y:this.max.lat-this.min.lat};x.x=typeof n!="undefined"&&n&&
this.min.lon>this.max.lon?360-this.min.lon+this.max.lon:this.max.lon-this.min.lon;return x},toString:function(){return"min: "+this.min+", max: "+this.max},union:function(){for(var n=this.min.copy(),x=this.max.copy(),y=arguments.length;y--;)if(arguments[y]){var B=arguments[y].min,K=arguments[y].max;n.lat=Math.min(n.lat,B.lat);n.lon=Math.min(n.lon,B.lon);x.lat=Math.max(x.lat,K.lat);x.lon=Math.max(x.lon,K.lon)}return new aa(n,x)}};ua.prototype={add:function(){for(var n=this.meters,x=arguments.length;x--;){var y=
typeof arguments[x]=="string"?new ua(arguments[x]):arguments[x];n+=y.meters}return new ua(n)},radians:function(n){if(typeof n!="undefined"){this.meters=n*la;return this}else return this.meters/la},toString:function(){if(this.meters>1E3)return(this.meters/10|0)/100+"km";return this.meters+"m"}};na.prototype={add:function(){for(var n=new na(this.days,this.seconds),x=arguments.length;x--;){n.days+=arguments[x].days;n.seconds+=arguments[x].seconds}return n},toString:function(){var n,x,y,B="";if(this.days)B=
this.days+" days ";if(this.seconds){y=this.seconds;if(y>=3600){n=~~(y/3600);y-=n*3600}if(y>=60){x=Math.round(y/60);y-=x*60}if(n)B=B+n+(n>1?" hrs ":" hr ")+(x?(x>10?x:"0"+x)+" min ":"");else if(x)B=B+x+" min";else if(y>0)B=B+(y>10?y:"0"+y)+" sec"}return B}};var Oa=function(){var n=/^P(\d+Y)?(\d+M)?(\d+D)?$/,x=/^(\d+H)?(\d+M)?(\d+S)?$/,y={8601:function(B){B=B.split("T");var K=null;K=null;var Y=0,ga=0;n.lastIndex=-1;K=n.exec(B[0]);Y+=K[3]?parseInt(K[3].slice(0,-1),10):0;x.lastIndex=-1;K=x.exec(B[1]);
ga+=K[1]?parseInt(K[1].slice(0,-1),10)*3600:0;ga+=K[2]?parseInt(K[2].slice(0,-1),10)*60:0;ga+=K[3]?parseInt(K[3].slice(0,-1),10):0;return new na(Y,ga)}};return function(B,K){var Y=y[K];if(!Y)throw"No parser found for the duration format: "+K;return Y(B)}}(),Ba=this.GeoJS={Pos:Z,Line:Ua,BBox:aa,Distance:ua,generalize:function(n,x,y){var B=n.length,K=[],Y=null;y=(y||250)/1E3;for(B=B;B--;)if(B===0)K.unshift(n[B]);else{var ga=!Y||n[B].inArray(x)?y:Y.distanceTo(n[B]);if(n[B]&&ga>=y){K.unshift(n[B]);Y=
n[B]}}return K},Duration:na,parseDuration:Oa,define:function(n,x){oa[n]=x},require:La};if(Ma)module.exports=Ba})();
...
function d(m,p){return{x:m.x-(p?p.left:0),y:m.y-(p?p.top:0)}}function k(m,p){if(m.preventDefault){m.preventDefault();m.stopPropagation()}else if(typeof m.cancelBubble!="undefined")m.cancelBubble=true;p&&m.stopImmediatePropagation&&m.stopImmediatePropagation()}var g=[],h=function(m,p,v){v=Q({binder:null,unbinder:null,observable:null},v);for(var A=v.observable,G=[],O,t,E={bind:function(){return A.bind.apply(null,arguments)},unbind:function(){A.unbind();for(z=0;z<G.length;z++)G[z].unbind();return E}},
z=0;z<p.length;z++)G.push(p[z](m,A,v));A.bind("pointerDown",function(){t=O=0});A.bind("pointerMove",function(o,w,s,r){O+=r.x||0;t+=r.y||0});A.bind("pointerUp",function(o,w,s){Math.max(Math.abs(O),Math.abs(t))<=20&&A.triggerCustom("tap",o,w,s)});return E};i("pointer",{handler:function(m,p,v){function A(I){if(I.pageX&&I.pageY)return f(I.pageX,I.pageY);else{var L=document.documentElement,R=document.body;return f(I.clientX+(L&&L.scrollLeft||R&&R.scrollLeft||0)-(L&&L.clientLeft||R&&R.clientLeft||0),I.clientY+
(L&&L.scrollTop||R&&R.scrollTop||0)-(L&&L.clientTop||R&&R.clientTop||0))}}function G(I){if(l(I,m))if(F=z(I)){var L=A(I);m.style.cursor="move";k(I,true);T=L.x;V=L.y;D=f(T,V);p.triggerCustom("pointerDown",e("mouse",I),D,d(D,j(m)))}}function O(I){var L=A(I);J=L.x;U=L.y;if(l(I,m))w(I,F?"pointerMove":"pointerHover")}function t(I){if(F&&z(I)){F=false;if(l(I,m)){m.style.cursor="default";w(I,"pointerUp")}}}function E(I){if(l(I,m)){var L;I=I||window.event;if(I.detail){if(typeof I.axis=="undefined"||I.axis===
...
ra({});globalOpts=p}p.binder=(p.isIE?b:a)(p.bindTarget||document);p.unbinder=(p.isIE?b:c)(p.bindTarget||document);v=p.types;for(var A=capabilities,G=[],O=g.length;O--;){var t=g[O],E=!v||v.indexOf(t.type)>=0,z=true,o;for(o in t.checks){var w=t.checks[o];z=z&&w===A[o]}if(E&&z)G[G.length]=t.handler}return new h(m,G,p)}}}();var X=this.T5=ra({}),Na=function(){var a={};return{create:function(c,b){if(a[c][b])return a[c][b].apply(null,Array.prototype.slice.call(arguments,2));throw gb(c,b);},get:function(c,
b){return a[c]?a[c][b]:null},register:function(c,b,f){a[c]||(a[c]={});a[c][b]&&La(qb(c,b),"warn");a[c][b]=f}}}(),qb=Z("Registration of {0}: {1} will override existing definition"),gb=Z("Could not create {0} of type: {1}");Z("Could not create drawable of type: {0}");var la=Math.PI*2,fb=Math.PI/2,Oa=Math.PI/180,Ba=180/Math.PI,n=0.08181919084262157,x=Math.abs,y=Math.round,B=Math.min,K=Math.max,Y=Math.pow,ga=Math.sqrt,rb=Math.log,Wa=Math.sin,jb=Math.cos,yb=Math.atan,Pa="undefined",Ia="string",Ja="number",
...
f);var d,k,g,h,m=0,p=0,v={},A=null,G={},O=function(){},t=function(){this.fill&&h.fill();this.stroke&&h.stroke()},E=["fill","stroke","lineWidth","opacity"],z=["fillStyle","strokeStyle","lineWidth","globalAlpha"];W&&j();if(c){d=c.offsetWidth;k=c.offsetHeight;g=W?W.create("canvas",null,{position:"absolute","z-index":1}):new Canvas;g.width=d;g.height=k;a.attachFrame(g,true);h=null}b=Q(i,{applyStyle:function(o){var w=v[o],s=h&&h.canvas?h.canvas.id:"default",r;G[s]||(G[s]=[]);r=G[s].pop()||"reset";if(w){G[s].push(o);
w(h);return r}},applyTransform:function(o){if(o.translateX!==0||o.translateY!==0||o.scaling!==1||o.rotation!==0){h.save();A={undo:function(){h.restore();A=null},x:o.xy.x,y:o.xy.y};h.translate(o.xy.x-m+o.translateX,o.xy.y-p+o.translateY);o.rotation!==0&&h.rotate(o.rotation);o.scaling!==1&&h.scale(o.scaling,o.scaling)}return A},drawTiles:function(o,w,s){for(var r=w.length;r--;){o=w[r];if(!o.loaded&&s)o.load(a.invalidate);else o.image&&h.drawImage(o.image,o.x-m,o.y-p)}},prepArc:function(o,w,s){h.beginPath();
h.arc(o.xy.x-(A?A.x:m),o.xy.y-(A?A.y:p),o.size>>1,o.startAngle,o.endAngle,false);return l(w,s)},prepImage:function(o,w,s,r){var F=(r.x||o.xy.x)-(A?A.x:m),D=(r.y||o.xy.y)-(A?A.y:p),J=r.image||o.image;if(J){h.beginPath();h.rect(F,D,r.width||J.width,r.height||J.height);return l(w,s,function(){h.drawImage(J,F,D,r.width||J.width,r.height||J.height)})}},prepMarker:function(o,w,s){var r=o.xy.x-(A?A.x:m),F=o.xy.y-(A?A.y:p),D=o.size,J=undefined;h.beginPath();switch(o.markerType.toLowerCase()){case "image":J=
O;h.rect(r-(D>>1),F-(D>>1),D,D);if(o.reset&&o.image){o.image=null;o.reset=false}o.image?h.drawImage(o.image,r-(D>>1),F-(D>>1),D,D):kb(o.imageUrl,function(U){o.image=U;h.drawImage(o.image,r-(D>>1),F-(D>>1),D,D)});break;default:h.moveTo(r,F);h.lineTo(r-(D>>1),F-D);h.lineTo(r+(D>>1),F-D);h.lineTo(r,F)}return l(w,s,J)},prepPoly:function(o,w,s,r){var F=true;o=r.points||o.line().cull(w);r=A?A.x:m;var D=A?A.y:p;h.beginPath();for(var J=o.length;J--;){var U=o[J].x-r,T=o[J].y-D;if(F){h.moveTo(U,T);F=false}else h.lineTo(U,
...
clearTimeout(tb);S.trigger("doubleTap",u,C,q);if(c.scalable){u=S.center();r(ia+q.x-u.x,ja+q.y-u.y,U?Cb:null);s(2,Cb,true)}}function j(q,u,C){P=null;ka=true;O("down",u,C)}function e(q,u,C){O("hover",u,C)}function l(q,u,C,M){p(u,C,false);if(!P){wa+=M.x;Da+=M.y}}function d(q,u,C){p(u,C,true);ka=false}function k(){clearTimeout(Db);Db=setTimeout(function(){if(N)if(N.offsetWidth!==ab*2||N.offsetHeight!==bb*2){var q=E();A(a);E(q.x,q.y)}},250)}function g(q,u,C){O("tap",u,C);tb=setTimeout(function(){S.trigger("tap",
...
k);S.bind("layerRemove",function(q,u){var C=Ga(L,u.id);if(C>=0&&C<R){L.splice(C,1);ma++}R=L.length});hb(S,c,{container:A,captureHover:h,scalable:h,pannable:h,renderer:m});o("markers","draw",{zindex:20});A(a);if(W&&!$(window.attachEvent,Pa))window.attachEvent("onresize",k);else W&&window.addEventListener("resize",k,false);c.copyright&&t(c.copyright);Qa.attach(G);return S},Fa=function(){return{tween:function(a,c,b,f,i){function j(v){var A=v-p;v=p+m<=v;for(var G=e;G--;)l[G]=k(A,a[G],g[G],m);i&&i.invalidate();
if(v||h){Qa.detach(j);for(G=0;G<d.length;G++)d[G]&&d[G](l,A,h)}}b=Q({easing:"sine.out",duration:1E3,complete:null},b);var e=a.length,l=[].concat(a),d=[f,b.complete],k=va(b.easing),g=[],h=false,m=b.duration,p=(new Date).getTime();for(b=e;b--;)g[b]=c[b]-a[b];Qa.attach(j);return function(v){h=v;return l}},tweenDrawable:function(a,c,b,f,i){var j=Fa.tween([b],[f],i,function(){a[c]=f;for(var l=a.tweens.length;l--;)if(a.tweens[l]===e){a.tweens.splice(l,1);break}},a.view),e=function(){a[c]=j()[0]};return e}}}(),
Ta=function(a,c,b){b=Q({style:null,xy:null,size:20,fill:false,stroke:true,draggable:false,properties:{},typeName:"Shape",zindex:0},b);Q(this,b);if($(this.xy,Ia))this.xy=new a.XY(this.xy);this.id="drawable_"+Qb++;this.bounds=null;this.view=a;this.layer=c;this.tweens=[];this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.visible=true;ra(this)};Ta.prototype={constructor:Ta,applyTweens:function(){for(var a=this.tweens.length;a--;)this.tweens[a]()},drag:null,draw:null,
getProps:null,resync:function(){if(this.xy){this.xy.sync(this.view);if(this.size){var a=this.size>>1;this.updateBounds(new oa(this.xy.x-a,this.xy.y-a,this.size,this.size))}}return this},rotate:function(a,c,b){if($(a,Ja)){a=(b?a:this.rotation*Ba+a)*Oa;if(c)this.tweens.push(Fa.tweenDrawable(this,"rotation",this.rotation,a,c));else{this.rotation=a;this.view.invalidate()}return this}else return this.rotation*Ba},scale:function(a,c,b){if($(a,Ja)){a=b?a:this.scaling*a;if(c)this.tweens.push(Fa.tweenDrawable(this,
...
c){a&&(!this.bounds||a.x!=this.bounds.x||a.y!=this.bounds.y)&&this.trigger("move",this,a,this.bounds);this.bounds=a;if(c)this.xy=this.bounds.center()}};xa("drawable","marker",function(a,c,b){b=Q({fill:true,stroke:false,markerType:"simple",hoverStyle:"highlight",typeName:"Marker"},b);return new Ta(a,c,b)});xa("drawable","poly",function(a,c,b){function f(){var g,h,m,p,v,A,G,O;d=b.simplify?l.simplify():l;O=d.points;for(g=O.length;g--;){h=O[g].x;m=O[g].y;A=$(A,Pa)||h<A?h:A;G=$(G,Pa)||m<G?m:G;p=$(p,Pa)||
h>p?h:p;v=$(v,Pa)||m>v?m:v}k.updateBounds(new oa(A,G,p-A,v-G),true);k.trigger("pointsUpdate",k,O);a.invalidate()}function i(g){if($(g,"array")){var h;l=new ua(b.allowCull);h=l.points;Ab.process(g,function(m,p){for(var v=0;v<p;v++)h.push(new a.XY(m[v]))},j,e);return k}else return d}function j(){l.points.length&&Ab.process(l.points,function(g,h){for(var m=h;m--;)g[m].sync(a)},f,e)}b=Q({allowCull:false,simplify:false,fill:true,points:[],typeName:"Poly"},b);var e=500,l=new ua(b.allowCull),d=new ua(b.allowCull),
k=Q(new Ta(a,c,b),{line:i,resync:j});i(b.points);return k});xa("drawable","line",function(a,c,b){b.fill=false;b.allowCull=true;return Xa("drawable","poly",a,c,b)});xa("drawable","image",function(a,c,b){function f(){if(d&&d.width>0){if(!this.centerOffset)this.centerOffset=new aa(-d.width>>1,-d.height>>1);this.updateBounds(new oa(this.xy.x+this.centerOffset.x,this.xy.y+this.centerOffset.y,d.width,d.height),false)}}function i(g){if(this.imageUrl=g){var h=this;kb(this.imageUrl,function(m,p){d=m;if(p){var v=
k.layer?k.layer.view:null;v&&v.invalidate()}f.apply(h)})}}b=Q({image:null,imageUrl:null,centerOffset:null,typeName:"Image"},b);var j=Ta.prototype.resync,e=0,l=0,d=b.image,k=Q(new Ta(a,c,b),{changeImage:i,getProps:function(){this.bounds||f(this,d);return{image:d,x:this.xy.x+e,y:this.xy.y+l}},resync:function(g){j.call(this,g);f.call(this)}});d||i.call(this,this.imageUrl);if(this.centerOffset){e=this.centerOffset.x;l=this.centerOffset.y}return k});xa("drawable","arc",function(a,c,b){b=Q({startAngle:0,
endAngle:Math.PI*2,typeName:"Arc"},b);return new Ta(a,c,b)});Ca.prototype={constructor:Ca,clip:null,cycle:function(){},draw:function(){},hitGuess:null,remove:function(){this.view.trigger("layerRemove",this)}};xa("layer","tile",function(a,c,b,f){f=Q({generator:"osm",imageLoadArgs:{}},f);var i=Xa("generator",f.generator,a,f).run,j=null,e=[];c=Q(new Ca(a,c,b,f),{draw:function(l,d,k){l.drawTiles&&l.drawTiles(d,j.search(d.buffer(128)),k.panSpeed<2)}});a.bind("resync",function(){var l=a&&a.zoom?a.zoom():
0;e[l]||(e[l]=Va(l));j=e[l]});a.bind("refresh",function(){j&&i(j,a.invalidate)});a.bind("reset",function(){j.clear()});return c});xa("layer","draw",function(a,c,b,f){function i(k,g,h,m){var p=this.dragOffset;if(!p)p=this.dragOffset=new a.XY(k.startX-this.xy.x,k.startY-this.xy.y);this.xy.x=g-p.x;this.xy.y=h-p.y;if(m){delete this.dragOffset;a.invalidate();this.xy.sync(a,true);this.trigger("dragDrop")}return true}function j(k,g,h,m){if(e){m&&e.remove(m,g);e.insert(h,g)}}f=Q({zindex:10},f);var e,l,d=
Q(new Ca(a,c,b,f),{clear:function(){if(e){e.clear();d.trigger("cleared");a.invalidate()}},create:function(k,g){var h=Xa("drawable",k,a,d,g);h.resync();e&&h.bounds&&e.insert(h.bounds,h);h.bind("move",j);h.trigger("created");d.trigger(k+"Added",h);return h},draw:function(k,g,h,m,p){h={};m=e&&g?e.search(g):[];for(var v=m.length;v--;){var A=m[v],G=A.style||d.style,O,t,E=A.getProps?A.getProps(k):h;O=k["prep"+A.typeName];A.tweens.length>0&&A.applyTweens();t=k.applyTransform(A);if(E=A.visible&&O?O.call(k,
A,g,p,E):null){if(p&&E.hit){p.elements.push(Hits.initHit(A.type,A,A.draggable?i:null));O=p.type+"Style";G=A[O]||d[O]||G}G=G?k.applyStyle(G,true):null;(O=A.draw||E.draw)&&O.call(A,E);G&&k.applyStyle(G)}t&&t.undo&&t.undo()}},find:function(){return e.all()},hitGuess:function(k,g){return e&&e.search({x:k-5,y:g-5,w:10,h:10}).length>0}});l=a.bind("resync",function(){var k=e?e.all():[];e=Va(a.zoom(),e);for(var g=k.length;g--;)k[g].resync()});a.bind("layerRemove",function(k,g){if(g===d){e=null;a.unbind("resync",
l)}});return d});Ha.prototype={constructor:Ha};xa("control","zoombar",function(a,c,b,f){function i(s,r){var F=E[s]=W.create("div","t5-zoombar-button",{position:"absolute",background:j(s),"z-index":51,width:f.width+"px",height:f.buttonHeight+"px","margin-top":(r||0)+"px"});z.appendChild(F)}function j(s,r){return"url("+f.images+") 0 -"+(h+p*3+(s||0)*k*3+(r||0)*k)+"px"}function e(s){return"url("+f.images+") 0 -"+(h+(s||0)*p)+"px"}function l(s,r){var F;if(s===m){m.style.background=e(r);F="thumb"}else for(var D=
0;D<E.length;D++)if(s===E[D]){F="button"+D;E[D].style.background=j(D,r);break}return F}function d(s){if(s!==O){O=s;G=A-O/t*(A-v)|0;W.move(m,0,G-v);clearTimeout(o);o=setTimeout(function(){a.zoom(O)},500)}}f=Q({width:24,height:200,images:"img/zoom.png",align:"right",marginTop:10,spacing:10,thumbHeight:16,buttonHeight:16},f);var k=f.buttonHeight,g,h=f.height,m,p=f.thumbHeight,v=f.spacing+k-(p>>1),A=f.height-k-(p>>1),G=v,O=-1;c=a.minZoom();var t=a.maxZoom()-c+1,E=[],z,o=0,w={button0:function(){a.zoom(a.zoom()+
1)},button1:function(){a.zoom(a.zoom()-1)}};z=W.create("div","t5-zoombar",{position:"absolute",background:"url("+f.images+")","z-index":50,overflow:"hidden",width:f.width+"px",height:f.height+"px",margin:function(){var s=f.spacing,r=Z("{0}px 0 0 {1}px");if(f.align==="right")s=b.offsetWidth-f.width-f.spacing;return r(f.marginTop,s)}()});b.childNodes[0]?b.insertBefore(z,b.childNodes[0]):b.appendChild(z);z.appendChild(m=W.create("div","t5-zoombar-thumb",{position:"absolute",background:e(),"z-index":51,
width:f.width+"px",height:f.thumbHeight+"px",margin:"10px 0 0 0",top:G-v+"px"}));i(0);i(1,f.height-f.buttonHeight);g=INTERACT.watch(z,{bindTarget:z});g.bind("pointerMove",function(s,r,F){G=Math.min(Math.max(v,F.y-(p>>1)),A);d(t-(G-v)/A*t|0)});g.bind("pointerDown",function(s){l(s.target,2)});g.bind("pointerUp",function(s){l(s.target,0)});g.bind("tap",function(s){(s=w[l(s.target,2)])&&s()});c=new Ha(a);c.bind("detach",function(){g.unbind();b.removeChild(z)});a.bind("zoom",function(s,r){d(r)});d(a.zoom());
return c});xa("control","copyright",function(a,c,b,f){function i(){j(a.getCopy())}function j(l){if(e){e.innerHTML=l;l=e.style;var d;a.viewport();d=W.rect(b);var k=f.spacing,g=d.h-e.offsetHeight-f.spacing,h=Z("{0}px 0 0 {1}px");if(f.align==="right")k=d.w-e.offsetWidth-f.spacing;d=h(g,k);l.margin=d}}f=Q({align:"right",text:null,spacing:5},f);var e;(function(){var l=W.rect(b),d=f.text||a.getCopy();e=W.create("div","t5-copyright",{position:"absolute",overflow:"hidden","max-width":Math.max(l.w>>1,Math.min(400,
l.w-f.spacing*2))+"px","z-index":49});b.childNodes[0]?b.insertBefore(e,b.childNodes[0]):b.appendChild(e);d&&j(d)})();c=Q(new Ha(a),{getText:function(){return e?e.innerHTML:""},setText:j});c.bind("detach",function(){e&&b.removeChild(e)});f.text||a.bind("copyright",i);return c});Q(X,{ex:Q,log:La,observable:ra,configurable:hb,formatter:Z,wordExists:function(a,c){for(var b=a.split(/\s|\,/).length;b--;)if(a.toLowerCase()==c.toLowerCase())return true;return false},is:$,indexOf:Ga,fn:function(a){return Pb("fn",
a)},project:ib,unproject:sb,getImage:kb,Registry:Na,Style:Bb,DOM:W,Rect:oa,XY:aa,GeoXY:na,Line:ua,Hits:Hits,Control:Ha,Tile:Ma,Tweener:Fa,ViewLayer:Ca,View:Lb,Map:function(a,c){function b(g,h){var m=k.viewport();if(g){m=K(g.bestZoomLevel(m.w,m.h)-1,h||0);return f(m).center(g.center())}else return new GeoJS.BBox((new na(m.x,m.y2)).sync(k,true).pos(),(new na(m.x2,m.y)).sync(k,true).pos())}function f(g,h,m){if($(g,Ja)){g=K(c.minZoom,B(c.maxZoom,g|0));if(g!==e){var p=k.viewport(),v=k.offset(),A=p.w/2;
p=p.h/2;var G=Y(2,g-e),O=A/G|0,t=p/G|0;e=g;k.offset(((h||v.x+A)-O)*G,((m||v.y+p)-t)*G);k.trigger("zoom",g);k.trigger("reset");j=k.rpp=la/(256<<e);k.setMaxOffset(la/j|0,la/j|0,true,false);k.scale(1+l,false,true);l=0;k.trigger("resync");k.refresh()}return k}else return e}c=Q({controls:["zoombar","copyright"],minZoom:1,maxZoom:18,renderer:"canvas/dom",zoom:1,zoombar:{}},c);var i=new na,j,e=c.zoom||c.zoomLevel,l=0,d=0,k=Q(new Lb(a,c),{XY:na,bounds:b,zoom:f});j=k.rpp=la/(256<<e);k.bind("refresh",function(){var g=
k.viewport();if(i.x!=g.x||i.y!=g.y){k.trigger("boundsChange",b());i.x=g.x;i.y=g.y}});k.bind("scaleChanged",function(g,h){var m=rb(h)/Math.LN2|0;if(m!==0){l=h-Y(2,m);clearTimeout(d);d=setTimeout(function(){f(e+m)},500)}});return k}});if(typeof module!="undefined"&&module.exports)module.exports=X;X.Decarta=function(){function a(t){return m(e.clientName,e.clientPassword,e.sessionID,e.configuration,e.release,t)}function c(t,E){return p(t.maxResponses,t.version,t.requestID,t.methodName,E)}function b(t,
E){e.server||X.log("No server configured for deCarta - we are going to have issues","warn");return v(e.server,t.requestID,escape(E))}function f(t,E,z){La("making request: "+a(c(t,t.getRequestBody())));$a(b(t,a(c(t,t.getRequestBody()))),function(o){console.log(o.response);var w=o.response.XLS.Response;if(w.numberOfResponses>0&&w[t.methodName+"Response"]){o=[];if(t.parseResponse)o=t.parseResponse(w[t.methodName+"Response"]);E&&E.apply(null,o)}else z&&z("Server returned no responses",o.response)})}function i(t,
E){var z=new A({json:t.StreetAddress}),o=[];if(t.Place){if(!t.Place.length)t.Place=[t.Place];for(var w=t.Place.length;w--;)o[o.length]=t.Place[w].content}return{building:z.building,street:z.street,regions:o,countryCode:t.countryCode||"",postalCode:t.PostalCode||"",pos:E}}function j(t){if(!t)return"";t=t.toUpperCase();if(!k){var E=[],z;for(z in g)E.push(z);k=RegExp("(\\s)("+E.join("|")+")(\\s|$)","i")}k.lastIndex=-1;if(E=k.exec(t))t=t.replace(k,"$1"+g[E[2]]);return t}var e={sessionID:(new Date).getTime(),
server:"",clientName:"",clientPassword:"",configuration:"",maxResponses:25,release:"4.4.2sp03",tileFormat:"PNG",fixedGrid:true,useCache:true,tileHosts:[],requestTimeout:3E4,geocoding:{countryCode:"US",language:"EN"}};TL=function(){function t(z,o){if(typeof z=="number"){this.days=z||0;this.seconds=o||0}else if(typeof z!="undefined"){this.days=z.days||0;this.seconds=z.seconds||0}}t.prototype={add:function(){for(var z=new t(this.days,this.seconds),o=arguments.length;o--;){z.days+=arguments[o].days;z.seconds+=
arguments[o].seconds}return z},toString:function(){var z,o,w,s="";if(this.days)s=this.days+" days ";if(this.seconds){w=this.seconds;if(w>=3600){z=~~(w/3600);w-=z*3600}if(w>=60){o=Math.round(w/60);w-=o*60}if(z)s=s+z+(z>1?" hrs ":" hr ")+(o?(o>10?o:"0"+o)+" min ":"");else if(o)s=s+o+" min";else if(w>0)s=s+(w>10?w:"0"+w)+" sec"}return s}};var E=function(){var z=/^P(\d+Y)?(\d+M)?(\d+D)?$/,o=/^(\d+H)?(\d+M)?(\d+S)?$/,w={8601:function(s){s=s.split("T");var r=null;r=null;var F=0,D=0;z.lastIndex=-1;r=z.exec(s[0]);
F+=r[3]?parseInt(r[3].slice(0,-1),10):0;o.lastIndex=-1;r=o.exec(s[1]);D+=r[1]?parseInt(r[1].slice(0,-1),10)*3600:0;D+=r[2]?parseInt(r[2].slice(0,-1),10)*60:0;D+=r[3]?parseInt(r[3].slice(0,-1),10):0;return new t(F,D)}};return function(s,r){var F=w[r];if(!F)throw"No parser found for the duration format: "+r;return F(s)}}();return{Duration:t,parse:E}}();var l=/^(\d+).*$/,d=/(\d+)\s?\-\s?(\d+)/,k=null,g={RD:"ROAD",ST:"STREET",CR:"CRESCENT",CRES:"CRESCENT",CT:"COURT",LN:"LANE",HWY:"HIGHWAY",MWY:"MOTORWAY"},
h=1,m=Z("<xls:XLS version='1' xls:lang='en' xmlns:xls='http://www.opengis.net/xls' rel='{4}' xmlns:gml='http://www.opengis.net/gml'><xls:RequestHeader clientName='{0}' clientPassword='{1}' sessionID='{2}' configuration='{3}' />{5}</xls:XLS>"),p=Z("<xls:Request maximumResponses='{0}' version='{1}' requestID='{2}' methodName='{3}Request'>{4}</xls:Request>"),v=Z("{0}/JSON?reqID={1}&chunkNo=1&numChunks=1&data={2}&responseFormat=JSON"),A=function(t){t=X.ex({json:{}},t);var E="",z="";if(t.json.Street)E=
t.json.Street.content?t.json.Street.content:t.json.Street;E=E&&E.replace?E.replace(/\/\d+$/,""):"";if(t.json.Building)if(t.json.Building.number)z=t.json.Building.number;return{building:z,street:E,calcMatchPercentage:function(o){var w=0,s=j(o),r=j(E);if(t.json.Building){a:{var F=t.json.Building.number.toString();l.lastIndex=-1;if(l.test(o)){o=o.replace(l,"$1");F=F.split(",");for(var D=0;D<F.length;D++){d.lastIndex=-1;if(d.test(F[D])){var J=d.exec(F[D]);if(o>=parseInt(J[1],10)&&o<=parseInt(J[2],10)){o=
true;break a}}else if(o==F[D]){o=true;break a}}}o=false}if(o)w+=0.2}if(s&&r&&X.wordExists(s,r))w+=0.8;return w},toString:function(){if(E)return(z?z+" ":"")+E;return""}}},G=function(){return{methodName:"",maxResponses:25,version:"1.0",requestID:h++,getRequestBody:function(){return""},parseResponse:function(t){return[t]}}},O=function(){return X.ex(new G,{methodName:"RUOK",parseResponse:function(t){return[{aliasCount:t.maxHostAliases,host:t.hostName}]}})};X.Registry.register("generator","decarta",function(t,
E){function z(s,r){var F=t.zoom?t.zoom():0,D=t.viewport();if(F){var J=2<<F-1,U=J>>1,T=E.tileSize,V=(D.w/T|0)+1,I=(D.h/T|0)+1,L=(D.x/T|0)-U;D=J-(D.y/T|0)-U-I;var R=s.search({x:(U+L)*T,y:(U+D*-1-I)*T,w:V*T,h:I*T});J={};var ea=w.hosts,H;for(H=R.length;H--;)J[R[H].id]=true;for(R=0;R<=V;R++)for(H=0;H<=I;H++){var N=L+R,P=D+H-1,ba=N+"_"+P;if(!J[ba]){N=new X.Tile((U+L+R)*T,(U+D*-1-H)*T,ea[R%ea.length]+"/openls/image-cache/TILE?LLMIN=0.0,0.0&LLMAX="+o[F]+"&CACHEABLE=true&DS=navteq-world&WIDTH=256&HEIGHT=256&CLIENTNAME="+
w.clientName+"&SESSIONID="+w.sessionID+"&FORMAT=PNG&CONFIG="+w.configuration+"&N="+P+"&E="+N,T,T,ba);s.insert(N,N)}}r&&r()}}E=X.ex({tileSize:256},E);var o=["89.787438015348100000,360.00000000000000000","85.084059050110410000,180.00000000000000000","66.653475896509040000,90.00000000000000000","41.170427238429790000,45.000000000000000000","22.076741328793200000,22.500000000000000000","11.251819676168665000,11.250000000000000000","5.653589942659626000,5.625000000000000000","2.830287664051185000,2.812500000000000000",
...
"0.000345636472797214,0.000343322753906250"],w=e.tileConfig;t.addCopy("&copy; deCarta, Inc. Map and Imagery Data &copy; NAVTEQ or Tele Atlas or DigitalGlobe");return{run:function(s,r){if(w)z(s,r);else{var F=e.clientName.replace(/.*?\:/,"");X.Decarta.getTileConfig(F,function(D){w=D;z(s,r)})}},setTileConfig:function(s){w=s}}});X.Registry.register("service","geocoder",function(){var t=X.formatter('<xls:GeocodeRequest><xls:Address countryCode="{0}" language="{1}">{2}</xls:Address></xls:GeocodeRequest>'),
