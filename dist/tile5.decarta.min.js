(function(){function K(p,A,E){if(p&&p.split){var F=p.split(Ja);if(F.length>1){p=F[0];A=F[1]}}else if(p&&p.lat){A=p.lon;p=p.lat}this.lat=parseFloat(p||0);this.lon=parseFloat(A||0);this.radius=E||$a}function sa(p){this.positions=[];for(var A=p.length;A--;)this.positions[A]=typeof p[A]=="string"?new K(p[A]):p[A]}function ja(p,A){if(p&&p.splice){for(var E=A,F=new K(Aa,Ka),S=new K(La,ab),ia=p.length;ia--;){var ea=typeof p[ia]=="string"?new K(p[ia]):p[ia];if(ea.lat<F.lat)F.lat=ea.lat;if(ea.lat>S.lat)S.lat=
...
"undefined"&&module.exports,oa=Math.PI*2,ta=Math.PI/2,fa=Math.PI/180,ma=180/Math.PI,Aa=90,La=-Aa,Ka=180,ab=-Ka,pa=Aa*fa,Ta=-pa,Ma=Ka*fa,Z=-Ma,$a=6371,U=$a*1E3,Ja=/[\,\s]+/;K.prototype={constructor:K,bearing:function(p){var A=this.lat*fa,E=p.lat*fa;p=(p.lon-this.lon)*fa;return(Math.atan2(Math.sin(p)*Math.cos(E),Math.cos(A)*Math.sin(E)-Math.sin(A)*Math.cos(E)*Math.cos(p))*ma+360)%360},copy:function(){return new K(this.lat,this.lon)},distanceTo:function(p){if(!p||this.empty()||p.empty())return 0;var A=
...
return d}else return b[g]}}return function(d,b,e){b=b||d;for(var g in b)typeof b[g]!="function"&&a(d,b,e,g);return d}}(),Ma=Array.prototype.indexOf||function(a){for(var d=0;d<this.length;d++)if(this[d]===a)return d;return-1},Z=function(){var a={NaN:1,Infinity:1,"-Infinity":1},d=String.prototype.toLowerCase,b=Object.prototype.toString;return function(e,g){g=d.call(g);if(g=="finite")return!a.hasOwnProperty(+e);return g=="null"&&e===null||g==typeof e||g=="object"&&e===Object(e)||g=="array"&&Array.isArray&&
...
o.observable=pa({});globalOpts=o}o.binder=(o.isIE?b:a)(o.bindTarget||document);o.unbinder=(o.isIE?b:d)(o.bindTarget||document);q=o.types;for(var B=capabilities,H=[],T=k.length;T--;){var v=k[T],I=!q||q.indexOf(v.type)>=0,x=true,m;for(m in v.checks){var u=v.checks[m];sa("checking "+m+" capability. require: "+u+", capability = "+B[m]);x=x&&u===B[m]}if(I&&x)H[H.length]=v.handler}return new h(n,H,o)}}}();var U=this.T5=pa({}),Ja=function(){var a={};return{create:function(d,b){if(a[d][b])return a[d][b].apply(null,
...
Na=Math.sin,eb=Math.cos,rb=Math.atan,Ga="undefined",Ca="string",Da="number",ua=Ja.register,Oa=Ja.create,Ib=Ja.get,Jb=0,Gb=0,sb=/[\,\s]+/,Pa=function(){function a(f){g++;f=aa?window.mozAnimationStartTime||f||(new Date).getTime():(new Date).getTime();for(var l=e.length;l--;){var c=e[l];g%c.every===0&&c.cb(f)}i&&animFrame(a)}var d=1E3/60,b=["r","webkitR","mozR","oR","msR"],e=[],g=0,i=aa&&function(){for(var f=0;f<b.length;f++)window.animFrame=window.animFrame||window[b[f]+"equestAnimationFrame"];return animFrame}();
...
f;k<=i;k++){var h=d(c,k),n=Ma.call(h,l);n>=0&&h.splice(n,1)}}},search:function(i){var f=i.y/a|0,l=(i.x+i.w)/a|0,c=(i.y+i.h)/a|0,j=[],k=[];for(i=i.x/a|0;i<=l;i++)for(var h=f;h<=c;h++)j=j.concat(d(i,h));j.sort();for(f=j.length;f--;){l=j[f];if(c=e[l])k[k.length]=c;for(;f>0&&j[f-1]==l;)f--}return k}}},fb=function(){var a={},d=0;(new Date).getTime();var b={},e=[];Pa.attach(function(g){g=g||(new Date).getTime();for(var i=0;i<e.length;){var f=e[i],l=b[f],c=b[f].image&&b[f].image.complete&&b[f].image.width>
0,j=g-l.start;j=c||j>=3E4;if(c){c=l.callbacks;a[f]=l.image;for(var k=0;k<c.length;k++)c[k](l.image,true)}if(j){e.splice(i,1);delete b[f]}else i++}},250);return function(g,i){var f=g&&i?a[g]:null;if(f&&f&&f.complete&&f.width>0)i(f);else if(f=b[g])f.callbacks.push(i);else{f=new Image;f.id="_ldimg"+ ++d;b[g]={start:(new Date).getTime(),image:f,callbacks:[i]};f.src=g;e[e.length]=g}}}();ma.prototype={constructor:ma,load:function(a){var d=this;fb(this.url,function(b){d.loaded=true;d.image=b;a&&a()})}};
...
2;ob=hb/2;Va=G.offsetWidth/2;Wa=G.offsetHeight/2;Xa=new ba(nb,ob);P.appendChild(V=aa.create("div","",aa.styles({width:gb+"px",height:hb+"px","z-index":2,margin:-ra.y+"px 0 0 "+-ra.x+"px"})));n(d.renderer);t=d.controls;for(w=0;w<z.length;w++)z[w].trigger("detach");z=[];for(w=0;w<t.length;w++)z[z.length]=Oa("control",t[w],R,P,G,d[t[w]])}else n("canvas")}function H(t,w,y){var O=(new ba(y.x-Va+nb,y.y-Wa+ob)).rotate(-va*F,Xa).scale(1/da,Xa);na=Hits.init(t,w,y,e(y.x,y.y,true),O);W=false;for(t=M;t--;)if(C[t].visible)W=
W||(C[t].hitGuess?C[t].hitGuess(na.gridX,na.gridY,R):false);W&&la++}function T(t,w,y){if(typeof t!="undefined"&&(Z(t,Ca)||Z(t,"object"))){w=new R.XY(t);w.sync(R);t=w.x;w=w.y}if(Z(t,Da)){u(t-Va-ra.x,w-Wa-ra.y,y);return R}else return u().offset(Va+ra.x|0,Wa+ra.y|0).sync(R,true)}function v(){var t=new ta(Y,$,gb,hb);t.scaleFactor=da;t.padding=ra?ra.copy():new ba;return t}function I(t,w,y){if(Z(t,Ca)&&Z(w,Ga))for(w=0;w<M;w++){if(C[w].id===t)return C[w]}else if(Z(t,Ca)){w=Oa("layer",w,R,y);y=q(t);y!==M&&
...
w,y,O){if(Z(t,Da)){var X=y?t:da*t;if(!r){w=undefined;t=cb(lb(X)/Math.LN2);X=Fa(2,t)}if(O){t=R.center();u(Y+O.x-t.x,$+O.y-t.y,w)}if(w){kb=da;Qa=ya.tween([da],[X],w,function(){da=X;kb=Qa=null;la++})}else{da=X;la++}return R}else return da}function u(t,w,y){if(Z(t,Da)){if(y)za=ya.tween([Y,$],[t,w],y,function(){Y=t|0;$=w|0;wa=Ha=0;za=null;la++});else{Y=t|0;$=w|0;la++}return R}else return(new R.XY(Y,$)).sync(R,true)}d=K({captureHover:true,controls:[],drawOnScale:true,padding:128,inertia:true,refreshDistance:128,
pannable:true,scalable:true,renderer:"canvas",useTransforms:true},d);var s="auto",r=true,D=false,z=[],C=[],M=0,V=null,P=null,G,L=null,W=false,ca,J=false,Q=0,N=0,ha=d.refreshDistance,Y=0,$=0,wa=0,Ha=0,Ab=0,Bb=0,Ra=null,Sa=null,ib=false,jb=false,za=null,ra,na=null,Ya=null,ga,zb=0,Xa=new ba,va=0,Za=null,da=1,kb,Qa=null,pb=1,qa=null,qb={index:0,draw:false},yb={easing:"sine.out",duration:500},la=0,gb,hb,nb,ob,Va,Wa,xb=0,R={XY:ba,id:d.id,panSpeed:0,attachFrame:function(t,w){w&&V.appendChild(t)},center:T,
...
R.bind("layerRemove",function(t,w){var y=Ma(C,w.id);if(y>=0&&y<M){C.splice(y,1);la++}M=C.length});Ta(R,d,{container:B,captureHover:h,scalable:h,pannable:h,renderer:n});I("markers","draw",{zindex:20});B(a);if(aa&&!Z(window.attachEvent,Ga))window.attachEvent("onresize",j);else aa&&window.addEventListener("resize",j,false);Pa.attach(function(t){var w=[],y,O,X;if(!D){R.panSpeed=panSpeed=ea(Q)+ea(N);(y=da!==pb)&&R.trigger("scale");if(panSpeed>0||y||za||Qa||Za){la++;if(za&&panSpeed>0){za(true);za=null}}if(!J&&
panSpeed<=0&&(ea(Y-Ab)>=ha||ea($-Bb)>=ha))x();qb.index++;qb.draw=la||panSpeed||0;if(ga&&qb.draw){if(Qa)da=Qa()[0];if(Za)va=Za()[0];wa+=Q;Ha+=N;if(Q||N)R.trigger("pan");if(r){if(da!==1)w[w.length]="scale("+da+")";if(va!==0)w[w.length]="rotate("+va+"deg)"}y=W||!ca||!J&&!za&&(d.drawOnScale||da===1)&&panSpeed<=2;if(za){O=za();X=1;if(kb)X=da/kb;wa=(Y-O[0]|0)*X;Ha=($-O[1]|0)*X}if(y){O=-va*F;y=eb(O)*wa+-Na(O)*Ha;O=Na(O)*wa+eb(O)*Ha;Y=Y-y/da|0;$=$-O/da|0;X=v();ga.trigger("predraw",X);if(ga.prepare(C,X,t,
na)){O=Ha=y=wa=la=0;for(ii=M;ii--;){var ka=C[ii];if(ka.visible){var xa=ka.style?ga.applyStyle(ka.style,true):null;ka.draw(ga,X,R,t,na);xa&&ga.applyStyle(xa)}}ga.trigger("render",X);R.trigger("drawComplete",X,t);aa.move(V,y,O,w,Xa)}}else aa.move(V,wa,Ha,w,Xa);if(J||!d.inertia)N=Q=0;else if(Q!=0||N!=0){Q*=0.8;N*=0.8;if(ea(Q)<0.5)Q=0;if(ea(N)<0.5)N=0}if(na){t=true;w=na?na.elements:[];if(Ya&&Ya.type==="hover"){diffElements=Hits.diffHits(Ya.elements,w);if(diffElements.length>0)Hits.triggerEvent(Ya,R,"Out",
diffElements);else t=false}if(w.length>0){O=na.gridX;X=na.gridY;for(y=w.length;J&&y--;){ka=w[y];xa=O;var Cb=X,Db=ka&&ka.drag&&(!ka.canDrag||ka.canDrag(ka,xa,Cb));if(Db){L=ka;L.startX=xa;L.startY=Cb}if(Db)break}t&&Hits.triggerEvent(na,R)}Ya=w.length>0?K({},na):null;na=null}if(pb!==da){R.trigger("scaleChanged",da);pb=da}}}});return R},ya=function(){return{tween:function(a,d,b,e,g){function i(q){var B=q-o;q=o+n<=q;for(var H=f;H--;)l[H]=j(B,a[H],k[H],n);g&&g.invalidate();if(q||h){Pa.detach(i);for(H=0;H<
c.length;H++)c[H]&&c[H](l,B,h)}}b=K({easing:"sine.out",duration:1E3,complete:null},b);var f=a.length,l=[].concat(a),c=[e,b.complete],j=ab(b.easing),k=[],h=false,n=b.duration,o=(new Date).getTime();for(b=f;b--;)k[b]=d[b]-a[b];Pa.attach(i);return function(q){h=q;return l}},tweenDrawable:function(a,d,b,e,g){var i=ya.tween([b],[e],g,function(){a[d]=e;for(var l=a.tweens.length;l--;)if(a.tweens[l]===f){a.tweens.splice(l,1);break}},a.view),f=function(){a[d]=i()[0]};return f}}}(),Ia=function(a,d,b){b=K({style:null,
xy:null,size:20,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape",zindex:0},b);K(this,b);if(Z(this.xy,Ca))this.xy=new a.XY(this.xy);this.id="drawable_"+Jb++;this.bounds=null;this.view=a;this.layer=d;this.tweens=[];this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.observable&&pa(this)};Ia.prototype={constructor:Ia,applyTweens:function(){for(var a=this.tweens.length;a--;)this.tweens[a]()},drag:null,draw:null,getProps:null,resync:function(){if(this.xy){this.xy.sync(this.view);
if(this.size){var a=this.size>>1;this.updateBounds(new ta(this.xy.x-a,this.xy.y-a,this.size,this.size))}}},rotate:function(a,d,b){if(Z(a,Da)){a=(b?a:this.rotation*S+a)*F;if(d)this.tweens.push(ya.tweenDrawable(this,"rotation",this.rotation,a,d));else{this.rotation=a;this.view.invalidate()}return this}else return this.rotation*S},scale:function(a,d,b){if(Z(a,Da)){a=b?a:this.scaling*a;if(d)this.tweens.push(ya.tweenDrawable(this,"scaling",this.scaling,a,d));else{this.scaling=a;this.view.invalidate()}return this}else return this.scaling},
...
a;if(d)this.xy=this.bounds.center()}};ua("drawable","marker",function(a,d,b){b=K({fill:true,stroke:false,markerType:"simple",hoverStyle:"highlight",typeName:"Marker"},b);return new Ia(a,d,b)});ua("drawable","poly",function(a,d,b){function e(){var k,h,n,o,q,B,H;c=b.simplify?l.simplify():l;for(k=c.length;k--;){h=c[k].x;n=c[k].y;B=Z(B,Ga)||h<B?h:B;H=Z(H,Ga)||n<H?n:H;o=Z(o,Ga)||h>o?h:o;q=Z(q,Ga)||n>q?n:q}j.updateBounds(new ta(B,H,o-B,q-H),true);j.trigger("pointsUpdate",j,c);a.invalidate()}function g(k){if(Z(k,
"array")){l=new Ba(b.allowCull);vb.process(k,function(h,n){for(var o=0;o<n;o++)l.push(new a.XY(h[o]))},i,f);return j}else return c}function i(){l.length&&vb.process(l,function(k,h){for(var n=h;n--;)k[n].sync(a)},e,f)}b=K({allowCull:false,simplify:false,fill:true,points:[],typeName:"Poly"},b);var f=500,l=new Ba(b.allowCull),c=[],j=K(new Ia(a,d,b),{points:g,resync:i});g(b.points);return j});ua("drawable","line",function(a,d,b){b.fill=false;b.allowCull=true;return Oa("drawable","poly",a,d,b)});ua("drawable",
"image",function(a,d,b){function e(){if(c&&c.width>0){if(!this.centerOffset)this.centerOffset=new ba(-c.width>>1,-c.height>>1);this.updateBounds(new ta(this.xy.x+this.centerOffset.x,this.xy.y+this.centerOffset.y,c.width,c.height),false)}}function g(k){if(this.imageUrl=k){var h=this;fb(this.imageUrl,function(n,o){c=n;if(o){var q=j.layer?j.layer.view:null;q&&q.invalidate()}e.apply(h)})}}b=K({image:null,imageUrl:null,centerOffset:null,typeName:"Image"},b);var i=Ia.prototype.resync,f=0,l=0,c=b.image,
j=K(new Ia(a,d,b),{changeImage:g,getProps:function(){this.bounds||e(this,c);return{image:c,x:this.xy.x+f,y:this.xy.y+l}},resync:function(k){i.call(this,k);e.call(this)}});c||g.call(this,this.imageUrl);if(this.centerOffset){f=this.centerOffset.x;l=this.centerOffset.y}});ua("drawable","arc",function(a,d,b){b=K({startAngle:0,endAngle:Math.PI*2,typeName:"Arc"},b);return new Ia(a,d,b)});Aa.prototype={constructor:Aa,clip:null,cycle:function(){},draw:function(){},hitGuess:null,remove:function(){this.view.trigger("layerRemove",
this)}};ua("layer","tile",function(a,d){d=K({generator:"osm",imageLoadArgs:{}},d);var b=Oa("generator",d.generator,d).run,e=null,g=[],i=K(new Aa(a,d),{draw:function(f,l,c){f.drawTiles&&f.drawTiles(l,e.search(l.buffer(128)),c.panSpeed<2)}});a.bind("resync",function(){var f=a&&a.zoom?a.zoom():0;g[f]||(g[f]=fa(f));e=g[f]});a.bind("refresh",function(f,l,c){e&&b(l,c,e,function(){l.invalidate()})});return i});ua("layer","draw",function(a,d){function b(c,j,k,h){var n=this.dragOffset;if(!n)n=this.dragOffset=
new a.XY(c.startX-this.xy.x,c.startY-this.xy.y);this.xy.x=j-n.x;this.xy.y=k-n.y;if(h){delete this.dragOffset;a.invalidate();this.xy.sync(a,true);this.trigger("dragDrop")}return true}function e(c,j,k,h){if(i){h&&i.remove(h,j);i.insert(k,j)}}d=K({zindex:10},d);var g=[],i,f,l=K(new Aa(a,d),{itemCount:0,clear:function(){i=new tb;g=[];l.trigger("cleared");l.itemCount=0;a.invalidate()},create:function(c,j,k){j=Oa("drawable",c,a,l,j);if(k)g.unshift(j);else g[g.length]=j;j.resync();i&&j.bounds&&i.insert(j.bounds,
j);j.bind("move",e);l.itemCount=g.length;l.trigger(c+"Added",j);return j},draw:function(c,j,k,h,n){k={};h=i&&j?i.search(j):[];for(var o=h.length;o--;){var q=h[o],B=q.style||l.style,H,T,v=q.getProps?q.getProps(c):k;H=c["prep"+q.typeName];q.tweens.length>0&&q.applyTweens();T=c.applyTransform(q);if(v=H?H.call(c,q,j,n,v):null){if(n&&v.hit){n.elements.push(Hits.initHit(q.type,q,q.draggable?b:null));H=n.type+"Style";B=q[H]||l[H]||B}B=B?c.applyStyle(B,true):null;(H=q.draw||v.draw)&&H.call(q,v);B&&c.applyStyle(B)}T&&
T.undo&&T.undo()}},find:function(){return[].concat(g)},hitGuess:function(c,j){return i&&i.search({x:c-10,y:j-10,w:20,h:20}).length>0}});f=a.bind("resync",function(){i=fa(a.zoom(),i);for(var c=g.length;c--;)g[c].resync()});a.bind("layerRemove",function(c,j){if(j===l){i=null;g=[];a.unbind("resync",f)}});return l});La.prototype={constructor:La};ua("control","zoombar",function(a,d,b,e){function g(s,r){var D=I[s]=aa.create("div","t5-zoombar-button",{position:"absolute",background:i(s),"z-index":51,width:e.width+
"px",height:e.buttonHeight+"px","margin-top":(r||0)+"px"});x.appendChild(D)}function i(s,r){return"url("+e.images+") 0 -"+(h+o*3+(s||0)*j*3+(r||0)*j)+"px"}function f(s){return"url("+e.images+") 0 -"+(h+(s||0)*o)+"px"}function l(s,r){var D;if(s===n){n.style.background=f(r);D="thumb"}else for(var z=0;z<I.length;z++)if(s===I[z]){D="button"+z;I[z].style.background=i(z,r);break}return D}function c(s){if(s!==T){T=s;H=B-T/v*(B-q)|0;aa.move(n,0,H-q);clearTimeout(m);m=setTimeout(function(){a.zoom(T)},500)}}
e=K({width:24,height:200,images:"img/zoom.png",align:"right",marginTop:10,spacing:10,thumbHeight:16,buttonHeight:16},e);var j=e.buttonHeight,k,h=e.height,n,o=e.thumbHeight,q=e.spacing+j-(o>>1),B=e.height-j-(o>>1),H=q,T=-1;d=a.minZoom();var v=a.maxZoom()-d+1,I=[],x,m=0,u={button0:function(){a.zoom(a.zoom()+1)},button1:function(){a.zoom(a.zoom()-1)}};x=aa.create("div","t5-zoombar",{position:"absolute",background:"url("+e.images+")","z-index":50,overflow:"hidden",width:e.width+"px",height:e.height+"px",
margin:function(){var s=e.spacing,r=ja("{0}px 0 0 {1}px");if(e.align==="right")s=b.offsetWidth-e.width-e.spacing;return r(e.marginTop,s)}()});b.childNodes[0]?b.insertBefore(x,b.childNodes[0]):b.appendChild(x);x.appendChild(n=aa.create("div","t5-zoombar-thumb",{position:"absolute",background:f(),"z-index":51,width:e.width+"px",height:e.thumbHeight+"px",margin:"10px 0 0 0",top:H-q+"px"}));g(0);g(1,e.height-e.buttonHeight);k=INTERACT.watch(x,{bindTarget:x});k.bind("pointerMove",function(s,r,D){H=Math.min(Math.max(q,
D.y-(o>>1)),B);c(v-(H-q)/B*v|0)});k.bind("pointerDown",function(s){l(s.target,2)});k.bind("pointerUp",function(s){l(s.target,0)});k.bind("tap",function(s){(s=u[l(s.target,2)])&&s()});d=new La(a);d.bind("detach",function(){k.unbind();b.removeChild(x)});a.bind("zoom",function(s,r){c(r)});c(a.zoom());return d});K(U,{ex:K,log:sa,observable:pa,configurable:Ta,formatter:ja,wordExists:function(a,d){for(var b=a.split(/\s|\,/).length;b--;)if(a.toLowerCase()==d.toLowerCase())return true;return false},is:Z,
indexOf:Ma,fn:function(a){return Ib("fn",a)},project:bb,unproject:mb,getImage:fb,userMessage:function(a,d,b){U.trigger("userMessage",a,d,b)},Registry:Ja,Style:wb,DOM:aa,Rect:ta,XY:ba,Line:Ba,Hits:Hits,Control:La,Tile:ma,Tweener:ya,View:Eb,Map:function(a,d){function b(j,k){var h=c.viewport();if(j){h=db(j.bestZoomLevel(h.w,h.h)-1,k||0);return e(h).center(j.center())}else return new GeoJS.BBox((new oa(h.x,h.y2)).sync(c,true).pos(),(new oa(h.x2,h.y)).sync(c,true).pos())}function e(j,k,h){if(Z(j,Da)){j=
db(d.minZoom,ub(d.maxZoom,j|0));if(j!==f){var n=c.viewport(),o=c.offset(),q=n.w/2;n=n.h/2;var B=Fa(2,j-f),H=q/B|0,T=n/B|0;f=j;c.offset(((k||o.x+q)-H)*B,((h||o.y+n)-T)*B);c.trigger("zoom",j);c.trigger("reset");i=c.rpp=A/(256<<f);c.setMaxOffset(A/i|0,A/i|0,true,false);c.scale(1,false,true);c.trigger("resync");c.refresh()}return c}else return f}d=K({controls:["zoombar"],minZoom:1,maxZoom:18,renderer:"canvas/dom",zoom:1,zoombar:{}},d);var g=new oa,i,f=d.zoom||d.zoomLevel,l=0,c=K(new Eb(a,d),{XY:oa,bounds:b,
zoom:e});i=c.rpp=A/(256<<f);c.bind("refresh",function(){var j=c.viewport();if(g.x!=j.x||g.y!=j.y){c.trigger("boundsChange",b());g.x=j.x;g.y=j.y}});c.bind("scaleChanged",function(j,k){var h=lb(k)/Math.LN2|0;if(h!==0){k=Fa(2,h);clearTimeout(l);l=setTimeout(function(){e(f+h)},500)}});return c}});if(typeof module!="undefined"&&module.exports)module.exports=U;U.Decarta=function(){function a(v){return n(f.clientName,f.clientPassword,f.sessionID,f.configuration,f.release,v)}function d(v,I){return o(v.maxResponses,
v.version,v.requestID,v.methodName,I)}function b(v,I){f.server||U.log("No server configured for deCarta - we are going to have issues","warn");return q(f.server,v.requestID,escape(I))}function e(v,I,x){sa("making request: "+a(d(v,v.getRequestBody())));$a(b(v,a(d(v,v.getRequestBody()))),function(m){console.log(m.response);var u=m.response.XLS.Response;if(u.numberOfResponses>0&&u[v.methodName+"Response"]){m=[];if(v.parseResponse)m=v.parseResponse(u[v.methodName+"Response"]);I&&I.apply(null,m)}else x&&
x("Server returned no responses",m.response)})}function g(v,I){var x=new B({json:v.StreetAddress}),m=[];if(v.Place){if(!v.Place.length)v.Place=[v.Place];for(var u=v.Place.length;u--;)m[m.length]=v.Place[u].content}return{building:x.building,street:x.street,regions:m,countryCode:v.countryCode||"",postalCode:v.PostalCode||"",pos:I}}function i(v){if(!v)return"";v=v.toUpperCase();if(!j){var I=[],x;for(x in k)I.push(x);j=RegExp("(\\s)("+I.join("|")+")(\\s|$)","i")}j.lastIndex=-1;if(I=j.exec(v))v=v.replace(j,
"$1"+k[I[2]]);return v}var f={sessionID:(new Date).getTime(),server:"",clientName:"",clientPassword:"",configuration:"",maxResponses:25,release:"4.4.2sp03",tileFormat:"PNG",fixedGrid:true,useCache:true,tileHosts:[],requestTimeout:3E4,geocoding:{countryCode:"US",language:"EN"}};TL=function(){function v(x,m){if(typeof x=="number"){this.days=x||0;this.seconds=m||0}else if(typeof x!="undefined"){this.days=x.days||0;this.seconds=x.seconds||0}}v.prototype={add:function(){for(var x=new v(this.days,this.seconds),
m=arguments.length;m--;){x.days+=arguments[m].days;x.seconds+=arguments[m].seconds}return x},toString:function(){var x,m,u,s="";if(this.days)s=this.days+" days ";if(this.seconds){u=this.seconds;if(u>=3600){x=~~(u/3600);u-=x*3600}if(u>=60){m=Math.round(u/60);u-=m*60}if(x)s=s+x+(x>1?" hrs ":" hr ")+(m?(m>10?m:"0"+m)+" min ":"");else if(m)s=s+m+" min";else if(u>0)s=s+(u>10?u:"0"+u)+" sec"}return s}};var I=function(){var x=/^P(\d+Y)?(\d+M)?(\d+D)?$/,m=/^(\d+H)?(\d+M)?(\d+S)?$/,u={8601:function(s){s=s.split("T");
var r=null;r=null;var D=0,z=0;x.lastIndex=-1;r=x.exec(s[0]);D+=r[3]?parseInt(r[3].slice(0,-1),10):0;m.lastIndex=-1;r=m.exec(s[1]);z+=r[1]?parseInt(r[1].slice(0,-1),10)*3600:0;z+=r[2]?parseInt(r[2].slice(0,-1),10)*60:0;z+=r[3]?parseInt(r[3].slice(0,-1),10):0;return new v(D,z)}};return function(s,r){var D=u[r];if(!D)throw"No parser found for the duration format: "+r;return D(s)}}();return{Duration:v,parse:I}}();var l=/^(\d+).*$/,c=/(\d+)\s?\-\s?(\d+)/,j=null,k={RD:"ROAD",ST:"STREET",CR:"CRESCENT",CRES:"CRESCENT",
CT:"COURT",LN:"LANE",HWY:"HIGHWAY",MWY:"MOTORWAY"},h=1,n=ja("<xls:XLS version='1' xls:lang='en' xmlns:xls='http://www.opengis.net/xls' rel='{4}' xmlns:gml='http://www.opengis.net/gml'><xls:RequestHeader clientName='{0}' clientPassword='{1}' sessionID='{2}' configuration='{3}' />{5}</xls:XLS>"),o=ja("<xls:Request maximumResponses='{0}' version='{1}' requestID='{2}' methodName='{3}Request'>{4}</xls:Request>"),q=ja("{0}/JSON?reqID={1}&chunkNo=1&numChunks=1&data={2}&responseFormat=JSON"),B=function(v){v=
U.ex({json:{}},v);var I="",x="";if(v.json.Street)I=v.json.Street.content?v.json.Street.content:v.json.Street;I=I&&I.replace?I.replace(/\/\d+$/,""):"";if(v.json.Building)if(v.json.Building.number)x=v.json.Building.number;return{building:x,street:I,calcMatchPercentage:function(m){var u=0,s=i(m),r=i(I);if(v.json.Building){a:{var D=v.json.Building.number.toString();l.lastIndex=-1;if(l.test(m)){m=m.replace(l,"$1");D=D.split(",");for(var z=0;z<D.length;z++){c.lastIndex=-1;if(c.test(D[z])){var C=c.exec(D[z]);
if(m>=parseInt(C[1],10)&&m<=parseInt(C[2],10)){m=true;break a}}else if(m==D[z]){m=true;break a}}}m=false}if(m)u+=0.2}if(s&&r&&U.wordExists(s,r))u+=0.8;return u},toString:function(){if(I)return(x?x+" ":"")+I;return""}}},H=function(){return{methodName:"",maxResponses:25,version:"1.0",requestID:h++,getRequestBody:function(){return""},parseResponse:function(v){return[v]}}},T=function(){return U.ex(new H,{methodName:"RUOK",parseResponse:function(v){return[{aliasCount:v.maxHostAliases,host:v.hostName}]}})};
