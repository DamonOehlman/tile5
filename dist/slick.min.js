...
function l(t){var z=Array(arguments.length-1),D=0;for(D=1;D<arguments.length;D++)z[D-1]=arguments[D];for(D=F.length;D--;)F[D][t]&&F[D][t].apply(M,z)}function k(t){s=p?j(t.touches):[new SLICK.Vector(t.pageX,t.pageY)];y=new SLICK.Vector;x=new SLICK.Vector;E=true;r=false;if(t.target&&t.target===c.element){h(t);I.current=SLICK.Clock.getTime();if(t=s.length>0?s[0]:null){l("touchStartHandler",t.x,t.y);if(I.current-I.last<M.THRESHOLD_DOUBLETAP)if((t=u?SLICK.V.diff(s[0],u[0]):null)&&Math.abs(t.x)<c.maxDistDoubleTap&&
Math.abs(t.y)<c.maxDistDoubleTap)r=true;B=b.TAP;u=[].concat(s)}else GRUNT.Log.warn("Touch start fired, but no touch vector found")}}function m(t){if(E)if(t.target&&t.target===c.element)try{h(t);var z=p?j(t.touches):[new SLICK.Vector(t.pageX,t.pageY)];t=0;if(z.length>1){if(s.length===1)s=[].concat(z);t=SLICK.VectorMath.distance(s)-SLICK.VectorMath.distance(u)}if(B===b.TAP){var D=o(z,s);if(D&&(Math.abs(D.x)>=g||Math.abs(D.y)>=g))B=b.MOVE}if(B!==b.TAP)if(!t||Math.abs(t)<c.pinchZoomThreshold){if(y=o(z,
u)){x.x+=y.x;x.y+=y.y;A.x+=y.x;A.y+=y.y}if(SLICK.V.absSize(A)>c.panEventThreshhold){l("moveHandler",A.x,A.y);A=SLICK.V.create()}B=b.MOVE}else{l("pinchZoomHandler",i(s),i(z));B=b.PINCHZOOM}u=[].concat(z)}catch(J){GRUNT.Log.exception(J)}}function q(t){if(t.target&&t.target===c.element)try{h(t);SLICK.Clock.getTime();I.last=I.current;if(B===b.TAP)n||(n=setTimeout(function(){n=0;var D=r?"doubleTapHandler":"tapHandler",J=s[0],P=null;if(c.element)P=SLICK.V.offset(J,-c.element.offsetLeft,-c.element.offsetTop);
...
d.getOffset();return new SLICK.Vector(e.x-f.x,e.y-f.y)},viewPixToGridPix:function(e){var f=d.getOffset();return new SLICK.Vector(e.x+f.x,e.y+f.y)}});return d}};return g}();
...
k.gotoPosition(q,15,m)},errorCallback:function(){}})},gotoPosition:function(m,q,r){var n=f,p=(new Date).getTime();f=q?q:f;if(!f)throw"Zoom level required to goto a position.";if(h.provider)f=h.provider.checkZoomLevel(f);if(!g||f!==n){k.removeLayer("grid");SLICK.Resources.resetImageLoadQueue();SLICK.Animation.cancel();g&&k.freeze();d=p;h.provider.zoomLevel=f;h.provider.getMapTiles(k,m,function(s){if(p===d){k.setTileLayer(s);k.panToPosition(m,function(){k.unfreeze();r&&r()})}else GRUNT.Log.info("request time mismatch - ignoring update")});
