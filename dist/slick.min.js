...
e);var c=true,h=null,a=[],d=0,g=[],i=[],b=GRUNT.extend(new SLICK.Graphics.ViewLayer(e),{getAnimation:function(f,j,k,l){if(c)return null;var p="routeAnimation"+o++;i.push(p);return new SLICK.Graphics.AnimatedPathLayer({id:p,path:a,zindex:e.zindex+1,easing:f?f:SLICK.Animation.Easing.Sine.InOut,duration:j?j:5E3,drawIndicator:k,autoCenter:l?l:false})},draw:function(f,j,k,l,p){k=0;l=e.data?e.data.geometry:null;if(c){c=false;h=null;a=[];d=0}if(l&&d<l.length){a:{p=p.getTileLayer();g=[];l=GRUNT.Log.getTraceTicks();
var r,m,q,s=e.data?e.data.geometry:[],u=s.length,x=e.data?e.data.instructions:[],y=x.length,C=e.calculationsPerCycle,z=0;for(r=d;r<u;r++){m=p.getGridXYForPosition(s[r]);if(q=!h||r===0||Math.abs(m.x-h.x)+Math.abs(m.y-h.y)>e.pixelGeneralization){a.push(m);h=m}z++;if(z>=C){d=r;break a}}d=u;GRUNT.Log.trace(u+" geometry points generalized to "+a.length+" coordinates",l);h=null;for(r=y;r--;)if(x[r].position){m=p.getGridXYForPosition(x[r].position);if(q=!h||r===0||Math.abs(m.x-h.x)+Math.abs(m.y-h.y)>e.pixelGeneralization){g.push(m);
h=m}}}k++}p=a.length;if(p>0&&(!k||e.partialDraw)){f.strokeStyle=e.strokeStyle;f.lineWidth=e.lineWidth;f.beginPath();f.moveTo(a[p-1].x-j.x,a[p-1].y-j.y);for(p=p;p--;)f.lineTo(a[p].x-j.x,a[p].y-j.y);f.stroke();f.fillStyle=e.waypointFillStyle;for(p=g.length;p--;){f.beginPath();f.arc(g[p].x-j.x,g[p].y-j.y,2,0,Math.PI*2,false);f.stroke();f.fill()}}return k}});GRUNT.WaterCooler.listen("grid.updated",function(){for(var f=i.length;f--;)GRUNT.WaterCooler.say("layer.remove",{id:i[f]});i=[];c=true;b.wakeParent()});
