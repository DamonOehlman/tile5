...
j.length+" positions");return j},generalizePositions:function(b,f,j){var k=b.length,l=[],p=null;j||(j=250);j/=1E3;GRUNT.Log.info("generalizing positions, must include "+f.length+" positions");for(var r=k;r--;)if(r===0)l.push(b[r]);else if((!p||i.P.inArray(b[r],f)?j:i.P.calcDistance(p,b[r]))>=j){l.push(b[r]);p=b[r]}GRUNT.Log.info("generalized "+k+" positions into "+l.length+" positions");return l},posToMercatorPixels:function(b,f){return new SLICK.Vector(SLICK.Geo.Utilities.lon2pix(b.lon,f),SLICK.Geo.Utilities.lat2pix(b.lat,
f))},mercatorPixelsToPos:function(b,f,j){return new i.Position(SLICK.Geo.Utilities.pix2lat(f,j),SLICK.Geo.Utilities.normalizeLon(SLICK.Geo.Utilities.pix2lon(b,j)))},emptyBounds:function(b){return!b||i.emptyPos(b.min)||i.emptyPos(b.max)},posInBounds:function(b,f){var j=!(i.emptyPos(b)||i.emptyBounds(f));return j=(j=j&&b.lat>=f.min.lat&&b.lat<=f.max.lat)&&b.lon>=f.min.lon&&b.lon<=f.max.lon},calculateBoundsSize:function(b,f,j){var k=new SLICK.Vector(0,f.lat-b.lat);if(typeof j==="undefined")j=true;k.x=
j&&b.lon>f.lon?360-b.lon+f.lon:f.lon-b.lon;return k},getBoundingBoxZoomLevel:function(b,f){var j=b.getCenter();j=e[Math.min(Math.round(Math.abs(j.lat)*0.05),e.length)];var k=i.calculateBoundsSize(b.min,b.max);return Math.min(Math.ceil(Math.log(e[3]*f.height/k.y)/Math.log(2)),Math.ceil(Math.log(j*f.width/k.x)/Math.log(2)))},getBoundsForPositions:function(b,f){var j=null,k=SLICK.Clock.getTime();f||(f="auto");for(var l=b.length;l--;)if(j){var p=i.calculateBoundsSize(j.min,b[l],false),r=i.calculateBoundsSize(b[l],
j.max,false);if(p.x<0)j.min.lon=b[l].lon;if(p.y<0)j.min.lat=b[l].lat;if(r.x<0)j.max.lon=b[l].lon;if(r.y<0)j.max.lat=b[l].lat}else j=new SLICK.Geo.BoundingBox(i.copyPos(b[l]),i.copyPos(b[l]));if(f){if(f=="auto"){l=i.calculateBoundsSize(j.min,j.max);f=Math.max(l.x,l.y)*0.3}j.expand(f)}GRUNT.Log.trace("calculated bounds for "+b.length+" positions",k);return j},normalizeAddress:function(b){if(!b)return"";b=b.toUpperCase();if(!a){var f=[];for(var j in d)f.push(j);a=RegExp("(\\s)("+f.join("|")+")(\\s|$)",
"i")}a.lastIndex=-1;if(f=a.exec(b))b=b.replace(a,"$1"+d[f[2]]);return b},buildingMatch:function(b,f){h.lastIndex=-1;if(h.test(b))for(var j=b.replace(h,"$1"),k=f.split(","),l=0;l<k.length;l++){c.lastIndex=-1;if(c.test(k[l])){var p=c.exec(k[l]);if(j>=parseInt(p[1],10)&&j<=parseInt(p[2],10))return true}else if(j==k[l])return true}return false},rankGeocodeResponses:function(b,f,j){var k=[],l=i.AddressCompareFns;if(j&&j.compareFns)l=GRUNT.extend({},l,j.compareFns);for(j=0;j<f.length;j++)k.push(new i.GeoSearchResult({caption:f[j].toString(),
data:f[j],pos:f[j].pos,matchWeight:n(b,f[j],l,i.GeocodeFieldWeights)}));k.sort(function(p,r){return r.matchWeight-p.matchWeight});return k}};return i}();
...
e.pixelGeneralization){g.push(m);h=m}}GRUNT.Log.trace(y+" instructions generalized to "+g.length+" coordinates",l)}k++}p=a.length;if(p>0&&(!k||e.partialDraw)){f.strokeStyle=e.strokeStyle;f.lineWidth=e.lineWidth;f.beginPath();f.moveTo(a[p-1].x-j.x,a[p-1].y-j.y);for(p=p;p--;)f.lineTo(a[p].x-j.x,a[p].y-j.y);f.stroke();f.fillStyle=e.waypointFillStyle;for(p=g.length;p--;){f.beginPath();f.arc(g[p].x-j.x,g[p].y-j.y,2,0,Math.PI*2,false);f.stroke();f.fill()}}return k}});GRUNT.WaterCooler.listen("grid.updated",
function(){for(var f=i.length;f--;)GRUNT.WaterCooler.say("layer.remove",{id:i[f]});i=[];c=true;b.wakeParent()});return b},Annotation:function(e){e=GRUNT.extend({xy:null,pos:null,draw:null,tweenIn:n.AnnotationTween},e);var c=false,h={xy:e.xy,pos:e.pos,isNew:false,isAnimating:function(){return c},draw:function(a,d,g,i){if(h.xy){if(h.isNew&&e.tweenIn){var b=h.xy.y;h.xy.y=d.y-20;c=true;SLICK.Animation.tween(h.xy,"y",b,e.tweenIn,function(){h.xy.y=b;c=false},250+Math.random()*500)}if(e.draw)e.draw(a,d,
new SLICK.Vector(h.xy.x-d.x,h.xy.y-d.y),g,i);else{a.beginPath();a.arc(h.xy.x-d.x,h.xy.y-d.y,4,0,Math.PI*2,false);a.fill()}h.isNew=false}}};return h},ImageAnnotation:function(e){e=GRUNT.extend({imageUrl:null},e);var c=null;e.draw=function(h,a,d,g,i){if(a=SLICK.Resources.getImage(e.imageUrl)){if(a.complete){c||(c=new SLICK.Vector(-a.width>>1,-a.height>>1));d=SLICK.V.offset(d,c.x,c.y);h.drawImage(a,d.x,d.y,a.width,a.height)}}else SLICK.Resources.loadImage(e.imageUrl,function(){i.wakeParent()})};return new n.Annotation(e)},
AnnotationsOverlay:function(e){function c(i){for(var b=e.map?e.map.getTileLayer():null,f=0;b&&f<i.length;f++)i[f].xy=b.getGridXYForPosition(i[f].pos)}e=GRUNT.extend({pois:null,map:null,createAnnotationForPOI:null,zindex:100},e);var h=[],a=false,d=[],g=GRUNT.extend(new SLICK.Graphics.ViewLayer(e),{draw:function(i,b,f,j){i.save();try{var k;a=false;i.fillStyle="rgba(255, 0, 0, 0.75)";i.globalCompositeOperation="source-over";for(k=h.length;k--;){h[k].draw(i,b,j,g);a=a||h[k].isAnimating()}for(k=d.length;k--;){d[k].draw(i,
b,j,g);a=a||h[k].isAnimating()}a&&g.wakeParent()}finally{i.restore()}},add:function(i){d.push(i);c(d)},isAnimating:function(){return a}});GRUNT.WaterCooler.listen("geo.pois-updated",function(i){if(e.pois&&e.pois.id==i.srcID){i=i.pois;try{h=[];for(var b=0;b<i.length;b++)if(i[b].pos){var f;var j=i[b];if(j&&j.pos){var k=null;if(k=e.createAnnotationForPOI?e.createAnnotationForPOI(j):new n.Annotation({pos:j.pos})){k.isNew=j.isNew;j.isNew=false}f=k}else f=void 0;f&&h.push(f)}c(h)}catch(l){GRUNT.Log.exception(l)}g.wakeParent()}});
GRUNT.WaterCooler.listen("grid.updated",function(){c(h);c(d)});return g},Tiler:function(e){e=GRUNT.extend({tapExtent:10,provider:null,crosshair:true,copyright:undefined,zoomLevel:0,boundsChange:null,tapPOI:null,boundsChangeThreshold:30,pois:new SLICK.Geo.POIStorage,createAnnotationForPOI:null},e);if(typeof e.copyright==="undefined")e.copyright=e.provider?e.provider.getCopyright():"";var c=new SLICK.Vector,h=e.copyright,a=false,d=[],g=0,i=null,b=e.zoomLevel;if(!e.provider)e.provider=new SLICK.Geo.MapProvider;
var f=e.panHandler,j=e.tapHandler;e.onPan=function(p,r){f&&f(p,r)};e.tapHandler=function(p,r){var m=l.getTileLayer(),q=null;if(m){q=l.viewPixToGridPix(new SLICK.Vector(r.x,r.y));var s=m.pixelsToPos(SLICK.V.offset(q,-e.tapExtent,e.tapExtent)),u=m.pixelsToPos(SLICK.V.offset(q,e.tapExtent,-e.tapExtent));GRUNT.Log.info("grid tapped @ "+SLICK.Geo.posToStr(m.pixelsToPos(q)));q=new SLICK.Geo.BoundingBox(s,u);d=l.pois.findByBounds(q);e.tapPOI&&e.tapPOI(d)}j&&j(p,r,q)};e.doubleTapHandler=function(p,r){l.animate(2,
l.getDimensions().getCenter(),new SLICK.Vector(r.x,r.y),SLICK.Animation.Easing.Sine.Out)};e.onScale=function(p,r){var m=0;p=Math.sqrt(p);if(p<1)m=-(0.5/p);else if(p>1)m=p;l.gotoPosition(l.getXYPosition(r),b+Math.floor(m))};var k=new SLICK.Tiling.Tiler(e),l=GRUNT.extend({},k,{pois:e.pois,getBoundingBox:function(){var p=new SLICK.Geo.BoundingBox,r=l.getTileLayer(),m=l.getOffset(),q=l.getDimensions();if(r)p=r.getBoundingBox(m.x,m.y,q.width,q.height);return p},getCenterPosition:function(){return l.getXYPosition(l.gridDimensions.getCenter())},
getXYPosition:function(p){return l.getTileLayer().pixelsToPos(l.viewPixToGridPix(p))},gotoBounds:function(p,r){var m=SLICK.Geo.getBoundingBoxZoomLevel(p,l.getDimensions());l.gotoPosition(p.getCenter(),m,r)},gotoCurrentPosition:function(p){SLICK.Geo.Location.get({successCallback:function(r){l.clearBackground();l.gotoPosition(r,15,p)},errorCallback:function(){}})},gotoPosition:function(p,r,m){var q=b,s=(new Date).getTime();b=r?r:b;if(!b)throw"Zoom level required to goto a position.";if(e.provider)b=
e.provider.checkZoomLevel(b);if(!a||b!==q){SLICK.Resources.resetImageLoadQueue();SLICK.Animation.cancel();a&&l.freeze();g=s;e.provider.zoomLevel=b;e.provider.getMapTiles(l,p,function(u){if(s===g){l.setTileLayer(u);l.panToPosition(p,function(){l.unfreeze();m&&m()})}else GRUNT.Log.info("request time mismatch - ignoring update")});a=true}else l.panToPosition(p,m)},panToPosition:function(p,r,m){var q=l.getTileLayer();if(q){p=q.getGridXYForPosition(p);q=l.getDimensions();p.x-=q.width>>1;p.y-=q.height>>
1;if(i)i=null;l.updateOffset(p.x,p.y,m);GRUNT.WaterCooler.say("view.wake",{id:l.id});e.boundsChange&&e.boundsChange(l.getBoundingBox());r&&r(l)}},setZoomLevel:function(p){l.gotoPosition(l.getCenterPosition(),p)},zoomIn:function(){l.scale(2,SLICK.Animation.Easing.Sine.Out)||l.setZoomLevel(b+1)},zoomOut:function(){l.scale(0.5,SLICK.Animation.Easing.Sine.Out)||l.setZoomLevel(b-1)},animateRoute:function(p,r,m,q){var s=l.getLayer("route");if(s)(p=s.getAnimation(p,r,m,q))&&p.addToView(l)}},k);k=new SLICK.Mapping.AnnotationsOverlay({pois:l.pois,
map:l,createAnnotationForPOI:e.createAnnotationForPOI});l.setLayer("annotations",k);e.crosshair&&l.setLayer("crosshair",new SLICK.Mapping.CrosshairOverlay);h&&l.setLayer("copyright",new SLICK.Graphics.ViewLayer({zindex:999,draw:function(p,r,m){p.lineWidth=2.5;p.fillStyle="rgb(50, 50, 50)";p.strokeStyle="rgba(255, 255, 255, 0.8)";p.font="bold 10px sans";p.textBaseline="bottom";p.strokeText(h,10,m.height-10);p.fillText(h,10,m.height-10)}}));GRUNT.WaterCooler.listen("view-idle",function(p){if(p.id&&
p.id==l.id)if(SLICK.V.absSize(SLICK.V.diff(c,l.getOffset()))>e.boundsChangeThreshold&&e.boundsChange){c=l.getOffset();e.boundsChange(l.getBoundingBox())}});return l}};return n}();
