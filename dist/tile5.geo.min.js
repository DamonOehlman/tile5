...
v+": "+l[v])}})})();
(function(){function P(L,A){var k=L.obsHandlers;k[A]||(k[A]=[]);return k[A]}var ha=0;COG.observable=function(L){if(!L)return null;if(!L.obsHandlers)L.obsHandlers={};if(!(L.bind||L.trigger||L.unbind)){L.bind=function(A,k){var l="callback"+ha++;P(L,A).unshift({fn:k,id:l});return l};L.triggerCustom=function(A,k){var l=P(L,A),v={cancel:false,name:A,source:this},D;k&&COG.extend(v,k);if(!l)return null;D=Array.prototype.slice.call(arguments,2);L.eventInterceptor&&L.eventInterceptor(A,v,D);D.unshift(v);for(var U=
