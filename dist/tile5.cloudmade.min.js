(function(){function J(){var a=arguments[0]||{},d=Array.prototype.slice.call(arguments,1),b=d.length,e,h;for(h=0;h<b;h++)if((e=d[h])!==null)for(var k in e){var g=e[k];if(a!==g)if(g!==undefined)a[k]=g}return a}function wa(a,d){if(typeof console!=="undefined")console[d||"log"](a)}function xa(a){var d=a.match(Bb),b=[],e=0,h;for(h=d?d.length:0;h--;){var k=d[h].slice(1);b[k]||(b[k]=RegExp("\\{"+k+"\\}","g"))}e=b.length;return function(){var g=a;for(h=0;h<e;h++){var i=arguments[h];if(typeof i=="undefined")i=
"";g=g.replace(b[h],i)}return g}}function kb(a,d,b){da.trigger("userMessage",a,d,b)}function lb(a,d){var b=Pa*ea(a);return mb-2*nb(d*ya((1-b)/(1+b),Pa/2))}function V(a,d){if(R(a,qa)){var b=a.split(ob);this.x=parseFloat(b[0]);this.y=parseFloat(b[1])}else if(a&&a.x){this.x=a.x;this.y=a.y}else{this.x=a||0;this.y=d||0}}function Fa(a){this.allowCull=a}function ia(a,d,b,e){this.mercX=b;this.mercY=e;if(R(a,qa))J(this,Cb.parseXY(a));else a&&a.toBounds?J(this,Qa(a.lon,a.lat)):V.call(this,a,d)}function ra(a,
...
2)+l},elasticin:function(c,l,j,f,n,m){if(c==0)return l;if((c/=f)==1)return l+j;m||(m=f*0.3);if(!n||n<b(j)){n=j;j=m/4}else j=m/fa*k(j/n);return-(n*e(2,10*(c-=1))*h((c*f-j)*fa/m))+l},elasticout:function(c,l,j,f,n,m){var p;if(c==0)return l;if((c/=f)==1)return l+j;m||(m=f*0.3);if(!n||n<b(j)){n=j;p=m/4}else p=m/fa*k(j/n);return n*e(2,-10*c)*h((c*f-p)*fa/m)+j+l},elasticinout:function(c,l,j,f,n,m){var p;if(c==0)return l;if((c/=f/2)==2)return l+j;m||(m=f*0.3*1.5);if(!n||n<b(j)){n=j;p=m/4}else p=m/fa*k(j/
n);if(c<1)return-0.5*n*e(2,10*(c-=1))*h((c*f-p)*fa/m)+l;return n*e(2,-10*(c-=1))*h((c*f-p)*fa/m)*0.5+j+l},quadin:function(c,l,j,f){return j*(c/=f)*c+l},quadout:function(c,l,j,f){return-j*(c/=f)*(c-2)+l},quadinout:function(c,l,j,f){if((c/=f/2)<1)return j/2*c*c+l;return-j/2*(--c*(c-2)-1)+l},sinein:function(c,l,j,f){return-j*g(c/f*d)+j+l},sineout:function(c,l,j,f){return j*h(c/f*d)+l},sineinout:function(c,l,j,f){return-j/2*(g(Math.PI*c/f)-1)+l}};return function(c){c=c.replace(/[\-\_\s\.]/g,"").toLowerCase();
return i[c]||i.linear}}(),sa=function(){function a(b,e){var h=b.hasOwnProperty("obsHandlers")?b.obsHandlers:null;h[e]||(h[e]=[]);return h[e]}var d=0;return function(b){if(!b)return null;if(!(b.hasOwnProperty("obsHandlers")?b.obsHandlers:null))b.obsHandlers={};if(!b.hasOwnProperty("bind")){b.bind=function(e,h){var k="callback"+d++;a(b,e).unshift({fn:h,id:k});return k};b.triggerCustom=function(e,h){var k=a(b,e),g={cancel:false,name:e,source:this},i;for(i in h)g[i]=h[i];if(!k)return null;i=Array.prototype.slice.call(arguments,
...
z.wheelDeltaY:z.wheelDelta;if(window.opera)H=-H}if(H){var I=e(F,N);m.triggerCustom("zoom",g("mouse",z),I,c(I,k(n)),H/t,"wheel");l(z);z.returnValue=false}}}function v(z){z=z||window.event;return(z.which||z.button)==1}function o(z,H,I,T,ka){I=typeof I!="undefined"?I:F;T=typeof T!="undefined"?T:N;var D=I-M,O=T-P,G=e(I,T);m.triggerCustom(H,g("mouse",z),G,c(G,k(n)),e(D,O));if(typeof ka=="undefined"||ka){M=I;P=T}}p=J({},p);var u=120,t=u*8,y=false,C,F,N,M,P;p.binder("mousedown",A);p.binder("mousemove",B);
p.binder("mouseup",K);p.binder("dblclick",function(z){wa("captured double click");if(i(z,n)){var H=w(z);m.triggerCustom("doubleTap",g("mouse",z),H,c(H,k(n)))}});p.binder("mousewheel",q);p.binder("DOMMouseScroll",q);return{unbind:function(){p.unbinder("mousedown",A);p.unbinder("mousemove",B);p.unbinder("mouseup",K);p.unbinder("mousewheel",q);p.unbinder("DOMMouseScroll",q)}}},checks:{touch:false}});h("pointer",{handler:function(n,m,p){function w(D){if(D.count<2)return 0;var O=D.x-D.next.x;D=D.y-D.next.y;
...
y;m.triggerCustom("pointerDown",g("touch",D),O,G)}T&&m.triggerCustom("pointerDownMulti",g("touch",D),O,G);P=B(D);if(P.count>1)H=w(P);ka=1;I=A(P)}}function q(D){if(i(D,n)){l(D);z=B(D);if(M==y)M=Math.abs(P.x-z.x)>u||Math.abs(P.y-z.y)>u?t:y;if(M!=y){M=z.count>1?F:C;if(M==F)if(P.count===1){P=A(z);H=w(P)}else{var O=w(z);if(Math.abs(H-O)<o)M=C;else{var G;G=z.x;var U=z.next.x,ba=z.y,S=z.next.y;G={x:(G<U?G:U)+(Math.abs(G-U)>>1),y:(ba<S?ba:S)+(Math.abs(ba-S)>>1)};O=O/H;U=O-ka;m.triggerCustom("zoom",g("touch",
D),G,c(G,N),U,"pinch");ka=O}}M==C&&m.triggerCustom("pointerMove",g("touch",D),z,A(z,N.left,N.top),e(z.x-I.x,z.y-I.y));T&&m.triggerCustom("pointerMoveMulti",g("touch",D),z,A(z,N.left,N.top))}I=A(z)}}function v(D){if(i(D,n)){var O=B(D,"changedTouches"),G=A(O,N.left,N.top);z=B(D);if(!z){m.triggerCustom("pointerUp",g("touch",D),O,G);P=null}T&&m.triggerCustom("pointerUpMulti",g("touch",D),O,G)}}p=J({detailed:false,inertia:false},p);var o=20,u=7,t=0,y=1,C=2,F=3,N,M,P,z,H,I,T=p.detailed,ka=1;p.binder("touchstart",
K);p.binder("touchmove",q);p.binder("touchend",v);wa("initialized touch handler");return{unbind:function(){p.unbinder("touchstart",K);p.unbinder("touchmove",q);p.unbinder("touchend",v)}}},checks:{touch:true}});return{register:h,watch:function(n,m,p){m=J({bindTarget:null,observable:null,isIE:typeof window.attachEvent!="undefined",types:null},m);capabilities=J({touch:"ontouchstart"in window},p);if(!m.observable){wa("creating observable");m.observable=sa({});globalOpts=m}m.binder=(m.isIE?b:a)(m.bindTarget||
document);m.unbinder=(m.isIE?b:d)(m.bindTarget||document);p=m.types;for(var w=capabilities,A=[],B=j.length;B--;){var K=j[B],q=!p||p.indexOf(K.type)>=0,v=true,o;for(o in K.checks){var u=K.checks[o];wa("checking "+o+" capability. require: "+u+", capability = "+w[o]);v=v&&u===w[o]}if(q&&v)A[A.length]=K.handler}return new f(n,A,m)}}}();(function(){function a(q,v){if(q&&q.split){var o=q.split(B);if(o.length>1){q=o[0];v=o[1]}}else if(q&&q.lat){v=q.lon;q=q.lat}this.lat=parseFloat(q||0);this.lon=parseFloat(v||
...
K})();var da=this.T5=sa({}),Ua=function(){var a={};return{create:function(d,b){if(a[d][b])return a[d][b].apply(null,Array.prototype.slice.call(arguments,2));throw Gb(d,b);},get:function(d,b){return a[d]?a[d][b]:null},register:function(d,b,e){a[d]||(a[d]={});a[d][b]&&wa(Hb(d,b),"warn");a[d][b]=e}}}(),Hb=xa("Registration of {0}: {1} will override existing definition"),Gb=xa("Could not create {0} of type: {1}");xa("Could not create drawable of type: {0}");var fa=Math.PI*2,mb=Math.PI/2,ma=Math.PI/180,
Va=180/Math.PI,Pa=0.08181919084262157,na=Math.abs,Wa=Math.round,db=Math.min,Ga=Math.max,ya=Math.pow,cb=Math.sqrt,eb=Math.log,ea=Math.sin,sb=Math.asin,ta=Math.cos,nb=Math.atan,Fb=Math.atan2,za="undefined",qa="string",Ib="object",ua="number",Jb="array",ja=Ua.register,Ba=Ua.create,Kb=Ua.get,Lb=0,Db=0,ob=/[\,\s]+/,Ca=function(){function a(g){h++;g=Q?window.mozAnimationStartTime||g||(new Date).getTime():(new Date).getTime();for(var i=e.length;i--;){var c=e[i];h%c.every===0&&c.cb(g)}k&&animFrame(a)}var d=
...
"undefined"?function(){function a(h){for(var k=0;k<h.length;k++){var g=h[k];if(typeof document.body.style[g]!="undefined")return g}}var d={"-webkit-user-select":"none",position:"absolute"},b,e;b=a(["-webkit-transform","MozTransform"]);css3DTransformProp=a(["WebkitPerspective","MozPerspective"]);e=a(["-webkit-transform-origin","MozTransformOrigin"]);return{transforms:R(b,qa),create:function(h,k,g){h=document.createElement(h);var i=[];g=g||{};h.className=k||"";for(var c in g)i[i.length]=c+": "+g[c];
h.style.cssText=i.join(";");return h},move:function(h,k,g,i,c){if(b){h.style[b]="translate("+k+"px, "+g+"px) "+(i||[]).join(" ");if(c&&e)h.style[e]=c.x+"px "+c.y+"px"}else{h.style.left=k+"px";h.style.top=g+"px"}},styles:function(h){return J({},d,h)}}}():null,tb=function(){function a(){for(var h=e.length,k=h;k--;)try{e[k](h)}catch(g){wa(g.toString(),"error")}}var d=20,b=50,e=[];return{process:function(h,k,g,i){function c(f){f=(l||b)/f;if(j<h.length){var n=h.slice(j,j+f);f=n.length;var m=l?0:(new Date).getTime();
k(n,f);if(!l)l=(n=((new Date).getTime()-m)/f)?d/n|0:h.length;j+=f}else{for(f=e.length;f--;)if(e[f]===c){e.splice(f,1);break}e.length===0&&Ca.detach(a);g&&g()}}var l,j=0;if(Q&&h.length>(i||0))e.push(c)===1&&Ca.attach(a);else{k(h,h.length);g&&g()}}}}(),Qa=Qa||function(a,d){var b=parseFloat(d)*ma;b=ea(b);var e=Pa*b;b=eb((1+b)/(1-b)*ya((1-e)/(1+e),Pa))/2;return new ia(0,0,parseFloat(a)*ma,b)},fb=fb||function(a,d){for(var b=ya(Math.E,-d),e=mb-2*nb(b),h=lb(e,b),k=0;k<12&&na(e-h)>1.0E-7;){e=h;h=lb(e,b);
...
0);var b=this.x-d.x,e=this.y-d.y;return new V(d.x+ta(a)*b+-ea(a)*e,d.y+ea(a)*b+ta(a)*e)},scale:function(a,d){d=d||new V(0,0);return new V(d.x+(this.x-d.x)*a,d.y+(this.y-d.y)*a)},sync:function(){return this},toString:function(){return this.x+", "+this.y}};Fa.prototype=J([],{cull:function(a){if(this.allowCull){var d=a.x,b=a.y,e=a.x+a.w;a=a.y+a.h;for(var h=Infinity,k=0,g,i=this.length;i--;)if(g=this[i].x>=d&&this[i].x<=e&&this[i].y>=b&&this[i].y<=a){h=i<h?i:h;k=i>k?i:k}return this.slice(Ga(h-1,0),db(k+
1,this.length))}return this},simplify:function(a){a=a||3;for(var d=new Fa(this.allowCull),b=null,e=this.length;e--;){var h=this[e];if(include=!b||e===0||na(h.x-b.x)+na(h.y-b.y)>a){d.unshift(h);b=h}}return d}});ia.prototype=J(new V,{constructor:ia,pos:function(){return fb(this.mercX,this.mercY)},sync:function(a,d){var b;if(!(b=a.rpp)){b=a.zoom();b=fa/(256<<b)}b=b;if(d){this.mercX=this.x*b-Math.PI;this.mercY=Math.PI-this.y*b}else{this.x=Wa(((this.mercX||0)+Math.PI)/b);this.y=Wa((Math.PI-(this.mercY||
0))/b)}return this}});ra.prototype={constructor:ra,buffer:function(a,d){return new ra(this.x-a,this.y-(d||a),this.w+a*2,this.h+(d||a)*2)},center:function(){return new V(this.x+(this.w>>1),this.y+(this.h>>1))}};Hits=function(){return{diffHits:function(a,d){var b=[],e={},h;for(h=d.length;h--;)e[d[h].target.id]=true;for(h=a.length;h--;)e[a[h].target.id]||(b[b.length]=a[h]);return b},init:function(a,d,b,e,h){return{type:a,x:h.x,y:h.y,gridX:e.x|0,gridY:e.y|0,elements:[],absXY:d,relXY:b}},initHit:function(a,
...
d=0;(new Date).getTime();var b={},e=[];Ca.attach(function(h){h=h||(new Date).getTime();for(var k=0;k<e.length;){var g=e[k],i=b[g],c=b[g].image&&b[g].image.complete&&b[g].image.width>0,l=h-i.start;l=c||l>=3E4;if(c){c=i.callbacks;a[g]=i.image;for(var j=0;j<c.length;j++)c[j](i.image,true)}if(l){e.splice(k,1);delete b[g]}else k++}},250);return function(h,k){var g=h&&k?a[h]:null;if(g&&g&&g.complete&&g.width>0)k(g);else if(g=b[h])g.callbacks.push(k);else{g=new Image;g.id="_ldimg"+ ++d;b[h]={start:(new Date).getTime(),
image:g,callbacks:[k]};g.src=h;e[e.length]=h}}}();Ra.prototype={constructor:Ra,load:function(a){var d=this;Xa(this.url,function(b){d.loaded=true;d.image=b;a&&a()})}};var Mb=function(){return sa({fastpan:true,applyStyle:function(){},applyTransform:function(a,d,b){return{restore:null,x:d,y:b}},getDimensions:function(){return{width:0,height:0}},getViewport:function(){},hitTest:function(){return false},prepare:function(){},projectXY:null,reset:function(){}})};ja("renderer","canvas",function(a,d,b,e,h){function k(){function o(y,
...
C);f.lineTo(y-(F>>1),C-F);f.lineTo(y+(F>>1),C-F);f.lineTo(y,C)}return i(u,t,N)},prepPoly:function(o,u,t,y){var C=true;o=y.points||o.points().cull(u);y=w?w.x:n;var F=w?w.y:m;f.beginPath();for(var N=o.length;N--;){var M=o[N].x-y,P=o[N].y-F;if(C){f.moveTo(M,P);C=false}else f.lineTo(M,P)}return i(u,t)},getContext:function(){return f}});(function(){ub.each(function(o,u){g(null,o,u)});da.bind("styleDefined",g)})();b.bind("detach",function(){d.removeChild(j)});b.bind("resize",function(){});return b});ja("renderer",
...
null))&&Q.move(m,j.x-p,j.y-w);l[j.id]=j}}}});b.bind("predraw",function(){g(c,l);l={}});b.bind("detach",function(){d.removeChild(i)});a.bind("reset",function(){for(g(c,l={});i.childNodes.length>0;)i.removeChild(i.childNodes[0])});return b});var ub=function(){function a(b,e){if(R(b,qa)){da.trigger("styleDefined",b,d[b]=e);return b}else{var h=[],k;for(k in b)h[h.length]=a(k,b[k]);return h}}var d={};a({reset:{fill:"#ffffff",opacity:1},highlight:{fill:"#ff0000"},waypoints:{lineWidth:4,stroke:"#003377",
fill:"#ffffff"},waypointsHover:{lineWidth:4,stroke:"#ff0000",fill:"#ffffff"}});return{resetStyle:"reset",each:function(b){for(var e in d)b(e,d[e])},get:function(b){return d[b]},define:a}}(),Ab=function(a,d){function b(r,s,x,E){r=C?Ga(W+ya(2,E)-1,0.125):E>0?2:0.5;o(r,false,true)}function e(r,s){var x=X&&X.projectXY?X.projectXY(r,s):null;if(!x){x=K();var E=1/W;x=new L.XY(x?x.x+r*E:r,x?x.y+s*E:s)}return x.sync(L,true)}function h(r,s,x){u("doubleTap",s,x,e(x.x,x.y));d.scalable&&o(2,{easing:"quad.out",
duration:300},true)}function k(r,s,x){T=null;O=true;A("down",s,x)}function g(r,s,x){A("hover",s,x)}function i(r,s,x,E){m(s,x,false);if(!T){G=E.x;U=E.y}}function c(r,s,x){m(s,x,true);O=false}function l(){clearTimeout(vb);vb=setTimeout(function(){if(I)if(I.offsetWidth!==Ha*2||I.offsetHeight!==Ia*2){var r=B();w(a);B(r.x,r.y)}},250)}function j(r,s,x){A("tap",s,x);L.trigger("tap",s,x,e(x.x,x.y,true))}function f(){ga&&ga.unbind();if(Q&&X){ga=INTERACT.watch(X.interactTarget||I);if(d.scalable){ga.bind("zoom",
b);ga.bind("doubleTap",h)}ga.bind("pointerDown",k);ga.bind("pointerMove",i);ga.bind("pointerUp",c);d.captureHover&&ga.bind("pointerHover",g);ga.bind("tap",j)}}function n(r){if(X){X.trigger("detach");X=null}var s=L,x=z,E=I,Z=d;r=r.split("/");for(var $=new Mb(s,x,E,Z),oa=0;oa<r.length;oa++)$=Ba("renderer",r[oa],s,x,E,Z,$);X=$;D=Q&&X.fastpan&&Q.transforms;C=Q&&Q.transforms&&d.useTransforms;f();L.trigger("reset");v()}function m(r,s,x){if(T){r=e(s.x,s.y);T.drag.call(T.target,T,r.x,r.y,x)&&aa++;if(x)T=
null}}function p(r){for(var s=P;s--;)if(M[s].id===r)return s;return P}function w(r){if(Q){I=document.getElementById(r);I.appendChild(H=Q.create("div","",Q.styles({overflow:"hidden",width:I.offsetWidth+"px",height:I.offsetHeight+"px"})));r=d.padding;if(r===y){r=I.offsetWidth;var s=I.offsetHeight,x=cb(r*r+s*s);ha=new V(x-r>>1,x-s>>1)}else ha=new V(r,r);Ya=H.offsetWidth+ha.x*2;Za=H.offsetHeight+ha.y*2;gb=Ya/2;hb=Za/2;Ha=I.offsetWidth/2;Ia=I.offsetHeight/2;Ja=new V(gb,hb);H.appendChild(z=Q.create("div",
"",Q.styles({width:Ya+"px",height:Za+"px","z-index":2,margin:-ha.y+"px 0 0 "+-ha.x+"px"})));n(d.renderer);r=d.controls;for(s=0;s<N.length;s++)N[s].trigger("detach");N=[];for(s=0;s<r.length;s++)N[N.length]=Ba("control",r[s],L,H,I,d[r[s]])}else n("canvas")}function A(r,s,x){var E=(new V(x.x-Ha+gb,x.y-Ia+hb)).rotate(-la*ma,Ja).scale(1/W,Ja);ca=Hits.init(r,s,x,e(x.x,x.y,true),E);for(r=P;r--;)if(M[r].visible)ka=ka||(M[r].hitGuess?M[r].hitGuess(ca.gridX,ca.gridY,L):false);ka&&aa++}function B(r,s,x){if(R(r,
qa)||R(r,Ib)){s=new L.XY(r);s.sync(L);r=s.x;s=s.y}if(R(r,ua)){t(r-Ha-ha.x,s-Ia-ha.y,x);return L}else return t().offset(Ha+ha.x|0,Ia+ha.y|0).sync(L,true)}function K(){var r=new ra(S,Y,Ya,Za);r.scaleFactor=W;r.padding=ha?ha.copy():new V;return r}function q(r,s,x){if(R(r,qa)&&R(s,za))for(s=0;s<P;s++){if(M[s].id===r)return M[s]}else if(R(r,qa)){s=Ba("layer",s,L,x);s.added=(new Date).getTime();s.id=r;M[p(r)]=s;M.sort(function(E,Z){return Z.zindex-E.zindex||Z.added-E.added});P=M.length;L.trigger("resync");
v();L.trigger("layerChange",L,s);aa++;return s}else return[].concat(M)}function v(){var r=K();if(r){if(Da||Ea)if(r){var s=$a?S+(r.w>>1):S,x=ab?Y+(r.h>>1):Y,E=r.w,Z=r.h;if(Da&&Da>E)if(s+E>Da)S=$a?S:Da-E;else if(s<0)S=$a?S:0;if(Ea&&Ea>Z)if(x+Z>Ea)Y=ab?Y:Ea-Z;else if(x<0)Y=ab?Y:0}wb=S;xb=Y;L.trigger("refresh",L,r);aa++}}function o(r,s,x){if(R(r,ua)){var E=x?r:W*r;if(!Nb){s=null;r=Wa(eb(E)/Math.LN2);E=ya(2,r)}if(s)Ka=pa.tween([W],[E],s,function(){W=E;Ka=null;aa++});else{W=E;aa++}return L}else return W}
function u(){for(var r=L.trigger.apply(null,arguments).cancel,s=M.length;s--;)r=M[s].trigger.apply(null,arguments).cancel||r;return!r}function t(r,s,x){if(R(r,ua)){if(x)va=pa.tween([S,Y],[r,s],x,function(){va=null});else{S=r|0;Y=s|0;aa++}return L}else return(new L.XY(S,Y)).sync(L,true)}d=J({captureHover:true,controls:[],drawOnScale:true,padding:128,inertia:true,refreshDistance:128,pannable:true,scalable:true,renderer:"canvas",useTransforms:true},d);var y="auto",C=true,F=false,N=[],M=[],P=0,z=null,
H=null,I,T=null,ka=false,D,O=false,G=0,U=0,ba=d.refreshDistance,S=0,Y=0,La=0,Ma=0,wb=0,xb=0,Da=null,Ea=null,$a=false,ab=false,va=null,ha,ca=null,Na=null,X,vb=0,Ja=new V,la=0,Oa=null,W=1,Ka=null,ib=1,ga=null,jb={index:0,draw:false},Nb=true,aa=0,Ya,Za,gb,hb,Ha,Ia,L={XY:V,id:d.id,panSpeed:0,attachFrame:function(r,s){s&&z.appendChild(r)},center:B,detach:function(){X&&X.trigger("detach");ga&&ga.unbind();if(H){I.removeChild(H);z=H=null}},frozen:function(r){if(R(r,za))return F;else{F=r;return L}},layer:q,
invalidate:function(){aa++},pan:function(r,s,x){t(S+r,Y+s,x)},refresh:v,rotate:function(r,s,x){if(R(r,ua)){var E=x?r:la+r;if(s)Oa=pa.tween([la],[E],s,function(){la=E%360;Oa=null;aa++});else{la=E%360;aa++}return L}else return la},scale:o,triggerAll:u,setMaxOffset:function(r,s,x,E){Da=r;Ea=s;$a=x;ab=E},offset:t,viewport:K};sa(L);L.bind("resize",l);L.bind("layerRemove",function(r,s){var x=bb(M,s.id);if(x>=0&&x<P){M.splice(x,1);aa++}P=M.length});rb(L,d,{container:w,captureHover:f,scalable:f,pannable:f,
renderer:n});q("markers","draw",{zindex:20});w(a);if(Q&&!R(window.attachEvent,za))window.attachEvent("onresize",l);else Q&&window.addEventListener("resize",l,false);Ca.attach(function(r){var s=[],x,E,Z;if(!F){L.panSpeed=panSpeed=na(G)+na(U);(x=W!==ib)&&L.trigger("scale");if(panSpeed>0||x||va||Ka||Oa){aa++;if(va&&panSpeed>0){va(true);va=null}}if(panSpeed<2&&(na(S-wb)>=ba||na(Y-xb)>=ba))v();jb.index++;jb.draw=aa||panSpeed||0;if(X&&jb.draw){if(Ka)W=Ka()[0];if(Oa)la=Oa()[0];La+=G;Ma+=U;if(G||U)L.trigger("pan");
if(C){if(W!==1)s[s.length]="scale("+W+")";if(la!==0)s[s.length]="rotate("+la+"deg)"}if(x=!D||(d.drawOnScale||W===1)&&panSpeed<5){if(va){x=va();S=x[0]|0;Y=x[1]|0}else{E=-la*ma;x=ta(E)*La+-ea(E)*Ma;E=ea(E)*La+ta(E)*Ma;S=S-x/W|0;Y=Y-E/W|0}Z=K();X.trigger("predraw",Z);if(X.prepare(M,Z,r,ca)){E=Ma=x=La=aa=0;for(ii=P;ii--;){var $=M[ii];if($.visible){var oa=$.style?X.applyStyle($.style,true):null;$.draw(X,Z,L,r,ca);oa&&X.applyStyle(oa)}}X.trigger("render",Z);L.trigger("drawComplete",Z,r);Q.move(z,x,E,s,
Ja)}}else Q.move(z,La,Ma,s,Ja);if(O||!d.inertia)U=G=0;else if(G!=0||U!=0){G*=0.8;U*=0.8;if(na(G)<0.5)G=0;if(na(U)<0.5)U=0}if(ca){r=true;s=ca?ca.elements:[];if(Na&&Na.type==="hover"){diffElements=Hits.diffHits(Na.elements,s);if(diffElements.length>0)Hits.triggerEvent(Na,L,"Out",diffElements);else r=false}if(s.length>0){E=ca.gridX;Z=ca.gridY;for(x=s.length;x--;){$=s[x];oa=E;var yb=Z,zb=$&&$.drag&&(!$.canDrag||$.canDrag($,oa,yb));if(zb){T=$;T.startX=oa;T.startY=yb}if(zb)break}r&&Hits.triggerEvent(ca,
L)}Na=s.length>0?J({},ca):null;ca=null}if(ib!==W){L.trigger("scaleChanged",W);ib=W}}}});return L},pa=function(){return{tween:function(a,d,b,e,h){function k(p){var w=p-m;p=m+n<=p;for(var A=g;A--;)i[A]=l(w,a[A],j[A],n);h&&h.invalidate();if(p||f){Ca.detach(k);for(A=0;A<c.length;A++)c[A]&&c[A](i,w,f)}}b=J({easing:"sine.out",duration:1E3,complete:null},b);var g=a.length,i=[].concat(a),c=[e,b.complete],l=Eb(b.easing),j=[],f=false,n=b.duration,m=(new Date).getTime();for(b=g;b--;)j[b]=d[b]-a[b];Ca.attach(k);
return function(p){f=p;return i}},tweenDrawable:function(a,d,b,e,h){var k=pa.tween([b],[e],h,function(){a[d]=e;for(var i=a.tweens.length;i--;)if(a.tweens[i]===g){a.tweens.splice(i,1);break}},a.view),g=function(){a[d]=k()[0]};return g}}}(),Aa=function(a,d,b){b=J({style:null,xy:null,size:20,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape",zindex:0},b);J(this,b);if(R(this.xy,qa))this.xy=new a.XY(this.xy);this.id="drawable_"+Lb++;this.bounds=null;this.view=a;this.layer=
d;this.tweens=[];this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.observable&&sa(this)};Aa.prototype={constructor:Aa,applyTweens:function(){for(var a=this.tweens.length;a--;)this.tweens[a]()},drag:null,draw:null,getProps:null,resync:function(){if(this.xy){this.xy.sync(this.view);if(this.size){var a=this.size>>1;this.updateBounds(new ra(this.xy.x-a,this.xy.y-a,this.size,this.size))}}},rotate:function(a,d,b){if(R(a,ua)){a=(b?a:this.rotation*Va+a)*ma;if(d)this.tweens.push(pa.tweenDrawable(this,
"rotation",this.rotation,a,d));else{this.rotation=a;this.view.invalidate()}return this}else return this.rotation*Va},scale:function(a,d,b){if(R(a,ua)){a=b?a:this.scaling*a;if(d)this.tweens.push(pa.tweenDrawable(this,"scaling",this.scaling,a,d));else{this.scaling=a;this.view.invalidate()}return this}else return this.scaling},translate:function(a,d,b,e){if(R(a,ua)){a=e?a:this.translateX+a;d=e?d:this.translateY+d;if(b){this.tweens.push(pa.tweenDrawable(this,"translateX",this.translateX,a,b));this.tweens.push(pa.tweenDrawable(this,
"translateY",this.translateY,d,b))}else{this.translateX=a;this.translateY=d;this.view.invalidate()}return this}else return new V(this.translateX,this.translateY)},updateBounds:function(a,d){a&&(!this.bounds||a.x!=this.bounds.x||a.y!=this.bounds.y)&&this.trigger("move",this,a,this.bounds);this.bounds=a;if(d)this.xy=this.bounds.center()}};ja("drawable","marker",function(a,d,b){b=J({fill:true,stroke:false,markerType:"simple",hoverStyle:"highlight",typeName:"Marker"},b);return new Aa(a,d,b)});ja("drawable",
"poly",function(a,d,b){function e(){var j,f,n,m,p,w,A;c=b.simplify?i.simplify():i;for(j=c.length;j--;){f=c[j].x;n=c[j].y;w=R(w,za)||f<w?f:w;A=R(A,za)||n<A?n:A;m=R(m,za)||f>m?f:m;p=R(p,za)||n>p?n:p}l.updateBounds(new ra(w,A,m-w,p-A),true);a.invalidate()}function h(j){if(R(j,Jb)){i=new Fa(b.allowCull);tb.process(j,function(f,n){for(var m=0;m<n;m++)i.push(new a.XY(f[m]))},k,g);return l}else return c}function k(){i.length&&tb.process(i,function(j,f){for(var n=f;n--;)j[n].sync(a)},e,g)}b=J({allowCull:false,
simplify:false,fill:true,points:[],typeName:"Poly"},b);var g=500,i=new Fa(b.allowCull),c,l=J(new Aa(a,d,b),{points:h,resync:k});h(b.points);return l});ja("drawable","line",function(a,d,b){b.fill=false;b.allowCull=true;return Ba("drawable","poly",a,d,b)});ja("drawable","image",function(a,d,b){function e(){if(c&&c.width>0){if(!this.centerOffset)this.centerOffset=new V(-c.width>>1,-c.height>>1);this.updateBounds(new ra(this.xy.x+this.centerOffset.x,this.xy.y+this.centerOffset.y,c.width,c.height),false)}}
function h(j){if(this.imageUrl=j){var f=this;Xa(this.imageUrl,function(n,m){c=n;if(m){var p=l.layer?l.layer.view:null;p&&p.invalidate()}e.apply(f)})}}b=J({image:null,imageUrl:null,centerOffset:null,typeName:"Image"},b);var k=Aa.prototype.resync,g=0,i=0,c=b.image,l=J(new Aa(a,d,b),{changeImage:h,getProps:function(){this.bounds||e(this,c);return{image:c,x:this.xy.x+g,y:this.xy.y+i}},resync:function(j){k.call(this,j);e.call(this)}});c||h.call(this,this.imageUrl);if(this.centerOffset){g=this.centerOffset.x;
i=this.centerOffset.y}});ja("drawable","arc",function(a,d,b){b=J({startAngle:0,endAngle:Math.PI*2,typeName:"Arc"},b);return new Aa(a,d,b)});Sa.prototype={constructor:Sa,clip:null,cycle:function(){},draw:function(){},hitGuess:null,remove:function(){this.view.trigger("layerRemove",this)}};ja("layer","tile",function(a,d){d=J({generator:"osm",imageLoadArgs:{}},d);var b=Ba("generator",d.generator,d).run,e=null,h=[],k=J(new Sa(a,d),{draw:function(g,i,c){g.drawTiles&&g.drawTiles(i,e.search(i.buffer(128)),
c.panSpeed<2)}});a.bind("resync",function(){var g=a&&a.zoom?a.zoom():0;h[g]||(h[g]=pb(g));e=h[g]});a.bind("refresh",function(g,i,c){e&&b(i,c,e,function(){i.invalidate()})});return k});ja("layer","draw",function(a,d){function b(i,c,l,j){var f=this.dragOffset;if(!f)f=this.dragOffset=new a.XY(i.startX-this.xy.x,i.startY-this.xy.y);this.xy.x=c-f.x;this.xy.y=l-f.y;if(j){delete this.dragOffset;a.invalidate();this.xy.sync(a,true);this.trigger("dragDrop")}return true}function e(i,c,l,j){j&&k.remove(j,c);
k.insert(l,c)}d=J({zindex:10},d);var h=[],k,g=J(new Sa(a,d),{itemCount:0,clear:function(){k=new qb;h=[];g.trigger("cleared");g.itemCount=0;a.invalidate()},create:function(i,c,l){c=Ba("drawable",i,a,g,c);if(l)h.unshift(c);else h[h.length]=c;c.resync();k&&c.bounds&&k.insert(c.bounds,c);c.bind("move",e);g.itemCount=h.length;g.trigger(i+"Added",c);return c},draw:function(i,c,l,j,f){l={};j=k&&c?k.search(c):[];for(var n=j.length;n--;){var m=j[n],p=m.style||g.style,w,A,B=m.getProps?m.getProps(i):l;w=i["prep"+
...
1)},button1:function(){a.zoom(a.zoom()-1)}};v=Q.create("div","t5-zoombar",{position:"absolute",background:"url("+e.images+")","z-index":50,overflow:"hidden",width:e.width+"px",height:e.height+"px",margin:function(){var t=e.spacing,y=xa("{0}px 0 0 {1}px");if(e.align==="right")t=b.offsetWidth-e.width-e.spacing;return y(e.marginTop,t)}()});b.childNodes[0]?b.insertBefore(v,b.childNodes[0]):b.appendChild(v);v.appendChild(n=Q.create("div","t5-zoombar-thumb",{position:"absolute",background:g(),"z-index":51,
...
return d});J(da,{ex:J,log:wa,observable:sa,configurable:rb,formatter:xa,wordExists:function(a,d){for(var b=a.split(/\s|\,/).length;b--;)if(a.toLowerCase()==d.toLowerCase())return true;return false},is:R,indexOf:bb,fn:function(a){return Kb("fn",a)},project:Qa,unproject:fb,getImage:Xa,userMessage:kb,Registry:Ua,Style:ub,DOM:Q,Rect:ra,XY:V,Line:Fa,Hits:Hits,Control:Ta,Tile:Ra,Tweener:pa,View:Ab,Map:function(a,d){function b(l,j){var f=c.viewport();if(l){f=Ga(l.bestZoomLevel(f.w,f.h),j||0);return e(f).center(l.center())}else return new GeoJS.BBox((new ia(f.x,
f.y2)).sync(c,true).pos(),(new ia(f.x2,f.y)).sync(c,true).pos())}function e(l,j,f){if(R(l,ua)){l=Ga(d.minZoom,db(d.maxZoom,l|0));if(l!==g){var n=c.viewport(),m=c.offset(),p=n.w/2;n=n.h/2;var w=ya(2,l-g),A=p/w|0,B=n/w|0;g=l;c.offset(((j||m.x+p)-A)*w,((f||m.y+n)-B)*w);refreshY=refreshX=0;c.trigger("zoom",l);c.trigger("reset");k=c.rpp=fa/(256<<g);c.setMaxOffset(fa/k|0,fa/k|0,true,false);c.scale(1,false,true);c.trigger("resync");c.refresh()}return c}else return g}d=J({controls:["zoombar"],minZoom:1,maxZoom:18,
renderer:"canvas/dom",zoom:1,zoombar:{}},d);var h=new ia,k,g=d.zoom||d.zoomLevel,i=0,c=J(new Ab(a,d),{XY:ia,bounds:b,zoom:e});k=c.rpp=fa/(256<<g);c.bind("refresh",function(){var l=c.viewport();if(h.x!=l.x||h.y!=l.y){c.trigger("boundsChange",b());h.x=l.x;h.y=l.y}});c.bind("scaleChanged",function(l,j){var f=eb(j)/Math.LN2|0;if(f!==0){j=ya(2,f);clearTimeout(i);i=setTimeout(function(){e(g+f)},500)}});return c}});if(typeof module!="undefined"&&module.exports)module.exports=da;ja("generator","osm",function(a){a=
J({flipY:false,tileSize:256,tilePath:"{0}/{1}/{2}.png",osmDataAck:true},a);var d=Math.PI/180,b=180/Math.PI,e=null,h=[],k,g=xa(a.tilePath),i={buildTileUrl:function(c,l,j,f,n){if(l>=0&&l<f){c=(c%f+f)%f;l=g(j,c,n?Math.abs(l-f+1):l);if(e)l=k(h[c%(h.length||1)])+l;return l}},run:function(c,l,j,f){var n=c.zoom?c.zoom():0;if(n){var m=2<<n-1,p=a.tileSize,w=(l.w/p|0)+1,A=(l.h/p|0)+1;l=(new ia(l.x,l.y)).sync(c,true).pos();var B=l.lat;l=new ia((l.lon+180)/360*m|0,(1-Math.log(Math.tan(B*d)+1/Math.cos(B*d))/Math.PI)/
2*m%m|0);B=Math.PI-2*Math.PI*l.y/m;B=new GeoJS.Pos(b*Math.atan(0.5*(Math.exp(B)-Math.exp(-B))),l.x/m*360-180);B=new ia(B);B.sync(c);c=a.flipY;var K=j.search({x:B.x,y:B.y,w:w*p,h:A*p}),q={},v;for(v=K.length;v--;)q[K[v].id]=true;h=(e=i.getServerDetails?i.getServerDetails():null)&&e.subDomains?e.subDomains:[];k=xa(e?e.baseUrl:"");for(K=0;K<=w;K++)for(v=0;v<=A;v++){var o=l.x+K,u=l.y+v,t=o+"_"+u;if(!q[t]){tileUrl=i.buildTileUrl(o,u,n,m,c);o=new Ra(B.x+K*p,B.y+v*p,tileUrl,p,p,t);j.insert(o,o)}}f&&f()}}};
a.osmDataAck&&kb("ack","osm",'Map data (c) <a href="http://openstreetmap.org/" target="_blank">OpenStreetMap</a> (and) contributors, CC-BY-SA');return i});da.Cloudmade=function(){da.Registry.register("generator","osm.cloudmade",function(a){a=da.ex({apikey:null,styleid:1},a);var d=da.formatter("http://{3}.tile.cloudmade.com/{0}/{1}/{2}/");da.userMessage("ack","osm.cloudmade",'This product uses the <a href="http://cloudmade.com/" target="_blank">CloudMade</a> APIs, but is not endorsed or certified by CloudMade.');
return da.ex(da.Registry.create("generator","osm",a),{getServerDetails:function(){return{baseUrl:d(a.apikey,a.styleid,256,"{0}"),subDomains:["a","b","c"]}}})})}()})();
