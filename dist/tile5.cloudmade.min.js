...
w(s);l.style.cursor="move";e(s,true);T=B.x;N=B.y;D=i(T,N);m.triggerCustom("pointerDown",g("mouse",s),D,h(D,j(l)))}}function y(s){var B=w(s);H=B.x;I=B.y;if(d(s,l))n(s,u?"pointerMove":"pointerHover")}function E(s){if(u&&v(s)){u=false;if(d(s,l)){l.style.cursor="default";n(s,"pointerUp")}}}function J(s){if(d(s,l)){var B;s=s||window.event;if(s.detail)B=s.axis===2?-s.detail*A:0;else{B=s.wheelDeltaY?s.wheelDeltaY:s.wheelDelta;if(window.opera)B=-B}if(B!==0){var M=i(H,I);m.triggerCustom("zoom",g("mouse",s),
M,h(M,j(l)),B/C,"wheel");e(s);s.returnValue=false}}}function v(s){s=s||window.event;return(s.which||s.button)==1}function n(s,B,M,P,U){M=typeof M!="undefined"?M:H;P=typeof P!="undefined"?P:I;var z=M-T,K=P-N,F=i(M,P);m.triggerCustom(B,g("mouse",s),F,h(F,j(l)),i(z,K));if(typeof U=="undefined"||U){T=M;N=P}}q=G({},q);var A=120,C=A*8,u=false,D,H,I,T,N;q.binder("mousedown",x);q.binder("mousemove",y);q.binder("mouseup",E);q.binder("dblclick",function(s){ha("captured double click");if(d(s,l)){var B=w(s);
...
W=Z;z.next;){z=z.next;W=W.next={x:z.x-K,y:z.y-F,id:z.id}}return Z}function y(z,K){var F=z[K?K:"touches"],Z,W;if(F.length===0)return null;W=Z={x:F[0].pageX,y:F[0].pageY,id:F[0].identifier,count:F.length};for(var Y=1,va=F.length;Y<va;Y++)W=W.next={x:F[Y].pageX,y:F[Y].pageY,id:F[Y].identifier};return Z}function E(z){if(d(z,l)){I=j(l);var K=y(z,"changedTouches"),F=x(K,I.left,I.top);if(!N){T=u;m.triggerCustom("pointerDown",g("touch",z),K,F)}P&&m.triggerCustom("pointerDownMulti",g("touch",z),K,F);N=y(z);
if(N.count>1)B=w(N);U=1;M=x(N)}}function J(z){if(d(z,l)){e(z);s=y(z);if(T==u)T=Math.abs(N.x-s.x)>A||Math.abs(N.y-s.y)>A?C:u;if(T!=u){T=s.count>1?H:D;if(T==H)if(N.count===1){N=x(s);B=w(N)}else{var K=w(s);if(Math.abs(B-K)<n)T=D;else{var F;F=s.x;var Z=s.next.x,W=s.y,Y=s.next.y;F={x:(F<Z?F:Z)+(Math.abs(F-Z)>>1),y:(W<Y?W:Y)+(Math.abs(W-Y)>>1)};K=K/B;Z=K-U;m.triggerCustom("zoom",g("touch",z),F,h(F,I),Z,"pinch");U=K}}T==D&&m.triggerCustom("pointerMove",g("touch",z),s,x(s,I.left,I.top),i(s.x-M.x,s.y-M.y));
P&&m.triggerCustom("pointerMoveMulti",g("touch",z),s,x(s,I.left,I.top))}M=x(s)}}function v(z){if(d(z,l)){var K=y(z,"changedTouches"),F=x(K,I.left,I.top);s=y(z);if(!s){m.triggerCustom("pointerUp",g("touch",z),K,F);N=null}P&&m.triggerCustom("pointerUpMulti",g("touch",z),K,F)}}q=G({detailed:false,inertia:false},q);var n=20,A=7,C=0,u=1,D=2,H=3,I,T,N,s,B,M,P=q.detailed,U=1;q.binder("touchstart",E);q.binder("touchmove",J);q.binder("touchend",v);ha("initialized touch handler");return{unbind:function(){q.unbinder("touchstart",
...
J=!q||q.indexOf(E.type)>=0,v=true,n;for(n in E.checks){var A=E.checks[n];ha("checking "+n+" capability. require: "+A+", capability = "+w[n]);v=v&&A===w[n]}if(J&&v)x[x.length]=E.handler}return new p(l,x,m)}}}();var S={ex:G,log:ha,observable:qa,formatter:Ea,wordExists:function(a,b){for(var c=a.split(/\s|\,/).length;c--;)if(a.toLowerCase()==b.toLowerCase())return true;return false},is:X,indexOf:vb};qa(S);var Ta=function(){var a={};return{create:function(b,c){if(a[b][c])return a[b][c].apply(null,Array.prototype.slice.call(arguments,
...
Qb=Math.atan2,Lb="hasOwnProperty",Kb={NaN:1,Infinity:1,"-Infinity":1},Jb=String.prototype.toLowerCase,Mb=Object.prototype.toString,ma="undefined",Ga="string",Wa="number",Gb="array",ba=Ta.register,ra=Ta.create,Rb=0,Fb=0,sb=/[\,\s]+/,Sb=S.newCanvas=function(a,b){var c=document.createElement("canvas");c.width=a?a:0;c.height=b?b:0;S.trigger("createCanvas",c);return c},Tb=function(){function a(k){i++;k=k||(new Date).getTime();for(var j=c.length;j--;){var g=c[j];i%g.every===0&&g.cb(k)}animFrame(a)}var b=
...
p+n.translateX,n.xy.y-l+n.translateY);n.rotation!==0&&f.rotate(n.rotation);n.scaling!==1&&f.scale(n.scaling,n.scaling)}return q},drawTiles:function(n,A,C){for(var u,D=p-256,H=l-256,I=n.x2,T=n.y2,N=A.length;N--;){n=A[N];if(u=n.x>=D&&n.x<=I&&n.y>=H&&n.y<=T)if(!n.loaded&&C)n.load(a.invalidate);else n.image&&f.drawImage(n.image,n.x-p,n.y-l)}},prepare:function(n,A){var C,u=false,D=A.scaled||A;if(f)f.restore();else f=e.getContext("2d");for(C=n.length;C--;)u=u||n[C].clip;p=D.x;l=D.y;if(f){u||f.clearRect(0,
...
p),D=n.xy.y-(q?q.y:l),H=n.size,I=undefined;f.beginPath();switch(n.markerStyle.toLowerCase()){case "image":I=y;f.rect(u-(H>>1),D-(H>>1),H,H);if(n.reset&&n.image){n.image=null;n.reset=false}n.image?f.drawImage(n.image,u-(H>>1),D-(H>>1),H,H):Ha(n.imageUrl,function(T){n.image=T;f.drawImage(n.image,u-(H>>1),D-(H>>1),H,H)});break;default:f.moveTo(u,D);f.lineTo(u-(H>>1),D-H);f.lineTo(u+(H>>1),D-H);f.lineTo(u,D)}return g(A,C,I)},prepPoly:function(n,A,C,u){var D=true;n=u.points||n.points;u=q?q.x:p;var H=q?
q.y:l;f.beginPath();for(var I=n.length;I--;){var T=n[I].x-u,N=n[I].y-H;if(D){f.moveTo(T,N);D=false}else f.lineTo(T,N)}return g(A,C)},getContext:function(){return f}});(function(){for(var n in S.styles)j(null,n,S.styles[n]);S.bind("styleDefined",j)})();c.bind("detach",function(){b.removeChild(e)});return c});ba("renderer","dom",function(a,b,c,i,k){function j(f){var p=f.image=new Image;h[f.id]=f;p.src=f.url;p.onload=function(){if(e[f.id])d.appendChild(this);else f.image=null};p.style.cssText="-webkit-user-select: none; -webkit-box-shadow: none; -moz-box-shadow: none; box-shadow: none; border-top-width: 0px; border-right-width: 0px; border-bottom-width: 0px; border-left-width: 0px; border-style: initial; border-color: initial; padding-top: 0px; padding-right: 0px; padding-bottom: 0px; padding-left: 0px; margin-top: 0px; margin-right: 0px; margin-bottom: 0px; margin-left: 0px; position: absolute;";
...
null))&&ea.move(m,f.x-q,f.y-w);e[f.id]=f}}}});a.bind("predraw",function(){g(h,e);e={}});a.bind("detach",function(){b.removeChild(d)});a.bind("reset",function(){for(g(h,e={});d.childNodes.length>0;)d.removeChild(d.childNodes[0])});return a});var Xa=S.styles={},Wb=S.defineStyle=function(a,b){Xa[a]=b;S.trigger("styleDefined",a,Xa[a]);return a},yb=S.defineStyles=function(a){for(var b in a)Wb(b,a[b])};S.getStyle=function(a){return Xa[a]};S.loadStyles=function(a){Nb(a,function(b){yb(b)})};yb({basic:{fill:"#ffffff"},
highlight:{fill:"#ff0000"},waypoints:{lineWidth:4,stroke:"#003377",fill:"#ffffff"},waypointsHover:{lineWidth:4,stroke:"#ff0000",fill:"#ffffff"}});ba("view","simple",function(a){function b(o,r,t,L){y(La(Ra(V+ka(2,L)-1,0.5),2))}function c(o){var r=o|0;o=r+Math.round((o-r)*50)/50;if(o!==V){V=o;o=ja(V)/Math.LN2|0;if(o!==0){V=ka(2,o);m(Ba+o,Xb,Yb)}fa++}}function i(o,r){var t=renderer&&renderer.projectXY?renderer.projectXY(o,r):null;if(!t){t=Q.getViewport();var L=1/V;t=new O(t?t.x+o*L:o,t?t.y+r*L:r)}return t}
...
r,t){aa=Hits.init(o,r,t,i(t.x,t.y,true));for(o=n;o--;)I=I||(v[o].hitGuess?v[o].hitGuess(aa.x,aa.y,Q):false);I&&fa++}function m(o,r,t){if(X(o,Wa)){o=Ra(a.minZoom,La(a.maxZoom,o|0));if(o!==Ba){var L=ka(2,o-Ba),R=Ya/L|0,$=Za/L|0;Ba=o;J(((r||P+Ya)-R)*L,((t||U+Za)-$)*L);Z=F=0;E("zoom",o);V=1;renderer.trigger("reset");x()}return Q}else return Ba}function q(){var o=new la(P,U,Ma,Na);o.scaleFactor=V;return o}function w(o,r,t){if(X(r,ma))for(r=0;r<n;r++){if(v[r].id===o)return v[r]}else if(X(o,Ga)){r=ra("layer",
r,Q,t);r.added=(new Date).getTime();r.id=o;v[v.length]=r;v.sort(function(L,R){return R.zindex-L.zindex||R.added-L.added});n=v.length;r.trigger("resync");r.trigger("refresh",Q,q());Q.trigger("layerChange",Q,r);fa++;return r}else return[].concat(v)}function x(){var o=q();if(o){if(W||Y)if(o){var r=va?P+(o.w>>1):P,t=Oa?U+(o.h>>1):U,L=o.w,R=o.h;if(W&&W>L)if(r+L>W)P=va?P:W-L;else if(r<0)P=va?P:0;if(Y&&Y>R)if(t+R>Y)U=Oa?U:Y-R;else if(t<0)U=Oa?U:0}F=P;Z=U;E("refresh",Q,o);fa++}}function y(o,r,t,L,R){var $;
if(!ac){t=false;$=wa(ja(o)/Math.LN2);o=ka(2,$)}t?db(V,o,t,R,function(sa,Pa){o=sa;if(Pa){$=wa(ja(o)/Math.LN2);o=ka(2,$);L&&L()}c(o)}):c(o);return Q}function E(){for(var o=Q.trigger.apply(null,arguments).cancel,r=v.length;r--;)o=v[r].trigger.apply(null,arguments).cancel||o;return!o}function J(o,r,t){if(X(o,Wa)){if(!(t&&panSpeed===0)){P=o|0;U=r|0}}else return new O(P,U)}a=G({container:"",captureHover:true,drawOnScale:true,padding:0,inertia:true,refreshDistance:256,pannable:false,scalable:false,minZoom:1,
maxZoom:1,renderer:"canvas/dom",zoom:1},a);var v=[],n=0,A=null,C=null,u,D=null,H=!X(window.attachEvent,ma),I=false,T=true,N=false,s=0,B=0,M=a.refreshDistance,P=0,U=0,z=0,K=0,F=0,Z=0,W=null,Y=null,va=false,Oa=false,Ca=a.padding,aa=null,Da=null,zb=0,V=1,Ab=1,ca=null,$a={index:0,draw:false},ac=true,fa=0,Ma,Na,Ya,Za,ab,bb,Xb,Yb,Ba=a.zoom||a.zoomLevel,Zb=Qa("quad.out"),$b=300,Q={id:a.id,padding:Ca,panSpeed:0,attachFrame:function(o,r){r&&A.appendChild(o)},center:function(o,r,t){if(X(o,Ga)){r=kb.parseXY(o).sync(Q.rpp);
o=r.x;r=r.y}if(X(o,Wa)){J(o-ab,r-bb,t);return Q}else{r=J().offset(ab,bb);Q.rpp&&r.sync(Q.rpp,true);return r}},detach:function(){renderer&&renderer.trigger("detach");ca&&ca.unbind();if(C){u.removeChild(C);A=C=null}},layer:w,invalidate:function(){fa++},refresh:x,resetScale:function(){V=1},scale:y,triggerAll:E,removeLayer:function(o){a:{for(var r=n;r--;)if(v[r].id===o){o=r;break a}o=-1}if(o>=0&&o<n){Q.trigger("layerRemove",Q,v[o]);v.splice(o,1);fa++}n=v.length},getRenderer:function(){return renderer},
getScaleFactor:function(){return V},setMaxOffset:function(o,r,t,L){W=o;Y=r;va=t;Oa=L},getViewport:q,offset:J,pan:function(o,r){s=o;B=r;fa++},zoom:m};qa(Q);Q.bind("resize",function(){renderer.checkSize()});Sa.init(function(){w("markers","draw",{zindex:20});u=document.getElementById(a.container);u.appendChild(C=ea.create("div","",ea.styles({overflow:"hidden",width:u.offsetWidth+"px",height:u.offsetHeight+"px"})));Ma=C.offsetWidth+Ca*2;Na=C.offsetHeight+Ca*2;Ya=Ma/2;Za=Na/2;ab=u.offsetWidth/2;bb=u.offsetHeight/
2;C.appendChild(A=ea.create("div","",ea.styles({width:Ma+"px",height:Na+"px","z-index":2,margin:-Ca+"px 0 0 "+-Ca+"px"})));renderer=Vb(a.renderer,Q,A,u,a);T=renderer.fastpan;ca&&ca.unbind();if(renderer){ca=INTERACT.watch(renderer.interactTarget||u);if(a.scalable){ca.bind("zoom",b);ca.bind("doubleTap",k)}ca.bind("pointerDown",j);ca.bind("pointerMove",d);ca.bind("pointerUp",h);a.captureHover&&ca.bind("pointerHover",g);ca.bind("tap",f)}H?window.attachEvent("onresize",e):window.addEventListener("resize",
e,false)});Tb.attach(function(o){var r=[],t,L,R;self.panSpeed=panSpeed=na(s)+na(B);t=V!==Ab;if(panSpeed>0||t)fa++;if(panSpeed<2&&(na(P-F)>=M||na(U-Z)>=M))x();$a.index++;$a.draw=fa||panSpeed||0;if(renderer&&$a.draw){z+=s;K+=B;if(s||B)Q.trigger("pan");if(V!==1&&ea.supportTransforms)r[r.length]="scale("+V+")";if(t=!T||(a.drawOnScale||V===1)&&panSpeed<5){P=P-z/V|0;U=U-K/V|0;R=q();renderer.trigger("predraw",R);if(renderer.prepare(v,R,o,aa)){L=K=t=z=fa=0;for(ii=n;ii--;){var $=v[ii];if($.visible){var sa=
$.style?renderer.applyStyle($.style,true):null;$.draw(renderer,R,Q,o,aa);sa&&renderer.applyStyle(sa)}}renderer.trigger("render",R);Q.trigger("drawComplete",R,o);Ab=V;ea.move(A,t,L,r)}}else ea.move(A,z,K,r);if(N||!a.inertia)B=s=0;else if(s!=0||B!=0){s*=0.8;B*=0.8;if(na(s)<0.5)s=0;if(na(B)<0.5)B=0}if(aa){o=aa?aa.elements:[];if(Da&&Da.type==="hover"){r=Hits.diffHits(Da.elements,o);r.length>0&&Hits.triggerEvent(Da,Q,"Out",r)}if(o.length>0){t=aa.x;L=aa.y;for(r=o.length;r--;){R=o[r];$=t;sa=L;var Pa=R&&
...
0;E<=w;E++)for(v=0;v<=x;v++){var n=e.x+E,A=e.y+v,C=n+"_"+A;if(!J[C]){tileUrl=d.buildTileUrl(n,A,h,l,q);n=new Ia(y.x+E*m,y.y+v*m,tileUrl,m,m,C);f.insert(n,n)}}p&&p()}}};a.osmDataAck&&fb("ack","osm",'Map data (c) <a href="http://openstreetmap.org/" target="_blank">OpenStreetMap</a> (and) contributors, CC-BY-SA');return d});S.Cloudmade=function(){S.Registry.register("generator","osm.cloudmade",function(a){a=S.ex({apikey:null,styleid:1},a);var b=S.formatter("http://{3}.tile.cloudmade.com/{0}/{1}/{2}/");
S.userMessage("ack","osm.cloudmade",'This product uses the <a href="http://cloudmade.com/" target="_blank">CloudMade</a> APIs, but is not endorsed or certified by CloudMade.');return S.ex(S.Registry.create("generator","osm",a),{getServerDetails:function(){return{baseUrl:b(a.apikey,a.styleid,256,"{0}"),subDomains:["a","b","c"]}}})})}();cb.T5=S;cb.Tile5=function(a,b){b=G({container:a,type:"map",renderer:"canvas",starpos:null,zoom:1,fastpan:false,drawOnScale:true,zoombar:{}},b);return ra("view",b.type,
b)}})(window);
