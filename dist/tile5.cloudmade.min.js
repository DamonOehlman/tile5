(function(){function z(q,A){if(q&&q.split){var B=q.split(Pa);if(B.length>1){q=B[0];A=B[1]}}else if(q&&q.lat){A=q.lon;q=q.lat}this.lat=parseFloat(q||0);this.lon=parseFloat(A||0)}function ja(q,A){if(q&&q.splice){for(var B=A,I=new z(Ha,ta),P=new z(Ia,Ca),pa=q.length;pa--;){var fa=typeof q[pa]=="string"?new z(q[pa]):q[pa];if(fa.lat<I.lat)I.lat=fa.lat;if(fa.lat>P.lat)P.lat=fa.lat;if(fa.lon<I.lon)I.lon=fa.lon;if(fa.lon>P.lon)P.lon=fa.lon}this.min=I;this.max=P;if(typeof B=="undefined"){B=this.size();B=Math.max(B.x,
B.y)*0.3}this.min=new z(I.lat-B,(I.lon-B)%360);this.max=new z(P.lat+B,(P.lon+B)%360)}else{this.min=q;this.max=A}}function ma(q){if(typeof q=="string"){var A=(q.replace(/\d|\.|\s/g,"")||"m").toLowerCase();q=parseFloat(q)*({km:1E3}[A]||1)}this.meters=q||0}var Ja=[1.406245461070741,1.321415085624082,1.077179995861952,0.703119412486786,0.488332580888611],Qa=typeof module!="undefined"&&module.exports,aa=Math.PI*2,za=Math.PI/2,ba=Math.PI/180,ga=180/Math.PI,Ha=90,Ia=-Ha,ta=180,Ca=-ta,Ra=Ha*ba,Ya=-Ra,qa=
ta*ba,Za=-qa,Pa=/[\,\s]+/;z.prototype={constructor:z,copy:function(){return new z(this.lat,this.lon)},distanceTo:function(q){if(!q||this.empty()||q.empty())return 0;var A=(q.lat-this.lat)*ba/2,B=(q.lon-this.lon)*ba/2;q=Math.sin(A)*Math.sin(A)+Math.cos(this.lat*ba)*Math.cos(q.lat*ba)*Math.sin(B)*Math.sin(B);return 6371*2*Math.atan2(Math.sqrt(q),Math.sqrt(1-q))},equalTo:function(q){return pos&&this.lat===q.lat&&this.lon===q.lon},empty:function(){return this.lat===0&&this.lon===0},inArray:function(q){if(q)for(var A=
q.length;A--;)if(this.equal(q[A]))return true;return false},offset:function(q,A){var B=this.lat*ba,I=B+q/6371;B=this.lon*ba+Math.asin(Math.sin(A/6371)/Math.cos(B));I=(I+za)%Math.PI-za;B%=aa;return new z(I*ga,B*ga)},toBounds:function(q){var A=q.radians(),B=this.lat*ba,I=this.lon*ba;q=B-A;var P=B+A;if(q>Ya&&P<Ra){B=Math.asin(Math.sin(A)/Math.cos(B));A=I-B;if(A<Za)A+=aa;I=I+B;if(I>qa)I-=aa}else{q=Math.max(q,Ya);P=Math.min(P,Ra);A=Ca;I=ta}return new ja(new z(q*ga,A*ga),new z(P*ga,I*ga))},toString:function(q){return this.lat+
(q||" ")+this.lon},valid:function(){return!(isNaN(this.lat)||isNaN(this.lon))}};ja.prototype={constructor:ja,bestZoomLevel:function(q,A){var B=this.center(),I=Ja[Math.min(Math.round(Math.abs(B.lat)*0.05),Ja.length)],P=this.size();B=Math.ceil(Math.log(Ja[3]*A/P.y)/Math.LN2);I=Math.ceil(Math.log(I*q/P.x)/Math.LN2);return Math.min(isNaN(B)?1E3:B,isNaN(I)?1E3:I)},center:function(){var q=this.size();return new z(this.min.lat+q.y/2,this.min.lon+q.x/2)},expand:function(q){return new ja(new z(this.min.lat-
q,(this.min.lon-q)%360),new z(this.max.lat+q,(this.max.lon+q)%360))},size:function(q){var A={x:0,y:this.max.lat-this.min.lat};A.x=typeof q!="undefined"&&q&&this.min.lon>this.max.lon?360-this.min.lon+this.max.lon:this.max.lon-this.min.lon;return A},toString:function(){return"min: "+this.min+", max: "+this.max},union:function(){for(var q=this.min.copy(),A=this.max.copy(),B=arguments.length;B--;)if(arguments[B]){var I=arguments[B].min,P=arguments[B].max;q.lat=Math.min(q.lat,I.lat);q.lon=Math.min(q.lon,
I.lon);A.lat=Math.max(A.lat,P.lat);A.lon=Math.max(A.lon,P.lon)}return new ja(q,A)}};ma.prototype={add:function(){for(var q=this.meters,A=arguments.length;A--;){var B=typeof arguments[A]=="string"?new ma(arguments[A]):arguments[A];q+=B.meters}return new ma(q)},radians:function(q){if(typeof q!="undefined"){this.meters=q*6371E3;return this}else return this.meters/6371E3},toString:function(){if(this.meters>1E3)return(this.meters/10|0)/100+"km";return this.meters+"m"}};var S=this.GeoJS={Pos:z,BBox:ja,
Distance:ma,generalize:function(q,A,B){var I=q.length,P=[],pa=null;B=(B||250)/1E3;for(I=I;I--;)if(I===0)P.unshift(q[I]);else{var fa=!pa||q[I].inArray(A)?B:pa.distanceTo(q[I]);if(q[I]&&fa>=B){P.unshift(q[I]);pa=q[I]}}return P},include:function(q){if(Qa){q=q.split(",");for(var A=0;A<q.length;A++){var B=require("./plugins/"+q[A].trim()),I;for(I in B)S[I]=B[I]}}return S}};if(Qa)module.exports=S})();
...
"";g=g.replace(b[h],i)}return g}}function Ja(a,d,b){q.trigger("userMessage",a,d,b)}function Qa(a,d){var b=$a*Ka(a);return pa-2*qb(d*Da((1-b)/(1+b),$a/2))}function aa(a,d){if(S(a,Aa)){var b=a.split(rb);this.x=parseFloat(b[0]);this.y=parseFloat(b[1])}else if(a&&a.x){this.x=a.x;this.y=a.y}else{this.x=a||0;this.y=d||0}}function za(a){this.allowCull=a}function ba(a,d,b,e){this.mercX=b;this.mercY=e;if(S(a,Aa))z(this,Cb.parseXY(a));else a&&a.toBounds?z(this,ab(a.lon,a.lat)):aa.call(this,a,d)}function ga(a,
...
y=t?t.x:n;var F=t?t.y:m;f.beginPath();for(var G=s.length;G--;){var W=s[G].x-y,Z=s[G].y-F;if(N){f.moveTo(W,Z);N=false}else f.lineTo(W,Z)}return i(C,v)},getContext:function(){return f}});(function(){vb.each(function(s,C){g(null,s,C)});q.bind("styleDefined",g)})();b.bind("detach",function(){d.removeChild(j)});b.bind("resize",function(){});return b});ra("renderer","dom",function(a,d,b,e,h){function k(j){var f=j.image=new Image;c[j.id]=j;f.src=j.url;f.onload=function(){if(l[j.id])i.appendChild(this);else j.image=
...
l);l={}});b.bind("detach",function(){d.removeChild(i)});a.bind("reset",function(){for(g(c,l={});i.childNodes.length>0;)i.removeChild(i.childNodes[0])});return b});var vb=function(){function a(b,e){if(S(b,Aa)){q.trigger("styleDefined",b,d[b]=e);return b}else{var h=[],k;for(k in b)h[h.length]=a(k,b[k]);return h}}var d={};a({reset:{fill:"#ffffff",opacity:1},highlight:{fill:"#ff0000"},waypoints:{lineWidth:4,stroke:"#003377",fill:"#ffffff"},waypointsHover:{lineWidth:4,stroke:"#ff0000",fill:"#ffffff"}});
return{resetStyle:"reset",each:function(b){for(var e in d)b(e,d[e])},get:function(b){return d[b]},define:a}}(),Bb=function(a,d){function b(p,r,u,H){p=y?db(ca+Da(2,H)-1,0.125):H>0?2:0.5;s(p,false,true)}function e(p,r){var u=da&&da.projectXY?da.projectXY(p,r):null;if(!u){u=Q();var H=1/ca;u=new M.XY(u?u.x+(p+u.padding.x)*H:p,u?u.y+(r+u.padding.y)*H:r)}return u.sync(M,true)}function h(p,r,u){p=e(u.x,u.y);M.trigger("doubleTap",r,u,p);d.scalable&&s(2,{easing:"sine.inout",duration:700},true,p)}function k(p,
r,u){K=null;D=true;x("down",r,u)}function g(p,r,u){x("hover",r,u)}function i(p,r,u,H){m(r,u,false);if(!K){O+=H.x;L+=H.y}}function c(p,r,u){m(r,u,true);D=false}function l(){clearTimeout(wb);wb=setTimeout(function(){if(w)if(w.offsetWidth!==Sa*2||w.offsetHeight!==Ta*2){var p=E();t(a);E(p.x,p.y)}},250)}function j(p,r,u){x("tap",r,u);M.trigger("tap",r,u,e(u.x,u.y,true))}function f(){na&&na.unbind();if(V&&da){na=INTERACT.watch(da.interactTarget||w);if(d.scalable){na.bind("zoom",b);na.bind("doubleTap",h)}na.bind("pointerDown",
k);na.bind("pointerMove",i);na.bind("pointerUp",c);d.captureHover&&na.bind("pointerHover",g);na.bind("tap",j)}}function n(p){if(da){da.trigger("detach");da=null}var r=M,u=Z,H=w,$=d;p=p.split("/");for(var ha=new Hb(r,u,H,$),wa=0;wa<p.length;wa++)ha=La("renderer",p[wa],r,u,H,$,ha);da=ha;ka=V&&da.fastpan&&V.transforms;y=V&&V.transforms&&d.useTransforms;f();M.trigger("reset");J()}function m(p,r,u){if(K){p=e(r.x,r.y);K.drag.call(K.target,K,p.x,p.y,u)&&ia++;if(u)K=null}}function o(p){for(var r=W;r--;)if(G[r].id===
p)return r;return W}function t(p){if(V){w=document.getElementById(p);w.appendChild(R=V.create("div","",V.styles({overflow:"hidden",width:w.offsetWidth+"px",height:w.offsetHeight+"px"})));p=d.padding;if(p===v){p=w.offsetWidth;var r=w.offsetHeight,u=Eb(p*p+r*r);oa=new aa(u-p>>1,u-r>>1)}else oa=new aa(p,p);gb=R.offsetWidth+oa.x*2;hb=R.offsetHeight+oa.y*2;mb=gb/2;nb=hb/2;Sa=w.offsetWidth/2;Ta=w.offsetHeight/2;Ua=new aa(mb,nb);R.appendChild(Z=V.create("div","",V.styles({width:gb+"px",height:hb+"px","z-index":2,
margin:-oa.y+"px 0 0 "+-oa.x+"px"})));n(d.renderer);p=d.controls;for(r=0;r<F.length;r++)F[r].trigger("detach");F=[];for(r=0;r<p.length;r++)F[F.length]=La("control",p[r],M,R,w,d[p[r]])}else n("canvas")}function x(p,r,u){var H=(new aa(u.x-Sa+mb,u.y-Ta+nb)).rotate(-sa*fa,Ua).scale(1/ca,Ua);la=Hits.init(p,r,u,e(u.x,u.y,true),H);T=false;for(p=W;p--;)if(G[p].visible)T=T||(G[p].hitGuess?G[p].hitGuess(la.gridX,la.gridY,M):false);T&&ia++}function E(p,r,u){if(typeof p!="undefined"&&(S(p,Aa)||S(p,"object"))){r=
new M.XY(p);r.sync(M);p=r.x;r=r.y}if(S(p,Ba)){C(p-Sa-oa.x,r-Ta-oa.y,u);return M}else return C().offset(Sa+oa.x|0,Ta+oa.y|0).sync(M,true)}function Q(){var p=new ga(U,X,gb,hb);p.scaleFactor=ca;p.padding=oa?oa.copy():new aa;return p}function Y(p,r,u){if(S(p,Aa)&&S(r,Ea))for(r=0;r<W;r++){if(G[r].id===p)return G[r]}else if(S(p,Aa)){r=La("layer",r,M,u);u=o(p);u!==W&&M.trigger("layerRemove",G[u]);r.added=(new Date).getTime();r.id=p;G[u]=r;G.sort(function(H,$){return $.zindex-H.zindex||$.added-H.added});
W=G.length;M.trigger("resync");J();M.trigger("layerChange",M,r);ia++;return r}else return[].concat(G)}function J(){var p=Q();if(p){if(Na||Oa)if(p){var r=ib?U+(p.w>>1):U,u=jb?X+(p.h>>1):X,H=p.w,$=p.h;if(Na&&Na>H)if(r+H>Na)U=ib?U:Na-H;else if(r<0)U=ib?U:0;if(Oa&&Oa>$)if(u+$>Oa)X=jb?X:Oa-$;else if(u<0)X=jb?X:0}xb=U;yb=X;M.trigger("refresh",M,p);ia++}}function s(p,r,u,H){if(S(p,Ba)){var $=u?p:ca*p;if(!y){r=undefined;p=cb(kb($)/Math.LN2);$=Da(2,p)}if(H){p=M.center();M.pan(H.x-p.x,H.y-p.y,r)}if(r)Va=xa.tween([ca],
[$],r,function(){ca=$;Va=null;ia++});else{ca=$;ia++}return M}else return ca}function C(p,r,u){if(S(p,Ba)){if(u)ya=xa.tween([U,X],[p,r],u,function(){U=p|0;X=r|0;ua=Fa=0;ya=null;ia++});else{U=p|0;X=r|0;ia++}return M}else return(new M.XY(U,X)).sync(M,true)}d=z({captureHover:true,controls:[],drawOnScale:true,padding:128,inertia:true,refreshDistance:128,pannable:true,scalable:true,renderer:"canvas",useTransforms:true},d);var v="auto",y=true,N=false,F=[],G=[],W=0,Z=null,R=null,w,K=null,T=false,ka,D=false,
O=0,L=0,ea=d.refreshDistance,U=0,X=0,ua=0,Fa=0,xb=0,yb=0,Na=null,Oa=null,ib=false,jb=false,ya=null,oa,la=null,Wa=null,da,wb=0,Ua=new aa,sa=0,Xa=null,ca=1,Va=null,ob=1,na=null,pb={index:0,draw:false},ia=0,gb,hb,mb,nb,Sa,Ta,M={XY:aa,id:d.id,panSpeed:0,attachFrame:function(p,r){r&&Z.appendChild(p)},center:E,detach:function(){da&&da.trigger("detach");na&&na.unbind();if(R){w.removeChild(R);Z=R=null}},frozen:function(p){if(S(p,Ea))return N;else{N=p;return M}},layer:Y,invalidate:function(){ia++},pan:function(p,
r,u){C(U+p,X+r,u)},refresh:J,rotate:function(p,r,u){if(S(p,Ba)){var H=u?p:sa+p;if(r)Xa=xa.tween([sa],[H],r,function(){sa=H%360;Xa=null;ia++});else{sa=H%360;ia++}return M}else return sa},scale:s,setMaxOffset:function(p,r,u,H){Na=p;Oa=r;ib=u;jb=H},offset:C,viewport:Q};qa(M);M.bind("resize",l);M.bind("layerRemove",function(p,r){var u=Pa(G,r.id);if(u>=0&&u<W){G.splice(u,1);ia++}W=G.length});Za(M,d,{container:t,captureHover:f,scalable:f,pannable:f,renderer:n});Y("markers","draw",{zindex:20});t(a);if(V&&
!S(window.attachEvent,Ea))window.attachEvent("onresize",l);else V&&window.addEventListener("resize",l,false);Ma.attach(function(p){var r=[],u,H,$;if(!N){M.panSpeed=panSpeed=va(O)+va(L);(u=ca!==ob)&&M.trigger("scale");if(panSpeed>0||u||ya||Va||Xa){ia++;if(ya&&panSpeed>0){ya(true);ya=null}}if(!D&&panSpeed<=0&&(va(U-xb)>=ea||va(X-yb)>=ea))J();pb.index++;pb.draw=ia||panSpeed||0;if(da&&pb.draw){if(Va)ca=Va()[0];if(Xa)sa=Xa()[0];ua+=O;Fa+=L;if(O||L)M.trigger("pan");if(y){if(ca!==1)r[r.length]="scale("+
ca+")";if(sa!==0)r[r.length]="rotate("+sa+"deg)"}u=T||!ka||!D&&!ya&&(d.drawOnScale||ca===1)&&panSpeed<=2;if(ya){H=ya();ua=U-H[0]|0;Fa=X-H[1]|0}if(u){H=-sa*fa;u=eb(H)*ua+-Ka(H)*Fa;H=Ka(H)*ua+eb(H)*Fa;U=U-u/ca|0;X=X-H/ca|0;$=Q();da.trigger("predraw",$);if(da.prepare(G,$,p,la)){H=Fa=u=ua=ia=0;for(ii=W;ii--;){var ha=G[ii];if(ha.visible){var wa=ha.style?da.applyStyle(ha.style,true):null;ha.draw(da,$,M,p,la);wa&&da.applyStyle(wa)}}da.trigger("render",$);M.trigger("drawComplete",$,p);V.move(Z,u,H,r,Ua)}}else V.move(Z,
ua,Fa,r,Ua);if(D||!d.inertia)L=O=0;else if(O!=0||L!=0){O*=0.8;L*=0.8;if(va(O)<0.5)O=0;if(va(L)<0.5)L=0}if(la){p=true;r=la?la.elements:[];if(Wa&&Wa.type==="hover"){diffElements=Hits.diffHits(Wa.elements,r);if(diffElements.length>0)Hits.triggerEvent(Wa,M,"Out",diffElements);else p=false}if(r.length>0){H=la.gridX;$=la.gridY;for(u=r.length;D&&u--;){ha=r[u];wa=H;var zb=$,Ab=ha&&ha.drag&&(!ha.canDrag||ha.canDrag(ha,wa,zb));if(Ab){K=ha;K.startX=wa;K.startY=zb}if(Ab)break}p&&Hits.triggerEvent(la,M)}Wa=r.length>
...
return d});z(q,{ex:z,log:ja,observable:qa,configurable:Za,formatter:ma,wordExists:function(a,d){for(var b=a.split(/\s|\,/).length;b--;)if(a.toLowerCase()==d.toLowerCase())return true;return false},is:S,indexOf:Pa,fn:function(a){return Fb("fn",a)},project:ab,unproject:lb,getImage:fb,userMessage:Ja,Registry:A,Style:vb,DOM:V,Rect:ga,XY:aa,Line:za,Hits:Hits,Control:Ca,Tile:Ia,Tweener:xa,View:Bb,Map:function(a,d){function b(l,j){var f=c.viewport();if(l){f=db(l.bestZoomLevel(f.w,f.h),j||0);return e(f).center(l.center())}else return new GeoJS.BBox((new ba(f.x,
...
zoom:1,zoombar:{}},d);var h=new ba,k,g=d.zoom||d.zoomLevel,i=0,c=z(new Bb(a,d),{XY:ba,bounds:b,zoom:e});k=c.rpp=P/(256<<g);c.bind("refresh",function(){var l=c.viewport();if(h.x!=l.x||h.y!=l.y){c.trigger("boundsChange",b());h.x=l.x;h.y=l.y}});c.bind("scaleChanged",function(l,j){var f=kb(j)/Math.LN2|0;if(f!==0){j=Da(2,f);clearTimeout(i);i=setTimeout(function(){e(g+f)},500)}});return c}});if(typeof module!="undefined"&&module.exports)module.exports=q;ra("generator","osm",function(a){a=z({flipY:false,
...
Ja("ack","osm",'Map data (c) <a href="http://openstreetmap.org/" target="_blank">OpenStreetMap</a> (and) contributors, CC-BY-SA');return i});q.Cloudmade=function(){q.Registry.register("generator","osm.cloudmade",function(a){a=q.ex({apikey:null,styleid:1},a);var d=q.formatter("http://{3}.tile.cloudmade.com/{0}/{1}/{2}/");q.userMessage("ack","osm.cloudmade",'This product uses the <a href="http://cloudmade.com/" target="_blank">CloudMade</a> APIs, but is not endorsed or certified by CloudMade.');return q.ex(q.Registry.create("generator",
"osm",a),{getServerDetails:function(){return{baseUrl:d(a.apikey,a.styleid,256,"{0}"),subDomains:["a","b","c"]}}})})}()})();
