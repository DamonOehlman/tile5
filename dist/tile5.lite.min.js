...
!p;for(p=0;p<d.length;p++)o=o||d[p]==v;if(o)b:{o=v;for(var C in g)for(p=0;p<g[C].length;p++)if(RegExp(g[C][p]+"$","i").test(q.url)){v=C;break b}v=o?o:"DEFAULT"}try{k=f[v](this,q)}catch(P){k=f.DEFAULT(this,q)}}else b.error&&b.error(this)}catch(y){GT.Log.exception(y,"PROCESSING AJAX RESPONSE")}s&&k&&b.success&&b.success.call(this,k)}}try{b=GT.extend({method:"GET",data:null,url:null,async:true,success:null,handleResponse:null,error:null,contentType:"application/x-www-form-urlencoded"},b);var e=a.exec(b.url),
...
(function(){function c(){for(;j.length>0;){var k=j.shift();document.location=k}e=0}function m(k){for(var s=true,q=l.length;q--;)s=s&&k!=l[q];return s}function g(k,s){var q=[];for(var o in s)o&&q.push(o+"="+escape(s[o]));return"tile5://"+k+"/"+(q.length>0?"?"+q.join("&"):"")}function d(k,s){m(k)&&GT.Log.info("would push url: "+g(k,s))}function a(k,s){if(m(k)){j.push(g(k,s));e||(e=setTimeout(c,100))}}function f(){h={base:{name:"Unknown",eventTarget:document,supportsTouch:"createTouch"in document,imageCacheMaxSize:null,
...
p){return v.created-p.created});for(var o=q;o--;)delete a[e[o].url];e.splice(0,q)}}finally{k=false}GT.say("imagecache.cleared")}var a={},f={},h=0,b=[],i=[],e=[],j=[],l=0,k=false,s={avgImageSize:25,loadTimeout:10,addInterceptor:function(q){j.push(q)},cancelLoad:function(){i=[]},get:function(q){var o=null,v=null;k||(o=a[q]);if((v=o?o.image:null)&&(v.getContext||v.complete&&v.width>0))return v},load:function(q,o,v){var p=a[q];if(p){p.hitCount++;p.image.complete&&o&&o(p.image,true)}else{p=T5.ex({url:q,
...
g(t.touches):[o(t.pageX,t.pageY)];S=o();R=o();aa=true;H=false;X=T5.time();m(t);t.target.style.cursor="move";x("inertiaCancel");da=X;if(t=M.length>0?M[0]:null){x("touchStart",t.x,t.y);if(da-Z<h)if((t=U?T5.V.diff(M[0],U[0]):null)&&Math.abs(t.x)<y.maxDistDoubleTap&&Math.abs(t.y)<y.maxDistDoubleTap)H=true;E=e;U=[].concat(M)}else GT.Log.warn("Touch start fired, but no touch vector found")}}function A(t){if(t.target&&t.target===K){Y=(L?g(t.touches):[o(t.pageX,t.pageY)])[0];if(aa){L&&m(t);t=L?g(t.touches):
...
new T5.Vector(A.x-ca.x*fa,A.y-ca.y*fa))r=T5.V.offset(r,M.x,M.y)}n.save();try{aa=E;if(B!==1)n.scale(B,B);else if($){n.translate(A.x,A.y);n.scale(E,E)}for(ba=p.length;ba--;)if(p[ba].shouldDraw(F)){var ha=p[ba].draw(n,r,x,F,w);z+=ha?ha:0}}finally{n.restore()}GT.Log.trace("draw complete",Q);O=false;return z}function o(){var n=0,r=!I&&(K===Z||K===da);U=T5.time();y.x=Math.floor(y.x);y.y=Math.floor(y.y);if(r){T5.cancelAnimation(function(F){return F.cancelOnInteract});J=false;if(N!==0){clearTimeout(N);N=
0}}for(r=p.length;r--;){var z=p[r].cycle(U,y,K);n+=z?z:0}n+=q(P,y);H=0;if(D+n>0)v();else if(!J&&N===0)N=setTimeout(s,500);GT.Log.trace("Completed draw cycle",U)}function v(){D++;if(!(u||H!==0)){D=0;H=setTimeout(o,0)}}c=T5.ex({id:GT.objId("view"),container:"",fastDraw:false,inertia:true,pannable:true,scalable:true,panAnimationEasing:T5.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,adjustScaleFactor:null,autoSize:false},c);var p=[],C=document.getElementById(c.container),P=
...
Q||(Q=T5.D.getCenter(x));w.animate(n,F,Q,r,z);return w},removeLayer:function(n){a:{for(var r=p.length;r--;)if(p[r].getId()==n){n=r;break a}n=-1}if(n>=0&&n<p.length){GT.say("layer.removed",{layer:p[n]});p.splice(n,1)}},getOffset:function(){return T5.V.copy(y)},setOffset:function(n,r){y.x=n;y.y=r},updateOffset:function(n,r,z,F,Q){function $(){d(0,0);Q&&Q()}if(z){n=new T5.Vector(n,r);z=T5.tweenVector(y,n.x,n.y,z,$,F);for(F=z.length;F--;){z[F].cancelOnInteract=true;z[F].requestUpdates(v)}}else w.setOffset(n,
r)},zoom:function(n,r,z){I=false;E=r;S=E!==1;T=T5.D.getCenter(w.getDimensions());A=E>1?T5.V.copy(n):T5.D.getCenter(w.getDimensions());R=null;clearTimeout(L);if(S){K=Z;v();if(z)L=setTimeout(function(){i();E=1},parseInt(z,10))}}};GT.listen("layer.remove",function(n){n.id&&w.removeLayer(n.id)});B=T5.getConfig().getScaling();GT.observable(w);w.bind("wake",v);w.bind("invalidate",function(){O=true});if(c.pannable){w.bind("pan",m);w.bind("panEnd",d);w.bind("inertiaPan",g);w.bind("inertiaCancel",function(){I=
false;v()})}if(c.scalable){w.bind("pinchZoom",f);w.bind("pinchZoomEnd",h);w.bind("wheelZoom",b)}GT.configurable(w,["inertia","container"],GT.paramTweaker(c,null,{container:function(n,r){C=document.getElementById(r);e()}}),true);e();return w};
...
T5.TileGrid=function(c){function m(u,B){var x=T5.Vector(u*b-j.x,B*b-j.y);return new T5.ImageTile({gridX:x.x,gridY:x.y})}function g(u,B,x,D){var A=Math.floor(c.gridSize*0.2),J=new T5.Vector;if(u<0||u+x>c.gridSize)J.x=u<0?-A:A;if(B<0||B+D>c.gridSize)J.y=B<0?-A:A;return J}function d(u,B,x){var D=GT.Log.getTraceTicks(),A=0,J=c.gridSize,I=c.tileSize;new T5.Vector(J/2,J/2);if(x)f=[];if(u)for(x=0;x<J;x++)for(var H=0;H<J;H++){if(!f[A]){var O=u(H,x,i,J);O.gridX=H*I-j.x;O.gridY=x*I-j.y;f[A]=O}A++}e=u;l=B;GT.Log.trace("tile grid populated",
...
B,x){u=0;if(p.x!==0||p.y!==0){var D=p,A=c.shiftOrigin;if(!(D.x===0&&D.y===0)){var J=GT.Log.getTraceTicks(),I=Array(f.length);I.length=f.length;for(var H=0;H<c.gridSize;H++)for(var O=0;O<c.gridSize;O++)I[O*c.gridSize+H]=H+D.x>=0&&H+D.x<c.gridSize?f[(O+D.y)*c.gridSize+(H+D.x)]:null;f=I;i=A?A(i,D):T5.V.add(i,D);j.x+=-D.x*c.tileSize;j.y+=-D.y*c.tileSize;GT.Log.trace("tile storage shifted",J);d(e,l)}p=new T5.Vector;u++}x!==T5.viewState("PINCH")&&a(B,x);return u+G.dirty?1:0},deactivate:function(){q=false},
...
"rgba(50, 50, 50, 0.3)";u.beginPath();for(x=o.length;x--;){var N=o[x];if(N){var L=N.gridX-I,M=N.gridY-B;if(!(O?false:!N.dirty)){H=G.drawTile(u,N,L,M,D)&&H;N.x=L;N.y=M}}else H=false;c.drawGrid&&u.rect(L,M,c.tileSize,c.tileSize)}u.stroke();GT.Log.trace("drawn tiles",J);H&&!v&&A.trigger("tileDrawComplete");v=H;G.dirty=false}}},getTileAtXY:function(u,B){for(var x=c.tileSize,D=o?o.length:0;D--;){var A=o[D];if(A&&u>=A.x&&B>=A.y)if(u<=A.x+x&&B<=A.y+x)return A}return null},getTileVirtualXY:function(u,B,x){u=
T5.V.add(new T5.Vector(u,B),T5.V.invert(i),j);u=new T5.Vector(u.x*c.tileSize,u.y*c.tileSize);if(x){u.x+=k;u.y+=k}return u},populate:function(u){d(u,null,true)}});GT.listen("imagecache.cleared",function(){for(var u=f.length;u--;)if(f[u])f[u].loaded=false});GT.listen("tiler.repaint",function(){for(var u=f.length;u--;)if(f[u]){f[u].x=null;f[u].y=null}});return G};
