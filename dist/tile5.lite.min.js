...
c[b]}else if(c)e[e.length]=c;h()},clear:function(c){if(c)for(var b=0;b<e.length;b++)c(e[b])&&e.splice(b,1);else e=[];h()}});d.bind("tap",function(c,b,m,g){for(var j=[],i=e.length;i--;)if(e[i].hitTest(g))j[j.length]=e[i];if(j.length>0)c.cancel=d.trigger("markerTap",b,m,j).cancel});d.bind("gridUpdate",l);return d};
