...
g,j,o){for(var h=a.length;h--;)a[h].draw(f,g,j,c,o)},add:function(f){if(f&&typeof f.length!=="undefined")for(var g=f.length;g--;){if(f[g])a[a.length]=f[g]}else if(f)a[a.length]=f;i()},clear:function(f){if(f)for(var g=0;g<a.length;g++)f(a[g])&&a.splice(g,1);else a=[];i()},each:function(f){if(f)for(var g=a.length;g--;)f(a[g])},find:function(f){for(var g=[],j=a.length;j--;)if(!f||f(a[j]))g[g.length]=a[j];return g}});c.bind("tap",function(f,g,j,o){var h=[],k=o.x;o=o.y;for(var u=a.length;u--;)if(a[u].hitTest(k,
