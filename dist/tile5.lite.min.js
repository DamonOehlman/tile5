...
resize:function(n,r){if(D){f.autoSize=false;D.width=n;D.height=r;j();s.trigger("resize",D.width,D.height)}},scale:function(n,r,B,H,N){H||(H=T5.D.getCenter(t));N||(N=T5.D.getCenter(t));s.animate(n,H,N,r,B);return s},removeLayer:function(n){a:{for(var r=P;r--;)if(v[r].getId()==n){n=r;break a}n=-1}if(n>=0&&n<P){COG.say("layer.removed",{layer:v[n]});v.splice(n,1);p()}P=v.length},getOffset:function(){return C},updateOffset:g,zoom:function(n,r,B){E=false;F=r;R=F!==1;ba=T5.D.getCenter(t);A=F>1?T5.V.copy(n):
T5.D.getCenter(t);O=null;clearTimeout(S);if(R){V=ja;u();if(B)S=setTimeout(function(){b();F=1},parseInt(B,10))}}};Q=T5.getConfig().getScaling();s.markers=i("markers",new T5.MarkerLayer);COG.observable(s);s.bind("wake",u);s.bind("invalidate",p);if(f.pannable){s.bind("pan",m);s.bind("panEnd",h);s.bind("inertiaCancel",function(){E=false;u()})}if(f.scalable){s.bind("pinchZoom",a);s.bind("pinchZoomEnd",c);s.bind("wheelZoom",d)}s.bind("tap",function(n,r,B){for(var H=T5.V.offset(B,T,x),N=v.length;N--;)n.cancel=
n.cancel||v[N].trigger("tap",r,B,H).cancel});COG.configurable(s,["inertia","container","rotation","tapExtent"],COG.paramTweaker(f,null,{container:function(n,r){D=document.getElementById(r);j()},rotation:function(n,r){I=r}}),true);j();f.autoSize&&window.addEventListener("resize",k,false);return s};
