...
COG.Touch=function(){function f(w,D){return{x:w.x-D.x,y:w.y-D.y}}function m(w,D){var G=f(w,D);return Math.sqrt(G.x*G.x+G.y*G.y)}function h(w){if(w.count>1)return m(w.touches[0],w.touches[1]);return 0}function e(w,D){var G=w&&w.count>0?w.touches[0]:null;if(G&&D&&D.count>0)return f(G,D.touches[0]);return null}function a(w,D){w.count=D.count;for(var G=g;G--;){w.touches[G].x=D.touches[G].x;w.touches[G].y=D.touches[G].y}}function c(){for(var w={count:0,touches:Array(g)},D=g;D--;)w.touches[D]=k();return w}
function d(w){if(w.preventDefault){w.preventDefault();w.stopPropagation()}else w.cancelBubble&&w.cancelBubble()}function b(w,D,G){if(E){D=D[G?G:"touches"];G=D.length;for(G=w.count=G;G--;){w.touches[G].x=D[G].pageX;w.touches[G].y=D[G].pageY}}else if(D.button===0){w.count=1;w.touches[0].x=D.pageX?D.pageX:D.screenX;w.touches[0].y=D.pageY?D.pageY:D.screenY}else w.count=0}function k(w,D){return{x:w?w:0,y:D?D:0}}var g=10,j=120,i=100,l=250,p=300,x=5,z=0,n=1,o=2,t=7,u=0,Q=0,E=undefined,ga=function(w){function D(v,
B,y,q){var r=Math.asin((v.y-B.y)/y);y=y*(qa/q)>>0;y=y>na?na:y;r=B.x>v.x?r:Math.PI-r;R("pan",Math.cos(r)*-y,Math.sin(r)*y,true)}function G(v,B){var y,q;if(E){y=B-ia;if(y<l){q=m(I.touches[0],v);q>w.inertiaTrigger&&D(I.touches[0],v,q,y)}}else{X.x=v.x;X.y=v.y;COG.Loopage.join({execute:function(r,C){y=r-B;q=m(v,X);if(y<i&&q>w.inertiaTrigger){C.trigger("complete");D(v,X,q,y)}else y>i&&C.trigger("complete")},frequency:10})}}function ea(v){var B=0,y=0;if(v.offsetParent){do{B+=v.offsetLeft;y+=v.offsetTop;
v=v.offsetParent}while(v)}return k(B,y)}function da(v){var B=v.count,y=Array(B),q=ea(Y);for(B=B;B--;)y[B]=k(v.touches[B].x-q.x,v.touches[B].y-q.y);return y}function R(){oa&&oa.trigger.apply(null,arguments)}function s(v,B){var y=ea(Y);y=k(B.x-y.x,B.y-y.y);R(v,B,y)}function K(v){var B=v.target?v.target:v.srcElement;if(Z||B&&B===Y){b(I,v);if(I.count!==0){ba=null;V.x=0;V.y=0;ja=true;M=false;ia=(new Date).getTime();d(v);B.style.cursor="move";R("inertiaCancel");ka=ia;if(v=I.count>0?I.touches[0]:null){R("touchStart",
v.x,v.y);if(ka-la<p)if((v=f(I.touches[0],P.touches[0]))&&Math.abs(v.x)<w.maxDistDoubleTap&&Math.abs(v.y)<w.maxDistDoubleTap)M=true;H=z;a(P,I)}else COG.Log.warn("Touch start fired, but no touch vector found")}}}function A(v){var B=v.target?v.target:v.srcElement,y=0;if(Z||B&&B===Y){b(S,v);if(S.count>0){X.x=S.touches[0].x;X.y=S.touches[0].y}if(ja){E&&d(v);if(S.count>1){I.count===1&&a(I,S);y=h(I)-h(S)}if(H===z)if((v=e(S,I))&&(Math.abs(v.x)>=t||Math.abs(v.y)>=t))H=n;if(H!==z)if(!y||Math.abs(y)<x){if(ba=
e(S,P)){V.x-=ba.x;V.y-=ba.y;W.x-=ba.x;W.y-=ba.y}R("pan",W.x,W.y);W.x=0;W.y=0;H=n}else{R("pinchZoom",da(I),da(S));H=o}a(P,S)}}}function J(v){var B=v.target?v.target:v.srcElement;if(ja&&(Z||B&&B===Y)){b(aa,v,"changedTouches");var y=aa.touches[0];E&&d(v);v=(new Date).getTime();la=ka;if(H===z){s("tap",I.touches[0]);T||(T=setTimeout(function(){T=0;M&&s("doubleTap",I.touches[0])},p+50))}else if(H==n){R("panEnd",V.x,V.y);pa&&G(y,v)}else H==o&&R("pinchZoomEnd",da(I),da(P),v-ia);B.style.cursor="default";ja=
false}}function F(v){var B=v.target?v.target:v.srcElement;if(Z||B&&B===Y){if(v.detail){B=-v.detail*j;B=k(v.axis===1?B:0,v.axis===2?B:0)}else B=k(v.wheelDeltaX,v.wheelDeltaY);var y=B.y!==0?Math.abs(B.y/j):0;if(X&&y!==0){var q=k(X.x-Y.offsetLeft,X.y-Y.offsetTop);R("wheelZoom",q,Math.pow(2,B.y>0?y:-y))}d(v)}}w=COG.extend({element:null,observable:null,inertiaTrigger:20,maxDistDoubleTap:20,touchStartHandler:null,moveHandler:null,moveEndHandler:null,pinchZoomHandler:null,pinchZoomEndHandler:null,tapHandler:null,
doubleTapHandler:null,wheelZoomHandler:null},w);var M=false,T=0,N=T5.getConfig(),I=c(),S=c(),P=c(),aa=c(),ba=null,V=k(),W=k(),H=null,ja=false,ia=0,fa=[],X=k(),pa=false,qa,na,ka=0,la=0,Y=w.element,oa=w.observable,Z=typeof FlashCanvas!=="undefined",ra={supportsTouch:E,addListeners:function(v){fa.push(v)},decoupleListeners:function(v){for(var B=0;v&&B<fa.length;B++)if(fa[B].listenerId===v){fa.splice(B,1);break}},release:function(){N.unbindEvent(E?"touchstart":"mousedown",K,false);N.unbindEvent(E?"touchmove":
"mousemove",A,false);N.unbindEvent(E?"touchend":"mouseup",J,false);if(!E){window.removeEventListener("mousewheel",F,false);window.removeEventListener("DOMMouseScroll",F,false)}},inertiaEnable:function(v,B){pa=true;qa=v;na=B?Math.min(B.width,B.height):500},inertiaDisable:function(){pa=false}};if(typeof E==="undefined")E=T5.getConfig().supportsTouch;N.bindEvent(E?"touchstart":"mousedown",K,false);N.bindEvent(E?"touchmove":"mousemove",A,false);N.bindEvent(E?"touchend":"mouseup",J,false);if(!E){N.bindEvent("mousewheel",
F,window);N.bindEvent("DOMMouseScroll",F,window)}return ra},U=[];return{capture:function(w,D){if(!w)throw Error("Unable to capture touch of null element");if(!w.id)w.id="touchable_"+u++;var G=U[w.id];if(!G){G=new ga(COG.extend({element:w},D));U[w.id]=G}if(D){D.listenerId&&G.decoupleListeners(D.listenerId);D.listenerId=++Q;G.addListeners(D)}return G},release:function(w){if(w&&w.id&&U[w.id]){U[w.id].release();delete U[w.id]}}}}();
...
T5.View=function(f){function m(q,r,C,L){Z=la;t();if(L&&f.inertia){F=true;g(U+r,w+C,f.panAnimationEasing,f.panAnimationDuration)}else L||g(U+r,w+C)}function h(){Z=ka;F=false;COG.Loopage.join({execute:t,after:50,single:true})}function e(q,r){P=ra(q);aa=ra(r);var C=v(P.dimensions),L=v(aa.dimensions);X=B(P);A=B(aa);H=P&&C!==0?L/C:1}function a(q,r,C){e(r,C);if(S=H!==1){Z=Y;t()}}function c(q,r,C,L){e(r,C);if(f.adjustScaleFactor){H=f.adjustScaleFactor(H);COG.Log.info("scale factor adjusted to: "+H)}if(L<
f.pinchZoomAnimateTrigger){l(ia,H,X,p(),T5.easing("sine.out"),function(){b();H=1},300);H=ia}else{b();H=1}}function d(q,r,C){y.zoom(r,Math.min(Math.pow(2,Math.round(Math.log(C))),8),500)}function b(){var q=P?p():A;H=Math.round(H*10)/10;S=false;for(var r=u.length;r--;)u[r].trigger("scale",H,q);y.trigger("scale",H,q);Z=ka;t()}function k(){clearTimeout(W);W=setTimeout(j,50)}function g(q,r,C,L,O){function $(){ca+=1;if(ca>=2){h(0,0);O&&O()}}var ca=0;if(C){q=T5.tweenValue(U,q,C,$,L);r=T5.tweenValue(w,r,
C,$,L);q.cancelOnInteract=true;q.requestUpdates(function(ha){U=ha;F=true;t()});r.cancelOnInteract=true;r.requestUpdates(function(ha){w=ha;F=true;t()})}else{U=q;w=r}}function j(){var q;if(E){COG.Touch.release(E);if(f.autoSize&&E.parentNode){q=E.parentNode.getBoundingClientRect();if(q.height!==0&&q.width!==0){E.height=q.height;E.width=q.width}}try{if(!E.id)E.id=f.id+"_canvas";ga=E.getContext("2d");ga.clearRect(0,0,E.width,E.height)}catch(r){COG.Log.exception(r);throw Error("Could not initialise canvas on specified view element");
}na=COG.Touch.capture(E,{observable:y});if(s.height!==E.height||s.width!==E.width){s=new T5.Dimensions(E.width,E.height);y.trigger("resize",E.width,E.height);for(q=Q;q--;)u[q].trigger("resize",E.width,E.height)}f.inertia&&na.inertiaEnable(f.panAnimationDuration,s);for(q=Q;q--;)u[q].trigger("contextChanged",ga);o()}}function i(q,r){r.setId(q);r.added=T5.ticks();r.bind("remove",function(){y.removeLayer(q)});r.trigger("contextChanged",ga);r.setParent(y);u.push(r);u.sort(function(C,L){var O=L.zindex-
C.zindex;if(O===0)O=L.added-C.added;return O});Q=u.length;return r}function l(q,r,C,L,O,$,ca){function ha(){$&&$();b();H=1;fa=null}S=true;X=T5.V.copy(C);A=T5.V.copy(L);P=null;if(O)T5.tweenValue(0,r-q,O,ha,ca?ca:1E3).requestUpdates(function(ma){fa=ma/(r-q);H=q+ma;Z=Y;t();y.trigger("animate")});else{H=targetScaleFactor;ha()}}function p(){var q=T5.D.getCenter(s),r=T5.V.distance([A,q]),C=T5.V.theta(A,q,r),L=T5.V.diff(X,A);q=T5.V.pointOnEdge(A,q,C,r/H);q.x+=L.x;q.y+=L.y;return q}function x(){y.trigger("idle");
J=true;M=0}function z(q,r,C,L,O){if(da)r=T5.viewState("FROZEN");var $=(r&Y)!==0,ca=0;if(G||$){q.clearRect(0,0,E.width,E.height);G=false}if($){T5.D.getCenter(s);var ha=(fa?fa:1)/2,ma=T5.V.diff(X,A);if(P){N.x=A.x+ma.x;N.y=A.y+ma.y}else{N.x=A.x-ma.x*ha;N.y=A.y-ma.y*ha}C=T5.V.offset(C,N.x,N.y)}q.save();try{ia=H;if(R!==1)q.scale(R,R);else if($){q.translate(A.x,A.y);q.scale(H,H)}for(ca=Q;ca--;)if(u[ca].shouldDraw(r,C,L)){u[ca].draw(q,C,s,r,y,L,O);u[ca].trigger("drawComplete",C,O)}}finally{q.restore()}y.trigger("drawComplete",
C,O);COG.Log.trace("draw complete",O)}function n(q,r){var C=false,L=ba?ba:F?la:Z,O=!F&&(L===Y||L===la),$=V||L===la||L===Y||T5.isTweening();D.x=U>>0;D.y=w>>0;if(O){T5.cancelAnimation(function(ca){return ca.cancelOnInteract});J=false;if(M!==0){clearTimeout(M);M=0}}for(O=Q;O--;){if(u[O].animated)Z|=oa;C=u[O].cycle(q,D,Z,$)||C}$=$||(Z&oa)!==0;if(C=C||$||H!==1&&H!==ja){z(ga,L,D,$,q);ja=H;V=false}if(!C&&K===0&&!pa){if(!J&&M===0)M=setTimeout(x,500);r.trigger("complete")}K=0;COG.Log.trace("Completed draw cycle",
q)}function o(){V=true;t()}function t(){K+=1;if(!(da||ea)){ea=COG.Loopage.join({execute:n,frequency:qa});ea.bind("complete",function(){ea=null})}}f=T5.ex({id:COG.objId("view"),container:"",fastDraw:false,inertia:true,pannable:true,scalable:true,panAnimationEasing:T5.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,adjustScaleFactor:null,autoSize:true,tapExtent:10,fps:25},f);var u=[],Q=0,E=document.getElementById(f.container),ga=null,U=0,w=0,D=new T5.Vector,G=false,ea=null,da=
false,R=1,s=new T5.Dimensions,K=0,A=null,J=false,F=false,M=0,T=0,N=new T5.Vector,I=0,S=false,P=null,aa=null,ba=null,V=false,W=0,H=1,ja=0,ia=1,fa=null,X=null,pa=typeof FlashCanvas!=="undefined",qa=~~(1E3/f.fps),na=null,ka=T5.viewState("ACTIVE"),la=T5.viewState("PAN"),Y=T5.viewState("PINCH"),oa=T5.viewState("ANIMATING"),Z=ka,ra=T5.V.getRect,v=T5.D.getSize,B=T5.R.getCenter,y={id:f.id,deviceScaling:R,fastDraw:f.fastDraw||T5.getConfig().requireFastDraw,animate:function(q,r,C,L,O){l(H,q,r,C,L,O)},centerOn:function(q){U=
q.x-E.width/2;w=q.y-E.height/2},getDimensions:function(){return s},getZoomCenter:function(){return N},getLayer:function(q){for(var r=0;r<Q;r++)if(u[r].getId()==q)return u[r];return null},setLayer:function(q,r){for(var C=0;C<Q;C++)if(u[C].getId()===q){u.splice(C,1);break}r&&i(q,r);o()},eachLayer:function(q){for(var r=Q;r--;)q(u[r])},clearBackground:function(){COG.Log.info("CALL OF DEPRECATED METHOD CLEAR BACKGROUND");G=true;o()},invalidate:o,freeze:function(){da=true},unfreeze:function(){da=false;
t()},resize:function(q,r){if(E){f.autoSize=false;E.width=q;E.height=r;j();y.trigger("resize",E.width,E.height)}},scale:function(q,r,C,L,O){L||(L=T5.D.getCenter(s));O||(O=T5.D.getCenter(s));y.animate(q,L,O,r,C);return y},removeLayer:function(q){a:{for(var r=Q;r--;)if(u[r].getId()==q){q=r;break a}q=-1}if(q>=0&&q<Q){COG.say("layer.removed",{layer:u[q]});u.splice(q,1);o()}Q=u.length},stateOverride:function(q){ba=q},getOffset:function(){return D},updateOffset:g,zoom:function(q,r,C){F=false;H=r;S=H!==1;
X=T5.D.getCenter(s);A=H>1?T5.V.copy(q):T5.D.getCenter(s);P=null;clearTimeout(T);if(S){Z=Y;t();if(C)T=setTimeout(function(){b();H=1},parseInt(C,10))}}};R=T5.getConfig().getScaling();y.markers=i("markers",new T5.MarkerLayer);COG.observable(y);y.bind("wake",t);y.bind("invalidate",o);if(f.pannable){y.bind("pan",m);y.bind("panEnd",h);y.bind("inertiaCancel",function(){F=false;t()})}if(f.scalable){y.bind("pinchZoom",a);y.bind("pinchZoomEnd",c);y.bind("wheelZoom",d)}y.bind("tap",function(q,r,C){for(var L=
T5.V.offset(C,U,w),O=u.length;O--;)q.cancel=q.cancel||u[O].trigger("tap",r,C,L).cancel});COG.configurable(y,["inertia","container","rotation","tapExtent","scalable","pannable"],COG.paramTweaker(f,null,{container:function(q,r){E=document.getElementById(r);j()},rotation:function(q,r){I=r}}),true);j();f.autoSize&&window.addEventListener("resize",k,false);return y};
...
T5.TileGrid=function(f){function m(s,K){var A=T5.Vector(s*k-p.x,K*k-p.y);return new T5.EmptyTile({gridX:A.x,gridY:A.y,empty:true})}function h(s,K,A,J){var F=Math.max(d,b)*0.2>>0,M=new T5.Vector;if(s<0||s+A>d)M.x=s<0?-F:F;if(K<0||K+J>b)M.y=K<0?-F:F;return M}function e(s,K,A){var J=COG.Log.getTraceTicks(),F=0;new T5.Vector(d/2,b/2);if(A)c=[];if(s)for(A=0;A<b;A++)for(var M=0;M<d;M++){if(!c[F]){var T=s(M,A,i,d,b);T.gridX=M*k-p.x;T.gridY=A*k-p.y;c[F]=T}F++}l=s;z=K;COG.Log.trace("tile grid populated",J);
R.changed()}function a(s,K,A,J){var F,M=[],T=false;s=new T5.Vector((s.x+p.x)*t>>0,(s.y+p.y)*t>>0);tilesNeeded=false;if(!G){F=R.getParent().getDimensions();w=Math.ceil(F.width*t)+1;D=Math.ceil(F.height*t)+1;G=new T5.Vector((w-1)/2>>0,(D-1)/2>>0)}for(var N=D;N--;)for(var I=w;I--;){F=I+s.x>=0&&I+s.x<d?c[(N+s.y)*d+(I+s.x)]:null;var S=new T5.Vector(I-G.x,N-G.y);if(!F){U=h(s.x,s.y,w,D);F=m(I+s.x,N+s.y)}T=F?(F.dirty=A||F.dirty)||T:T;M[M.length]={tile:F,centerness:T5.V.absSize(S)}}M.sort(function(P,aa){return aa.centerness-
P.centerness});Q||(Q=Array(M.length));for(A=M.length;A--;){Q[A]=M[A].tile;R.prepTile(Q[A],K)}ga=J;return T}f=T5.ex({tileSize:T5.tileSize,center:new T5.Vector,gridSize:25,shiftOrigin:null,supportFastDraw:true,allowShift:true},f);var c=Array(Math.pow(f.gridSize,2)),d=f.gridSize,b=f.gridSize,k=f.tileSize,g=b*k,j=d*k,i=T5.V.offset(f.center,-Math.ceil(d>>1)),l=null,p=new T5.Vector,x=false,z=null,n=Math.round(k/2),o=false,t=k?1/k:0,u=true,Q=null,E=false,ga=0;new T5.Vector;var U=new T5.Vector;T5.getConfig();
var w,D,G,ea=T5.viewState("ANIMATING"),da=T5.viewState("PINCH"),R=T5.ex(new T5.ViewLayer(f),{gridDimensions:new T5.Dimensions(j,g),cycle:function(s,K,A,J){if(U.x!==0||U.y!==0){J=U;var F=f.shiftOrigin;if(!(!f.allowShift||J.x===0&&J.y===0)){var M=COG.Log.getTraceTicks(),T=Array(c.length);T.length=c.length;for(var N=0;N<d;N++)for(var I=0;I<b;I++)T[I*d+N]=N+J.x>=0&&N+J.x<d?c[(I+J.y)*d+(N+J.x)]:null;c=T;i=F?F(i,J):T5.V.add(i,J);p.x+=-J.x*k;p.y+=-J.y*k;COG.Log.trace("tile storage shifted",M);e(l,z)}U=new T5.Vector;
J=true}if(!o&&(A&da)===0)o=a(K,A,J,s);x=(A&ea)!==0||T5.isTweening();return o},find:function(s){if(!s)return null;for(var K=c.length;K--;){var A=c[K];if(A&&s(A))return A}return null},prepTile:function(){},drawTile:function(){return false},getTileSize:function(){return k},clearTileRect:function(s,K,A,J){s.clearRect(K,A,J,J)},draw:function(s,K,A,J,F,M,T){var N=K.x;K=K.y;var I=A.width,S=A.height,P,aa=true;if(Q){s.beginPath();if(u)for(var ba=Q.length;ba--;){P=Q[ba];var V=P.gridX-N,W=P.gridY-K;P.x=V;P.y=
W;if(P.empty){COG.Log.info("empty tile @ x: "+V+", y: "+W);R.clearTileRect(s,V,W,k,J);aa=false}else if(M||P.dirty){x&&R.clearTileRect(s,V,W,k,J);(P=R.drawTile(s,P,V,W,J,M,T))&&s.rect(V,W,k,k);aa=aa&&P}I=V<I?V:I;S=W<S?W:S}s.clip();I>0&&s.clearRect(0,0,I,A.height);S>0&&s.clearRect(0,0,A.width,S);aa&&!E&&F.trigger("tileDrawComplete");o=false;E=aa}},getTileAtXY:function(s,K){for(var A=null,J=Q?Q.length:0;J--;){var F=Q[J];if(F&&s>=F.x&&K>=F.y)if(s<=F.x+k&&K<=F.y+k){A=F;break}}COG.Log.info("looking for tile @ x: "+
s+", y: "+K+", found: "+A);return A},getTileVirtualXY:function(s,K,A){s=T5.V.add(new T5.Vector(s,K),T5.V.invert(i),p);s=new T5.Vector(s.x*k,s.y*k);if(A){s.x+=n;s.y+=n}return s},populate:function(s){e(s,null,true)},syncVectors:function(){}});R.bind("resize",function(){COG.Log.info("captured resize");G=null});R.bind("scale",function(){u=false});COG.listen("imagecache.cleared",function(){for(var s=c.length;s--;)if(c[s])c[s].loaded=false});COG.listen("tiler.repaint",function(){for(var s=c.length;s--;)if(c[s]){c[s].x=
...
i=T5.ex(new T5.TileGrid(f),{clearTileRect:function(l,p,x,z,n){(n&c)===0&&l.clearRect(p,x,z,z)},drawTile:function(l,p,x,z,n){var o=p.url?k(p.url):null,t=false;if(o){l.drawImage(o,x,z);t=true}else if((n&c)!==0){l.drawImage(e,x,z);t=true}p.dirty=false;return t},initTileUrl:function(){},prepTile:function(l,p){if(l&&!l.loading&&(!d||p===a)){l.url||i.initTileUrl(l);if(!k(l.url)){l.loading=true;g(l.url,function(){l.loadTime=(new Date).getTime();l.loaded=true;l.dirty=true;i.changed()},j)}}}});return i};
