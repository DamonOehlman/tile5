...
(function(){function c(e){return e<10?"0"+e:e}function m(e){a.lastIndex=0;return a.test(e)?'"'+e.replace(a,function(j){var l=b[j];return typeof l==="string"?l:"\\u"+("0000"+j.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+e+'"'}function g(e,j){var l,k,s,n,p=f,u,o=j[e];if(o&&typeof o==="object"&&typeof o.toJSON==="function")o=o.toJSON(e);if(typeof i==="function")o=i.call(j,e,o);switch(typeof o){case "string":return m(o);case "number":return isFinite(o)?String(o):"null";case "boolean":case "null":return String(o);
case "object":if(!o)return"null";f+=h;u=[];if(Object.prototype.toString.apply(o)==="[object Array]"){n=o.length;for(l=0;l<n;l+=1)u[l]=g(l,o)||"null";s=u.length===0?"[]":f?"[\n"+f+u.join(",\n"+f)+"\n"+p+"]":"["+u.join(",")+"]";f=p;return s}if(i&&typeof i==="object"){n=i.length;for(l=0;l<n;l+=1){k=i[l];if(typeof k==="string")if(s=g(k,o))u.push(m(k)+(f?": ":":")+s)}}else for(k in o)if(Object.hasOwnProperty.call(o,k))if(s=g(k,o))u.push(m(k)+(f?": ":":")+s);s=u.length===0?"{}":f?"{\n"+f+u.join(",\n"+f)+
"\n"+p+"}":"{"+u.join(",")+"}";f=p;return s}}if(typeof Date.prototype.toJSON!=="function"){Date.prototype.toJSON=function(){return isFinite(this.valueOf())?this.getUTCFullYear()+"-"+c(this.getUTCMonth()+1)+"-"+c(this.getUTCDate())+"T"+c(this.getUTCHours())+":"+c(this.getUTCMinutes())+":"+c(this.getUTCSeconds())+"Z":null};String.prototype.toJSON=Number.prototype.toJSON=Boolean.prototype.toJSON=function(){return this.valueOf()}}var d=/[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g,
...
{"":e})};if(typeof JSON.parse!=="function")JSON.parse=function(e,j){function l(s,n){var p,u,o=s[n];if(o&&typeof o==="object")for(p in o)if(Object.hasOwnProperty.call(o,p)){u=l(o,p);if(u!==undefined)o[p]=u;else delete o[p]}return j.call(s,n,o)}var k;e=String(e);d.lastIndex=0;if(d.test(e))e=e.replace(d,function(s){return"\\u"+("0000"+s.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(e.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
...
(function(){function c(b){var i=[],e=function(l,k){k=GT.isFunction(k)?k():k;i[i.length]=encodeURIComponent(l)+"="+encodeURIComponent(k)};if(GT.isArray(b))for(var j=0;j<b.length;j++)e(b[j].name,b[j].value);else for(j in b)e(j,b[j]);return i.join("&").replace(/%20/g,"+")}var m={HTML:"text/html",XML:"text/xml",TEXT:"text/plain",STREAM:"application/octet-stream"},g={JSON:["json"],PDON:["pdon.txt"]},d=["TEXT","STREAM"],a=/^(\w+:)?\/\/([^\/?#]+)/,f={XML:function(b){return b.responseXML},JSON:function(b){return GT.parseData(b.responseText)},
PDON:function(b){return GT.parseData(b.responseText,"PDON")},DEFAULT:function(b){return b.responseText}},h={CONTENT_TYPE:"Content-Type"};GT.xhr=function(b){function i(){if(this.readyState===4){var k=null,s=!this.status&&location.protocol==="file:"||this.status>=200&&this.status<300||this.status===304||this.status===1223||this.status===0;try{if(s)if(b.handleResponse)b.handleResponse(this);else{var n=b,p=this.getResponseHeader(h.CONTENT_TYPE),u,o=false;for(u in m)if(p&&p.indexOf(m[u])>=0){o=true;break}p=
!o;for(o=0;o<d.length;o++)p=p||d[o]==u;if(p)b:{p=u;for(var C in g)for(o=0;o<g[C].length;o++)if(RegExp(g[C][o]+"$","i").test(n.url)){u=C;break b}u=p?p:"DEFAULT"}try{k=f[u](this,n)}catch(z){k=f.DEFAULT(this,n)}}else b.error&&b.error(this)}catch(D){GT.Log.exception(D,"PROCESSING AJAX RESPONSE")}s&&k&&b.success&&b.success.call(this,k)}}try{b=GT.extend({method:"GET",data:null,url:null,async:true,success:null,handleResponse:null,error:null,contentType:"application/x-www-form-urlencoded"},b);var e=a.exec(b.url),
j=e&&(e[1]&&e[1]!==location.protocol||e[2]!==location.host);if(b.data)b.method="POST";if(b.url){e=null;if(b.xhr)e=b.xhr(b);e||(e=new XMLHttpRequest);e.open(b.method,b.url,b.async);b.data&&e.setRequestHeader("Content-Type",b.contentType);j||e.setRequestHeader("X-Requested-With","XMLHttpRequest");e.onreadystatechange=i;e.send(b.method=="POST"?c(b.data):null)}else GT.Log.warn("ajax request issued with no url - that ain't going to work...")}catch(l){GT.Log.exception(l)}}})();
...
(function(){function c(){for(;j.length>0;){var k=j.shift();document.location=k}e=0}function m(k){for(var s=true,n=l.length;n--;)s=s&&k!=l[n];return s}function g(k,s){var n=[];for(var p in s)p&&n.push(p+"="+escape(s[p]));return"tile5://"+k+"/"+(n.length>0?"?"+n.join("&"):"")}function d(k,s){m(k)&&GT.Log.info("would push url: "+g(k,s))}function a(k,s){if(m(k)){j.push(g(k,s));e||(e=setTimeout(c,100))}}function f(){h={base:{name:"Unknown",eventTarget:document,supportsTouch:"createTouch"in document,imageCacheMaxSize:null,
...
T5.Images=function(){function c(){var n=f[this.id];if(n&&n.image.complete&&n.image.width>0){n.loaded=true;n.hitCount=1;for(var p=i.length;p--;)if(i[p].image.src==this.src){i.splice(p,1);break}if(n.background||n.postProcess)if(n.image){p=T5.newCanvas(n.realSize?n.realSize.width:image.width,n.realSize?n.realSize.height:image.height);var u=p.getContext("2d"),o=n.offset?n.offset:new T5.Vector;n.background&&u.drawImage(n.background,0,0);u.drawImage(n.image,o.x,o.y);n.postProcess&&n.postProcess(u,n);n.image=
p}n.loadCallback&&n.loadCallback(this,false);e.push({url:this.src,created:n.requested});delete f[this.id];m()}}function m(){for(var n=T5.getConfig().maxImageLoads;b.length>0&&(!n||i.length<n);){var p=b.shift();if(p.imageLoader){i.push(p);p.imageLoader(p,c)}}}function g(n){for(var p=null,u=j.length;u--&&!p;)p=j[u](n);return p?p:function(o,C){o.image.onload=C;o.image.src=T5.Resources.getPath(o.url);o.requested=T5.time()}}function d(){k=true;try{var n=Math.floor(e.length/2);if(n>0){e.sort(function(u,
o){return u.created-o.created});for(var p=n;p--;)delete a[e[p].url];e.splice(0,n)}}finally{k=false}GT.say("imagecache.cleared")}var a={},f={},h=0,b=[],i=[],e=[],j=[],l=0,k=false,s={avgImageSize:25,loadTimeout:10,addInterceptor:function(n){j.push(n)},cancelLoad:function(){i=[]},get:function(n){var p=null,u=null;k||(p=a[n]);if((u=p?p.image:null)&&(u.getContext||u.complete&&u.width>0))return u},load:function(n,p,u){var o=a[n];if(o){o.hitCount++;o.image.complete&&p&&p(o.image,true)}else{o=T5.ex({url:n,
image:new Image,loaded:false,imageLoader:g(n),created:T5.time(),requested:null,hitCount:0,loadCallback:p},u);o.image.id="resourceLoaderImage"+h++;a[n]=o;f[o.image.id]=o;b.push(o);m()}return o},stats:function(){return{imageLoadingCount:i.length,queuedImageCount:b.length,imageCacheFullness:l}}};setInterval(function(){for(var n=T5.time(),p=false,u=0,o=T5.getConfig();u<i.length;)if(n-i[u].requested>s.loadTimeout*1E3){i.splice(u,1);p=true}else u++;p&&m();if(o&&o.imageCacheMaxSize){l=e.length*s.avgImageSize/
o.imageCacheMaxSize;l>=1&&d()}},1E3);return s}();
(function(){function c(z,D){var I=z&&z.length>0?z[0]:null;if(I&&D&&D.length>0)return u(I,D[0]);return null}function m(z){z.preventDefault();z.stopPropagation()}function g(z){for(var D=Array(z.length),I=z.length;I--;)D[I]=n(z[I].pageX,z[I].pageY);return D}var d=120,a=100,f=250,h=300,b=5,i=2,e=0,j=1,l=7,k=0,s=0,n=T5.V.create,p=T5.V.distance,u=T5.V.diff,o=function(z){function D(t,E,w,q){var r=Math.asin((t.y-E.y)/w);w=Math.min(Math.floor(w*(Z.duration/q)),Z.max);r=E.x>t.x?r:Math.PI-r;t=n(Math.cos(r)*
-w,Math.sin(r)*w);B("inertiaPan",t.x,t.y)}function I(t,E){var w,q;if(G){w=E-$;if(w<f){q=p([O[0],t]);q>z.inertiaTrigger&&D(O[0],t,q,w)}}else{T=t;var r=setInterval(function(){w=T5.time()-E;q=p([t,T]);if(w<a&&q>z.inertiaTrigger){clearInterval(r);D(t,T,q,w)}else w>a&&clearInterval(r)},5)}}function v(t){for(var E=[],w=P?-P.offsetLeft:0,q=P?-P.offsetTop:0,r=t.length;r--;)E.push(T5.V.offset(t[r],w,q));return E}function B(){U&&U.trigger.apply(null,arguments)}function x(t){if(t.target&&t.target===P){O=G?g(t.touches):
...
t.pageY)];var E=0;if(t.length>1){if(O.length===1)O=[].concat(t);E=p(O)-p(t)}if(V===e){var w=c(t,O);if(w&&(Math.abs(w.x)>=l||Math.abs(w.y)>=l))V=j}if(V!==e)if(!E||Math.abs(E)<b){if(X=c(t,R)){W.x-=X.x;W.y-=X.y;S.x-=X.x;S.y-=X.y}if(T5.V.absSize(S)>i){B("pan",S.x,S.y);S=n()}V=j}else{B("pinchZoom",v(O),v(t));V=TOUCH_MODE_PINCH}R=[].concat(t)}}}function y(t){if(t.target&&t.target===P){var E=(G?g(t.changedTouches):[n(t.pageX,t.pageY)])[0];G&&m(t);var w=T5.time();ea=ba;if(V===e)J||(J=setTimeout(function(){J=
0;var q=M?"doubleTap":"tap",r=O[0],A=null;if(P)A=T5.V.offset(r,-P.offsetLeft,-P.offsetTop);B(q,r,A)},h+50));else if(V==j){B("panEnd",W.x,W.y);Z&&I(E,w)}else V==TOUCH_MODE_PINCH&&B("pinchZoomEnd",v(O),v(R),w-$);t.target.style.cursor="default";L=false}}function K(t){if(t.target&&t.target===P){var E;if(t.detail){E=-t.detail*d;E=n(t.axis===1?E:0,t.axis===2?E:0)}else E=n(t.wheelDeltaX,t.wheelDeltaY);var w=E.y!==0?Math.abs(E.y/d):0;if(T&&w!==0){var q=T5.V.offset(T,-P.offsetLeft,-P.offsetTop);B("wheelZoom",
q,Math.pow(2,E.y>0?w:-w))}t.preventDefault()}}z=T5.ex({element:null,observable:null,inertiaTrigger:20,maxDistDoubleTap:20,touchStartHandler:null,moveHandler:null,moveEndHandler:null,pinchZoomHandler:null,pinchZoomEndHandler:null,tapHandler:null,doubleTapHandler:null,wheelZoomHandler:null},z);var M=false,J=0,N=T5.getConfig(),G=T5.getConfig().supportsTouch,O=null,R=null,X=null,W=null,S=n(),V=null,L=false,$=0,Y=[],T=null,Z=null,ba=0,ea=0,P=z.element,U=z.observable,ga={supportsTouch:G,addListeners:function(t){Y.push(t)},
decoupleListeners:function(t){for(var E=0;t&&E<Y.length;E++)if(Y[E].listenerId===t){Y.splice(E,1);break}},release:function(){N.eventTarget.removeEventListener(G?"touchstart":"mousedown",x,false);N.eventTarget.removeEventListener(G?"touchmove":"mousemove",F,false);N.eventTarget.removeEventListener(G?"touchend":"mouseup",y,false);if(!G){window.removeEventListener("mousewheel",K,false);window.removeEventListener("DOMMouseScroll",K,false)}},inertiaEnable:function(t,E){Z={duration:t,max:E?Math.min(E.width,
E.height):500}},inertiaDisable:function(){Z=null}};N.eventTarget.addEventListener(G?"touchstart":"mousedown",x,false);N.eventTarget.addEventListener(G?"touchmove":"mousemove",F,false);N.eventTarget.addEventListener(G?"touchend":"mouseup",y,false);if(!G){window.addEventListener("mousewheel",K,false);window.addEventListener("DOMMouseScroll",K,false)}return ga},C=[];T5.captureTouch=function(z,D){if(!z)throw Error("Unable to capture touch of null element");if(!z.id)z.id="touchable_"+k++;var I=C[z.id];
if(!I){I=new o(T5.ex({element:z},D));C[z.id]=I}D.listenerId&&I.decoupleListeners(D.listenerId);D.listenerId=++s;I.addListeners(D);return I};T5.resetTouch=function(z){if(z&&z.id&&C[z.id]){C[z.id].release();delete C[z.id]}}})();
...
c.tweenFn(c.duration,a,f,c.duration);for(var j=g.length;j--;)g[j](i,true)}}catch(l){GT.Log.exception(l)}},requestUpdates:function(b){g.push(b)}};a=c.target&&c.property&&c.target[c.property]?c.target[c.property]:c.startValue;if(typeof c.endValue!=="undefined")f=c.endValue-a;if(f==0)d=true;T5.wakeTweens();return h};
...
T5.View=function(c){function m(q,r,A,H){M=typeof A!=="undefined";U=ea;u();w.updateOffset(D.x+q,D.y+r,A,H)}function g(q,r){c.inertia&&m(q,r,c.panAnimationEasing,c.panAnimationDuration)}function d(){U=ba;M=false;setTimeout(u,50)}function a(q,r){S=ga(q);V=ga(r);var A=t(S.dimensions),H=t(V.dimensions);T=E(S);y=E(V);L=S&&A!==0?H/A:1}function f(q,r){a(q,r);if(W=L!==1){U=P;u()}}function h(q,r,A){a(q,r);if(c.adjustScaleFactor){L=c.adjustScaleFactor(L);GT.Log.info("scale factor adjusted to: "+L)}if(A<c.pinchZoomAnimateTrigger){l($,
L,T,k(),T5.easing("sine.out"),function(){i();L=1},300);L=$}else{i();L=1}}function b(q,r){w.zoom(q,Math.min(Math.pow(2,Math.round(Math.log(r))),8),500)}function i(){W=false;w.trigger("scale",L,S?k():y);U=ba;u()}function e(){if(C){T5.resetTouch(C);if(c.autoSize){C.height=window.innerHeight-C.offsetTop;C.width=window.innerWidth-C.offsetLeft}try{z=C.getContext("2d");z.globalCompositeOperation="source-over";z.clearRect(0,0,C.width,C.height)}catch(q){GT.Log.exception(q);throw Error("Could not initialise canvas on specified view element");
}Z=T5.captureTouch(C,{observable:w});c.inertia&&Z.inertiaEnable(c.panAnimationDuration,x);x=w.getDimensions();for(var r=o.length;r--;)layerContextChange(o[r]);u()}}function j(q,r){r.setId(q);r.added=T5.time();r.trigger("contextChanged",z);r.setParent(w);o.push(r);o.sort(function(A,H){var Q=H.zindex-A.zindex;if(Q===0)Q=H.added-A.added;return Q})}function l(q,r,A,H,Q,aa,ca){function fa(){aa&&aa();i();L=1;Y=null}W=true;T=T5.V.copy(A);y=T5.V.copy(H);S=null;if(Q)T5.tweenValue(0,r-q,Q,fa,ca?ca:1E3).requestUpdates(function(da){Y=
da/(r-q);L=q+da;U=P;u();w.trigger("animate")});else{L=targetScaleFactor;fa()}}function k(){var q=T5.D.getCenter(x),r=T5.V.distance([y,q]),A=T5.V.theta(y,q,r),H=T5.V.diff(T,y);q=T5.V.pointOnEdge(y,q,A,r/L);q.x+=H.x;q.y+=H.y;return q}function s(){w.trigger("idle");K=true;G=0}function n(q,r){var A=0,H=v?T5.viewState("FROZEN"):U,Q=T5.time(),aa=(H&P)!==0,ca=0;if(I||aa){q.clearRect(0,0,C.width,C.height);I=false}if(aa){T5.D.getCenter(x);var fa=(Y?Y:1)/2,da=T5.V.diff(T,y);if(R=S?new T5.Vector(y.x+da.x,y.y+
da.y):new T5.Vector(y.x-da.x*fa,y.y-da.y*fa))r=T5.V.offset(r,R.x,R.y)}q.save();try{$=L;if(B!==1)q.scale(B,B);else if(aa){q.translate(y.x,y.y);q.scale(L,L)}for(ca=o.length;ca--;)if(o[ca].shouldDraw(H)){var ha=o[ca].draw(q,r,x,H,w);A+=ha?ha:0}}finally{q.restore()}GT.Log.trace("draw complete",Q);N=false;return A}function p(){var q=0,r=!M&&(U===P||U===ea);X=T5.time();D.x=Math.floor(D.x);D.y=Math.floor(D.y);if(r){T5.cancelAnimation(function(H){return H.cancelOnInteract});K=false;if(G!==0){clearTimeout(G);
G=0}}for(r=o.length;r--;){var A=o[r].cycle(X,D,U);q+=A?A:0}q+=n(z,D);J=0;if(F+q>0)u();else if(!K&&G===0)G=setTimeout(s,500);GT.Log.trace("Completed draw cycle",X)}function u(){F++;if(!(v||J!==0)){F=0;J=setTimeout(p,0)}}c=T5.ex({id:GT.objId("view"),container:"",fastDraw:false,inertia:true,pannable:true,scalable:true,panAnimationEasing:T5.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,adjustScaleFactor:null,autoSize:false},c);var o=[],C=document.getElementById(c.container),
z=null,D=new T5.Vector,I=false,v=false,B=1,x=null,F=0,y=null,K=false,M=false,J=0,N=false,G=0,O=0,R=null,X=0,W=false,S=null,V=null,L=1,$=1,Y=null,T=null,Z=null,ba=T5.viewState("ACTIVE"),ea=T5.viewState("PAN"),P=T5.viewState("PINCH"),U=ba,ga=T5.V.getRect,t=T5.D.getSize,E=T5.R.getCenter,w={id:c.id,deviceScaling:B,fastDraw:c.fastDraw||T5.getConfig().requireFastDraw,animate:function(q,r,A,H,Q){l(L,q,r,A,H,Q)},centerOn:function(q){w.setOffset(q.x-C.width/2,q.y-C.height/2)},getDimensions:function(){if(C)return new T5.Dimensions(C.width,
C.height)},getZoomCenter:function(){return R},getLayer:function(q){for(var r=0;r<o.length;r++)if(o[r].getId()==q)return o[r];return null},setLayer:function(q,r){for(var A=0;A<o.length;A++)if(o[A].getId()===q){o.splice(A,1);break}r&&j(q,r);u()},eachLayer:function(q){for(var r=0;r<o.length;r++)q(o[r])},clearBackground:function(){I=true},freeze:function(){v=true},unfreeze:function(){v=false;u()},needRepaint:function(){return N},snapshot:function(){},scale:function(q,r,A,H,Q){H||(H=T5.D.getCenter(x));
Q||(Q=T5.D.getCenter(x));w.animate(q,H,Q,r,A);return w},removeLayer:function(q){a:{for(var r=o.length;r--;)if(o[r].getId()==q){q=r;break a}q=-1}if(q>=0&&q<o.length){GT.say("layer.removed",{layer:o[q]});o.splice(q,1)}},getOffset:function(){return T5.V.copy(D)},setOffset:function(q,r){D.x=q;D.y=r},updateOffset:function(q,r,A,H,Q){function aa(){d(0,0);Q&&Q()}if(A){q=new T5.Vector(q,r);A=T5.tweenVector(D,q.x,q.y,A,aa,H);for(H=A.length;H--;){A[H].cancelOnInteract=true;A[H].requestUpdates(u)}}else w.setOffset(q,
r)},zoom:function(q,r,A){M=false;L=r;W=L!==1;T=T5.D.getCenter(w.getDimensions());y=L>1?T5.V.copy(q):T5.D.getCenter(w.getDimensions());S=null;clearTimeout(O);if(W){U=P;u();if(A)O=setTimeout(function(){i();L=1},parseInt(A,10))}}};GT.listen("layer.remove",function(q){q.id&&w.removeLayer(q.id)});B=T5.getConfig().getScaling();GT.observable(w);w.bind("wake",u);w.bind("invalidate",function(){N=true});if(c.pannable){w.bind("pan",m);w.bind("panEnd",d);w.bind("inertiaPan",g);w.bind("inertiaCancel",function(){M=
false;u()})}if(c.scalable){w.bind("pinchZoom",f);w.bind("pinchZoomEnd",h);w.bind("wheelZoom",b)}GT.configurable(w,["inertia","container"],GT.paramTweaker(c,null,{container:function(q,r){C=document.getElementById(r);e()}}),true);e();return w};
...
T5.TileGrid=function(c){function m(v,B){var x=T5.Vector(v*b-j.x,B*b-j.y);return new T5.ImageTile({gridX:x.x,gridY:x.y})}function g(v,B,x,F){var y=Math.floor(c.gridSize*0.2),K=new T5.Vector;if(v<0||v+x>c.gridSize)K.x=v<0?-y:y;if(B<0||B+F>c.gridSize)K.y=B<0?-y:y;return K}function d(v,B,x){var F=GT.Log.getTraceTicks(),y=0,K=c.gridSize,M=c.tileSize;new T5.Vector(K/2,K/2);if(x)f=[];if(v)for(x=0;x<K;x++)for(var J=0;J<K;J++){if(!f[y]){var N=v(J,x,i,K);N.gridX=J*M-j.x;N.gridY=x*M-j.y;f[y]=N}y++}e=v;l=B;GT.Log.trace("tile grid populated",
F);I.dirty=true;I.wakeParent()}function a(v,B){if(D){var x,F=[],y=new T5.Vector(Math.floor((v.x+j.x)*s),Math.floor((v.y+j.y)*s));tilesNeeded=false;for(var K=z;K--;)for(var M=C;M--;){x=M+y.x>=0&&M+y.x<c.gridSize?f[(K+y.y)*c.gridSize+(M+y.x)]:null;var J=new T5.Vector(M-D.x,K-D.y);if(!x){o=g(y.x,y.y,C,z);x=m(M+y.x,K+y.y)}F.push({tile:x,centerness:T5.V.absSize(J)})}F.sort(function(N,G){return G.centerness-N.centerness});p||(p=Array(F.length));for(x=F.length;x--;){p[x]=F[x].tile;I.prepTile(p[x],B)}}}c=
T5.ex({tileSize:T5.tileSize,drawGrid:false,center:new T5.Vector,gridSize:25,shiftOrigin:null,supportFastDraw:true},c);var f=Array(Math.pow(c.gridSize,2)),h=c.gridSize,b=c.tileSize,i=T5.V.offset(c.center,-Math.ceil(c.gridSize>>1)),e=null,j=new T5.Vector,l=null,k=Math.round(c.tileSize/2),s=c.tileSize?1/c.tileSize:0,n=true,p=null,u=false;new T5.Vector;var o=new T5.Vector;T5.getConfig();h=h*c.tileSize;var C,z,D,I=T5.ex(new T5.ViewLayer(c),{gridDimensions:new T5.Dimensions(h,h),dirty:false,cycle:function(v,
B,x){v=0;if(o.x!==0||o.y!==0){var F=o,y=c.shiftOrigin;if(!(F.x===0&&F.y===0)){var K=GT.Log.getTraceTicks(),M=Array(f.length);M.length=f.length;for(var J=0;J<c.gridSize;J++)for(var N=0;N<c.gridSize;N++)M[N*c.gridSize+J]=J+F.x>=0&&J+F.x<c.gridSize?f[(N+F.y)*c.gridSize+(J+F.x)]:null;f=M;i=y?y(i,F):T5.V.add(i,F);j.x+=-F.x*c.tileSize;j.y+=-F.y*c.tileSize;GT.Log.trace("tile storage shifted",K);d(e,l)}o=new T5.Vector;v++}x!==T5.viewState("PINCH")&&a(B,x);return v+I.dirty?1:0},deactivate:function(){n=false},
find:function(v){if(!v)return null;for(var B=f.length;B--;){var x=f[B];if(x&&v(x))return x}return null},prepTile:function(){},drawTile:function(){return false},getTileSize:function(){return c.tileSize},draw:function(v,B,x,F,y){if(n){var K=T5.time(),M=B.x;B=B.y;var J=true,N=y.needRepaint()||F===T5.viewState("PAN")||F===T5.viewState("PINCH")||T5.isTweening();if(!D){C=Math.ceil(x.width*s)+1;z=Math.ceil(x.height*s)+1;D=new T5.Vector(Math.floor((C-1)/2),Math.floor((z-1)/2))}if(p){if(c.drawGrid)v.strokeStyle=
"rgba(50, 50, 50, 0.3)";v.beginPath();for(x=p.length;x--;){var G=p[x];if(G){var O=G.gridX-M,R=G.gridY-B;if(!(N?false:!G.dirty)){J=I.drawTile(v,G,O,R,F)&&J;G.x=O;G.y=R}}else J=false;c.drawGrid&&v.rect(O,R,c.tileSize,c.tileSize)}v.stroke();GT.Log.trace("drawn tiles",K);J&&!u&&y.trigger("tileDrawComplete");u=J;I.dirty=false}}},getTileAtXY:function(v,B){for(var x=c.tileSize,F=p?p.length:0;F--;){var y=p[F];if(y&&v>=y.x&&B>=y.y)if(v<=y.x+x&&B<=y.y+x)return y}return null},getTileVirtualXY:function(v,B,x){v=
T5.V.add(new T5.Vector(v,B),T5.V.invert(i),j);v=new T5.Vector(v.x*c.tileSize,v.y*c.tileSize);if(x){v.x+=k;v.y+=k}return v},populate:function(v){d(v,null,true)}});GT.listen("imagecache.cleared",function(){for(var v=f.length;v--;)if(f[v])f[v].loaded=false});GT.listen("tiler.repaint",function(){for(var v=f.length;v--;)if(f[v]){f[v].x=null;f[v].y=null}});return I};
T5.ImageTileGrid=function(c){function m(){l.getParent().trigger("invalidate");l.dirty=true;l.wakeParent()}c=T5.ex({emptyTile:function(k){if(!g||k!==g.width){g=T5.newCanvas(k,k);k=g.getContext("2d");k.fillStyle="rgba(150, 150, 150, 0.01)";k.fillRect(0,0,g.width,g.height)}return g}(T5.tileSize),panningTile:function(k){function s(){var n=T5.newCanvas(32,32),p=n.getContext("2d");p.fillStyle="#BBBBBB";p.fillRect(0,0,32,32);p.fillStyle="#C3C3C3";p.fillRect(0,0,16,16);p.fillRect(16,16,16,16);return n}if(!d||
...
{drawTile:function(k,s,n,p,u){var o=s.url?i(s.url):null,C=false;if(o){k.drawImage(o,n,p);s.dirty=false;C=true}else if(u===f)d&&k.drawImage(d,n,p);else g&&k.drawImage(g,n,p);return C},prepTile:function(k,s){if(k)k.dirty=true;if(k&&(!h||s===a))i(k.url)||e(k.url,m,j)}});return l};
