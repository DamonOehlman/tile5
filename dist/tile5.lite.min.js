...
f(e,a[c])}if(!e.configure)e.configure=function(j,i){if(k[j]){for(var l=g.length;l--;){var q=g[l](j,i);if(typeof q!=="undefined")return q}return h[b]}return null}}}})();
...
COG.Loopage=function(){function f(i){return COG.extend({id:c++,frequency:0,after:0,single:false,lastTick:0,execute:function(){}},i)}function m(i){b.push(i);h()}function h(){k&&clearTimeout(k);k=setTimeout(e,0);j=true}function e(){for(var i,l=(new Date).getTime(),q=d.length;b.length>0;){var y=b.shift();for(i=q;i--;)if(d[i].id===y){d.splice(i,1);break}j=true;q=d.length}if(j){g=a;for(i=q;i--;)g=d[i].frequency<g?d[i].frequency:g}for(i=q;i--;){y=l-d[i].lastTick;if(d[i].lastTick===0||y>=d[i].frequency){d[i].execute(l,
d[i]);d[i].lastTick=l;d[i].single&&d[i].trigger("complete")}}k=q?setTimeout(e,g):0}var a=6E4,c=0,d=[],b=[],k=0,g=a,j=true;return{join:function(i){var l=new f(i);if(l.after>0)l.lastTick=(new Date).getTime()+l.after;COG.observable(l);l.bind("complete",function(){m(l.id)});d.unshift(l);h();return l},leave:m}}();if(!this.JSON)this.JSON={};
(function(){function f(g){return g<10?"0"+g:g}function m(g){a.lastIndex=0;return a.test(g)?'"'+g.replace(a,function(j){var i=b[j];return typeof i==="string"?i:"\\u"+("0000"+j.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+g+'"'}function h(g,j){var i,l,q,y,z=c,o,p=j[g];if(p&&typeof p==="object"&&typeof p.toJSON==="function")p=p.toJSON(g);if(typeof k==="function")p=k.call(j,g,p);switch(typeof p){case "string":return m(p);case "number":return isFinite(p)?String(p):"null";case "boolean":case "null":return String(p);
case "object":if(!p)return"null";c+=d;o=[];if(Object.prototype.toString.apply(p)==="[object Array]"){y=p.length;for(i=0;i<y;i+=1)o[i]=h(i,p)||"null";q=o.length===0?"[]":c?"[\n"+c+o.join(",\n"+c)+"\n"+z+"]":"["+o.join(",")+"]";c=z;return q}if(k&&typeof k==="object"){y=k.length;for(i=0;i<y;i+=1){l=k[i];if(typeof l==="string")if(q=h(l,p))o.push(m(l)+(c?": ":":")+q)}}else for(l in p)if(Object.hasOwnProperty.call(p,l))if(q=h(l,p))o.push(m(l)+(c?": ":":")+q);q=o.length===0?"{}":c?"{\n"+c+o.join(",\n"+c)+
...
{"":g})};if(typeof JSON.parse!=="function")JSON.parse=function(g,j){function i(q,y){var z,o,p=q[y];if(p&&typeof p==="object")for(z in p)if(Object.hasOwnProperty.call(p,z)){o=i(p,z);if(o!==undefined)p[z]=o;else delete p[z]}return j.call(q,y,p)}var l;g=String(g);e.lastIndex=0;if(e.test(g))g=g.replace(e,function(q){return"\\u"+("0000"+q.charCodeAt(0).toString(16)).slice(-4)});if(/^[\],:{}\s]*$/.test(g.replace(/\\(?:["\\\/bfnrt]|u[0-9a-fA-F]{4})/g,"@").replace(/"[^"\\\n\r]*"|true|false|null|-?\d+(?:\.\d*)?(?:[eE][+\-]?\d+)?/g,
...
PDON:function(b){return COG.parseData(b.responseText,"PDON")},DEFAULT:function(b){return b.responseText}},d={CONTENT_TYPE:"Content-Type"};COG.xhr=function(b){function k(){if(this.readyState===4){var l=null,q=!this.status&&location.protocol==="file:"||this.status>=200&&this.status<300||this.status===304||this.status===1223||this.status===0;try{if(q)if(b.handleResponse)b.handleResponse(this);else{var y=b,z=this.getResponseHeader(d.CONTENT_TYPE),o,p=false;for(o in m)if(z&&z.indexOf(m[o])>=0){p=true;
break}z=!p;for(p=0;p<e.length;p++)z=z||e[p]==o;if(z)b:{z=o;for(var u in h)for(p=0;p<h[u].length;p++)if(RegExp(h[u][p]+"$","i").test(y.url)){o=u;break b}o=z?z:"DEFAULT"}try{l=c[o](this,y)}catch(v){l=c.DEFAULT(this,y)}}else b.error&&b.error(this)}catch(P){COG.Log.exception(P,"PROCESSING AJAX RESPONSE")}q&&l&&b.success&&b.success.call(this,l)}}try{b=COG.extend({method:"GET",data:null,url:null,async:true,success:null,handleResponse:null,error:null,contentType:"application/x-www-form-urlencoded"},b);var g=
...
COG.Touch=function(){function f(x,C){return{x:x.x-C.x,y:x.y-C.y}}function m(x,C){var G=f(x,C);return Math.sqrt(G.x*G.x+G.y*G.y)}function h(x){if(x.count>1)return m(x.touches[0],x.touches[1]);return 0}function e(x,C){var G=x&&x.count>0?x.touches[0]:null;if(G&&C&&C.count>0)return f(G,C.touches[0]);return null}function a(x,C){x.count=C.count;for(var G=g;G--;){x.touches[G].x=C.touches[G].x;x.touches[G].y=C.touches[G].y}}function c(){for(var x={count:0,touches:Array(g)},C=g;C--;)x.touches[C]=k();return x}
function d(x){if(x.preventDefault){x.preventDefault();x.stopPropagation()}else x.cancelBubble&&x.cancelBubble()}function b(x,C,G){if(D){C=C[G?G:"touches"];G=C.length;for(G=x.count=G;G--;){x.touches[G].x=C[G].pageX;x.touches[G].y=C[G].pageY}}else if(C.button===0){x.count=1;x.touches[0].x=C.pageX?C.pageX:C.screenX;x.touches[0].y=C.pageY?C.pageY:C.screenY}else x.count=0}function k(x,C){return{x:x?x:0,y:C?C:0}}var g=10,j=120,i=100,l=250,q=300,y=5,z=0,o=1,p=2,u=7,v=0,P=0,D=undefined,ga=function(x){function C(w,
...
w=w.offsetParent}while(w)}return k(s,n)}function $(w){var s=w.count,n=Array(s),r=fa(aa);for(s=s;s--;)n[s]=k(w.touches[s].x-r.x,w.touches[s].y-r.y);return n}function Q(){V&&V.trigger.apply(null,arguments)}function t(w,s){var n=fa(aa);n=k(s.x-n.x,s.y-n.y);Q(w,s,n)}function K(w){var s=w.target?w.target:w.srcElement;if(la||s&&s===aa){b(I,w);if(I.count!==0){X=null;U.x=0;U.y=0;ia=true;L=false;ea=(new Date).getTime();d(w);s.style.cursor="move";Q("inertiaCancel");ma=ea;if(w=I.count>0?I.touches[0]:null){Q("touchStart",
w.x,w.y);if(ma-ja<q)if((w=f(I.touches[0],O.touches[0]))&&Math.abs(w.x)<x.maxDistDoubleTap&&Math.abs(w.y)<x.maxDistDoubleTap)L=true;Y=z;a(O,I)}else COG.Log.warn("Touch start fired, but no touch vector found")}}}function A(w){var s=w.target?w.target:w.srcElement,n=0;if(la||s&&s===aa){b(R,w);if(R.count>0){Z.x=R.touches[0].x;Z.y=R.touches[0].y}if(ia){D&&d(w);if(R.count>1){I.count===1&&a(I,R);n=h(I)-h(R)}if(Y===z)if((w=e(R,I))&&(Math.abs(w.x)>=u||Math.abs(w.y)>=u))Y=o;if(Y!==z)if(!n||Math.abs(n)<y){if(X=
e(R,O)){U.x-=X.x;U.y-=X.y;F.x-=X.x;F.y-=X.y}Q("pan",F.x,F.y);F.x=0;F.y=0;Y=o}else{Q("pinchZoom",$(I),$(R));Y=p}a(O,R)}}}function J(w){var s=w.target?w.target:w.srcElement;if(ia&&(la||s&&s===aa)){b(W,w,"changedTouches");var n=W.touches[0];D&&d(w);w=(new Date).getTime();ja=ma;if(Y===z){t("tap",I.touches[0]);S||(S=setTimeout(function(){S=0;L&&t("doubleTap",I.touches[0])},q+50))}else if(Y==o){Q("panEnd",U.x,U.y);pa&&G(n,w)}else Y==p&&Q("pinchZoomEnd",$(I),$(O),w-ea);s.style.cursor="default";ia=false}}
function E(w){var s=w.target?w.target:w.srcElement;if(la||s&&s===aa){if(w.detail){s=-w.detail*j;s=k(w.axis===1?s:0,w.axis===2?s:0)}else s=k(w.wheelDeltaX,w.wheelDeltaY);var n=s.y!==0?Math.abs(s.y/j):0;if(Z&&n!==0){var r=k(Z.x-aa.offsetLeft,Z.y-aa.offsetTop);Q("wheelZoom",r,Math.pow(2,s.y>0?n:-n))}d(w)}}x=COG.extend({element:null,observable:null,inertiaTrigger:20,maxDistDoubleTap:20,touchStartHandler:null,moveHandler:null,moveEndHandler:null,pinchZoomHandler:null,pinchZoomEndHandler:null,tapHandler:null,
doubleTapHandler:null,wheelZoomHandler:null},x);var L=false,S=0,M=T5.getConfig(),I=c(),R=c(),O=c(),W=c(),X=null,U=k(),F=k(),Y=null,ia=false,ea=0,ba=[],Z=k(),pa=false,oa,ka,ma=0,ja=0,aa=x.element,V=x.observable,la=typeof FlashCanvas!=="undefined",qa={supportsTouch:D,addListeners:function(w){ba.push(w)},decoupleListeners:function(w){for(var s=0;w&&s<ba.length;s++)if(ba[s].listenerId===w){ba.splice(s,1);break}},release:function(){M.unbindEvent(D?"touchstart":"mousedown",K,false);M.unbindEvent(D?"touchmove":
...
2)){for(var d=arguments[0].origin.y,b=arguments[0].origin.x,k=d+arguments[0].dimensions.height,g=b+arguments[0].dimensions.width,j=Array.prototype.slice.call(arguments,1),i=j.length;i--;){var l=j[i].origin.y,q=j[i].origin.x,y=l+j[i].dimensions.height,z=q+j[i].dimensions.width;d=d<l?d:l;b=b<q?b:q;k=k>y?k:y;g=g>z?g:z}arguments[0].origin.x=b;arguments[0].origin.y=d;arguments[0].dimensions.width=g-b;arguments[0].dimensions.height=k-d}}}}(),a=function(d,b){return{width:d?d:0,height:b?b:0}},c=function(){return{getAspectRatio:function(d){return d.height!==
...
regex:/froyo/i,eventTarget:document.body,supportsTouch:true,bridgeNotify:a}};i=[j.froyo,j.android,j.ipod,j.iphone,j.ipad,j.ie]}var j=null,i=[],l=null,q=0,y=[],z=["view.wake","tile.loaded"];T5.getConfig=function(){j||g();if(!l){COG.Log.info("ATTEMPTING TO DETECT PLATFORM: UserAgent = "+navigator.userAgent);for(var o=0;o<i.length;o++){var p=i[o];if(p.regex&&p.regex.test(navigator.userAgent)){l=T5.ex({},j.base,p);COG.Log.info("PLATFORM DETECTED AS: "+l.name);break}}if(!l){COG.Log.warn("UNABLE TO DETECT PLATFORM, REVERTING TO BASE CONFIGURATION");
...
T5.Images=function(){function f(){if(!q){var o=T5.getConfig().maxImageLoads;q=COG.Loopage.join({execute:function(p,u){if(!o||j.length<o){var v=g.shift();if(v){j[j.length]=v;v.image.onload=h;v.image.src=T5.Resources.getPath(v.url);v.requested=T5.ticks()}else u.trigger("complete")}},frequency:10});q.bind("complete",function(){q=null})}}function m(){y=true;try{var o=i.length/2>>0;if(o>0){i.sort(function(u,v){return u.created-v.created});for(var p=o;p--;)delete c[i[p].url];i.splice(0,o)}}finally{y=false}COG.say("imagecache.cleared")}
function h(){var o=b[this.id];if(o&&o.image.complete&&o.image.width>0){o.loaded=true;o.hitCount=1;for(var p=j.length;p--;)if(j[p].image.src==this.src){j.splice(p,1);break}if(o.background||o.postProcess||o.drawBackground||o.customDraw)if(o.image){p=a(o.realSize?o.realSize.width:image.width,o.realSize?o.realSize.height:image.height);var u=p.getContext("2d"),v=o.offset?o.offset:new T5.Vector;o.background&&u.drawImage(o.background,0,0);o.drawBackground&&o.drawBackground(u);o.customDraw?o.customDraw(u,
o):u.drawImage(o.image,v.x,v.y);o.postProcess&&o.postProcess(u,o);o.image=p}o.loadCallback&&o.loadCallback(this,false);i[i.length]={url:this.src,created:o.requested};delete b[this.id];f()}}function e(o,p,u){var v=c[o];if(v){v.hitCount++;v.image.complete&&p&&p(v.image,true)}else{v=T5.ex({url:o,image:new Image,loaded:false,created:T5.ticks(),requested:null,hitCount:0,loadCallback:p},u);v.image.id="resourceLoaderImage"+k++;c[o]=v;b[v.image.id]=v;g[g.length]=v;f()}return v}function a(o,p){var u=document.createElement("canvas");
u.width=o?o:0;u.height=p?p:0;if(typeof FlashCanvas!=="undefined"){u.id="tmpCanvas"+d++;u.style.cssText="position: absolute; top: -"+(p-1)+"px; left: -"+(o-1)+"px;";document.body.appendChild(u);FlashCanvas.initElement(u)}typeof G_vmlCanvasManager!=="undefined"&&G_vmlCanvasManager.initElement(u);return u}var c={},d=0,b={},k=0,g=[],j=[],i=[],l=0,q=null,y=false,z={avgImageSize:25,loadTimeout:10,cancelLoad:function(){j=[]},get:function(o,p){var u=null,v=null;y||(u=c[o]);if((v=u?u.image:null)&&(v.getContext||
v.complete&&v.width>0))return v;else p&&e(o,p);return null},load:e,newCanvas:a,stats:function(){return{imageLoadingCount:j.length,queuedImageCount:g.length,imageCacheFullness:l}}};COG.Loopage.join({execute:function(o){for(var p=false,u=0,v=T5.getConfig();u<j.length;)if(o-j[u].requested>z.loadTimeout*1E3){j.splice(u,1);p=true}else u++;p&&f();if(v&&v.imageCacheMaxSize){l=i.length*z.avgImageSize/v.imageCacheMaxSize;l>=1&&m()}},frequency:2E4});return z}();
...
i*7.5625*g*g+j:g<2/2.75?i*(7.5625*(g-=1.5/2.75)*g+0.75)+j:g<2.5/2.75?i*(7.5625*(g-=2.25/2.75)*g+0.9375)+j:i*(7.5625*(g-=2.625/2.75)*g+0.984375)+j},bounceinout:function(g,j,i,l){return g<l/2?k.bouncein(g*2,0,i,l)/2+j:k.bounceout(g*2-l,0,i,l)/2+i/2+j},cubicin:function(g,j,i,l){return i*(g/=l)*g*g+j},cubicout:function(g,j,i,l){return i*((g=g/l-1)*g*g+1)+j},cubicinout:function(g,j,i,l){if((g/=l/2)<1)return i/2*g*g*g+j;return i/2*((g-=2)*g*g+2)+j},elasticin:function(g,j,i,l,q,y){if(g==0)return j;if((g/=
l)==1)return j+i;y||(y=l*0.3);if(!q||q<h(i)){q=i;i=y/4}else i=y/f*c(i/q);return-(q*e(2,10*(g-=1))*a((g*l-i)*f/y))+j},elasticout:function(g,j,i,l,q,y){var z;if(g==0)return j;if((g/=l)==1)return j+i;y||(y=l*0.3);if(!q||q<h(i)){q=i;z=y/4}else z=y/f*c(i/q);return q*e(2,-10*g)*a((g*l-z)*f/y)+i+j},elasticinout:function(g,j,i,l,q,y){var z;if(g==0)return j;if((g/=l/2)==2)return j+i;y||(y=l*0.3*1.5);if(!q||q<h(i)){q=i;z=y/4}else z=y/f*c(i/q);if(g<1)return-0.5*q*e(2,10*(g-=1))*a((g*l-z)*f/y)+j;return q*e(2,
-10*(g-=1))*a((g*l-z)*f/y)*0.5+i+j},quadin:function(g,j,i,l){return i*(g/=l)*g+j},quadout:function(g,j,i,l){return-i*(g/=l)*(g-2)+j},quadinout:function(g,j,i,l){if((g/=l/2)<1)return i/2*g*g+j;return-i/2*(--g*(g-2)-1)+j},sinein:function(g,j,i,l){return-i*d(g/l*m)+i+j},sineout:function(g,j,i,l){return i*a(g/l*m)+j},sineinout:function(g,j,i,l){return-i/2*(d(Math.PI*g/l)-1)+j}};T5.easing=function(g){return k[g.replace(/[\-\_\s\.]/g,"").toLowerCase()]};T5.registerEasingType=function(g,j){k[g.replace(/[\-\_\s\.]/g,
"").toLowerCase()]=j}})();
...
T5.View=function(f){function m(n,r,B,H){V=ma;u();if(H&&f.inertia){E=true;g(T+r,x+B,f.panAnimationEasing,f.panAnimationDuration)}else H||g(T+r,x+B)}function h(){V=ka;E=false;COG.Loopage.join({execute:u,after:50,single:true})}function e(n,r){O=la(n);W=la(r);var B=qa(O.dimensions),H=qa(W.dimensions);ba=w(O);A=w(W);F=O&&B!==0?H/B:1}function a(n,r,B){e(r,B);if(R=F!==1){V=ja;u()}}function c(n,r,B,H){e(r,B);if(f.adjustScaleFactor){F=f.adjustScaleFactor(F);COG.Log.info("scale factor adjusted to: "+F)}if(H<
f.pinchZoomAnimateTrigger){l(ia,F,ba,q(),T5.easing("sine.out"),function(){b();F=1},300);F=ia}else{b();F=1}}function d(n,r,B){s.zoom(r,Math.min(Math.pow(2,Math.round(Math.log(B))),8),500)}function b(){var n=O?q():A;F=Math.round(F*10)/10;R=false;for(var r=v.length;r--;)v[r].trigger("scale",F,n);s.trigger("scale",F,n);V=ka;u()}function k(){clearTimeout(U);U=setTimeout(j,50)}function g(n,r,B,H,N){function ca(){da+=1;if(da>=2){h(0,0);N&&N()}}var da=0;if(B){n=T5.tweenValue(T,n,B,ca,H);r=T5.tweenValue(x,
r,B,ca,H);n.cancelOnInteract=true;n.requestUpdates(function(ha){T=ha;E=true;u()});r.cancelOnInteract=true;r.requestUpdates(function(ha){x=ha;E=true;u()})}else{T=n;x=r}}function j(){var n;if(D){COG.Touch.release(D);if(f.autoSize&&D.parentNode){n=D.parentNode.getBoundingClientRect();if(n.height!==0&&n.width!==0){D.height=n.height;D.width=n.width}}try{if(!D.id)D.id=f.id+"_canvas";ga=D.getContext("2d");ga.clearRect(0,0,D.width,D.height)}catch(r){COG.Log.exception(r);throw Error("Could not initialise canvas on specified view element");
...
B.zindex;if(N===0)N=H.added-B.added;return N});P=v.length;return r}function l(n,r,B,H,N,ca,da){function ha(){ca&&ca();b();F=1;ea=null}R=true;ba=T5.V.copy(B);A=T5.V.copy(H);O=null;if(N)T5.tweenValue(0,r-n,N,ha,da?da:1E3).requestUpdates(function(na){ea=na/(r-n);F=n+na;V=ja;u();s.trigger("animate")});else{F=targetScaleFactor;ha()}}function q(){var n=T5.D.getCenter(t),r=T5.V.distance([A,n]),B=T5.V.theta(A,n,r),H=T5.V.diff(ba,A);n=T5.V.pointOnEdge(A,n,B,r/F);n.x+=H.x;n.y+=H.y;return n}function y(){s.trigger("idle");
J=true;L=0}function z(n,r,B,H){var N=s.overrideState?s.overrideState:E?ma:$?T5.viewState("FROZEN"):V,ca=(N&ja)!==0,da=0;if(G||ca){n.clearRect(0,0,D.width,D.height);G=false}if(ca){T5.D.getCenter(t);var ha=(ea?ea:1)/2,na=T5.V.diff(ba,A);if(O){M.x=A.x+na.x;M.y=A.y+na.y}else{M.x=A.x-na.x*ha;M.y=A.y-na.y*ha}r=T5.V.offset(r,M.x,M.y)}n.save();try{ia=F;if(Q!==1)n.scale(Q,Q);else if(ca){n.translate(A.x,A.y);n.scale(F,F)}for(da=P;da--;)if(v[da].shouldDraw(N,r,B)){v[da].draw(n,r,t,N,s,B,H);v[da].trigger("drawComplete",
r,H)}}finally{n.restore()}s.trigger("drawComplete",r,H);COG.Log.trace("draw complete",H)}function o(n,r){var B=false,H=!E&&(V===ja||V===ma),N=X||V===T5.viewState("PAN")||V===T5.viewState("PINCH")||T5.isTweening();C.x=T>>0;C.y=x>>0;if(H){T5.cancelAnimation(function(ca){return ca.cancelOnInteract});J=false;if(L!==0){clearTimeout(L);L=0}}for(H=P;H--;){if(v[H].animated)V|=aa;B=v[H].cycle(n,C,V,N)||B}N=N||(V&aa)!==0;if(B=B||N||F!==1&&F!==Y){z(ga,C,N,n);Y=F;X=false}if(!B&&K===0&&!Z){if(!J&&L===0)L=setTimeout(y,
500);r.trigger("complete")}K=0;COG.Log.trace("Completed draw cycle",n)}function p(){X=true;u()}function u(){K+=1;if(!($||fa)){fa=COG.Loopage.join({execute:o,frequency:pa});fa.bind("complete",function(){fa=null})}}f=T5.ex({id:COG.objId("view"),container:"",fastDraw:false,inertia:true,pannable:true,scalable:true,panAnimationEasing:T5.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,adjustScaleFactor:null,autoSize:true,tapExtent:10,fps:25},f);var v=[],P=0,D=document.getElementById(f.container),
ga=null,T=0,x=0,C=new T5.Vector,G=false,fa=null,$=false,Q=1,t=new T5.Dimensions,K=0,A=null,J=false,E=false,L=0,S=0,M=new T5.Vector,I=0,R=false,O=null,W=null,X=false,U=0,F=1,Y=0,ia=1,ea=null,ba=null,Z=typeof FlashCanvas!=="undefined",pa=~~(1E3/f.fps),oa=null,ka=T5.viewState("ACTIVE"),ma=T5.viewState("PAN"),ja=T5.viewState("PINCH"),aa=T5.viewState("ANIMATING"),V=ka,la=T5.V.getRect,qa=T5.D.getSize,w=T5.R.getCenter,s={id:f.id,deviceScaling:Q,fastDraw:f.fastDraw||T5.getConfig().requireFastDraw,stateOverride:null,
animate:function(n,r,B,H,N){l(F,n,r,B,H,N)},centerOn:function(n){T=n.x-D.width/2;x=n.y-D.height/2},getDimensions:function(){return t},getZoomCenter:function(){return M},getLayer:function(n){for(var r=0;r<P;r++)if(v[r].getId()==n)return v[r];return null},setLayer:function(n,r){for(var B=0;B<P;B++)if(v[B].getId()===n){v.splice(B,1);break}r&&i(n,r);p()},eachLayer:function(n){for(var r=P;r--;)n(v[r])},clearBackground:function(){COG.Log.info("CALL OF DEPRECATED METHOD CLEAR BACKGROUND");G=true;p()},invalidate:p,
freeze:function(){$=true},unfreeze:function(){$=false;u()},resize:function(n,r){if(D){f.autoSize=false;D.width=n;D.height=r;j();s.trigger("resize",D.width,D.height)}},scale:function(n,r,B,H,N){H||(H=T5.D.getCenter(t));N||(N=T5.D.getCenter(t));s.animate(n,H,N,r,B);return s},removeLayer:function(n){a:{for(var r=P;r--;)if(v[r].getId()==n){n=r;break a}n=-1}if(n>=0&&n<P){COG.say("layer.removed",{layer:v[n]});v.splice(n,1);p()}P=v.length},getOffset:function(){return C},updateOffset:g,zoom:function(n,r,
B){E=false;F=r;R=F!==1;ba=T5.D.getCenter(t);A=F>1?T5.V.copy(n):T5.D.getCenter(t);O=null;clearTimeout(S);if(R){V=ja;u();if(B)S=setTimeout(function(){b();F=1},parseInt(B,10))}}};Q=T5.getConfig().getScaling();s.markers=i("markers",new T5.MarkerLayer);COG.observable(s);s.bind("wake",u);s.bind("invalidate",p);if(f.pannable){s.bind("pan",m);s.bind("panEnd",h);s.bind("inertiaCancel",function(){E=false;u()})}if(f.scalable){s.bind("pinchZoom",a);s.bind("pinchZoomEnd",c);s.bind("wheelZoom",d)}s.bind("tap",
function(n,r,B){for(var H=T5.V.offset(B,T,x),N=v.length;N--;)n.cancel=n.cancel||v[N].trigger("tap",r,B,H).cancel});COG.configurable(s,["inertia","container","rotation","tapExtent"],COG.paramTweaker(f,null,{container:function(n,r){D=document.getElementById(r);j()},rotation:function(n,r){I=r}}),true);j();f.autoSize&&window.addEventListener("resize",k,false);return s};
T5.PathLayer=function(f){function m(j){j.syncVectors(c);d&&j.syncVectors(d);g.trigger("tidy")}f=T5.ex({style:"waypoints",pixelGeneralization:8,zindex:50},f);var h=false,e=[],a=null,c=[],d=null,b=0,k=[],g=T5.ex(new T5.ViewLayer(f),{getAnimation:function(j,i,l,q){var y="pathAnimation"+b++;k.push(y);return new T5.AnimatedPathLayer({id:y,path:e,zindex:f.zindex+1,easing:j?j:T5.easing("sine.inout"),duration:i?i:5E3,drawIndicator:l,autoCenter:q?q:false})},cycle:function(j,i,l,q){return q},draw:function(j,
i){var l,q=e.length;j.save();try{T5.applyStyle(j,f.style);if(q>0){j.beginPath();j.moveTo(e[q-1].x-i.x,e[q-1].y-i.y);for(l=q;l--;)j.lineTo(e[l].x-i.x,e[l].y-i.y);j.stroke();if(a){j.fillStyle=f.waypointFillStyle;for(l=a.length;l--;){j.beginPath();j.arc(a[l].x-i.x,a[l].y-i.y,2,0,Math.PI*2,false);j.stroke();j.fill()}}}}finally{j.restore()}h=false},updateCoordinates:function(j,i){var l=g.getParent();l=l?l.getTileLayer():null;c=j;d=i;COG.Log.info("updating coordinates, grid = "+l);l&&m(l)}});g.bind("gridUpdate",
...
T5.Poly=function(f,m){m=T5.ex({fill:false,style:null},m);var h=false,e=m.fill,a=m.style,c=[],d={draw:function(b,k,g){if(h){var j=true,i=a?T5.applyStyle(b,m.style):null;b.beginPath();for(var l=c.length;l--;){var q=c[l].x-k,y=c[l].y-g;if(j){b.moveTo(q,y);j=false}else b.lineTo(q,y)}e&&b.fill();b.stroke();i&&T5.applyStyle(b,i)}},resync:function(b){b.syncVectors(f);c=f.length<=3?f:T5.V.simplify(f)}};h=f&&f.length>=2;return d};
...
T5.Marker=function(f){function m(g,j,i,l){e=g;a=j;c=i;d=l}f=T5.ex({xy:new T5.Vector,offset:true,tweenIn:null,animationSpeed:null},f);var h=false,e=0,a=0,c=0,d=0,b=f.offset,k=T5.ex(f,{isNew:true,isAnimating:function(){return h},draw:function(g,j,i,l,q){if(k.isNew&&f.tweenIn){var y=k.xy.y;k.xy.y=j.y-20;h=true;T5.tween(k.xy,"y",y,f.tweenIn,function(){k.xy.y=y;h=false},f.animationSpeed?f.animationSpeed:250+Math.random()*500)}k.drawMarker(g,j,k.xy.x-(b?j.x:0),k.xy.y-(b?j.y:0),i,l,q);k.isNew=false},drawMarker:function(g,
...
T5.TileGrid=function(f){function m(t,K){var A=T5.Vector(t*k-q.x,K*k-q.y);return new T5.EmptyTile({gridX:A.x,gridY:A.y,empty:true})}function h(t,K,A,J){var E=Math.max(d,b)*0.2>>0,L=new T5.Vector;if(t<0||t+A>d)L.x=t<0?-E:E;if(K<0||K+J>b)L.y=K<0?-E:E;return L}function e(t,K,A){var J=COG.Log.getTraceTicks(),E=0;new T5.Vector(d/2,b/2);if(A)c=[];if(t)for(A=0;A<b;A++)for(var L=0;L<d;L++){if(!c[E]){var S=t(L,A,i,d,b);S.gridX=L*k-q.x;S.gridY=A*k-q.y;c[E]=S}E++}l=t;z=K;COG.Log.trace("tile grid populated",J);
Q.changed()}function a(t,K,A,J){var E,L=[],S=false;t=new T5.Vector((t.x+q.x)*u>>0,(t.y+q.y)*u>>0);tilesNeeded=false;if(!G){E=Q.getParent().getDimensions();x=Math.ceil(E.width*u)+1;C=Math.ceil(E.height*u)+1;G=new T5.Vector((x-1)/2>>0,(C-1)/2>>0)}for(var M=C;M--;)for(var I=x;I--;){E=I+t.x>=0&&I+t.x<d?c[(M+t.y)*d+(I+t.x)]:null;var R=new T5.Vector(I-G.x,M-G.y);if(!E){T=h(t.x,t.y,x,C);E=m(I+t.x,M+t.y)}S=E?(E.dirty=A||E.dirty)||S:S;L[L.length]={tile:E,centerness:T5.V.absSize(R)}}L.sort(function(O,W){return W.centerness-
O.centerness});P||(P=Array(L.length));for(A=L.length;A--;){P[A]=L[A].tile;Q.prepTile(P[A],K)}ga=J;return S}f=T5.ex({tileSize:T5.tileSize,center:new T5.Vector,gridSize:25,shiftOrigin:null,supportFastDraw:true,allowShift:true},f);var c=Array(Math.pow(f.gridSize,2)),d=f.gridSize,b=f.gridSize,k=f.tileSize,g=b*k,j=d*k,i=T5.V.offset(f.center,-Math.ceil(d>>1)),l=null,q=new T5.Vector,y=false,z=null,o=Math.round(k/2),p=false,u=k?1/k:0,v=true,P=null,D=false,ga=0;new T5.Vector;var T=new T5.Vector;T5.getConfig();
var x,C,G,fa=T5.viewState("ANIMATING"),$=T5.viewState("PINCH"),Q=T5.ex(new T5.ViewLayer(f),{gridDimensions:new T5.Dimensions(j,g),cycle:function(t,K,A,J){if(T.x!==0||T.y!==0){J=T;var E=f.shiftOrigin;if(!(!f.allowShift||J.x===0&&J.y===0)){var L=COG.Log.getTraceTicks(),S=Array(c.length);S.length=c.length;for(var M=0;M<d;M++)for(var I=0;I<b;I++)S[I*d+M]=M+J.x>=0&&M+J.x<d?c[(I+J.y)*d+(M+J.x)]:null;c=S;i=E?E(i,J):T5.V.add(i,J);q.x+=-J.x*k;q.y+=-J.y*k;COG.Log.trace("tile storage shifted",L);e(l,z)}T=new T5.Vector;
J=true}if(!p&&(A&$)===0)p=a(K,A,J,t);y=(A&fa)!==0||T5.isTweening();return p},find:function(t){if(!t)return null;for(var K=c.length;K--;){var A=c[K];if(A&&t(A))return A}return null},prepTile:function(){},drawTile:function(){return false},getTileSize:function(){return k},clearTileRect:function(t,K,A,J){t.clearRect(K,A,J,J)},draw:function(t,K,A,J,E,L,S){var M=K.x;K=K.y;var I=A.width,R=A.height,O,W=true;if(P){t.beginPath();if(v)for(var X=P.length;X--;){O=P[X];var U=O.gridX-M,F=O.gridY-K;O.x=U;O.y=F;if(O.empty){COG.Log.info("empty tile @ x: "+
U+", y: "+F);Q.clearTileRect(t,U,F,k,J);W=false}else if(L||O.dirty){y&&Q.clearTileRect(t,U,F,k,J);(O=Q.drawTile(t,O,U,F,J,L,S))&&t.rect(U,F,k,k);W=W&&O}I=U<I?U:I;R=F<R?F:R}t.clip();I>0&&t.clearRect(0,0,I,A.height);R>0&&t.clearRect(0,0,A.width,R);W&&!D&&E.trigger("tileDrawComplete");p=false;D=W}},getTileAtXY:function(t,K){for(var A=null,J=P?P.length:0;J--;){var E=P[J];if(E&&t>=E.x&&K>=E.y)if(t<=E.x+k&&K<=E.y+k){A=E;break}}COG.Log.info("looking for tile @ x: "+t+", y: "+K+", found: "+A);return A},getTileVirtualXY:function(t,
K,A){t=T5.V.add(new T5.Vector(t,K),T5.V.invert(i),q);t=new T5.Vector(t.x*k,t.y*k);if(A){t.x+=o;t.y+=o}return t},populate:function(t){e(t,null,true)},syncVectors:function(){}});Q.bind("resize",function(){COG.Log.info("captured resize");G=null});Q.bind("scale",function(){v=false});COG.listen("imagecache.cleared",function(){for(var t=c.length;t--;)if(c[t])c[t].loaded=false});COG.listen("tiler.repaint",function(){for(var t=c.length;t--;)if(c[t]){c[t].x=null;c[t].y=null}});return Q};
...
i=T5.ex(new T5.TileGrid(f),{clearTileRect:function(l,q,y,z,o){(o&c)===0&&l.clearRect(q,y,z,z)},drawTile:function(l,q,y,z,o){var p=q.url?k(q.url):null,u=false;if(p){l.drawImage(p,y,z);u=true}else if((o&c)!==0){l.drawImage(e,y,z);u=true}q.dirty=false;return u},initTileUrl:function(){},prepTile:function(l,q){if(l&&!l.loading&&(!d||q===a)){l.url||i.initTileUrl(l);if(!k(l.url)){l.loading=true;g(l.url,function(){l.loaded=true;l.dirty=true;i.changed()},j)}}}});return i};
