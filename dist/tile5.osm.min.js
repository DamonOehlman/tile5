...
"";h=h.replace(b[g],j)}return h}}function sb(a,c,b){Ia.trigger("userMessage",a,c,b)}function tb(a,c){var b=Ta*ja(a);return Ua-2*ub(c*za((1-b)/(1+b),Ta/2))}function T(a,c){if(N(a,na)){var b=a.split(fb);this.x=parseFloat(b[0]);this.y=parseFloat(b[1])}else if(a&&a.x){this.x=a.x;this.y=a.y}else{this.x=a||0;this.y=c||0}}function Ja(a){this.allowCull=a}function ha(a,c,b,f){this.mercX=b;this.mercY=f;if(N(a,na))D(this,Mb.parseXY(a));else a&&a.toPixels?D(this,a.toPixels()):T.call(this,a,c)}function ta(a,c,
...
s.button)==1}function y(s,L,P,da,ka){P=typeof P!="undefined"?P:G;da=typeof da!="undefined"?da:R;var z=P-S,F=da-M,I=f(P,da);n.triggerCustom(L,h("mouse",s),I,e(I,i(l)),f(z,F));if(typeof ka=="undefined"||ka){S=P;M=da}}o=D({},o);var B=120,u=B*8,C=false,x,G,R,S,M;o.binder("mousedown",w);o.binder("mousemove",A);o.binder("mouseup",E);o.binder("dblclick",function(s){sa("captured double click");if(j(s,l)){var L=v(s);n.triggerCustom("doubleTap",h("mouse",s),L,e(L,i(l)))}});o.binder("mousewheel",K);o.binder("DOMMouseScroll",
...
O,Q;if(I.length===0)return null;Q=O={x:I[0].pageX,y:I[0].pageY,id:I[0].identifier,count:I.length};for(var $=1,Da=I.length;$<Da;$++)Q=Q.next={x:I[$].pageX,y:I[$].pageY,id:I[$].identifier};return O}function E(z){if(j(z,l)){R=i(l);var F=A(z,"changedTouches"),I=w(F,R.left,R.top);if(!M){S=C;n.triggerCustom("pointerDown",h("touch",z),F,I)}da&&n.triggerCustom("pointerDownMulti",h("touch",z),F,I);M=A(z);if(M.count>1)L=v(M);ka=1;P=w(M)}}function K(z){if(j(z,l)){k(z);s=A(z);if(S==C)S=Math.abs(M.x-s.x)>B||Math.abs(M.y-
s.y)>B?u:C;if(S!=C){S=s.count>1?G:x;if(S==G)if(M.count===1){M=w(s);L=v(M)}else{var F=v(s);if(Math.abs(L-F)<y)S=x;else{var I;I=s.x;var O=s.next.x,Q=s.y,$=s.next.y;I={x:(I<O?I:O)+(Math.abs(I-O)>>1),y:(Q<$?Q:$)+(Math.abs(Q-$)>>1)};F=F/L;O=F-ka;n.triggerCustom("zoom",h("touch",z),I,e(I,R),O,"pinch");ka=F}}S==x&&n.triggerCustom("pointerMove",h("touch",z),s,w(s,R.left,R.top),f(s.x-P.x,s.y-P.y));da&&n.triggerCustom("pointerMoveMulti",h("touch",z),s,w(s,R.left,R.top))}P=w(s)}}function p(z){if(j(z,l)){var F=
A(z,"changedTouches"),I=w(F,R.left,R.top);s=A(z);if(!s){n.triggerCustom("pointerUp",h("touch",z),F,I);M=null}da&&n.triggerCustom("pointerUpMulti",h("touch",z),F,I)}}o=D({detailed:false,inertia:false},o);var y=20,B=7,u=0,C=1,x=2,G=3,R,S,M,s,L,P,da=o.detailed,ka=1;o.binder("touchstart",E);o.binder("touchmove",K);o.binder("touchend",p);sa("initialized touch handler");return{unbind:function(){o.unbinder("touchstart",E);o.unbinder("touchmove",K);o.unbinder("touchend",p)}}},checks:{touch:true}});return{register:g,
watch:function(l,n,o){n=D({bindTarget:null,observable:null,isIE:typeof window.attachEvent!="undefined",types:null},n);capabilities=D({touch:"ontouchstart"in window},o);if(!n.observable){sa("creating observable");n.observable=ua({});globalOpts=n}n.binder=(n.isIE?b:a)(n.bindTarget||document);n.unbinder=(n.isIE?b:c)(n.bindTarget||document);o=n.types;for(var v=capabilities,w=[],A=d.length;A--;){var E=d[A],K=!o||o.indexOf(E.type)>=0,p=true,y;for(y in E.checks){var B=E.checks[y];sa("checking "+y+" capability. require: "+
B+", capability = "+v[y]);p=p&&B===v[y]}if(K&&p)w[w.length]=E.handler}return new m(l,w,n)}}}();var Ia=ua({}),Za=function(){var a={};return{create:function(c,b){if(a[c][b])return a[c][b].apply(null,Array.prototype.slice.call(arguments,2));throw Qb(c,b);},get:function(c,b){return a[c]?a[c][b]:null},register:function(c,b,f){a[c]||(a[c]={});a[c][b]&&sa(Rb(c,b),"warn");a[c][b]=f}}}(),Rb=ya("Registration of {0}: {1} will override existing definition"),Qb=ya("Could not create {0} of type: {1}");ya("Could not create drawable of type: {0}");
...
[];return{process:function(g,i,h,j){function e(m){m=(k||b)/m;if(d<g.length){var l=g.slice(d,d+m);m=l.length;var n=k?0:(new Date).getTime();i(l,m);if(!k){k=(l=((new Date).getTime()-n)/m)?c/l|0:g.length;sa("calculated that we can process "+k+" items per cycle")}d+=m}else{for(m=f.length;m--;)if(f[m]===e){f.splice(m,1);break}f.length===0&&Fa.detach(a);h&&h()}}var k,d=0;if(g.length>(j||0))f.push(e)===1&&Fa.attach(a);else{i(g,g.length);h&&h()}}}}(),$a=$a||function(a,c){var b=parseFloat(c)*aa;b=ja(b);var f=
Ta*b;b=Aa((1+b)/(1-b)*za((1-f)/(1+f),Ta))/2;return new ha(0,0,parseFloat(a)*aa,b)},mb=mb||function(a,c){for(var b=za(Math.E,-c),f=Ua-2*ub(b),g=tb(f,b),i=0;i<12&&ma(f-g)>1.0E-7;){f=g;g=tb(f,b);i++}return new ia(g*la,a%360*la)};T.prototype={constructor:T,add:function(){for(var a=this.x,c=this.y,b=arguments.length;b--;){a+=arguments[b].x;c+=arguments[b].y}return this.copy(a,c)},copy:function(a,c){var b=D({},this);b.x=a||b.x;b.y=c||b.y;return b},equals:function(a){return this.x===a.x&&this.y===a.y},offset:function(a,
c){return this.copy(this.x+a,this.y+c)},relative:function(a){a=a.viewport();return this.copy(this.x-a.x-a.padding.x,this.y-a.y-a.padding.y)},rotate:function(a,c){c=c||new T(0,0);var b=this.x-c.x,f=this.y-c.y;return new T(c.x+Ea(a)*b+-ja(a)*f,c.y+ja(a)*b+Ea(a)*f)},scale:function(a,c){c=c||new T(0,0);return new T(c.x+(this.x-c.x)*a,c.y+(this.y-c.y)*a)},sync:function(){return this},toString:function(){return this.x+", "+this.y}};Ja.prototype=D([],{cull:function(a){if(this.allowCull){var c=a.x,b=a.y,
f=a.x+a.w;a=a.y+a.h;for(var g=Infinity,i=0,h,j=this.length;j--;)if(h=this[j].x>=c&&this[j].x<=f&&this[j].y>=b&&this[j].y<=a){g=j<g?j:g;i=j>i?j:i}return this.slice(Ca(g-1,0),Ma(i+1,this.length))}return this},simplify:function(a){a=a||3;for(var c=new Ja,b=null,f=this.length;f--;){var g=this[f];if(include=!b||f===0||ma(g.x-b.x)+ma(g.y-b.y)>a){c.unshift(g);b=g}}return c}});ha.prototype=D(new T,{constructor:ha,pos:function(){return mb(this.mercX,this.mercY)},sync:function(a,c){var b;if(!(b=a.rpp)){b=a.zoom();
...
fill:"#ffffff"},waypointsHover:{lineWidth:4,stroke:"#ff0000",fill:"#ffffff"}});return{resetStyle:"reset",each:function(b){for(var f in c)b(f,c[f])},get:function(b){return c[b]},define:a}}();ba("view","view",function(a){function c(q,r,t,H){p(Ca(U+za(2,H)-1,0.125),false,true)}function b(q,r){var t=V&&V.projectXY?V.projectXY(q,r):null;if(!t){t=A();var H=1/U;t=new J.XY(t?t.x+q*H:q,t?t.y+r*H:r)}return t.sync(J,true)}function f(q,r,t){y("doubleTap",r,t,b(t.x,t.y));a.scalable&&p(2,{easing:"quad.out",duration:300},
true)}function g(q,r,t){s=null;ka=true;v("down",r,t)}function i(q,r,t){v("hover",r,t)}function h(q,r,t,H){l(r,t,false);if(!s){z=H.x;F=H.y}}function j(q,r,t){l(r,t,true);ka=false}function e(){clearTimeout(Gb);Gb=setTimeout(function(){if(M)if(M.offsetWidth!==Na*2||M.offsetHeight!==Oa*2){var q=w();o(a.container);w(q.x,q.y)}},250)}function k(q,r,t){v("tap",r,t);J.trigger("tap",r,t,b(t.x,t.y,true))}function d(){fa&&fa.unbind();if(V){fa=INTERACT.watch(V.interactTarget||M);if(a.scalable){fa.bind("zoom",
c);fa.bind("doubleTap",f)}fa.bind("pointerDown",g);fa.bind("pointerMove",h);fa.bind("pointerUp",j);a.captureHover&&fa.bind("pointerHover",i);fa.bind("tap",k)}}function m(q){if(V){V.trigger("detach");V=null}var r=J,t=R,H=M,W=a;q=q.split("/");for(var Y=new Xb(r,t,H,W),pa=0;pa<q.length;pa++)Y=wa("renderer",q[pa],r,t,H,W,Y);V=Y;da=V.fastpan&&X.transforms;d();J.trigger("reset");K()}function l(q,r,t){if(s){q=b(r.x,r.y);s.drag.call(s.target,s,q.x,q.y,t)&&ca++;if(t)s=null}}function n(q){for(var r=G;r--;)if(x[r].id===
...
margin:-ga.y+"px 0 0 "+-ga.x+"px"})));m(a.renderer);q=a.controls;for(r=0;r<C.length;r++)C[r].trigger("detach");C=[];for(r=0;r<q.length;r++)C[C.length]=wa("control",q[r],J,S,M,a[q[r]])}function v(q,r,t){var H=(new T(t.x-Na+nb,t.y-Oa+ob)).rotate(-qa*aa,Pa).scale(1/U,Pa);ea=Hits.init(q,r,t,b(t.x,t.y,true),H);for(q=G;q--;)if(x[q].visible)P=P||(x[q].hitGuess?x[q].hitGuess(ea.gridX,ea.gridY,J):false);P&&ca++}function w(q,r,t){if(N(q,na)||N(q,Ub)){r=new J.XY(q);r.sync(J);q=r.x;r=r.y}if(N(q,oa)){B(q-Na-ga.x,
r-Oa-ga.y,t);return J}else return B().offset(Na+ga.x|0,Oa+ga.y|0).sync(J,true)}function A(){var q=new ta(O,Q,bb,cb);q.scaleFactor=U;q.padding=ga?ga.copy():new T;return q}function E(q,r,t){if(N(q,na)&&N(r,va))for(r=0;r<G;r++){if(x[r].id===q)return x[r]}else if(N(q,na)){r=wa("layer",r,J,t);r.added=(new Date).getTime();r.id=q;x[n(q)]=r;x.sort(function(H,W){return W.zindex-H.zindex||W.added-H.added});G=x.length;J.trigger("resync");K();J.trigger("layerChange",J,r);ca++;return r}else return[].concat(x)}
function K(){var q=A();if(q){if(Ga||Ha)if(q){var r=db?O+(q.w>>1):O,t=eb?Q+(q.h>>1):Q,H=q.w,W=q.h;if(Ga&&Ga>H)if(r+H>Ga)O=db?O:Ga-H;else if(r<0)O=db?O:0;if(Ha&&Ha>W)if(t+W>Ha)Q=eb?Q:Ha-W;else if(t<0)Q=eb?Q:0}Hb=O;Ib=Q;J.trigger("refresh",J,q);ca++}}function p(q,r,t){if(N(q,oa)){var H=t?q:U*q;if(!Yb){r=null;q=La(Aa(H)/Math.LN2);H=za(2,q)}if(r)Qa=ra.tween([U],[H],r,function(){U=H;Qa=null;ca++});else{U=H;ca++}return J}else return U}function y(){for(var q=J.trigger.apply(null,arguments).cancel,r=x.length;r--;)q=
x[r].trigger.apply(null,arguments).cancel||q;return!q}function B(q,r,t){if(N(q,oa)){if(t)xa=ra.tween([O,Q],[q,r],t,function(){xa=null});else{O=q|0;Q=r|0;ca++}return J}else return(new J.XY(O,Q)).sync(J,true)}a=D({container:"",captureHover:true,controls:[],drawOnScale:true,padding:128,inertia:true,refreshDistance:256,pannable:true,scalable:true,renderer:"canvas"},a);var u="auto",C=[],x=[],G=0,R=null,S=null,M,s=null,L=!N(window.attachEvent,va),P=false,da,ka=false,z=0,F=0,I=a.refreshDistance,O=0,Q=0,
$=0,Da=0,Hb=0,Ib=0,Ga=null,Ha=null,db=false,eb=false,xa=null,ga,ea=null,Ra=null,V,Gb=0,Pa=new T,qa=0,Sa=null,U=1,Qa=null,pb=1,fa=null,qb={index:0,draw:false},Yb=true,ca=0,bb,cb,nb,ob,Na,Oa,J={XY:T,id:a.id,panSpeed:0,attachFrame:function(q,r){r&&R.appendChild(q)},center:w,detach:function(){V&&V.trigger("detach");fa&&fa.unbind();if(S){M.removeChild(S);R=S=null}},layer:E,invalidate:function(){ca++},pan:function(q,r,t){B(O+q,Q+r,t)},refresh:K,rotate:function(q,r,t){if(N(q,oa)){var H=t?q:qa+q;if(r)Sa=
ra.tween([qa],[H],r,function(){qa=H%360;Sa=null;ca++});else{qa=H%360;ca++}return J}else return qa},scale:p,triggerAll:y,setMaxOffset:function(q,r,t,H){Ga=q;Ha=r;db=t;eb=H},offset:B,viewport:A};ua(J);J.bind("resize",e);J.bind("layerRemove",function(q,r){var t=ib(x,r.id);if(t>=0&&t<G){x.splice(t,1);ca++}G=x.length});zb(J,a,{container:o,captureHover:d,scalable:d,pannable:d,renderer:m});E("markers","draw",{zindex:20});o(a.container);L?window.attachEvent("onresize",e):window.addEventListener("resize",
e,false);Fa.attach(function(q){var r=[],t,H,W;self.panSpeed=panSpeed=ma(z)+ma(F);t=U!==pb;if(panSpeed>0||t||xa||Qa||Sa){ca++;if(xa&&panSpeed>0){xa(true);xa=null}}if(panSpeed<2&&(ma(O-Hb)>=I||ma(Q-Ib)>=I))K();qb.index++;qb.draw=ca||panSpeed||0;if(V&&qb.draw){if(Qa)U=Qa()[0];if(Sa)qa=Sa()[0];$+=z;Da+=F;if(z||F)J.trigger("pan");if(X.transforms){if(U!==1)r[r.length]="scale("+U+")";if(qa!==0)r[r.length]="rotate("+qa+"deg)"}if(t=!da||(a.drawOnScale||U===1)&&panSpeed<5){if(xa){t=xa();O=t[0]|0;Q=t[1]|0}else{O=
O-$/U|0;Q=Q-Da/U|0}W=A();V.trigger("predraw",W);if(V.prepare(x,W,q,ea)){H=Da=t=$=ca=0;for(ii=G;ii--;){var Y=x[ii];if(Y.visible){var pa=Y.style?V.applyStyle(Y.style,true):null;Y.draw(V,W,J,q,ea);pa&&V.applyStyle(pa)}}V.trigger("render",W);J.trigger("drawComplete",W,q);X.move(R,t,H,r,Pa)}}else X.move(R,$,Da,r,Pa);if(ka||!a.inertia)F=z=0;else if(z!=0||F!=0){z*=0.8;F*=0.8;if(ma(z)<0.5)z=0;if(ma(F)<0.5)F=0}if(ea){q=true;r=ea?ea.elements:[];if(Ra&&Ra.type==="hover"){diffElements=Hits.diffHits(Ra.elements,
...
d.y2)).sync(j,true).pos(),(new ha(d.x2,d.y)).sync(j,true).pos())}function b(e,k,d){if(N(e,oa)){e=Ca(a.minZoom,Ma(a.maxZoom,e|0));if(e!==i){var m=j.viewport(),l=j.offset(),n=m.w/2;m=m.h/2;var o=za(2,e-i),v=n/o|0,w=m/o|0;i=e;j.offset(((k||l.x+n)-v)*o,((d||l.y+m)-w)*o);refreshY=refreshX=0;j.trigger("zoom",e);j.trigger("reset");g=j.rpp=Z/(256<<i);j.setMaxOffset(Z/g|0,Z/g|0,true,false);j.scale(1,false,true);j.trigger("resync");j.refresh()}return j}else return i}a=D({controls:["zoombar"],minZoom:1,maxZoom:18,
renderer:"canvas/dom",zoom:1,zoombar:{}},a);var f=new ha,g,i=a.zoom||a.zoomLevel,h=0,j=D(wa("view","view",a),{XY:ha,bounds:c,zoom:b});j.bind("refresh",function(){var e=j.viewport();if(f.x!=e.x||f.y!=e.y){j.trigger("boundsChange",c());f.x=e.x;f.y=e.y}});j.bind("scaleChanged",function(e,k){var d=Aa(k)/Math.LN2|0;if(d!==0){k=za(2,d);clearTimeout(h);h=setTimeout(function(){b(i+d)},500)}});return j});var ra=function(){return{tween:function(a,c,b,f,g){function i(o){var v=o-n;o=n+l<=o;for(var w=h;w--;){j[w]=
k(v,a[w],d[w],l);g&&g.invalidate()}if(o||m){Fa.detach(i);for(w=0;w<e.length;w++)e[w]&&e[w](j,v,m)}}b=D({easing:"sine.out",duration:1E3,complete:null},b);var h=a.length,j=[].concat(a),e=[f,b.complete],k=Pb(b.easing),d=[],m=false,l=b.duration,n=(new Date).getTime();for(b=h;b--;)d[b]=c[b]-a[b];Fa.attach(i);return function(o){m=o;return j}},tweenDrawable:function(a,c,b,f,g){var i=ra.tween([b],[f],g,function(){a[c]=f;for(var j=a.tweens.length;j--;)if(a.tweens[j]===h){a.tweens.splice(j,1);break}},a.view),
h=function(){a[c]=i()[0]};return h}}}(),Ba=function(a,c,b){b=D({style:null,xy:null,size:20,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape",zindex:0},b);D(this,b);if(N(this.xy,na))this.xy=new a.XY(this.xy);this.id="drawable_"+Wb++;this.bounds=null;this.view=a;this.layer=c;this.tweens=[];this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.observable&&ua(this)};Ba.prototype={constructor:Ba,applyTweens:function(){for(var a=this.tweens.length;a--;)this.tweens[a]()},
...
c){a&&(!this.bounds||a.x!=this.bounds.x||a.y!=this.bounds.y)&&this.trigger("move",this,a,this.bounds);this.bounds=a;if(c)this.xy=this.bounds.center()}};ba("drawable","marker",function(a,c,b){b=D({fill:true,stroke:false,markerType:"simple",hoverStyle:"highlight",typeName:"Marker"},b);return new Ba(a,c,b)});ba("drawable","poly",function(a,c,b){function f(){var d,m,l,n,o,v,w;e=b.simplify?j.simplify():j;for(d=e.length;d--;){m=e[d].x;l=e[d].y;v=N(v,va)||m<v?m:v;w=N(w,va)||l<w?l:w;n=N(n,va)||m>n?m:n;o=
N(o,va)||l>o?l:o}k.updateBounds(new ta(v,w,n-v,o-w),true)}function g(d){if(N(d,xb)){j=new Ja(b.allowCull);Eb.process(d,function(m,l){for(var n=0;n<l;n++)j.push(new a.XY(m[n]))},i,h);return k}else return e}function i(){j.length&&Eb.process(j,function(d,m){for(var l=m;l--;)d[l].sync(a)},f,h)}b=D({allowCull:false,simplify:false,fill:true,points:[],typeName:"Poly"},b);var h=500,j=new Ja(b.allowCull),e,k=D(new Ba(a,c,b),{points:g,resync:i});g(b.points);return k});ba("drawable","line",function(a,c,b){b.fill=
false;b.allowCull=true;return wa("drawable","poly",a,c,b)});ba("drawable","image",function(a,c,b){function f(){if(e&&e.width>0){if(!this.centerOffset)this.centerOffset=new T(-e.width>>1,-e.height>>1);this.updateBounds(new ta(this.xy.x+this.centerOffset.x,this.xy.y+this.centerOffset.y,e.width,e.height),false)}}function g(d){if(this.imageUrl=d){var m=this;ab(this.imageUrl,function(l,n){e=l;if(n){var o=k.layer?k.layer.view:null;o&&o.invalidate()}f.apply(m)})}}b=D({image:null,imageUrl:null,centerOffset:null,
typeName:"Image"},b);var i=Ba.prototype.resync,h=0,j=0,e=b.image,k=D(new Ba(a,c,b),{changeImage:g,getProps:function(){this.bounds||f(this,e);return{image:e,x:this.xy.x+h,y:this.xy.y+j}},resync:function(d){i.call(this,d);f.call(this)}});e||g.call(this,this.imageUrl);if(this.centerOffset){h=this.centerOffset.x;j=this.centerOffset.y}});ba("drawable","arc",function(a,c,b){b=D({startAngle:0,endAngle:Math.PI*2,typeName:"Arc"},b);return new Ba(a,c,b)});Wa.prototype={constructor:Wa,clip:null,cycle:function(){},
...
aa,g=b+a/6371;b=Db(ja(c/6371)/Ea(b));f=f+b;g=(g+Ua)%Math.PI-Ua;f%=Z;return new ia(g*la,f*la)},toPixels:function(){return $a(this.lon,this.lat)},toBounds:function(a){var c=a.radians(),b=this.lat*aa,f=this.lon*aa;a=b-c;var g=b+c;if(a>Ab&&g<kb){b=Db(ja(c)/Ea(b));c=f-b;if(c<Sb)c+=Z;f=f+b;if(f>Bb)f-=Z}else{a=Ca(a,Ab);g=Ma(g,kb);c=yb;f=Ya}return new Ka(new ia(a*la,c*la),new ia(g*la,f*la))},toString:function(a){return this.lat+(a||" ")+this.lon}};(function(){ba("fn","generalize",function(a,c,b){var f=a.length,
g=[],i=null;b=(b||250)/1E3;for(f=f;f--;)if(f===0)g.unshift(a[f]);else{var h=!i||a[f].inArray(c)?b:i.distanceTo(a[f]);if(a[f]&&h>=b){g.unshift(a[f]);i=a[f]}}return g})})();Ka.prototype={constructor:Ka,bestZoomLevel:function(a){var c=this.center();c=Ma(La(ma(c.lat)*0.05),jb.length);var b=jb[c],f=this.size();c=Cb(Aa(jb[3]*a.h/f.y)/Aa(2));a=Cb(Aa(b*a.w/f.x)/Aa(2));return Ma(isNaN(c)?1E3:c,isNaN(a)?1E3:a)},center:function(){var a=this.size();return new ia(this.min.lat+(a.y>>1),this.min.lon+(a.x>>1))},
expand:function(a){this.min.lat-=a;this.max.lat+=a;this.min.lon-=a%360;this.max.lon+=a%360},size:function(a){var c=new T(0,this.max.lat-this.min.lat);c.x=(a||N(a,va))&&this.min.lon>this.max.lon?360-this.min.lon+this.max.lon:this.max.lon-this.min.lon;return c},toString:function(){return"min: "+this.min+", max: "+this.max}};hb.prototype={constructor:hb,radians:function(a){if(N(a,oa)){this.meters=a*6371E3;return this}else return this.meters/6371E3},toString:function(){if(this.meters>1E3)return(this.meters/
10|0)/100+"km";return this.meters+"m"}};D(Ia,{ex:D,log:sa,observable:ua,configurable:zb,formatter:ya,wordExists:function(a,c){for(var b=a.split(/\s|\,/).length;b--;)if(a.toLowerCase()==c.toLowerCase())return true;return false},is:N,indexOf:ib,fn:function(a){return Vb("fn",a)},project:$a,unproject:mb,getImage:ab,userMessage:sb,Registry:Za,Style:Fb,DOM:X,Rect:ta,XY:T,Line:Ja,Pos:ia,BBox:Ka,Distance:hb,Hits:Hits,Control:Xa,Tile:Va,Tweener:ra});ba("generator","osm",function(a){a=D({flipY:false,tileSize:256,
tilePath:"{0}/{1}/{2}.png",osmDataAck:true},a);var c=Math.PI/180,b=180/Math.PI,f=null,g=[],i,h=ya(a.tilePath),j={buildTileUrl:function(e,k,d,m,l){if(k>=0&&k<m){e=(e%m+m)%m;k=h(d,e,l?Math.abs(k-m+1):k);if(f)k=i(g[e%(g.length||1)])+k;return k}},run:function(e,k,d,m){var l=e.zoom?e.zoom():0;if(l){var n=2<<l-1,o=a.tileSize,v=(k.w/o|0)+1,w=(k.h/o|0)+1;k=(new ha(k.x,k.y)).sync(e,true).pos();var A=k.lat;k=new ha((k.lon+180)/360*n|0,(1-Math.log(Math.tan(A*c)+1/Math.cos(A*c))/Math.PI)/2*n%n|0);A=Math.PI-2*
Math.PI*k.y/n;A=new ia(b*Math.atan(0.5*(Math.exp(A)-Math.exp(-A))),k.x/n*360-180);A=new ha(A);A.sync(e);e=a.flipY;var E=d.search({x:A.x,y:A.y,w:v*o,h:w*o}),K={},p;for(p=E.length;p--;)K[E[p].id]=true;g=(f=j.getServerDetails?j.getServerDetails():null)&&f.subDomains?f.subDomains:[];i=ya(f?f.baseUrl:"");for(E=0;E<=v;E++)for(p=0;p<=w;p++){var y=k.x+E,B=k.y+p,u=y+"_"+B;if(!K[u]){tileUrl=j.buildTileUrl(y,B,l,n,e);y=new Va(A.x+E*o,A.y+p*o,tileUrl,o,o,u);d.insert(y,y)}}m&&m()}}};a.osmDataAck&&sb("ack","osm",
'Map data (c) <a href="http://openstreetmap.org/" target="_blank">OpenStreetMap</a> (and) contributors, CC-BY-SA');return j});rb.T5=Ia;rb.Tile5=function(a,c){c=D({container:a,type:"map",renderer:"canvas",starpos:null,zoom:1,fastpan:false,drawOnScale:true,zoombar:{}},c);return wa("view",c.type,c)}})(window);
