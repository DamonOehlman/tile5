(function(gb){function J(){var a=arguments[0]||{},b=Array.prototype.slice.call(arguments,1),c,e;for(e=b.length;e--;)if((c=b[e])!==null)for(var k in c){var i=c[k];if(a!==i)if(i!==undefined)a[k]=i}return a}function ha(a,b){if(typeof console!=="undefined")console[b||"debug"](a)}function xa(a){var b=a.match(Fb),c=[],e=0,k;for(k=b?b.length:0;k--;){var i=b[k].slice(1);c[i]||(c[i]=RegExp("\\{"+i+"\\}","g"))}e=c.length;return function(){var h=a;for(k=0;k<e;k++)h=h.replace(c[k],arguments[k]||"");return h}}
function Ua(a){a=a.replace(/[\-\_\s\.]/g,"").toLowerCase();return ya[a]||ya.linear}function hb(a,b,c,e,k){function i(f){f=f?f:(new Date).getTime();if(d+ib<f){var g=f-h,q=c(g,a,j,e),l=h+e<=f;f=!l;if(k){g=k(q,l,g);f=typeof g!="undefined"?g&&f:f}f&&animFrame(i)}}var h=(new Date).getTime(),d=0,j=b-a;animFrame(i);return{}}function jb(a,b,c){za.trigger("userMessage",a,b,c)}function kb(a){a=parseFloat(a)*ia;a=qa(a);var b=Ia*a;return ka((1+a)/(1-a)*la((1-b)/(1+b),Ia))/2}function lb(a){return ea/(256<<a)}
function mb(a,b){var c=Ia*qa(a);return Aa-2*nb(b*la((1-c)/(1+c),Ia/2))}function S(a,b){this.mercY=this.mercX=null;if(V(a,Ba))ob.parseXY(a,this);else if(a&&a.toPixels){var c=a.toPixels();this.mercX=c.x;this.mercY=c.y}else{this.x=a||0;this.y=b||0}}function ma(a,b,c,e){this.x=a||0;this.y=b||0;this.w=c||0;this.h=e||0;this.x2=this.x+this.w;this.y2=this.y+this.h}function pb(a,b){var c=new qb(Math.sqrt(256<<a)|0);b&&b.zoomLevel===a&&b.copyInto(c);c.zoomLevel=a;return c}function rb(a){a=a||(new Date).getTime();
for(var b=0;b<ta.length;){var c=ta[b],e=ra[c],k=ra[c].image&&ra[c].image.complete&&ra[c].image.width>0,i=a-e.start;i=k||i>=Gb;if(k){k=e.callbacks;sb[c]=e.image;for(var h=0;h<k.length;h++)k[h](e.image,true)}if(i){ta.splice(b,1);delete ra[c]}else b++}ta.length>0&&animFrame(rb)}function Ja(a,b){var c=a&&b?sb[a]:null;if(c&&c&&c.complete&&c.width>0)b(c);else{if(c=ra[a])c.callbacks.push(b);else{c=new Image;c.id="_ldimg"+ ++Hb;ra[a]={start:(new Date).getTime(),image:c,callbacks:[b]};c.src=a;ta[ta.length]=
...
Q-M,N=Z-K,A=e(Q,Z);m.triggerCustom(I,h("mouse",t),A,j(A,i(l)),e(z,N));if(typeof W=="undefined"||W){M=Q;K=Z}}o=J({},o);var C=120,v=C*8,s=false,E,D,F,M,K;o.binder("mousedown",y);o.binder("mousemove",x);o.binder("mouseup",G);o.binder("dblclick",function(t){ha("captured double click");if(d(t,l)){var I=w(t);m.triggerCustom("doubleTap",h("mouse",t),I,j(I,i(l)))}});o.binder("mousewheel",H);o.binder("DOMMouseScroll",H);return{unbind:function(){o.unbinder("mousedown",y);o.unbinder("mousemove",x);o.unbinder("mouseup",
...
z),N,A);K=null}Z&&m.triggerCustom("pointerUpMulti",h("touch",z),N,A)}}o=J({detailed:false,inertia:false},o);var p=20,C=7,v=0,s=1,E=2,D=3,F,M,K,t,I,Q,Z=o.detailed,W=1;o.binder("touchstart",G);o.binder("touchmove",H);o.binder("touchend",B);ha("initialized touch handler");return{unbind:function(){o.unbinder("touchstart",G);o.unbinder("touchmove",H);o.unbinder("touchend",B)}}},checks:{touch:true}});return{register:k,watch:function(l,m,o){m=J({bindTarget:null,observable:null,isIE:typeof window.attachEvent!=
"undefined",types:null},m);capabilities=J({touch:"ontouchstart"in window},o);if(!m.observable){ha("creating observable");m.observable=ja({});globalOpts=m}m.binder=(m.isIE?c:a)(m.bindTarget||document);m.unbinder=(m.isIE?c:b)(m.bindTarget||document);o=m.types;for(var w=capabilities,y=[],x=g.length;x--;){var G=g[x],H=!o||o.indexOf(G.type)>=0,B=true,p;for(p in G.checks){var C=G.checks[p];ha("checking "+p+" capability. require: "+C+", capability = "+w[p]);B=B&&C===w[p]}if(H&&B)y[y.length]=G.handler}return new q(l,
y,m)}}}();var Xa=function(){var a={};return{create:function(b,c){if(a[b][c])return a[b][c].apply(null,Array.prototype.slice.call(arguments,2))},register:function(b,c,e){a[b]||(a[b]={});a[b][c]&&ha(Sb(b,c),"warn");a[b][c]=e}}}(),Sb=xa("Registration of {0}: {1} will override existing definition"),Ya=[1.406245461070741,1.321415085624082,1.077179995861952,0.703119412486786,0.488332580888611],ea=Math.PI*2,Aa=Math.PI/2,ia=Math.PI/180,Na=180/Math.PI,xb=Aa,Lb=-xb,yb=ea,Mb=-yb,Ia=0.08181919084262157,oa=Math.abs,
Ab=Math.ceil,ua=Math.round,Oa=Math.min,Va=Math.max,la=Math.pow,Bb=Math.sqrt,ka=Math.log,qa=Math.sin,Tb=Math.asin,Za=Math.cos,nb=Math.atan,Ub=Math.atan2,Qb="hasOwnProperty",Pb={NaN:1,Infinity:1,"-Infinity":1},Ob=String.prototype.toLowerCase,Rb=Object.prototype.toString,na="undefined",Ba="string",$a="number",Kb="array",aa=Xa.register,pa=Xa.create,Vb=0,Jb=0,wb=/[\,\s]+/,Wb=function(){function a(k){e++;k=k||(new Date).getTime();for(var i=c.length;i--;){var h=c[i];e%h.every===0&&h.cb(k)}animFrame(a)}var b=
1E3/60,c=[],e=0;animFrame(a);return{attach:function(k,i){c[c.length]={cb:k,every:i?ua(i/b):1}},detact:function(){}}}(),ob=function(){var a=/^xy\((.*)\).*$/i;return{parseXY:function(b,c){c=c||new S;if(!a.test(b)){var e=b.split(wb);c.mercX=parseFloat(e[1])*ia;c.mercY=kb(e[0])}return c}}}(),X=function(){var a={"-webkit-user-select":"none",position:"absolute"},b;b=function(c){for(var e=0;e<c.length;e++){var k=c[e];if(typeof document.body.style[k]!="undefined")return k}}(["-webkit-transform","MozTransform"]);
...
b)},equals:function(a){return this.x===a.x&&this.y===a.y},offset:function(a,b){return new S(this.x+a,this.y+b)},sync:function(a,b){if(b){this.mercX=this.x*a-Math.PI;this.mercY=Math.PI-this.y*a}else if(this.mercX||this.mercY){this.x=ua((this.mercX+Math.PI)/a);this.y=ua((Math.PI-this.mercY)/a)}return this},pos:function(){for(var a=la(Math.E,-this.mercY),b=Aa-2*nb(a),c=mb(b,a),e=0;e<12&&oa(b-c)>1.0E-7;){b=c;c=mb(b,a);e++}return new ga(c*Na,this.mercX%360*Na)},toString:function(){return this.x+", "+this.y}};
ma.prototype={constructor:ma,buffer:function(a,b){return new ma(this.x-a,this.y-(b||a),this.w+a*2,this.h+(b||a)*2)},center:function(){return new S(this.x+(this.w>>1),this.y+(this.h>>1))}};Hits=function(){return{diffHits:function(a,b){var c=[],e={},k;for(k=b.length;k--;)e[b[k].target.id]=true;for(k=a.length;k--;)e[a[k].target.id]||(c[c.length]=a[k]);return c},init:function(a,b,c,e){return{type:a,x:e.x|0,y:e.y|0,elements:[],absXY:b,relXY:c}},initHit:function(a,b,c){return{type:a,target:b,drag:c}},triggerEvent:function(a,
...
h.id;if(e[d]){var j=i.x/a|0;h=i.y/a|0;var f=(i.x+i.w)/a|0;i=(i.y+i.h)/a|0;delete e[d];for(j=j;j<=f;j++)for(var g=h;g<=i;g++){var q=b(j,g),l=zb.call(q,d);l>=0&&q.splice(l,1)}}},search:function(i){var h=i.y/a|0,d=(i.x+i.w)/a|0,j=(i.y+i.h)/a|0,f=[],g=[];for(i=i.x/a|0;i<=d;i++)for(var q=h;q<=j;q++)f=f.concat(b(i,q));f.sort();for(h=f.length;h--;){d=f[h];if(j=e[d])g[g.length]=j;for(;h>0&&f[h-1]==d;)h--}return g}}},Gb=3E4,sb={},Hb=0;(new Date).getTime();var ra={},ta=[];Ka.prototype={constructor:Ka,load:function(a){var b=
this;Ja(this.url,function(c){b.loaded=true;b.image=c;a&&a()})}};var Xb=function(){return ja({fastpan:true,applyStyle:function(){},applyTransform:function(a,b,c){return{restore:null,x:b,y:c}},checkSize:function(){},getDimensions:function(){return{width:0,height:0}},getViewport:function(){},hitTest:function(){return false},prepare:function(){},projectXY:null,reset:function(){}})};aa("renderer","canvas",function(a,b,c,e,k){function i(p,C,v){var s,E;m[C]=function(D){for(s=H.length;s--;)if(E=v[H[s]])D[B[s]]=
...
null))&&X.move(m,g.x-o,g.y-w);f[g.id]=g}}}});a.bind("predraw",function(){h(j,f);f={}});a.bind("detach",function(){b.removeChild(d)});a.bind("reset",function(){for(h(j,f={});d.childNodes.length>0;)d.removeChild(d.childNodes[0])});return a});var Cb=function(){function a(e,k){if(V(e,Ba)){c.trigger("defined",e,b[e]=k);return e}else{var i=[],h;for(h in e)i[i.length]=a(h,e[h]);return i}}var b={},c=ja({get:function(e){return b[e]},define:a});a({basic:{fill:"#ffffff"},highlight:{fill:"#ff0000"},waypoints:{lineWidth:4,
stroke:"#003377",fill:"#ffffff"},waypointsHover:{lineWidth:4,stroke:"#ff0000",fill:"#ffffff"}});return c}();aa("view","simple",function(a){function b(n,r,u,L){B(Oa(Va(T+la(2,L)-1,0.5),2))}function c(n){var r=n|0;n=r+Math.round((n-r)*50)/50;if(n!==T){T=n;n=ka(T)/Math.LN2|0;if(n!==0){T=la(2,n);y(Fa+n,Yb,Zb)}ca++}}function e(n,r){var u=renderer&&renderer.projectXY?renderer.projectXY(n,r):null;if(!u){u=O.getViewport();var L=1/T;u=new S(u?u.x+n*L:n,u?u.y+r*L:r)}return u}function k(n,r,u){p("doubleTap",
r,u,e(u.x,u.y));a.scalable&&B(2,e(u.x,u.y),$b,null,ac)}function i(n,r,u){K=null;Z=true;w("down",r,u)}function h(n,r,u){w("hover",r,u)}function d(n,r,u,L){m(r,u,false);if(!K){W=L.x;z=L.y}}function j(n,r,u){m(r,u,true);Z=false}function f(){clearTimeout(Db);Db=setTimeout(function(){renderer.checkSize()},250)}function g(n,r,u){w("tap",r,u);p("tap",r,u,e(u.x,u.y,true))}function q(n){var r=O,u=D,L=M,P=a;n=(n||a.renderer).split("/");for(var U=new Xb(r,u,L,P),fa=0;fa<n.length;fa++)U=pa("renderer",n[fa],r,
u,L,P,U);renderer=U;Q=renderer.fastpan;l()}function l(){da&&da.unbind();if(renderer){da=INTERACT.watch(renderer.interactTarget||M);if(a.scalable){da.bind("zoom",b);da.bind("doubleTap",k)}da.bind("pointerDown",i);da.bind("pointerMove",d);da.bind("pointerUp",j);a.captureHover&&da.bind("pointerHover",h);da.bind("tap",g)}}function m(n,r,u){if(K){n=e(r.x,r.y);K.drag.call(K.target,K,n.x,n.y,u)&&ca++;if(u)K=null}}function o(n){M=document.getElementById(n);M.appendChild(F=X.create("div","",X.styles({overflow:"hidden",
width:M.offsetWidth+"px",height:M.offsetHeight+"px"})));Pa=F.offsetWidth+Ga*2;Qa=F.offsetHeight+Ga*2;ab=Pa/2;bb=Qa/2;cb=M.offsetWidth/2;db=M.offsetHeight/2;F.appendChild(D=X.create("div","",X.styles({width:Pa+"px",height:Qa+"px","z-index":2,margin:-Ga+"px 0 0 "+-Ga+"px"})));q()}function w(n,r,u){ba=Hits.init(n,r,u,e(u.x,u.y,true));for(n=E;n--;)I=I||(s[n].hitGuess?s[n].hitGuess(ba.x,ba.y,O):false);I&&ca++}function y(n,r,u){if(V(n,$a)){n=Va(a.minZoom,Oa(a.maxZoom,n|0));if(n!==Fa){var L=la(2,n-Fa),P=
ab/L|0,U=bb/L|0;Fa=n;C(((r||A+ab)-P)*L,((u||R+bb)-U)*L);eb=Ea=0;p("zoom",n);T=1;renderer.trigger("reset");H()}return O}else return Fa}function x(){var n=new ma(A,R,Pa,Qa);n.scaleFactor=T;return n}function G(n,r,u){if(V(r,na))for(r=0;r<E;r++){if(s[r].id===n)return s[r]}else if(V(n,Ba)){r=pa("layer",r,O,u);r.added=(new Date).getTime();r.id=n;s[s.length]=r;s.sort(function(L,P){return P.zindex-L.zindex||P.added-L.added});E=s.length;r.trigger("resync");r.trigger("refresh",O,x());O.trigger("layerChange",
O,r);ca++;return r}else return[].concat(s)}function H(){var n=x();if(n){if(va||wa)if(n){var r=Ra?A+(n.w>>1):A,u=Sa?R+(n.h>>1):R,L=n.w,P=n.h;if(va&&va>L)if(r+L>va)A=Ra?A:va-L;else if(r<0)A=Ra?A:0;if(wa&&wa>P)if(u+P>wa)R=Sa?R:wa-P;else if(u<0)R=Sa?R:0}Ea=A;eb=R;p("refresh",O,n);ca++}}function B(n,r,u,L,P){var U;if(!bc){u=false;U=ua(ka(n)/Math.LN2);n=la(2,U)}u?hb(T,n,u,P,function(fa,Ta){n=fa;if(Ta){U=ua(ka(n)/Math.LN2);n=la(2,U);L&&L()}c(n)}):c(n);return O}function p(){for(var n=O.trigger.apply(null,
arguments).cancel,r=s.length;r--;)n=s[r].trigger.apply(null,arguments).cancel||n;return!n}function C(n,r,u){if(V(n,$a)){if(!(u&&panSpeed===0)){A=n|0;R=r|0}}else return new S(A,R)}a=J({container:"",captureHover:true,controls:["zoombar"],drawOnScale:true,padding:0,inertia:true,refreshDistance:256,pannable:false,scalable:false,minZoom:1,maxZoom:1,renderer:"canvas/dom",zoom:1,zoombar:{}},a);var v=[],s=[],E=0,D=null,F=null,M,K=null,t=!V(window.attachEvent,na),I=false,Q=true,Z=false,W=0,z=0,N=a.refreshDistance,
A=0,R=0,Y=0,$=0,Ea=0,eb=0,va=null,wa=null,Ra=false,Sa=false,Ga=a.padding,ba=null,Ha=null,Db=0,T=1,Eb=1,da=null,fb={index:0,draw:false},bc=true,ca=0,Pa,Qa,ab,bb,cb,db,Yb,Zb,Fa=a.zoom||a.zoomLevel,$b=Ua("quad.out"),ac=300,O={id:a.id,padding:Ga,panSpeed:0,attachFrame:function(n,r){r&&D.appendChild(n)},center:function(n,r,u){if(V(n,Ba)){r=ob.parseXY(n).sync(O.rpp);n=r.x;r=r.y}if(V(n,$a)){C(n-cb,r-db,u);return O}else{r=C().offset(cb,db);O.rpp&&r.sync(O.rpp,true);return r}},detach:function(){renderer&&
renderer.trigger("detach");da&&da.unbind();if(F){M.removeChild(F);D=F=null}},layer:G,invalidate:function(){ca++},refresh:H,resetScale:function(){T=1},scale:B,triggerAll:p,removeLayer:function(n){a:{for(var r=E;r--;)if(s[r].id===n){n=r;break a}n=-1}if(n>=0&&n<E){O.trigger("layerRemove",O,s[n]);s.splice(n,1);ca++}E=s.length},getRenderer:function(){return renderer},getScaleFactor:function(){return T},setMaxOffset:function(n,r,u,L){va=n;wa=r;Ra=u;Sa=L},getViewport:x,offset:C,pan:function(n,r){W=n;z=r;
ca++},zoom:y};ja(O);O.bind("resize",function(){renderer.checkSize()});Nb(O,a,{container:o,captureHover:l,scalable:l,pannable:l,renderer:function(n){if(renderer){renderer.trigger("detach");renderer=null}q(n);ca++}});Wa.init(function(){G("markers","draw",{zindex:20});o(a.container);t?window.attachEvent("onresize",f):window.addEventListener("resize",f,false)});Wb.attach(function(n){var r=[],u,L,P;self.panSpeed=panSpeed=oa(W)+oa(z);u=T!==Eb;if(panSpeed>0||u)ca++;if(panSpeed<2&&(oa(A-Ea)>=N||oa(R-eb)>=
N))H();fb.index++;fb.draw=ca||panSpeed||0;if(renderer&&fb.draw){Y+=W;$+=z;if(W||z)O.trigger("pan");if(T!==1&&X.supportTransforms)r[r.length]="scale("+T+")";if(u=!Q||(a.drawOnScale||T===1)&&panSpeed<5){A=A-Y/T|0;R=R-$/T|0;P=x();renderer.trigger("predraw",P);if(renderer.prepare(s,P,n,ba)){L=$=u=Y=ca=0;for(ii=E;ii--;){var U=s[ii];if(U.visible){var fa=U.style?renderer.applyStyle(U.style,true):null;U.draw(renderer,P,O,n,ba);fa&&renderer.applyStyle(fa)}}renderer.trigger("render",P);O.trigger("drawComplete",
P,n);Eb=T;X.move(D,u,L,r)}}else X.move(D,Y,$,r);if(Z||!a.inertia)z=W=0;else if(W!=0||z!=0){W*=0.8;z*=0.8;if(oa(W)<0.5)W=0;if(oa(z)<0.5)z=0}if(ba){n=ba?ba.elements:[];if(Ha&&Ha.type==="hover"){r=Hits.diffHits(Ha.elements,n);r.length>0&&Hits.triggerEvent(Ha,O,"Out",r)}if(n.length>0){u=ba.x;L=ba.y;for(r=n.length;r--;){P=n[r];U=u;fa=L;var Ta=P&&P.drag&&(!P.canDrag||P.canDrag(P,U,fa));if(Ta){K=P;K.startX=U;K.startY=fa}if(Ta)break}Hits.triggerEvent(ba,O)}Ha=n.length>0?J({},ba):null;ba=null}}});(function(n){v=
[];for(var r=0;r<n.length;r++)v[v.length]=pa("control",n[r],O,F,M,a[n[r]])})(a.controls);return O});aa("view","map",function(a){function b(d,j,f){h.zoom(j);c(d,f)}function c(d,j,f,g){d=new S(d);d.sync(lb(h.zoom()));h.center(d.x,d.y,f,g,function(){j&&j(h)})}a=J({zoomLevel:1,minZoom:1,maxZoom:18,pannable:true,scalable:true},a);var e=new S,k=0,i=a.tapExtent;a.adjustScaleFactor=function(d){return Math.pow(2,(d<1?Math.floor:Math.ceil)(Math.log(d)))};var h=J(pa("view","simple",a),{getBoundingBox:function(){var d=
...
f,d,new Da(q,g))});h.bind("refresh",function(d,j,f){if(e.x!=f.x||e.y!=f.y){h.trigger("boundsChange",h.getBoundingBox());e.x=f.x;e.y=f.y}});h.bind("zoom",function(d,j){var f;k=h.rpp=lb(j);f=ea/k|0;h.setMaxOffset(f,f,true,false);h.resetScale();h.triggerAll("resync")});return h});var sa=function(a,b,c){c=J({style:null,xy:null,size:20,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape"},c);J(this,c);if(V(this.xy,Ba))this.xy=new S(this.xy);this.id="drawable_"+Vb++;this.bounds=
null;this.view=a;this.layer=b;this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.observable&&ja(this)};sa.prototype={constructor:sa,animate:function(a,b,c,e){Ib(this,a,b,c,e)},drag:null,draw:null,getProps:null,resync:function(){if(this.xy){this.xy.sync(this.view.rpp);if(this.size){var a=this.size>>1;this.updateBounds(new ma(this.xy.x-a,this.xy.y-a,this.size,this.size))}}},rotate:function(a){this.rotation=a},scale:function(a){this.scaling=a},translate:function(a,
b){this.translateX=a;this.translateY=b},updateBounds:function(a,b){a&&(!this.bounds||a.x!=this.bounds.x||a.y!=this.bounds.y)&&this.trigger("move",this,a,this.bounds);this.bounds=a;if(b)this.xy=this.bounds.center()}};var ib=1E3/60|0,Ca=[],ub=0;aa("drawable","marker",function(a,b,c){c=J({fill:true,stroke:false,markerStyle:"simple",hoverStyle:"highlight",typeName:"Marker"},c);return new sa(a,b,c)});aa("drawable","poly",function(a,b,c){c=J({simplify:false,fill:true,points:[],typeName:"Poly"},c);var e=
c.points,k=c.simplify;sa.call(this,c);J(this,{getPoints:function(){return[].concat(e)},resync:function(){var i,h,d,j,f,g,q,l;for(i=e.length;i--;)e[i].sync(a.rpp);l=this.points=k?k(e):e;for(i=l.length;i--;){h=l[i].x;d=l[i].y;g=V(g,na)||h<g?h:g;q=V(q,na)||d<q?d:q;j=V(j,na)||h>j?h:j;f=V(f,na)||d>f?d:f}this.updateBounds(new ma(g,q,j-g,f-q),true)}});this.haveData=e&&e.length>=2});aa("drawable","line",function(a,b,c){c.fill=false;return pa("drawable","poly",a,b,c)});aa("drawable","image",function(a,b,c){function e(){if(j&&
j.width>0){if(!this.centerOffset)this.centerOffset=new S(-j.width>>1,-j.height>>1);this.updateBounds(new ma(this.xy.x+this.centerOffset.x,this.xy.y+this.centerOffset.y,j.width,j.height),false)}}function k(g){if(this.imageUrl=g){var q=this;Ja(this.imageUrl,function(l,m){j=l;if(m){var o=f.layer?f.layer.view:null;o&&o.invalidate()}e.apply(q)})}}c=J({image:null,imageUrl:null,centerOffset:null,typeName:"Image"},c);var i=sa.prototype.resync,h=0,d=0,j=c.image,f=J(new sa(a,b,c),{changeImage:k,getProps:function(){this.bounds||
e(this,j);return{image:j,x:this.xy.x+h,y:this.xy.y+d}},resync:function(g){i.call(this,g);e.call(this)}});j||k.call(this,this.imageUrl);if(this.centerOffset){h=this.centerOffset.x;d=this.centerOffset.y}});aa("drawable","arc",function(a,b,c){c=J({startAngle:0,endAngle:Math.PI*2,typeName:"Arc"},c);return new sa(a,b,c)});La.prototype={constructor:La,clip:null,cycle:function(){},draw:function(){},hitGuess:null};aa("layer","tile",function(a,b){b=J({generator:"osm",imageLoadArgs:{}},b);var c=pa("generator",
b.generator,b).run,e=null,k=[],i=J(new La(a,b),{draw:function(h,d,j){h.drawTiles&&h.drawTiles(d,e.search(d.buffer(128)),j.panSpeed<2)}});i.bind("refresh",function(h,d,j){var f=(new Date).getTime();e&&c(d,j,e,function(){d.invalidate();ha("GEN COMPLETED IN "+((new Date).getTime()-f)+" ms")})});i.bind("resync",function(){var h=a&&a.zoom?a.zoom():0;k[h]||(k[h]=pb(h));e=k[h]});return i});aa("layer","draw",function(a,b){function c(f,g,q,l){var m=this.dragOffset;if(!m)m=this.dragOffset=new S(f.startX-this.xy.x,
f.startY-this.xy.y);this.xy.x=g-m.x;this.xy.y=q-m.y;if(l){delete this.dragOffset;a.invalidate();this.trigger("dragDrop")}return true}function e(f){clearTimeout(d);d=setTimeout(function(){i.sort(function(g,q){if(q.xy&&g.xy){var l=q.xy.y-g.xy.y;return l!=0?l:q.xy.x-g.xy.x}});f&&f.invalidate()},50)}function k(f,g,q,l){l&&h.remove(l,g);h.insert(q,g)}b=J({zindex:10},b);var i=[],h,d=0,j=J(new La(a,b),{itemCount:0,clear:function(){h=new qb;i=[];j.itemCount=0},create:function(f,g,q){f=pa("drawable",f,a,j,
...
function(v){d(v.target,2)});g.bind("pointerUp",function(v){d(v.target,0)});g.bind("tap",function(v){(v=C[d(v.target,2)])&&v()});b=new Ma(a);b.bind("detach",function(){g.unbind();c.removeChild(B)});a.bind("zoom",function(v,s){j(s)});j(a.zoom());return b});ga.prototype={constructor:ga,distanceTo:function(){if(!targetPos||this.empty()||targetPos.empty())return 0;var a=(targetPos.lat-this.lat)*ia>>1,b=(targetPos.lon-this.lon)*ia>>1;a=qa(a)*qa(a)+Za(this.lat*ia)*Za(targetPos.lat*ia)*qa(b)*qa(b);return 6371*
2*Ub(Bb(a),Bb(1-a))},equalTo:function(a){return pos&&this.lat===a.lat&&this.lon===a.lon},empty:function(){return this.lat===0&&this.lon===0},inArray:function(a){for(var b=a.length;b--;)if(this.equal(a[b]))return true;return false},offset:function(a,b){var c=this.lat*ia,e=this.lon*ia,k=c+a/6371;c=Tb(qa(b/6371)/Za(c));e=e+c;k=(k+Aa)%Math.PI-Aa;e%=ea;return new ga(k*Na,e*Na)},toPixels:function(){return new S(parseFloat(this.lon)*ia,kb(this.lat))},toString:function(a){return this.lat+(a||" ")+this.lon}};
var cc=function(){return{generalize:function(a,b,c){var e=a.length,k=[],i=null;c||(c=250);c/=1E3;ha("generalizing positions, must include "+b.length+" positions");for(var h=e;h--;)if(h===0)k.unshift(a[h]);else{var d=!i||a[h].inArray(b)?c:i.distanceTo(a[h]);if(a[h]&&d>=c){k.unshift(a[h]);i=a[h]}}ha("generalized "+e+" positions into "+k.length+" positions");return k},parseArray:function(a){var b=a.length,c=Array(b);for(b=b;b--;)c[b]=new ga(a[b]);return c},vectorize:function(a,b){function c(){for(var h=
0,d=b.chunkSize,j=e;j--;){k[j]=new S(a[j]);h+=1;if(h>d)break}e=j;if(e<=0)b.callback&&b.callback(k);else animFrame(c)}var e=a.length,k=Array(e);b=J({chunkSize:500,async:true,callback:null},b);if(!b.async){for(var i=e;i--;)k[i]=new S(a[i]);return k}animFrame(c);return null}}}();Da.prototype={constructor:Da,bestZoomLevel:function(a){var b=this.center();b=Oa(ua(oa(b.lat)*0.05),Ya.length);var c=Ya[b],e=this.size();b=Ab(ka(Ya[3]*a.h/e.y)/ka(2));a=Ab(ka(c*a.w/e.x)/ka(2));return Oa(isNaN(b)?1E3:b,isNaN(a)?
1E3:a)},center:function(){var a=this.size();return new ga(this.min.lat+(a.y>>1),this.min.lon+(a.x>>1))},expand:function(a){this.min.lat-=a;this.max.lat+=a;this.min.lon-=a%360;this.max.lon+=a%360},size:function(a){var b=new S(0,this.max.lat-this.min.lat);b.x=(a||isType(a,na))&&this.min.lon>this.max.lon?360-this.min.lon+this.max.lon:this.max.lon-this.min.lon;return b},toString:function(){return"min: "+this.min+", max: "+this.max}};var za={ex:J,log:ha,observable:ja,formatter:xa,wordExists:function(a,
b){for(var c=a.split(/\s|\,/).length;c--;)if(a.toLowerCase()==b.toLowerCase())return true;return false},is:V,indexOf:zb,userMessage:jb,Registry:Xa,Style:Cb,DOM:X,Rect:ma,XY:S,Hits:Hits,tweenValue:hb,easing:Ua,Control:Ma,Tile:Ka,getImage:Ja,Pos:ga,PosFns:cc,BBox:Da};ja(za);aa("generator","osm",function(a){a=J({flipY:false,tileSize:256,tilePath:"{0}/{1}/{2}.png",osmDataAck:true},a);var b=Math.PI/180,c=180/Math.PI,e=null,k=[],i,h=xa(a.tilePath),d={buildTileUrl:function(j,f,g,q,l){if(f>=0&&f<q){j=(j%
q+q)%q;f=h(g,j,l?Math.abs(f-q+1):f);if(e)f=i(k[j%(k.length||1)])+f;return f}},run:function(j,f,g,q){if(j=j.zoom?j.zoom():0){var l=2<<j-1,m=a.tileSize,o=Math.PI*2/(m<<j),w=(f.w/m|0)+1,y=(f.h/m|0)+1;f=(new S(f.x,f.y)).sync(o,true).pos();var x=f.lat;f=new S((f.lon+180)/360*l|0,(1-Math.log(Math.tan(x*b)+1/Math.cos(x*b))/Math.PI)/2*l%l|0);x=Math.PI-2*Math.PI*f.y/l;x=new ga(c*Math.atan(0.5*(Math.exp(x)-Math.exp(-x))),f.x/l*360-180);x=new S(x);x.sync(o);o=a.flipY;var G=g.search({x:x.x,y:x.y,w:w*m,h:y*m}),
H={},B;for(B=G.length;B--;)H[G[B].id]=true;k=(e=d.getServerDetails?d.getServerDetails():null)&&e.subDomains?e.subDomains:[];i=xa(e?e.baseUrl:"");for(G=0;G<=w;G++)for(B=0;B<=y;B++){var p=f.x+G,C=f.y+B,v=p+"_"+C;if(!H[v]){tileUrl=d.buildTileUrl(p,C,j,l,o);p=new Ka(x.x+G*m,x.y+B*m,tileUrl,m,m,v);g.insert(p,p)}}q&&q()}}};a.osmDataAck&&jb("ack","osm",'Map data (c) <a href="http://openstreetmap.org/" target="_blank">OpenStreetMap</a> (and) contributors, CC-BY-SA');return d});gb.T5=za;gb.Tile5=function(a,
b){b=J({container:a,type:"map",renderer:"canvas",starpos:null,zoom:1,fastpan:false,drawOnScale:true,zoombar:{}},b);return pa("view",b.type,b)}})(window);
