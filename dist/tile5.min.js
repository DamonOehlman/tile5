...
l[p])}if(!k.configure)k.configure=function(E,w){if(oa[E]){for(var m=n.length;m--;){var r=n[m](E,w);if(typeof r!=="undefined")return r}return t[L]}return null}}}})();
...
U+m<=da;da=!sa;if(r){W=r(ra,sa,W);da=typeof W!="undefined"?W&&da:da}da&&animFrame(u)}}var U=(new Date).getTime(),$=0,ka=E-n;animFrame(u);return{}};COG.easing=function(n){return oa[n.replace(/[\-\_\s\.]/g,"").toLowerCase()]};COG.registerEasingType=function(n,E){oa[n.replace(/[\-\_\s\.]/g,"").toLowerCase()]=E}})();
...
ra(y)){var I=U(y);m.style.cursor="move";n(y);O=I.x;ma=I.y;ia=k(O,ma);r.triggerCustom("pointerDown",z("mouse",y),ia,oa(ia,p(m)))}}function ka(y){var I=U(y);ea=I.x;N=I.y;if(L(y,m))sa(y,aa?"pointerMove":"pointerHover")}function da(y){if(aa&&ra(y)){aa=false;if(L(y,m)){m.style.cursor="default";sa(y,"pointerUp")}}}function W(y){if(L(y,m)){var I;y=y||window.event;if(y.detail)I=y.axis===2?-y.detail*wa:0;else{I=y.wheelDeltaY?y.wheelDeltaY:y.wheelDelta;if(window.opera)I=-I}if(I!==0){var Y=k(ea,N);r.triggerCustom("zoom",
z("mouse",y),Y,oa(Y,p(m)),I/Ea,"wheel");n(y);y.returnValue=false}}}function ra(y){y=y||window.event;return(y.which||y.button)==1}function sa(y,I,Y,xa,ya){Y=typeof Y!="undefined"?Y:ea;xa=typeof xa!="undefined"?xa:N;var J=Y-O,P=xa-ma,R=k(Y,xa);r.triggerCustom(I,z("mouse",y),R,oa(R,p(m)),k(J,P));if(typeof ya=="undefined"||ya){O=Y;ma=xa}}u=COG.extend({},u);var wa=120,Ea=wa*8,aa=false,ia,ea,N,O,ma;u.binder("mousedown",$,false);u.binder("mousemove",ka,false);u.binder("mouseup",da,false);u.binder("dblclick",
function(y){COG.info("captured double click");if(L(y,m)){var I=U(y);r.triggerCustom("doubleTap",z("mouse",y),I,oa(I,p(m)))}},false);u.binder("mousewheel",W,document);u.binder("DOMMouseScroll",W,document);return{unbind:function(){u.unbinder("mousedown",$,false);u.unbinder("mousemove",ka,false);u.unbinder("mouseup",da,false);u.unbinder("mousewheel",W,document);u.unbinder("DOMMouseScroll",W,document)}}},checks:{touch:false}});l("pointer",{handler:function(m,r,u){function U(J){if(J.count<2)return 0;var P=
...
ua?R:ua)+(Math.abs(R-ua)>>1),y:Ya+(Math.abs(Aa-Ga)>>1)};P=P/I;ua=P-ya;r.triggerCustom("zoom",z("touch",J),R,oa(R,N),ua,"pinch");ya=P}}O==ia&&r.triggerCustom("pointerMove",z("touch",J),y,$(y,N.left,N.top),k(y.x-Y.x,y.y-Y.y));xa&&r.triggerCustom("pointerMoveMulti",z("touch",J),y,$(y,N.left,N.top))}Y=$(y)}}function ra(J){if(L(J,m)){var P=ka(J,"changedTouches"),R=$(P,N.left,N.top);y=ka(J);if(!y){r.triggerCustom("pointerUp",z("touch",J),P,R);ma=null}xa&&r.triggerCustom("pointerUpMulti",z("touch",J),P,
...
function(d){Cb(d)})};Cb({basic:{fill:"#ffffff"},highlight:{fill:"#ff0000"},waypoints:{lineWidth:4,stroke:"#003377",fill:"#ffffff"},waypointsHover:{lineWidth:4,stroke:"#ff0000",fill:"#ffffff"}});var Xb={NONE:0,ACTIVE:1,ANIMATING:4,PAN:8,ZOOM:16,FREEZE:128},Lb=function(b){function d(o,s,B){Ba||T(pa-s,qa-B)}function f(o,s,B,Q){S(y(I(ta+Y(2,Q)-1,0.5),2))}function e(){var o=ya(ta)/Math.LN2|0;if(o!==0){ta=Y(2,o);V(ab+o,dc,ec)}redraw=true}function a(o,s){var B=renderer&&renderer.projectXY?renderer.projectXY(o,
s):null;if(!B){B=Z.getViewport();var Q=1/ta;B=new q(B?B.x+o*Q:o,B?B.y+s*Q:s)}return B}function c(o,s,B){fa("doubleTap",s,B,a(B.x,B.y));b.scalable&&S(2,a(B.x,B.y),b.zoomEasing,null,b.zoomDuration)}function g(o,s,B){Ba=null;K("down",s,B)}function h(o,s,B){K("hover",s,B)}function i(o,s,B){A(s,B,false)}function j(o,s,B){A(s,B,true)}function x(){clearTimeout(Db);Db=setTimeout(function(){renderer.checkSize()},250)}function H(o,s,B){K("tap",s,B);fa("tap",s,B,a(B.x,B.y,true))}function ba(){renderer=bc(b.renderer,
Z,Ha);Eb=!renderer.preventPartialScale;D()}function C(o,s){s.id=o;s.added=X();s.view=Z;s.trigger("parentChange",Z,Ha,fc);M.push(s);M.sort(function(B,Q){var ga=Q.zindex-B.zindex;if(ga===0)ga=Q.added-B.added;return ga});ja=M.length;return s}function D(){Ca&&Ca.unbind();if(renderer){Ca=INTERACT.watch(renderer.interactTarget);b.pannable&&Ca.pannable().bind("pan",d);if(b.scalable){Ca.bind("zoom",f);Ca.bind("doubleTap",c)}Ca.bind("pointerDown",g);Ca.bind("pointerMove",i);Ca.bind("pointerUp",j);b.captureHover&&
Ca.bind("pointerHover",h);Ca.bind("tap",H)}}function A(o,s,B){if(Ba){o=a(s.x,s.y);if(Ba.drag.call(Ba.target,Ba,o.x,o.y,B))redraw=true;if(B)Ba=null}}function G(o){var s,B,Q=false;o=o||(new Date).getTime();if(Q=o-Fb>gc){var ga=O(pa-lb),za=O(qa-mb);s=pa!==nb||qa!==ob;B=ta!==Gb;if(s||B)Oa++;if(ga>=Hb||za>=Hb)v();B={index:hc++,draw:Oa};Z.trigger("enterFrame",o,B);Fb=o}if(Q&&B.draw){Ka=Ib|(ta!==1?Jb:0)|(s?pb:0)|(qb?Kb:0);rb=(s=(Ka&(Jb|pb))!==0)&&(Ka&Kb)===0;s=la();renderer.trigger("predraw",s,Ka);if(renderer.prepare(M,
s,Ka,o,Da)){Oa=0;for(ii=ja;ii--;){Q=M[ii];if(Q.visible&&(Ka&Q.validStates)!==0){B=Q.style?renderer.applyStyle(Q.style,true):null;Q.draw(renderer,s,Ka,Z,o,Da);B&&renderer.applyStyle(B)}}renderer.render(s);Z.trigger("drawComplete",s,o);nb=pa;ob=qa;Gb=ta}if(Da){o=Da?Da.elements:[];if(bb&&bb.type==="hover"){s=Hits.diffHits(bb.elements,o);s.length>0&&Hits.triggerEvent(bb,Z,"Out",s)}if(o.length>0){Q=Da.x;B=Da.y;for(s=o.length;s--;){ga=o[s];za=Q;var La=B,Ja=ga&&ga.drag&&(!ga.canDrag||ga.canDrag(ga,za,La));
if(Ja){Ba=ga;Ba.startX=za;Ba.startY=La}if(Ja)break}Hits.triggerEvent(Da,Z)}bb=o.length>0?COG.extend({},Da):null;Da=null}}animFrame(G)}function K(o,s,B){Da=Hits.init(o,s,B,a(B.x,B.y,true));for(o=ja;o--;)sb=sb||(M[o].hitGuess?M[o].hitGuess(Da.x,Da.y,Ka,Z):false);sb&&Oa++}function ca(){Oa++}function la(){var o=new k(pa,qa,Z.width,Z.height);o.scaleFactor=ta;if(ta!==1)o.scaled=$a.fromCenter(pa+(Z.width>>1),qa+(Z.height>>1),Z.width/ta|0,Z.height/ta|0);return o}function v(){var o=la();if(o){if(Wa||Xa)if(o){var s=
fb?pa+(o.w>>1):pa,B=gb?qa+(o.h>>1):qa,Q=o.w,ga=o.h;if(Wa&&Wa>Q)if(s+Q>Wa)pa=fb?pa:Wa-Q;else if(s<0)pa=fb?pa:0;if(Xa&&Xa>ga)if(B+ga>Xa)qa=gb?qa:Xa-ga;else if(B<0)qa=gb?qa:0}lb=pa;mb=qa;fa("refresh",Z,o);Oa++}}function S(o,s,B,Q,ga){var za;if(!Eb){B=false;za=J(ya(o)/Math.LN2);o=Y(2,za)}if(B)COG.tweenValue(ta,o,B,ga,function(La,Ja){ta=La;if(Ja){za=J(ya(ta)/Math.LN2);ta=Y(2,za);Q&&Q()}e()});else{ta=o;e()}return Z}function V(o,s,B){o=I(b.minZoom,y(b.maxZoom,o));if(o!==ab){var Q=Y(2,o-ab),ga=Z.width>>1,
za=Z.height>>1,La=ga/Q|0,Ja=za/Q|0;ab=o;T(((s?s:pa+ga)-La)*Q,((B?B:qa+za)-Ja)*Q);nb=pa;ob=qa;mb=lb=0;fa("zoomLevelChange",o);ta=1;renderer.reset();v()}}function fa(){for(var o=Z.trigger.apply(null,arguments).cancel,s=M.length;s--;)o=M[s].trigger.apply(null,arguments).cancel||o;return!o}function T(o,s,B,Q,ga){function za(){La+=1;if(La>=2){qb=false;ga&&ga()}}var La=0;if(B){if((Ka&pb)===0){COG.tweenValue(pa,o,B,Q,function(Ja,tb){pa=Ja|0;(tb?za:ca)();return!rb});COG.tweenValue(qa,s,B,Q,function(Ja,tb){qa=
Ja|0;(tb?za:ca)();return!rb});qb=true}}else{pa=o|0;qa=s|0;ga&&ga()}}b=COG.extend({id:COG.objId("view"),container:"",captureHover:true,captureDrag:false,inertia:true,refreshDistance:256,pannable:false,clipping:true,scalable:false,panAnimationEasing:COG.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,autoSize:true,tapExtent:10,guides:false,turbo:false,fps:25,minZoom:1,maxZoom:1,renderer:"canvas",zoomEasing:COG.easing("quad.out"),zoomDuration:300,zoomLevel:1},b);var na={},M=[],
ja=0,Ha=document.getElementById(b.container),Ba=null,hc=0,fc=null,ic=!t(window.attachEvent,va),sb=false,Hb=b.refreshDistance,pa=0,qa=0,lb=0,mb=0,nb=0,ob=0,Wa=null,Xa=null,fb=false,gb=false,Da=null,rb=false,bb=null,Db=0,ta=1,Gb=1,Fb=0,Ca=null,Eb=true,qb=false,gc=1E3/b.fps|0,Oa=0,dc,ec,ab=b.zoomLevel,Ib=L("ACTIVE"),pb=L("PAN"),Jb=L("ZOOM"),Kb=L("ANIMATING"),Ka=Ib,Z={id:b.id,detach:function(){Ca&&Ca.unbind()},eachLayer:function(o){for(var s=ja;s--;)o(M[s])},getLayer:function(o){for(var s=0;s<ja;s++)if(M[s].id===
o)return M[s];return null},getZoomLevel:function(){return ab},setLayer:function(o,s){for(var B=0;B<ja;B++)if(M[B].id===o){M.splice(B,1);break}if(s){C(o,s);s.trigger("refresh",Z,la());Z.trigger("layerChange",Z,s)}Oa++;return s},invalidate:ca,refresh:v,resetScale:function(){ta=1},scale:S,setZoomLevel:V,syncXY:function(){},triggerAll:fa,removeLayer:function(o){a:{for(var s=ja;s--;)if(M[s].id===o){o=s;break a}o=-1}if(o>=0&&o<ja){Z.trigger("layerRemove",Z,M[o]);M.splice(o,1);Oa++}ja=M.length},getOffset:function(){return new q(pa,
qa)},getRenderer:function(){return renderer},getScaleFactor:function(){return ta},setMaxOffset:function(o,s,B,Q){Wa=o;Xa=s;fb=B;gb=Q},getViewport:la,updateOffset:T,pan:function(o,s,B,Q,ga){T(pa+o,qa+s,B,Q,ga)}};COG.observable(Z);Z.bind("resync",function(){});COG.configurable(Z,["container","captureHover","captureDrag","scalable","pannable","inertia","minZoom","maxZoom","zoom"],COG.paramTweaker(b,null,{container:function(o,s){Ha=document.getElementById(s);ba()},inertia:D,captureHover:D,scalable:D,
pannable:D}),true);CANI.init(function(o){Z.markers=C("markers",new hb({zindex:20}));na=o;ba();ic?window.attachEvent("onresize",x):window.addEventListener("resize",x,false)});animFrame(G);return Z},Fa=function(b){b=COG.extend({style:null,xy:null,size:10,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape"},b);COG.extend(this,b);this.id=COG.objId(this.typeName);this.view=this.bounds=null;this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=
...
this.trigger("move",this,b,this.bounds);this.bounds=b;if(d)this.xy=$a.center(this.bounds)}};var Yb=1E3/60|0,Za=[],xb=0;w.prototype=COG.extend(Fa.prototype,{constructor:w});m.prototype=COG.extend({},Fa.prototype,{constructor:m});r.prototype=COG.extend({},m.prototype);u.prototype=COG.extend({},Fa.prototype,{constructor:u});U.prototype=COG.extend({},u.prototype,{constructor:U});$.prototype=COG.extend(Fa.prototype,{constructor:$});ka.prototype={constructor:ka,clip:null,cycle:function(){},draw:function(){},
hitGuess:null};Va=function(b,d){function f(h,i){var j=i&&i.getZoomLevel?i.getZoomLevel():0;c[j]||(c[j]=new jb);a=c[j]}d=COG.extend({imageLoadArgs:{}},d);var e=b?Ab.init(b,d).run:null,a=null,c=[],g=COG.extend(new ka(d),{draw:function(h,i){h.drawTiles(i,a.search(i))}});g.bind("refresh",function(h,i,j){var x=(new Date).getTime();a&&e(i,j,a,function(){i.invalidate();COG.info("GEN COMPLETED IN "+((new Date).getTime()-x)+" ms")})});g.bind("parentChange",f);g.bind("resync",f);return g};var Mb=function(b){function d(i){clearTimeout(g);
g=setTimeout(function(){a.sort(function(j,x){if(x.xy&&j.xy){var H=x.xy.y-j.xy.y;return H!=0?H:x.xy.x-j.xy.x}});i&&i.invalidate()},50)}function f(i,j,x,H){H&&c.remove(H,j);c.insert(x,j)}function e(i,j){for(var x=a.length;x--;)a[x].resync(j)}b=COG.extend({zindex:10},b);var a=[],c=new jb,g=0,h=COG.extend(new ka(b),{itemCount:0,add:function(i,j){if(i){i.layer=h;if(j)a.unshift(i);else a[a.length]=i;var x=h.view;if(x){i.resync(x);c&&i.bounds&&c.insert(i.bounds,i);d(x)}i.bind("move",f)}h.itemCount=a.length},
clear:function(){c=new jb;a=[];h.itemCount=0},draw:function(i,j,x,H,ba,C){H={};ba=c&&j?c.search(j):[];for(var D=ba.length;D--;){var A=ba[D],G=A.style||h.style,K,ca=i.applyTransform(A),la=A.getProps?A.getProps(i,x):H;if(la=(K=i["prep"+A.typeName])?K.call(i,A,j,C,x,la):null){if(C&&la.hit){C.elements.push(Hits.initHit(A.type,A,A.draggable?A.drag:null));K=C.type+"Style";G=A[K]||h[K]||G}G=G?i.applyStyle(G,true):null;(K=A.draw||la.draw)&&K.call(A,la);G&&i.applyStyle(G)}ca&&ca.undo&&ca.undo()}},find:function(){return[].concat(a)},
hitGuess:function(i,j){return c&&c.search({x:i-10,y:j-10,w:20,h:20}).length>0}});h.bind("parentChange",e);h.bind("resync",e);return h},hb=function(b){return new Mb(b)};COG.extend(F,{ex:COG.extend,is:t,ticks:X,userMessage:function(b,d,f){F.trigger("userMessage",b,d,f)},indexOf:ia,Rect:k,XY:Ra,XYRect:$a,Vector:ib,Hits:Hits,Generator:Ab,tweenValue:COG.tweenValue,easing:COG.easing,Tile:z,TileLayer:Va,getImage:p,viewState:L,View:Lb,ViewLayer:ka,ImageLayer:Va,Drawable:Fa,Marker:w,Poly:m,Line:r,Arc:$,ImageDrawable:u,
ImageMarker:U,DrawLayer:Mb,ShapeLayer:hb});var ub=[1.406245461070741,1.321415085624082,1.077179995861952,0.703119412486786,0.488332580888611],Ia=Math.PI/180,Pa=180/Math.PI,vb=N,Nb=-vb,Ob=ea,jc=-Ob,eb=0.08181919084262157,cb=null,Pb={RD:"ROAD",ST:"STREET",CR:"CRESCENT",CRES:"CRESCENT",CT:"COURT",LN:"LANE",HWY:"HIGHWAY",MWY:"MOTORWAY"},ha=function(){function b(a){return a?f(a.lat,a.lon):null}function d(a){return!a||a.lat===0&&a.lon===0}function f(a,c){return{lat:parseFloat(a?a:0),lon:parseFloat(c?c:
0)}}function e(a){if(a)if(t(a.lat,zb))return b(a);else{if(a.split)for(var c=[" ",","],g=0;g<c.length;g++){var h=a.split(c[g]);if(h.length===2)return f(h[0],h[1])}}else return f();return null}return{calcDistance:function(a,c){if(d(a)||d(c))return 0;var g=(c.lat-a.lat)*Ia>>1,h=(c.lon-a.lon)*Ia>>1;g=P(g)*P(g)+ua(a.lat*Ia)*ua(c.lat*Ia)*P(h)*P(h);return 6371*2*Ga(xa(g),xa(1-g))},copy:b,empty:d,equal:function(a,c){return a&&c&&a.lat==c.lat&&a.lon==c.lon},fromMercatorPixels:function(a,c){for(var g=Y(Math.E,
-c),h=N-2*Aa(g),i=wa(h,g),j=0;j<12&&O(h-i)>1.0E-7;){h=i;i=wa(h,g);j++}return f(i*Pa,a%360*Pa)},generalize:function(a,c,g){var h=a.length,i=[],j=null;g||(g=250);g/=1E3;COG.info("generalizing positions, must include "+c.length+" positions");for(var x=h;x--;)if(x===0)i.unshift(a[x]);else{var H=!j||ha.inArray(a[x],c)?g:ha.calcDistance(j,a[x]);if(a[x]&&H>=g){i.unshift(a[x]);j=a[x]}}COG.info("generalized "+h+" positions into "+i.length+" positions");return i},inArray:function(a,c){for(var g=ha.equal,h=
c.length;h--;)if(g(a,c[h]))return true;return false},inBounds:function(a,c){var g=!(ha.empty(a)||ha.empty(c));return g=(g=g&&a.lat>=c.min.lat&&a.lat<=c.max.lat)&&a.lon>=c.min.lon&&a.lon<=c.max.lon},init:f,offset:function(a,c,g){var h=a.lat*Ia;a=a.lon*Ia;c=h+c/6371;g=R(P(g/6371)/ua(h));g=a+g;c=(c+N)%Math.PI-N;g%=ea;return f(c*Pa,g*Pa)},parse:e,parseArray:function(a){var c=a.length,g=Array(c);for(c=c;c--;)g[c]=e(a[c]);return g},toMercatorPixels:function(a){return T5.XY.init(parseFloat(a.lon)*Ia,W(a.lat))},
toString:function(a){return a?a.lat+" "+a.lon:""},vectorize:function(a,c){function g(){for(var x=0,H=c.chunkSize,ba=h;ba--;){i[ba]=T5.GeoXY.init(a[ba]);x+=1;if(x>H)break}h=ba;if(h<=0)c.callback&&c.callback(i);else animFrame(g)}var h=a.length,i=Array(h);c=COG.extend({chunkSize:500,async:true,callback:null},c);if(!c.async){for(var j=h;j--;)i[j]=T5.GeoXY.init(a[j]);return i}animFrame(g);return null}}}(),Ma=function(){function b(a,c,g){var h=new q(0,c.lat-a.lat);h.x=(g||t(g,va))&&a.lon>c.lon?360-a.lon+
c.lon:c.lon-a.lon;return h}function d(a,c){return Ma.init(ha.init(a.min.lat-c,a.min.lon-c%360),ha.init(a.max.lat+c,a.max.lon+c%360))}function f(a){var c=b(a.min,a.max);return ha.init(a.min.lat+(c.y>>1),a.min.lon+(c.x>>1))}function e(a,c){return{min:ha.parse(a),max:ha.parse(c)}}return{calcSize:b,createBoundsFromCenter:function(a,c){var g=c/6371,h=a.lat*Ia,i=a.lon*Ia,j=h-g,x=h+g;if(j>Nb&&x<vb){h=R(P(g)/ua(h));g=(i-h)%ea;i=(i+h)%ea}else{j=I(j,Nb);x=y(x,vb);g=jc;i=Ob}return Ma.init(ha.init(j*Pa,g*Pa),
ha.init(x*Pa,i*Pa))},expand:d,forPositions:function(a,c){var g=null,h=T5.ticks();c||(c="auto");for(var i=a.length;i--;)if(g){var j=b(g.min,a[i],false),x=b(a[i],g.max,false);if(j.x<0)g.min.lon=a[i].lon;if(j.y<0)g.min.lat=a[i].lat;if(x.x<0)g.max.lon=a[i].lon;if(x.y<0)g.max.lat=a[i].lat}else g=e(a[i],a[i]);if(c){if(c=="auto"){i=b(g.min,g.max);c=I(i.x,i.y)*0.3}g=d(g,c)}COG.trace("calculated bounds for "+a.length+" positions",h);return g},getCenter:f,getGeoHash:function(a){var c=T5.Geo.GeoHash.encode(a.min.lat,
