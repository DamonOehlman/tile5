...
m&&d.push(m)}b(d)}catch(s){GRUNT.Log.exception(s)}e.wakeParent()}});GRUNT.WaterCooler.listen("grid.updated",function(){b(d);b(k);e.wakeParent()});return e},Tiler:function(a){function b(x){try{var y=new TILE5.Geo.Position(x.coords.latitude,x.coords.longitude),A=x.coords.accuracy/1E3;if(v){if(!s){s=new g.LocationAnnotation({pos:y,accuracy:A});u.bind("tileDrawComplete",function(){s.drawAccuracyIndicator=true})}o.add(s);var E=TILE5.Geo.B.createBoundsFromCenter(y,Math.max(A,1));u.gotoBounds(E)}else{s.pos=
y;s.accuracy=A;s.calcXY(u.getTileLayer());u.panToPosition(y,null,TILE5.Animation.Easing.Sine.Out)}v=false}catch(G){GRUNT.Log.exception(G)}}function d(x){GRUNT.Log.info("caught location tracking error:",x)}a=GRUNT.extend({tapExtent:10,provider:null,crosshair:false,zoomLevel:0,boundsChange:null,tapPOI:null,boundsChangeThreshold:30,pois:new TILE5.Geo.POIStorage,createAnnotationForPOI:null,zoomAnimation:TILE5.Animation.Easing.Quad.Out},a);var h=new TILE5.Vector,k=l.NONE,e=false,f=[],o=null,m=0,n=null,
