...
T5.PolyLayer=function(f){f=T5.ex({zindex:80,style:null},f);var l=[],k=false,e=T5.ex(new T5.ViewLayer(f),{add:function(b){l.unshift(b)},cycle:function(){return k},draw:function(b,g,d,a){b.save();try{T5.applyStyle(b,f.style);for(var m=l.length;m--;)l[m].draw(b,void 0,void 0,a)}finally{b.restore()}k=false}});e.bind("gridUpdate",function(b,g){for(var d=l.length;d--;)l[d].resync(g);k=true;e.wakeParent(true)});e.bind("parentChange",function(b,g){var d=g?g.getTileLayer():null;if(d)for(var a=l.length;a--;)l[a].resync(d)});
