...
E-y)*TWO_PI/r)+C;return t*j(2,-10*(l-=1))*k((l*E-y)*TWO_PI/r)*0.5+w+C},quadin:function(l,C,w,E){return w*(l/=E)*l+C},quadout:function(l,C,w,E){return-w*(l/=E)*(l-2)+C},quadinout:function(l,C,w,E){if((l/=E/2)<1)return w/2*l*l+C;return-w/2*(--l*(l-2)-1)+C},sinein:function(l,C,w,E){return-w*x(l/E*U)+w+C},sineout:function(l,C,w,E){return w*k(l/E*U)+C},sineinout:function(l,C,w,E){return-w/2*(x(Math.PI*l/E)-1)+C}};COG.tweenValue=function(l,C,w,E,t){function r(V){V=V?V:(new Date).getTime();if(ca+v<V){var da=
V-y,Z=w(da,l,Y,E),ma=y+E<=V;V=!ma;if(t){da=t(Z,ma,da);V=typeof da!="undefined"?da&&V:V}V&&animFrame(r)}}var y=(new Date).getTime(),ca=0,Y=C-l;animFrame(r);return{}};COG.easing=function(l){return N[l.replace(/[\-\_\s\.]/g,"").toLowerCase()]};COG.registerEasingType=function(l,C){N[l.replace(/[\-\_\s\.]/g,"").toLowerCase()]=C}})();
...
INTERACT=function(){function H(t){return function(r,y,ca){(ca?ca:t?document.body:document).addEventListener(r,y,false)}}function U(t){return function(r,y,ca){(ca?ca:t?document.body:document).removeEventListener(r,y,false)}}function v(t,r,y){(y?y:document).attachEvent("on"+t,r)}function q(t,r,y){(y?y:document).detachEvent("on"+t,r)}function j(t,r){return{x:t?t:0,y:r?r:0,count:1}}function k(t,r){w.push(COG.extend({handler:null,checks:{},type:t},r))}function s(t){var r=0,y=0;if(t.offsetParent){do{r+=
t.offsetLeft;y+=t.offsetTop;t=t.offsetParent}while(t)}return{left:r,top:y}}function x(t,r){return{source:t,target:r.target?r.target:r.srcElement}}function N(t,r){for(var y=t.target?t.target:t.srcElement;y&&y!==r&&y.nodeName&&y.nodeName.toUpperCase()!="CANVAS";)y=y.parentNode;return y&&y===r}function l(t,r){return{x:t.x-(r?r.left:0),y:t.y-(r?r.top:0)}}function C(t){if(t.preventDefault){t.preventDefault();t.stopPropagation()}else t.cancelBubble&&t.cancelBubble()}var w=[],E=function(t,r,y){function ca(W,
ea,ha,M){var Q=0,$=0,z=0;COG.tweenValue(0,W,ha,M,function(K){z=Q;Q=K});COG.tweenValue(0,ea,ha,M,function(K){Y.trigger("pan",Q-z,K-$);$=K})}y=COG.extend({binder:null,unbinder:null,observable:null},y);for(var Y=y.observable,V=null,da=[],Z=[],ma,oa,ra={bind:function(){return Y.bind.apply(null,arguments)},pannable:function(W){V=COG.extend({inertia:true},W);return ra},unbind:function(){Y.unbind();for(var W=0;W<da.length;W++)da[W].unbind();return ra}},Ca=0;Ca<r.length;Ca++)da.push(r[Ca](t,Y,y));Y.bind("pointerDown",
function(){Z=[];oa=ma=0});Y.bind("pointerMove",function(W,ea,ha,M){if(V){Z[Z.length]={ticks:(new Date).getTime(),x:M.x,y:M.y};Y.trigger("pan",M.x,M.y)}ma+=M.x?M.x:0;oa+=M.y?M.y:0});Y.bind("pointerUp",function(W,ea,ha){if(Math.abs(ma)>20||Math.abs(oa)>20){if(V){ha=Z.length;var M,Q;ea=0;for(Q=Z.length;--Q>1&&ea<50;)ea+=Z[Q].ticks-Z[Q-1].ticks;if(ha-Q>1){W=Z[ha-1].x-Z[Q].x;M=Z[ha-1].y-Z[Q].y;Q=Math.sqrt(W*W+M*M)|0;if(Q>15){W=Z[ha-1].x-Z[0].x;M=Z[ha-1].y-Z[0].y;Q=Math.sqrt(W*W+M*M)|0;ha=Math.asin(M/Q);
ea=Q*500/ea|0;ea=ea>300?300:ea;ca(Math.cos(W>0?ha:Math.PI-ha)*ea,Math.sin(ha)*ea,COG.easing("sine.out"),500)}}}}else Y.trigger("tap",ea,ha)});return ra};k("pointer",{handler:function(t,r,y){function ca(z){if(z.pageX&&z.pageY)return j(z.pageX,z.pageY);else{var K=document.documentElement,T=document.body;return j(z.clientX+(K&&K.scrollLeft||T&&T.scrollLeft||0)-(K&&K.clientLeft||T&&T.clientLeft||0),z.clientY+(K&&K.scrollTop||T&&T.scrollTop||0)-(K&&K.clientTop||T&&T.clientTop||0))}}function Y(z){if(N(z,
t))if(W=ma(z)){var K=ca(z);t.style.cursor="move";C(z);Q=K.x;$=K.y;ea=j(Q,$);r.triggerCustom("pointerDown",x("mouse",z),ea,l(ea,s(t)))}}function V(z){var K=ca(z);ha=K.x;M=K.y;if(N(z,t))oa(z,W?"pointerMove":"pointerHover")}function da(z){if(W&&ma(z)){W=false;if(N(z,t)){t.style.cursor="default";oa(z,"pointerUp")}}}function Z(z){if(N(z,t)){var K;z=z||window.event;if(z.detail)K=z.axis===2?-z.detail*ra:0;else{K=z.wheelDeltaY?z.wheelDeltaY:z.wheelDelta;if(window.opera)K=-K}if(K!==0){var T=j(ha,M);r.triggerCustom("zoom",
x("mouse",z),T,l(T,s(t)),K/Ca,"wheel");C(z);z.returnValue=false}}}function ma(z){z=z||window.event;return(z.which||z.button)==1}function oa(z,K,T,ka,Aa){T=typeof T!="undefined"?T:ha;ka=typeof ka!="undefined"?ka:M;var J=T-Q,X=ka-$,L=j(T,ka);r.triggerCustom(K,x("mouse",z),L,l(L,s(t)),j(J,X));if(typeof Aa=="undefined"||Aa){Q=T;$=ka}}y=COG.extend({},y);var ra=120,Ca=ra*8,W=false,ea,ha,M,Q,$;y.binder("mousedown",Y,false);y.binder("mousemove",V,false);y.binder("mouseup",da,false);y.binder("dblclick",function(z){COG.info("captured double click");
if(N(z,t)){var K=ca(z);r.triggerCustom("doubleTap",x("mouse",z),K,l(K,s(t)))}},false);y.binder("mousewheel",Z,document);y.binder("DOMMouseScroll",Z,document);return{unbind:function(){y.unbinder("mousedown",Y,false);y.unbinder("mousemove",V,false);y.unbinder("mouseup",da,false);y.unbinder("mousewheel",Z,document);y.unbinder("DOMMouseScroll",Z,document)}}},checks:{touch:false}});k("pointer",{handler:function(t,r,y){function ca(J){if(J.count<2)return 0;var X=J.x-J.next.x;J=J.y-J.next.y;return~~Math.sqrt(X*
X+J*J)}function Y(J,X,L){X=X?X:0;L=L?L:0;for(var sa={x:J.x-X,y:J.y-L,id:J.id,count:J.count},qa=sa;J.next;){J=J.next;qa=qa.next={x:J.x-X,y:J.y-L,id:J.id}}return sa}function V(J,X){var L=J[X?X:"touches"],sa,qa;if(L.length===0)return null;qa=sa={x:L[0].pageX,y:L[0].pageY,id:L[0].identifier,count:L.length};for(var ya=1,Xa=L.length;ya<Xa;ya++)qa=qa.next={x:L[ya].pageX,y:L[ya].pageY,id:L[ya].identifier};return sa}function da(J){if(N(J,t)){M=s(t);var X=V(J,"changedTouches"),L=Y(X,M.left,M.top);if(!$){Q=
W;r.triggerCustom("pointerDown",x("touch",J),X,L)}ka&&r.triggerCustom("pointerDownMulti",x("touch",J),X,L);$=V(J);if($.count>1)K=ca($);Aa=1;T=Y($)}}function Z(J){if(N(J,t)){J.preventDefault();z=V(J);if(Q==W)Q=Math.abs($.x-z.x)>ra||Math.abs($.y-z.y)>ra?Ca:W;if(Q!=W){Q=z.count>1?ha:ea;if(Q==ha)if($.count===1){$=Y(z);K=ca($)}else{var X=ca(z);if(Math.abs(K-X)<oa)Q=ea;else{var L;L=z.x;var sa=z.next.x,qa=z.y,ya=z.next.y,Xa=qa<ya?qa:ya;L={x:(L<sa?L:sa)+(Math.abs(L-sa)>>1),y:Xa+(Math.abs(qa-ya)>>1)};X=X/
K;sa=X-Aa;r.triggerCustom("zoom",x("touch",J),L,l(L,M),sa,"pinch");Aa=X}}Q==ea&&r.triggerCustom("pointerMove",x("touch",J),z,Y(z,M.left,M.top),j(z.x-T.x,z.y-T.y));ka&&r.triggerCustom("pointerMoveMulti",x("touch",J),z,Y(z,M.left,M.top))}T=Y(z)}}function ma(J){if(N(J,t)){var X=V(J,"changedTouches"),L=Y(X,M.left,M.top);z=V(J);if(!z){r.triggerCustom("pointerUp",x("touch",J),X,L);$=null}ka&&r.triggerCustom("pointerUpMulti",x("touch",J),X,L)}}y=COG.extend({detailed:false,inertia:false},y);var oa=20,ra=
7,Ca=0,W=1,ea=2,ha=3,M,Q,$,z,K,T,ka=y.detailed,Aa=1;y.binder("touchstart",da,false);y.binder("touchmove",Z,false);y.binder("touchend",ma,false);COG.info("initialized touch handler");return{unbind:function(){y.unbinder("touchstart",da,false);y.unbinder("touchmove",Z,false);y.unbinder("touchend",ma,false)}}},checks:{touch:true}});return{register:k,watch:function(t,r,y){r=COG.extend({bindToBody:false,observable:null,isIE:typeof window.attachEvent!="undefined",types:null},r);capabilities=COG.extend({touch:"ontouchstart"in
window},y);if(!r.observable){COG.info("creating observable");r.observable=COG.observable({});globalOpts=r}r.binder=r.isIE?v:H(r.bindToBody);r.unbinder=r.isIE?q:U(r.bindToBody);y=r.types;for(var ca=capabilities,Y=[],V=w.length;V--;){var da=w[V],Z=!y||y.indexOf(da.type)>=0,ma=true;for(var oa in da.checks){var ra=da.checks[oa];COG.info("checking "+oa+" capability. require: "+ra+", capability = "+ca[oa]);ma=ma&&ra===ca[oa]}if(Z&&ma)Y[Y.length]=da.handler}return new E(t,Y,r)}}}();var T5={};
...
g=b[d],h=d=="translate",i=a.autoInvalidate,n=f.length&&e.length&&f.length==e.length?f.length:0,p=Array(n),I=Array(n),G=COG.easing(a.easing),D=a.duration,P=a.progress,m=function(A){var F=A-c;A=c+D<=A;for(var R=b.layer?b.layer.view:null,O,aa=n;aa--;){O=G(F,f[aa],p[aa],D);I[aa]=h?O|0:O}g.apply(b,I);i&&R&&R.invalidate();if(P){F=[].concat(A?e:I);F.unshift(A);P.apply(b,F)}if(A){b.animations--;g.apply(b,e);a.complete&&a.complete.apply(b,e)}else w(m)};if(g&&g.apply&&n>0){D=D?D:DEFAULT_DURATION;for(d=n;d--;)p[d]=
...
b,d)}function ca(b){function d(){if(h&&h.width>0){if(!this.centerOffset)this.centerOffset=new q(-h.width>>1,-h.height>>1);this.updateBounds(new j(this.xy.x+this.centerOffset.x,this.xy.y+this.centerOffset.y,h.width,h.height),false)}}function f(n){if(this.imageUrl=n){var p=this;s(this.imageUrl,function(I,G){h=I;if(G){var D=i.layer?i.layer.view:null;D&&D.invalidate()}d.apply(p)})}}b=COG.extend({image:null,imageUrl:null,centerOffset:null,typeName:"Image"},b);var e=null,a=za.prototype.resync,c=0,g=0,h=
...
for(c=h;c<=n;c++)for(h=i;h<=a;h++){var p=d(c,h),I=ha.call(p,g);I>=0&&p.splice(I,1)}},search:function(a){var c=a.y/b|0,g=(a.x+a.w)/b|0,h=(a.y+a.h)/b|0,i=[],n=[];for(a=a.x/b|0;a<=g;a++)for(var p=c;p<=h;p++)i=i.concat(d(a,p));i.sort();for(c=i.length;c--;){g=i[c];if(h=e[g])n[n.length]=h;for(;c>0&&i[c-1]==g;)c--}return n}}},Vb=3E4,ub={},Wb=0;(new Date).getTime();var Ma={},Sa=[];x.prototype={constructor:x,load:function(b){var d=this;s(this.url,function(f){d.loaded=true;d.image=f;b&&b()})}};var ac=function(){return{applyStyle:function(){},
...
m.scaling)}return I},drawTiles:function(m,A){for(var F,R,O=n-256,aa=p-256,va=m.x2,ta=m.y2,Ba=A.length;Ba--;){F=A[Ba];if(R=F.x>=O&&F.x<=va&&F.y>=aa&&F.y<=ta)F.loaded?i.drawImage(F.image,F.x-n,F.y-p):F.load(b.invalidate)}},hitTest:function(m,A,F){return i.isPointInPath(A,F)},prepare:function(m,A){var F,R=false,O=A.scaled||A,aa=A.scaleFactor;if(i)i.restore();else i=h.getContext("2d");for(F=m.length;F--;)R=R||m[F].clip;n=O.x;p=O.y;if(i){R||i.clearRect(0,0,c,g);i.save();i.scale(aa,aa)}i.globalCompositeOperation=
"source-over";return i},prepArc:function(m,A,F,R){i.beginPath();i.arc(m.xy.x-(I?I.x:n),m.xy.y-(I?I.y:p),m.size>>1,m.startAngle,m.endAngle,false);return a(A,F,R)},prepImage:function(m,A,F,R,O){var aa=(O.x||m.xy.x)-(I?I.x:n),va=(O.y||m.xy.y)-(I?I.y:p),ta=O.image||m.image;if(ta){i.beginPath();i.rect(aa,va,O.width||ta.width,O.height||ta.height);return a(A,F,R,function(){i.drawImage(ta,aa,va,O.width||ta.width,O.height||ta.height)})}},prepPoly:function(m,A,F,R,O){var aa=true;m=O.points||m.points;O=I?I.x:
n;var va=I?I.y:p;i.beginPath();for(var ta=m.length;ta--;){var Ba=m[ta].x-O,Pa=m[ta].y-va;if(aa){i.moveTo(Ba,Pa);aa=false}else i.lineTo(Ba,Pa)}return a(A,F,R)},getContext:function(){return i},getDimensions:function(){return{width:c,height:g}},getOffset:function(){return new q(n,p)}})});var Ab={},dc=H.defineStyle=function(b,d){Ab[b]=N(d);return b},Bb=H.defineStyles=function(b){for(var d in b)dc(d,b[d])},cc=H.getStyle=function(b){return Ab[b]};H.loadStyles=function(b){COG.jsonp(b,function(d){Bb(d)})};
Bb({basic:{lineWidth:1,strokeStyle:"#000",fillStyle:"#fff"},waypoints:{lineWidth:4,strokeStyle:"rgba(0, 51, 119, 0.9)",fillStyle:"#FFF"},waypointsHover:{lineWidth:4,strokeStyle:"#f00",fillStyle:"#FFF"}});var Xb={NONE:0,ACTIVE:1,ANIMATING:4,PAN:8,ZOOM:16,FREEZE:128},Kb=function(b){function d(o,u,B){Ea||Pa(ia-u,ja-B)}function f(o,u,B,S){va(K(T(la+ka(2,S)-1,0.5),2))}function e(){var o=J(la)/Math.LN2|0;if(o!==0){la=ka(2,o);ta(Za+o,ec,fc)}redraw=true}function a(o,u){var B=ga.getViewport(),S=1/la;return new q(B?
B.x+o*S:o,B?B.y+u*S:u)}function c(o,u,B){Ba("doubleTap",u,B,a(B.x,B.y));b.scalable&&va(2,a(B.x,B.y),b.zoomEasing,null,b.zoomDuration)}function g(o,u,B){Ea=null;F("down",u,B)}function h(o,u,B){F("hover",u,B)}function i(o,u,B){m(u,B,false)}function n(o,u,B){m(u,B,true)}function p(){clearTimeout(Cb);Cb=setTimeout(function(){renderer.checkSize()},250)}function I(o,u,B){F("tap",u,B);Ba("tap",u,B,a(B.x,B.y,true))}function G(){renderer=bc(b.renderer,ga,ib);Db=!renderer.preventPartialScale;P()}function D(o,
u){u.id=o;u.added=U();u.view=ga;u.trigger("parentChange",ga,ib,gc);na.push(u);na.sort(function(B,S){var ba=S.zindex-B.zindex;if(ba===0)ba=S.added-B.added;return ba});Ga=na.length;return u}function P(){wa&&wa.unbind();if(renderer){wa=INTERACT.watch(renderer.interactTarget);b.pannable&&wa.pannable().bind("pan",d);if(b.scalable){wa.bind("zoom",f);wa.bind("doubleTap",c)}wa.bind("pointerDown",g);wa.bind("pointerMove",i);wa.bind("pointerUp",n);b.captureHover&&wa.bind("pointerHover",h);wa.bind("tap",I)}}
function m(o,u,B){if(Ea){o=a(u.x,u.y);if(Ea.drag.call(Ea.target,Ea,o.x,o.y,B))redraw=true;if(B)Ea=null}}function A(o){var u,B,S=false;o=o||(new Date).getTime();if(S=o-Eb>hc){var ba=$(ia-jb),ua=$(ja-kb);u=ia!==lb||ja!==mb;B=la!==Fb;if(u||B)Qa++;if(ba>=Gb||ua>=Gb)aa();ga.trigger("enterFrame",o,ic++);Eb=o}if(S&&Qa){Ka=Hb|(la!==1?Ib:0)|(u?nb:0)|(ob?Jb:0);pb=(u=(Ka&(Ib|nb))!==0)&&(Ka&Jb)===0;u=O();if(renderer.prepare(na,u,Ka,o,xa)){Qa=0;for(ii=Ga;ii--;){B=na[ii];if((Ka&B.validStates)!==0){S=B.style?renderer.applyStyle(B.style):
null;B.draw(renderer,u,Ka,ga,o,xa);S&&renderer.applyStyle(S)}}renderer.render(u);lb=ia;mb=ja;Fb=la}if(xa){o=xa?xa.elements:[];if($a&&$a.type==="hover"){u=Hits.diffHits($a.elements,o);u.length>0&&Hits.triggerEvent($a,ga,"Out",u)}if(o.length>0){B=xa.x;S=xa.y;for(u=o.length;u--;){ba=o[u];ua=B;var Ha=S,Fa=ba&&ba.drag&&(!ba.canDrag||ba.canDrag(ba,ua,Ha));if(Fa){Ea=ba;Ea.startX=ua;Ea.startY=Ha}if(Fa)break}Hits.triggerEvent(xa,ga)}$a=o.length>0?COG.extend({},xa):null;xa=null}}animFrame(A)}function F(o,u,
B){xa=Hits.init(o,u,B,a(B.x,B.y,true));for(o=Ga;o--;)qb=qb||(na[o].hitGuess?na[o].hitGuess(xa.x,xa.y,Ka,ga):false);qb&&Qa++}function R(){Qa++}function O(){var o=new j(ia,ja,ga.width,ga.height);o.scaleFactor=la;if(la!==1)o.scaled=Ta.fromCenter(ia+(ga.width>>1),ja+(ga.height>>1),ga.width/la|0,ga.height/la|0);return o}function aa(){var o=O();if(o){if(Va||Wa)if(o){var u=db?ia+(o.w>>1):ia,B=eb?ja+(o.h>>1):ja,S=o.w,ba=o.h;if(Va&&Va>S)if(u+S>Va)ia=db?ia:Va-S;else if(u<0)ia=db?ia:0;if(Wa&&Wa>ba)if(B+ba>Wa)ja=
eb?ja:Wa-ba;else if(B<0)ja=eb?ja:0}jb=ia;kb=ja;Ba("refresh",ga,o);Qa++}}function va(o,u,B,S,ba){var ua;if(!Db){B=false;ua=X(J(o)/Math.LN2);o=ka(2,ua)}if(B)COG.tweenValue(la,o,B,ba,function(Ha,Fa){la=Ha;if(Fa){ua=X(J(la)/Math.LN2);la=ka(2,ua);S&&S()}e()});else{la=o;e()}return ga}function ta(o,u,B){o=T(b.minZoom,K(b.maxZoom,o));if(o!==Za){var S=ka(2,o-Za),ba=ga.width>>1,ua=ga.height>>1,Ha=ba/S|0,Fa=ua/S|0;Za=o;Pa(((u?u:ia+ba)-Ha)*S,((B?B:ja+ua)-Fa)*S);lb=ia;mb=ja;kb=jb=0;Ba("zoomLevelChange",o);la=
1;renderer.reset();aa()}}function Ba(){for(var o=ga.trigger.apply(null,arguments).cancel,u=na.length;u--;)o=na[u].trigger.apply(null,arguments).cancel||o;return!o}function Pa(o,u,B,S,ba){function ua(){Ha+=1;if(Ha>=2){ob=false;ba&&ba()}}var Ha=0;if(B){if((Ka&nb)===0){COG.tweenValue(ia,o,B,S,function(Fa,rb){ia=Fa|0;(rb?ua:R)();return!pb});COG.tweenValue(ja,u,B,S,function(Fa,rb){ja=Fa|0;(rb?ua:R)();return!pb});ob=true}}else{ia=o|0;ja=u|0;ba&&ba()}}b=COG.extend({id:COG.objId("view"),container:"",captureHover:true,
captureDrag:false,inertia:true,refreshDistance:256,pannable:false,clipping:true,scalable:false,panAnimationEasing:COG.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,autoSize:true,tapExtent:10,guides:false,turbo:false,fps:25,minZoom:1,maxZoom:1,renderer:"canvas",zoomEasing:COG.easing("quad.out"),zoomDuration:300,zoomLevel:1},b);var jc={},na=[],Ga=0,ib=document.getElementById(b.container),Ea=null,ic=0,gc=null,kc=!v(window.attachEvent,pa),qb=false,Gb=b.refreshDistance,ia=0,ja=
0,jb=0,kb=0,lb=0,mb=0,Va=null,Wa=null,db=false,eb=false,xa=null,pb=false,$a=null,Cb=0,la=1,Fb=1,Eb=0,wa=null,Db=true,ob=false,hc=1E3/b.fps|0,Qa=0,ec,fc,Za=b.zoomLevel,Hb=l("ACTIVE"),nb=l("PAN"),Ib=l("ZOOM"),Jb=l("ANIMATING"),Ka=Hb,ga={id:b.id,detach:function(){wa&&wa.unbind()},eachLayer:function(o){for(var u=Ga;u--;)o(na[u])},getLayer:function(o){for(var u=0;u<Ga;u++)if(na[u].id===o)return na[u];return null},getZoomLevel:function(){return Za},setLayer:function(o,u){for(var B=0;B<Ga;B++)if(na[B].id===
o){na.splice(B,1);break}u&&D(o,u);aa();return u},invalidate:R,refresh:aa,resetScale:function(){la=1},scale:va,setZoomLevel:ta,syncXY:function(){},triggerAll:Ba,removeLayer:function(o){a:{for(var u=Ga;u--;)if(na[u].id===o){o=u;break a}o=-1}if(o>=0&&o<Ga){ga.trigger("layerRemoved",na[o]);na.splice(o,1);Qa++}Ga=na.length},getOffset:function(){return new q(ia,ja)},getRenderer:function(){return renderer},getScaleFactor:function(){return la},setMaxOffset:function(o,u,B,S){Va=o;Wa=u;db=B;eb=S},getViewport:O,
updateOffset:Pa,pan:function(o,u,B,S,ba){Pa(ia+o,ja+u,B,S,ba)}};COG.observable(ga);ga.bind("resync",function(){});COG.configurable(ga,["container","captureHover","captureDrag","scalable","pannable","inertia","minZoom","maxZoom","zoom"],COG.paramTweaker(b,null,{container:function(o,u){ib=document.getElementById(u);G()},inertia:P,captureHover:P,scalable:P,pannable:P}),true);CANI.init(function(o){ga.markers=D("markers",new fb({zindex:20}));jc=o;G();kc?window.attachEvent("onresize",p):window.addEventListener("resize",
p,false)});animFrame(A);return ga},za=function(b){b=COG.extend({style:null,xy:null,size:null,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape"},b);COG.extend(this,b);this.id=COG.objId(this.typeName);this.view=this.bounds=null;this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.observable&&COG.observable(this)};za.prototype={constructor:za,animate:function(b,d,f,e){E(this,b,d,f,e)},drag:null,draw:null,getProps:null,resync:function(b){if(this.xy){b.syncXY([this.xy]);
...
this.xy.y,this.size>>1,0,Math.PI*2)}});r.prototype=COG.extend({},za.prototype,{constructor:r});y.prototype=COG.extend({},r.prototype);ca.prototype=COG.extend({},za.prototype,{constructor:ca});Y.prototype=COG.extend({},ca.prototype,{constructor:Y});V.prototype=COG.extend(za.prototype,{constructor:V});da.prototype={constructor:da,clip:null,cycle:function(){},draw:function(){},hitGuess:null};Ua=function(b,d){function f(n,p){if(a&&h){g=a.search(h);h=null}var I=p&&p.getZoomLevel?p.getZoomLevel():0;c[I]||
(c[I]=new hb);a=c[I]}d=COG.extend({imageLoadArgs:{}},d);var e=b?yb.init(b,d).run:null,a=null,c=[],g=[],h=null,i=COG.extend(new da(d),{draw:function(n,p){n.drawTiles(p,a.search(p))}});i.bind("refresh",function(n,p,I){var G=(new Date).getTime();if(a){g=h?a.search(h):[];e(p,I,a,function(){p.invalidate();COG.info("GEN COMPLETED IN "+((new Date).getTime()-G)+" ms")});h=Ta.copy(I)}});i.bind("parentChange",f);i.bind("resync",f);return i};var Lb=function(b){function d(i){clearTimeout(g);g=setTimeout(function(){a.sort(function(n,
p){if(p.xy&&n.xy){var I=p.xy.y-n.xy.y;return I!=0?I:p.xy.x-n.xy.x}});i&&i.invalidate()},50)}function f(i,n,p,I){I&&c.remove(I,n);c.insert(p,n)}function e(i,n){for(var p=a.length;p--;)a[p].resync(n)}b=COG.extend({zindex:10},b);var a=[],c=new hb,g=0,h=COG.extend(new da(b),{add:function(i,n){if(i){i.layer=h;if(n)a.unshift(i);else a[a.length]=i;var p=h.view;if(p){i.resync(p);c&&i.bounds&&c.insert(i.bounds,i);d(p)}i.bind("move",f)}},clear:function(){c=new hb;a=[]},draw:function(i,n,p,I,G,D){I={};G=c&&
...
h.bind("parentChange",e);h.bind("resync",e);return h},fb=function(b){return new Lb(b)};COG.extend(H,{ex:COG.extend,is:v,ticks:U,userMessage:function(b,d,f){H.trigger("userMessage",b,d,f)},indexOf:ha,Rect:j,XY:Na,XYRect:Ta,Vector:gb,Hits:Hits,Generator:yb,tweenValue:COG.tweenValue,easing:COG.easing,Tile:x,TileLayer:Ua,getImage:s,viewState:l,View:Kb,ViewLayer:da,ImageLayer:Ua,Drawable:za,Marker:t,Poly:r,Line:y,Arc:V,ImageDrawable:ca,ImageMarker:Y,DrawLayer:Lb,ShapeLayer:fb});COG.observable(H);var sb=
[1.406245461070741,1.321415085624082,1.077179995861952,0.703119412486786,0.488332580888611],Da=Math.PI/180,La=180/Math.PI,tb=Q,Mb=-tb,Nb=M,lc=-Nb,cb=0.08181919084262157,ab=null,Ob={RD:"ROAD",ST:"STREET",CR:"CRESCENT",CRES:"CRESCENT",CT:"COURT",LN:"LANE",HWY:"HIGHWAY",MWY:"MOTORWAY"},fa=function(){function b(a){return a?f(a.lat,a.lon):null}function d(a){return!a||a.lat===0&&a.lon===0}function f(a,c){return{lat:parseFloat(a?a:0),lon:parseFloat(c?c:0)}}function e(a){if(a)if(v(a.lat,xb))return b(a);else{if(a.split)for(var c=
...
La,a%360*La)},generalize:function(a,c,g){var h=a.length,i=[],n=null;g||(g=250);g/=1E3;COG.info("generalizing positions, must include "+c.length+" positions");for(var p=h;p--;)if(p===0)i.unshift(a[p]);else{var I=!n||fa.inArray(a[p],c)?g:fa.calcDistance(n,a[p]);if(a[p]&&I>=g){i.unshift(a[p]);n=a[p]}}COG.info("generalized "+h+" positions into "+i.length+" positions");return i},inArray:function(a,c){for(var g=fa.equal,h=c.length;h--;)if(g(a,c[h]))return true;return false},inBounds:function(a,c){var g=
!(fa.empty(a)||fa.empty(c));return g=(g=g&&a.lat>=c.min.lat&&a.lat<=c.max.lat)&&a.lon>=c.min.lon&&a.lon<=c.max.lon},init:f,offset:function(a,c,g){var h=a.lat*Da;a=a.lon*Da;c=h+c/6371;g=sa(L(g/6371)/qa(h));g=a+g;c=(c+Q)%Math.PI-Q;g%=M;return f(c*La,g*La)},parse:e,parseArray:function(a){var c=a.length,g=Array(c);for(c=c;c--;)g[c]=e(a[c]);return g},toMercatorPixels:function(a){return T5.XY.init(parseFloat(a.lon)*Da,ma(a.lat))},toString:function(a){return a?a.lat+" "+a.lon:""},vectorize:function(a,c){function g(){for(var p=
0,I=c.chunkSize,G=h;G--;){i[G]=T5.GeoXY.init(a[G]);p+=1;if(p>I)break}h=G;if(h<=0)c.callback&&c.callback(i);else animFrame(g)}var h=a.length,i=Array(h);c=COG.extend({chunkSize:500,async:true,callback:null},c);if(!c.async){for(var n=h;n--;)i[n]=T5.GeoXY.init(a[n]);return i}animFrame(g);return null}}}(),Ia=function(){function b(a,c,g){var h=new q(0,c.lat-a.lat);h.x=(g||v(g,pa))&&a.lon>c.lon?360-a.lon+c.lon:c.lon-a.lon;return h}function d(a,c){return Ia.init(fa.init(a.min.lat-c,a.min.lon-c%360),fa.init(a.max.lat+
c,a.max.lon+c%360))}function f(a){var c=b(a.min,a.max);return fa.init(a.min.lat+(c.y>>1),a.min.lon+(c.x>>1))}function e(a,c){return{min:fa.parse(a),max:fa.parse(c)}}return{calcSize:b,createBoundsFromCenter:function(a,c){var g=c/6371,h=a.lat*Da,i=a.lon*Da,n=h-g,p=h+g;if(n>Mb&&p<tb){h=sa(L(g)/qa(h));g=(i-h)%M;i=(i+h)%M}else{n=T(n,Mb);p=K(p,tb);g=lc;i=Nb}return Ia.init(fa.init(n*La,g*La),fa.init(p*La,i*La))},expand:d,forPositions:function(a,c){var g=null,h=T5.ticks();c||(c="auto");for(var i=a.length;i--;)if(g){var n=
...
c){var g=f(a);g=K(X($(g.lat)*0.05),sb.length);var h=sb[g],i=b(a.min,a.max);g=z(J(sb[3]*c.h/i.y)/J(2));h=z(J(h*c.w/i.x)/J(2));return K(isNaN(g)?1E3:g,isNaN(h)?1E3:h)},init:e,isEmpty:function(a){return!a||fa.empty(a.min)||fa.empty(a.max)},toString:function(a){return"min: "+fa.toString(a.min)+", max: "+fa.toString(a.max)}}}();gb=function(){var b=/^(\d+).*$/,d=/(\d+)\s?\-\s?(\d+)/;return{buildingMatch:function(f,e){b.lastIndex=-1;if(b.test(f))for(var a=f.replace(b,"$1"),c=e.split(","),g=0;g<c.length;g++){d.lastIndex=
...
a.push({regex:/arrive/i,turnType:f.Arrive});a.push({regex:/fell\sthrough/i,turnType:f.Merge});return a}();return{calculate:function(a){a=COG.extend({engineId:"",waypoints:[],map:null,error:null,autoFit:true,success:null,generalize:false},a);var c=Z("route");c&&c.route(a,function(g){if(a.generalize)g.geometry=fa.generalize(g.geometry,g.getInstructionPositions());if(a.map){b(a.map,g);a.autoFit&&a.map.gotoBounds(g.boundingBox)}a.success&&a.success(g)})},createMapOverlay:b,parseTurnType:d,TurnType:f,
...
c.push(a.instructions[g].position);return c}},a)}}}(),nc="featurecollection",oc="feature",wb={async:false},Ra={processor:null,group:"shapes",layerClass:fb},pc={point:COG.extend({},Ra,{processor:function(b,d,f,e){d=ea([d.coordinates],wb);if(d.length>0)if(f=e.marker(d[0],f)){b.add(f);return d.length}},group:"markers",layerClass:fb}),linestring:COG.extend({},Ra,{processor:function(b,d,f,e){d=ea(d&&d.coordinates?d.coordinates:[]);return W(b,d,f,e.line)}}),multilinestring:COG.extend({},Ra,{processor:function(b,
...
return c};H.Geo={distanceToString:function(b){if(b>1E3)return~~(b/10)/100+" km";return b?b+" m":""},dist2rad:function(b){return b/6371},getEngine:Z,radsPerPixel:oa,Position:fa,BoundingBox:Ia,Radius:function(b,d){return{distance:parseInt(b,10),uom:d}},Address:function(b){return b=COG.extend({streetDetails:"",location:"",country:"",postalCode:"",pos:null,boundingBox:null},b)},A:gb,GeocodeFieldWeights:{streetDetails:50,location:50},AddressCompareFns:{},Engine:function(b){if(!b.id)throw Error("A GEO.Engine cannot be registered without providing an id.");
...
fa.init(i.coords.latitude,i.coords.longitude);i=new Pb({id:1,caption:"Current Location",pos:n,accuracy:i.coords.accuracy/1E3,matchWeight:100});if(!c||i.accuracy<c.accuracy)c=i;if(!b.requiredAccuracy||c.accuracy<b.requiredAccuracy)d(c,h)},f,{enableHighAccuracy:true,timeout:1E4,maximumAge:5E3});if(b.searchTimeout)a=setTimeout(function(){c&&d(c,h)},b.searchTimeout)}}},b))},Routing:mc,GeoJSON:Ra}})(T5);
