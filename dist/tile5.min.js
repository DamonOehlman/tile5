...
Math.LN2);va=za(2,Ba);S&&S()}f()});else{va=q;f()}return aa}function ba(q,u,y){q=sa(b.minZoom,W(b.maxZoom,q));if(q!==$a){var S=za(2,q-$a),da=aa.width>>1,Ba=aa.height>>1,La=da/S|0,Ja=Ba/S|0;$a=q;ca(((u?u:pa+da)-La)*S,((y?y:qa+Ba)-Ja)*S);mb=pa;nb=qa;lb=kb=0;R("zoomLevelChange",q);va=1;renderer.reset();ja()}}function R(){for(var q=aa.trigger.apply(null,arguments).cancel,u=N.length;u--;)q=N[u].trigger.apply(null,arguments).cancel||q;return!q}function ca(q,u,y,S,da){function Ba(){La+=1;if(La>=2){pb=false;
da&&da()}}var La=0;if(y){if((Ka&ob)===0){COG.tweenValue(pa,q,y,S,function(Ja,sb){pa=Ja|0;(sb?Ba:X)();return!qb});COG.tweenValue(qa,u,y,S,function(Ja,sb){qa=Ja|0;(sb?Ba:X)();return!qb});pb=true}}else{pa=q|0;qa=u|0;da&&da()}}b=COG.extend({id:COG.objId("view"),container:"",captureHover:true,captureDrag:false,inertia:true,refreshDistance:256,pannable:false,clipping:true,scalable:false,panAnimationEasing:COG.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,autoSize:true,tapExtent:10,
guides:false,turbo:false,fps:25,minZoom:1,maxZoom:1,renderer:"canvas",zoomEasing:COG.easing("quad.out"),zoomDuration:300,zoomLevel:1},b);var $={},N=[],Y=0,ta=document.getElementById(b.container),wa=null,eb=0,hc=null,jc=!v(window.attachEvent,ya),rb=false,Hb=b.refreshDistance,pa=0,qa=0,kb=0,lb=0,mb=0,nb=0,Xa=null,Ya=null,fb=false,gb=false,Fa=null,qb=false,ab=null,Db=0,va=1,Gb=1,Fb=0,Ea=null,Eb=true,pb=false,ic=1E3/b.fps|0,Ia=0,fc,gc,$a=b.zoomLevel,Ib=ka("ACTIVE"),ob=ka("PAN"),Jb=ka("ZOOM"),Kb=ka("ANIMATING"),
Ka=Ib,aa={id:b.id,detach:function(){renderer&&renderer.trigger("detach");Ea&&Ea.unbind()},eachLayer:function(q){for(var u=Y;u--;)q(N[u])},getLayer:function(q){for(var u=0;u<Y;u++)if(N[u].id===q)return N[u];return null},getZoomLevel:function(){return $a},setLayer:function(q,u){for(var y=0;y<Y;y++)if(N[y].id===q){N.splice(y,1);break}if(u){G(q,u);u.trigger("refresh",aa,ma());aa.trigger("layerChange",aa,u)}Ia++;return u},invalidate:X,refresh:ja,resetScale:function(){va=1},scale:A,setZoomLevel:ba,syncXY:function(){},
triggerAll:R,removeLayer:function(q){a:{for(var u=Y;u--;)if(N[u].id===q){q=u;break a}q=-1}if(q>=0&&q<Y){aa.trigger("layerRemove",aa,N[q]);N.splice(q,1);Ia++}Y=N.length},getOffset:function(){return new r(pa,qa)},getRenderer:function(){return renderer},getScaleFactor:function(){return va},setMaxOffset:function(q,u,y,S){Xa=q;Ya=u;fb=y;gb=S},getViewport:ma,updateOffset:ca,pan:function(q,u,y,S,da){ca(pa+q,qa+u,y,S,da)}};COG.observable(aa);aa.bind("resync",function(){});COG.configurable(aa,["container",
"captureHover","captureDrag","scalable","pannable","inertia","minZoom","maxZoom","renderer","zoom"],COG.paramTweaker(b,null,{container:function(q,u){ta=document.getElementById(u);I()},inertia:t,captureHover:t,scalable:t,pannable:t,renderer:function(q,u){if(renderer){renderer.trigger("detach");renderer=null}I(u);Ia++}}),true);CANI.init(function(q){aa.markers=G("markers",new hb({zindex:20}));$=q;I();jc?window.attachEvent("onresize",o):window.addEventListener("resize",o,false)});animFrame(C);return aa},
Ga=function(b){b=COG.extend({style:null,xy:null,size:10,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape"},b);COG.extend(this,b);this.id=COG.objId(this.typeName);this.view=this.bounds=null;this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.observable&&COG.observable(this)};Ga.prototype={constructor:Ga,animate:function(b,d,e,f){j(this,b,d,e,f)},drag:null,draw:null,getProps:null,resync:function(b){if(this.xy){b.syncXY([this.xy]);
