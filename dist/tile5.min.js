...
f.splice(g,1);else f=[];n()},find:function(b){for(var g=[],d=f.length;d--;)if(!b||b(f[d]))g[g.length]=f[d];return g}});a.bind("tap",function(b,g,d){for(var o=[],e=d.x,k=d.y,h=f.length;h--;)if(f[h].hitTest(e,k))o[o.length]=f[h];if(o.length>0)b.cancel=a.trigger("markerTap",g,d,o).cancel});a.bind("parentChange",m);a.bind("resync",m);a.bind("changed",m);return a};
