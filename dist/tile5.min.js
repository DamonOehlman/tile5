...
l(2,-10*(o-=1))*q((o*n-Q)*u/x)*0.5+y+D},quadin:function(o,D,y,n){return y*(o/=n)*o+D},quadout:function(o,D,y,n){return-y*(o/=n)*(o-2)+D},quadinout:function(o,D,y,n){if((o/=n/2)<1)return y/2*o*o+D;return-y/2*(--o*(o-2)-1)+D},sinein:function(o,D,y,n){return-y*O(o/n*V)+y+D},sineout:function(o,D,y,n){return y*q(o/n*V)+D},sineinout:function(o,D,y,n){return-y/2*(O(Math.PI*o/n)-1)+D}};COG.tweenValue=function(o,D,y,n,r){function x(W){W=W?W:(new Date).getTime();if($+s<W){var S=W-Q,pa=y(S,o,ia,n),ja=Q+n<=W;
W=!ja;if(r){S=r(pa,ja,S);W=typeof S!="undefined"?S&&W:W}W&&animFrame(x)}}var Q=(new Date).getTime(),$=0,ia=D-o;animFrame(x);return{}};COG.easing=function(o){return ha[o.replace(/[\-\_\s\.]/g,"").toLowerCase()]};COG.registerEasingType=function(o,D){ha[o.replace(/[\-\_\s\.]/g,"").toLowerCase()]=D}})();
...
0,aa=0,C=0;COG.tweenValue(0,Y,ga,N,function(K){C=R;R=K});COG.tweenValue(0,da,ga,N,function(K){$.trigger("pan",R-C,K-aa);aa=K})}x=COG.extend({binder:null,unbinder:null,observable:null},x);for(var $=x.observable,ia=null,W=[],S=[],pa,ja,ta={bind:function(){return $.bind.apply(null,arguments)},pannable:function(Y){ia=COG.extend({inertia:true},Y);return ta},unbind:function(){$.unbind();for(var Y=0;Y<W.length;Y++)W[Y].unbind();return ta}},Ca=0;Ca<r.length;Ca++)W.push(r[Ca](n,$,x));$.bind("pointerDown",
function(){S=[];ja=pa=0});$.bind("pointerMove",function(Y,da,ga,N){if(ia){S[S.length]={ticks:(new Date).getTime(),x:N.x,y:N.y};$.trigger("pan",N.x,N.y)}pa+=N.x?N.x:0;ja+=N.y?N.y:0});$.bind("pointerUp",function(Y,da,ga){if(Math.abs(pa)>20||Math.abs(ja)>20){if(ia){ga=S.length;var N,R;da=0;for(R=S.length;--R>1&&da<50;)da+=S[R].ticks-S[R-1].ticks;if(ga-R>1){Y=S[ga-1].x-S[R].x;N=S[ga-1].y-S[R].y;R=Math.sqrt(Y*Y+N*N)|0;if(R>15){Y=S[ga-1].x-S[0].x;N=S[ga-1].y-S[0].y;R=Math.sqrt(Y*Y+N*N)|0;ga=Math.asin(N/
R);da=R*500/da|0;da=da>300?300:da;Q(Math.cos(Y>0?ga:Math.PI-ga)*da,Math.sin(ga)*da,COG.easing("sine.out"),500)}}}}else $.trigger("tap",da,ga)});return ta};l("pointer",{handler:function(n,r,x){function Q(C){if(C.pageX&&C.pageY)return j(C.pageX,C.pageY);else{var K=document.documentElement,U=document.body;return j(C.clientX+(K&&K.scrollLeft||U&&U.scrollLeft||0)-(K&&K.clientLeft||U&&U.clientLeft||0),C.clientY+(K&&K.scrollTop||U&&U.scrollTop||0)-(K&&K.clientTop||U&&U.clientTop||0))}}function $(C){if(O(C,
n))if(Y=pa(C)){var K=Q(C);n.style.cursor="move";o(C);R=K.x;aa=K.y;da=j(R,aa);r.triggerCustom("pointerDown",z("mouse",C),da,ha(da,q(n)))}}function ia(C){var K=Q(C);ga=K.x;N=K.y;if(O(C,n))ja(C,Y?"pointerMove":"pointerHover")}function W(C){if(Y&&pa(C)){Y=false;if(O(C,n)){n.style.cursor="default";ja(C,"pointerUp")}}}function S(C){if(O(C,n)){var K;C=C||window.event;if(C.detail)K=C.axis===2?-C.detail*ta:0;else{K=C.wheelDeltaY?C.wheelDeltaY:C.wheelDelta;if(window.opera)K=-K}if(K!==0){var U=j(ga,N);r.triggerCustom("zoom",
z("mouse",C),U,ha(U,q(n)),K/Ca,"wheel");o(C);C.returnValue=false}}}function pa(C){C=C||window.event;return(C.which||C.button)==1}function ja(C,K,U,ma,Aa){U=typeof U!="undefined"?U:ga;ma=typeof ma!="undefined"?ma:N;var I=U-R,Z=ma-aa,M=j(U,ma);r.triggerCustom(K,z("mouse",C),M,ha(M,q(n)),j(I,Z));if(typeof Aa=="undefined"||Aa){R=U;aa=ma}}x=COG.extend({},x);var ta=120,Ca=ta*8,Y=false,da,ga,N,R,aa;x.binder("mousedown",$,false);x.binder("mousemove",ia,false);x.binder("mouseup",W,false);x.binder("dblclick",
function(C){COG.info("captured double click");if(O(C,n)){var K=Q(C);r.triggerCustom("doubleTap",z("mouse",C),K,ha(K,q(n)))}},false);x.binder("mousewheel",S,document);x.binder("DOMMouseScroll",S,document);return{unbind:function(){x.unbinder("mousedown",$,false);x.unbinder("mousemove",ia,false);x.unbinder("mouseup",W,false);x.unbinder("mousewheel",S,document);x.unbinder("DOMMouseScroll",S,document)}}},checks:{touch:false}});l("pointer",{handler:function(n,r,x){function Q(I){if(I.count<2)return 0;var Z=
I.x-I.next.x;I=I.y-I.next.y;return~~Math.sqrt(Z*Z+I*I)}function $(I,Z,M){Z=Z?Z:0;M=M?M:0;for(var ua={x:I.x-Z,y:I.y-M,id:I.id,count:I.count},sa=ua;I.next;){I=I.next;sa=sa.next={x:I.x-Z,y:I.y-M,id:I.id}}return ua}function ia(I,Z){var M=I[Z?Z:"touches"],ua,sa;if(M.length===0)return null;sa=ua={x:M[0].pageX,y:M[0].pageY,id:M[0].identifier,count:M.length};for(var ya=1,Xa=M.length;ya<Xa;ya++)sa=sa.next={x:M[ya].pageX,y:M[ya].pageY,id:M[ya].identifier};return ua}function W(I){if(O(I,n)){N=q(n);var Z=ia(I,
"changedTouches"),M=$(Z,N.left,N.top);if(!aa){R=Y;r.triggerCustom("pointerDown",z("touch",I),Z,M)}ma&&r.triggerCustom("pointerDownMulti",z("touch",I),Z,M);aa=ia(I);if(aa.count>1)K=Q(aa);Aa=1;U=$(aa)}}function S(I){if(O(I,n)){I.preventDefault();C=ia(I);if(R==Y)R=Math.abs(aa.x-C.x)>ta||Math.abs(aa.y-C.y)>ta?Ca:Y;if(R!=Y){R=C.count>1?ga:da;if(R==ga)if(aa.count===1){aa=$(C);K=Q(aa)}else{var Z=Q(C);if(Math.abs(K-Z)<ja)R=da;else{var M;M=C.x;var ua=C.next.x,sa=C.y,ya=C.next.y,Xa=sa<ya?sa:ya;M={x:(M<ua?M:
ua)+(Math.abs(M-ua)>>1),y:Xa+(Math.abs(sa-ya)>>1)};Z=Z/K;ua=Z-Aa;r.triggerCustom("zoom",z("touch",I),M,ha(M,N),ua,"pinch");Aa=Z}}R==da&&r.triggerCustom("pointerMove",z("touch",I),C,$(C,N.left,N.top),j(C.x-U.x,C.y-U.y));ma&&r.triggerCustom("pointerMoveMulti",z("touch",I),C,$(C,N.left,N.top))}U=$(C)}}function pa(I){if(O(I,n)){var Z=ia(I,"changedTouches"),M=$(Z,N.left,N.top);C=ia(I);if(!C){r.triggerCustom("pointerUp",z("touch",I),Z,M);aa=null}ma&&r.triggerCustom("pointerUpMulti",z("touch",I),Z,M)}}x=
COG.extend({detailed:false,inertia:false},x);var ja=20,ta=7,Ca=0,Y=1,da=2,ga=3,N,R,aa,C,K,U,ma=x.detailed,Aa=1;x.binder("touchstart",W,false);x.binder("touchmove",S,false);x.binder("touchend",pa,false);COG.info("initialized touch handler");return{unbind:function(){x.unbinder("touchstart",W,false);x.unbinder("touchmove",S,false);x.unbinder("touchend",pa,false)}}},checks:{touch:true}});return{register:l,watch:function(n,r,x){r=COG.extend({bindToBody:false,observable:null,isIE:typeof window.attachEvent!=
"undefined",types:null},r);capabilities=COG.extend({touch:"ontouchstart"in window},x);if(!r.observable){COG.info("creating observable");r.observable=COG.observable({});globalOpts=r}r.binder=r.isIE?u:G(r.bindToBody);r.unbinder=r.isIE?s:V(r.bindToBody);x=r.types;for(var Q=capabilities,$=[],ia=D.length;ia--;){var W=D[ia],S=!x||x.indexOf(W.type)>=0,pa=true;for(var ja in W.checks){var ta=W.checks[ja];COG.info("checking "+ja+" capability. require: "+ta+", capability = "+Q[ja]);pa=pa&&ta===Q[ja]}if(S&&pa)$[$.length]=
W.handler}return new y(n,$,r)}}}();var T5={};
...
g=b[d],i=d=="translate",h=a.autoInvalidate,m=f.length&&e.length&&f.length==e.length?f.length:0,t=Array(m),E=Array(m),X=COG.easing(a.easing),H=a.duration,F=a.progress,k=function(v){var B=v-c;v=c+H<=v;for(var L=b.layer?b.layer.view:null,J,T=m;T--;){J=X(B,f[T],t[T],H);E[T]=i?J|0:J}g.apply(b,E);h&&L&&L.invalidate();if(F){B=[].concat(v?e:E);B.unshift(v);F.apply(b,B)}if(v){b.animations--;g.apply(b,e);a.complete&&a.complete.apply(b,e)}else D(k)};if(g&&g.apply&&m>0){H=H?H:DEFAULT_DURATION;for(d=m;d--;)t[d]=
...
i||f.call(this,this.imageUrl);if(this.centerOffset){c=this.centerOffset.x;g=this.centerOffset.y}}function $(b){b=COG.extend({imageAnchor:null},b);if(b.imageAnchor)b.centerOffset=Oa.invert(b.imageAnchor);Q.call(this,b)}function ia(b){b=COG.extend({startAngle:0,endAngle:Math.PI*2,typeName:"Arc"},b);za.call(this,b)}function W(b){b=COG.extend({id:COG.objId("layer"),zindex:0,animated:false,validStates:ha("ACTIVE","ANIMATING","PAN","ZOOM"),style:null,minXY:null,maxXY:null},b);this.view=null;this.visible=
...
a.x?e.x:a.x,e.y>a.y?e.y:a.y)},offset:function(e,a,c){return f(e.x+a,e.y+(c||a))},scale:function(e,a){return f(e.x/a|0,e.y/a|0)},simplify:function(e,a){if(!e)return null;a=a?a:Oa.VECTOR_SIMPLIFICATION;for(var c=[],g=null,i=e.length;i--;){var h=e[i];if(include=!g||i===0||aa(h.x-g.x)+aa(h.y-g.y)>a){c.unshift(h);g=h}}return c},theta:function(e,a,c){c=ua((e.y-a.y)/c);return e.x>a.x?c:Math.PI-c},toString:function(e){return e?e.x+", "+e.y:""}}}(),gb=function(){return{dotProduct:function(b,d){return b.x*
...
d){zb[b]=d},bc=G.attachRenderer=function(b,d,f,e){b=b.split("/");for(var a=new ac(d,f,e),c=0;c<b.length;c++){var g=zb[b[c]];if(g)a=new g(d,f,e,a)}return a};Ua("canvas",function(b,d,f,e){function a(k,v,B,L){var J=false;if(v)J=h.isPointInPath(X?v.x-m:v.relXY.x,X?v.y-t:v.relXY.y);return{draw:L||F,viewport:k,state:B,hit:J,vpX:m,vpY:t,context:h}}f=COG.extend({},f);var c,g,i,h,m=0,t=0,E=null,X=CANI.canvas.pipTransformed,H={},F=function(){this.fill&&h.fill();this.stroke&&h.stroke()};(function(){if(d){var k=
d.tagName=="CANVAS",v=k?d.parentNode:d;c=b.width=v.offsetWidth;g=b.height=v.offsetHeight;if(k){i=d;i.width=c;i.height=g;bb&&FlashCanvas.initElement(i)}else{i=$b(c,g);i.style.cssText="position: absolute; z-index: 1;";d.appendChild(i)}h=null}})();return COG.extend(e,{interactTarget:i,applyStyle:function(k){var v=cc(k),B=h&&h.canvas?h.canvas.id:"default",L;H[B]||(H[B]=[]);L=H[B].pop()||"basic";if(v){H[B].push(k);v.applyToContext(h);return L}},applyTransform:function(k){if(k.translateX!==0||k.translateY!==
0||k.scaling!==1||k.rotation!==0){h.save();E={undo:function(){h.restore();E=null},x:k.xy.x,y:k.xy.y};h.translate(k.xy.x-m+k.translateX,k.xy.y-t+k.translateY);k.rotation!==0&&h.rotate(k.rotation);k.scaling!==1&&h.scale(k.scaling,k.scaling)}return E},drawTiles:function(k,v){for(var B,L,J=m-256,T=t-256,fa=k.x2,ra=k.y2,Ba=v.length;Ba--;){B=v[Ba];if(L=B.x>=J&&B.x<=fa&&B.y>=T&&B.y<=ra)B.loaded?h.drawImage(B.image,B.x-m,B.y-t):B.load(b.invalidate)}},hitTest:function(k,v,B){return h.isPointInPath(v,B)},prepare:function(k,
v){var B,L=false,J=v.scaled||v,T=v.scaleFactor;if(h)h.restore();else h=i.getContext("2d");for(B=k.length;B--;)L=L||k[B].clip;m=J.x;t=J.y;if(h){L||h.clearRect(0,0,c,g);h.save();h.scale(T,T)}h.globalCompositeOperation="source-over";return h},prepArc:function(k,v,B,L){h.beginPath();h.arc(k.xy.x-(E?E.x:m),k.xy.y-(E?E.y:t),k.size>>1,k.startAngle,k.endAngle,false);return a(v,B,L)},prepImage:function(k,v,B,L,J){var T=(J.x||k.xy.x)-(E?E.x:m),fa=(J.y||k.xy.y)-(E?E.y:t),ra=J.image||k.image;if(ra){h.beginPath();
h.rect(T,fa,J.width||ra.width,J.height||ra.height);return a(v,B,L,function(){h.drawImage(ra,T,fa,J.width||ra.width,J.height||ra.height)})}},prepMarker:function(k,v,B,L){var J=k.xy.x-(E?E.x:m),T=k.xy.y-(E?E.y:t),fa=k.size;h.beginPath();switch(k.markerStyle.toLowerCase()){case "simple":h.moveTo(J,T);h.lineTo(J-(fa>>1),T-fa);h.lineTo(J+(fa>>1),T-fa);h.lineTo(J,T);break;case "image":k.image?h.drawImage(k.image,J-(fa>>1),T-(fa>>1),fa,fa):q(k.imageUrl,function(ra){k.image=ra;h.drawImage(k.image,J-(fa>>
1),T-(fa>>1),fa,fa)});break}return a(v,B,L)},prepPoly:function(k,v,B,L,J){var T=true;k=J.points||k.points;J=E?E.x:m;var fa=E?E.y:t;h.beginPath();for(var ra=k.length;ra--;){var Ba=k[ra].x-J,Qa=k[ra].y-fa;if(T){h.moveTo(Ba,Qa);T=false}else h.lineTo(Ba,Qa)}return a(v,B,L)},getContext:function(){return h},getDimensions:function(){return{width:c,height:g}},getOffset:function(){return new s(m,t)}})});var Ab={},dc=G.defineStyle=function(b,d){Ab[b]=O(d);return b},Bb=G.defineStyles=function(b){for(var d in b)dc(d,
b[d])},cc=G.getStyle=function(b){return Ab[b]};G.loadStyles=function(b){COG.jsonp(b,function(d){Bb(d)})};Bb({basic:{fillStyle:"#000"},highlight:{fillStyle:"#f00"},waypoints:{lineWidth:4,strokeStyle:"rgba(0, 51, 119, 0.9)",fillStyle:"#FFF"},waypointsHover:{lineWidth:4,strokeStyle:"#f00",fillStyle:"#FFF"}});var Xb={NONE:0,ACTIVE:1,ANIMATING:4,PAN:8,ZOOM:16,FREEZE:128},Kb=function(b){function d(p,w,A){Ea||Qa(ka-w,la-A)}function f(p,w,A,P){fa(K(U(na+ma(2,P)-1,0.5),2))}function e(){var p=I(na)/Math.LN2|
0;if(p!==0){na=ma(2,p);ra(Za+p,ec,fc)}redraw=true}function a(p,w){var A=renderer&&renderer.projectXY?renderer.projectXY(p,w):null;if(!A){A=ba.getViewport();var P=1/na;A=new s(A?A.x+p*P:p,A?A.y+w*P:w)}return A}function c(p,w,A){Ba("doubleTap",w,A,a(A.x,A.y));b.scalable&&fa(2,a(A.x,A.y),b.zoomEasing,null,b.zoomDuration)}function g(p,w,A){Ea=null;B("down",w,A)}function i(p,w,A){B("hover",w,A)}function h(p,w,A){k(w,A,false)}function m(p,w,A){k(w,A,true)}function t(){clearTimeout(Cb);Cb=setTimeout(function(){renderer.checkSize()},
250)}function E(p,w,A){B("tap",w,A);Ba("tap",w,A,a(A.x,A.y,true))}function X(){renderer=bc(b.renderer,ba,ib);Db=!renderer.preventPartialScale;F()}function H(p,w){w.id=p;w.added=V();w.view=ba;w.trigger("parentChange",ba,ib,gc);oa.push(w);oa.sort(function(A,P){var ca=P.zindex-A.zindex;if(ca===0)ca=P.added-A.added;return ca});Ga=oa.length;return w}function F(){wa&&wa.unbind();if(renderer){wa=INTERACT.watch(renderer.interactTarget);b.pannable&&wa.pannable().bind("pan",d);if(b.scalable){wa.bind("zoom",
f);wa.bind("doubleTap",c)}wa.bind("pointerDown",g);wa.bind("pointerMove",h);wa.bind("pointerUp",m);b.captureHover&&wa.bind("pointerHover",i);wa.bind("tap",E)}}function k(p,w,A){if(Ea){p=a(w.x,w.y);if(Ea.drag.call(Ea.target,Ea,p.x,p.y,A))redraw=true;if(A)Ea=null}}function v(p){var w,A,P=false;p=p||(new Date).getTime();if(P=p-Eb>hc){var ca=aa(ka-jb),va=aa(la-kb);w=ka!==lb||la!==mb;A=na!==Fb;if(w||A)Ka++;if(ca>=Gb||va>=Gb)T();A={index:ic++,draw:Ka};ba.trigger("enterFrame",p,A);Eb=p}if(P&&A.draw){La=
Hb|(na!==1?Ib:0)|(w?nb:0)|(ob?Jb:0);pb=(w=(La&(Ib|nb))!==0)&&(La&Jb)===0;w=J();if(renderer.prepare(oa,w,La,p,xa)){Ka=0;for(ii=Ga;ii--;){P=oa[ii];if(P.visible&&(La&P.validStates)!==0){A=P.style?renderer.applyStyle(P.style,true):null;P.draw(renderer,w,La,ba,p,xa);A&&renderer.applyStyle(A)}}renderer.render(w);ba.trigger("drawComplete",w,p);lb=ka;mb=la;Fb=na}if(xa){p=xa?xa.elements:[];if($a&&$a.type==="hover"){w=Hits.diffHits($a.elements,p);w.length>0&&Hits.triggerEvent($a,ba,"Out",w)}if(p.length>0){P=
xa.x;A=xa.y;for(w=p.length;w--;){ca=p[w];va=P;var Ha=A,Fa=ca&&ca.drag&&(!ca.canDrag||ca.canDrag(ca,va,Ha));if(Fa){Ea=ca;Ea.startX=va;Ea.startY=Ha}if(Fa)break}Hits.triggerEvent(xa,ba)}$a=p.length>0?COG.extend({},xa):null;xa=null}}animFrame(v)}function B(p,w,A){xa=Hits.init(p,w,A,a(A.x,A.y,true));for(p=Ga;p--;)qb=qb||(oa[p].hitGuess?oa[p].hitGuess(xa.x,xa.y,La,ba):false);qb&&Ka++}function L(){Ka++}function J(){var p=new j(ka,la,ba.width,ba.height);p.scaleFactor=na;if(na!==1)p.scaled=Ta.fromCenter(ka+
(ba.width>>1),la+(ba.height>>1),ba.width/na|0,ba.height/na|0);return p}function T(){var p=J();if(p){if(Va||Wa)if(p){var w=db?ka+(p.w>>1):ka,A=eb?la+(p.h>>1):la,P=p.w,ca=p.h;if(Va&&Va>P)if(w+P>Va)ka=db?ka:Va-P;else if(w<0)ka=db?ka:0;if(Wa&&Wa>ca)if(A+ca>Wa)la=eb?la:Wa-ca;else if(A<0)la=eb?la:0}jb=ka;kb=la;Ba("refresh",ba,p);Ka++}}function fa(p,w,A,P,ca){var va;if(!Db){A=false;va=Z(I(p)/Math.LN2);p=ma(2,va)}if(A)COG.tweenValue(na,p,A,ca,function(Ha,Fa){na=Ha;if(Fa){va=Z(I(na)/Math.LN2);na=ma(2,va);
P&&P()}e()});else{na=p;e()}return ba}function ra(p,w,A){p=U(b.minZoom,K(b.maxZoom,p));if(p!==Za){var P=ma(2,p-Za),ca=ba.width>>1,va=ba.height>>1,Ha=ca/P|0,Fa=va/P|0;Za=p;Qa(((w?w:ka+ca)-Ha)*P,((A?A:la+va)-Fa)*P);lb=ka;mb=la;kb=jb=0;Ba("zoomLevelChange",p);na=1;renderer.reset();T()}}function Ba(){for(var p=ba.trigger.apply(null,arguments).cancel,w=oa.length;w--;)p=oa[w].trigger.apply(null,arguments).cancel||p;return!p}function Qa(p,w,A,P,ca){function va(){Ha+=1;if(Ha>=2){ob=false;ca&&ca()}}var Ha=
0;if(A){if((La&nb)===0){COG.tweenValue(ka,p,A,P,function(Fa,rb){ka=Fa|0;(rb?va:L)();return!pb});COG.tweenValue(la,w,A,P,function(Fa,rb){la=Fa|0;(rb?va:L)();return!pb});ob=true}}else{ka=p|0;la=w|0;ca&&ca()}}b=COG.extend({id:COG.objId("view"),container:"",captureHover:true,captureDrag:false,inertia:true,refreshDistance:256,pannable:false,clipping:true,scalable:false,panAnimationEasing:COG.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,autoSize:true,tapExtent:10,guides:false,
turbo:false,fps:25,minZoom:1,maxZoom:1,renderer:"canvas",zoomEasing:COG.easing("quad.out"),zoomDuration:300,zoomLevel:1},b);var jc={},oa=[],Ga=0,ib=document.getElementById(b.container),Ea=null,ic=0,gc=null,kc=!u(window.attachEvent,qa),qb=false,Gb=b.refreshDistance,ka=0,la=0,jb=0,kb=0,lb=0,mb=0,Va=null,Wa=null,db=false,eb=false,xa=null,pb=false,$a=null,Cb=0,na=1,Fb=1,Eb=0,wa=null,Db=true,ob=false,hc=1E3/b.fps|0,Ka=0,ec,fc,Za=b.zoomLevel,Hb=ha("ACTIVE"),nb=ha("PAN"),Ib=ha("ZOOM"),Jb=ha("ANIMATING"),
La=Hb,ba={id:b.id,detach:function(){wa&&wa.unbind()},eachLayer:function(p){for(var w=Ga;w--;)p(oa[w])},getLayer:function(p){for(var w=0;w<Ga;w++)if(oa[w].id===p)return oa[w];return null},getZoomLevel:function(){return Za},setLayer:function(p,w){for(var A=0;A<Ga;A++)if(oa[A].id===p){oa.splice(A,1);break}if(w){H(p,w);w.trigger("refresh",ba,J())}Ka++;return w},invalidate:L,refresh:T,resetScale:function(){na=1},scale:fa,setZoomLevel:ra,syncXY:function(){},triggerAll:Ba,removeLayer:function(p){a:{for(var w=
Ga;w--;)if(oa[w].id===p){p=w;break a}p=-1}if(p>=0&&p<Ga){ba.trigger("layerRemoved",oa[p]);oa.splice(p,1);Ka++}Ga=oa.length},getOffset:function(){return new s(ka,la)},getRenderer:function(){return renderer},getScaleFactor:function(){return na},setMaxOffset:function(p,w,A,P){Va=p;Wa=w;db=A;eb=P},getViewport:J,updateOffset:Qa,pan:function(p,w,A,P,ca){Qa(ka+p,la+w,A,P,ca)}};COG.observable(ba);ba.bind("resync",function(){});COG.configurable(ba,["container","captureHover","captureDrag","scalable","pannable",
"inertia","minZoom","maxZoom","zoom"],COG.paramTweaker(b,null,{container:function(p,w){ib=document.getElementById(w);X()},inertia:F,captureHover:F,scalable:F,pannable:F}),true);CANI.init(function(p){ba.markers=H("markers",new fb({zindex:20}));jc=p;X();kc?window.attachEvent("onresize",t):window.addEventListener("resize",t,false)});animFrame(v);return ba},za=function(b){b=COG.extend({style:null,xy:null,size:10,fill:false,stroke:true,draggable:false,observable:true,properties:{},typeName:"Shape"},b);
COG.extend(this,b);this.id=COG.objId(this.typeName);this.view=this.bounds=null;this.rotation=this.animations=0;this.scaling=1;this.translateY=this.translateX=0;this.observable&&COG.observable(this)};za.prototype={constructor:za,animate:function(b,d,f,e){y(this,b,d,f,e)},drag:null,draw:null,getProps:null,resync:function(b){if(this.xy){b.syncXY([this.xy]);this.size&&this.updateBounds(Ta.fromCenter(this.xy.x,this.xy.y,this.size,this.size))}},rotate:function(b){this.rotation=b},scale:function(b){this.scaling=
b},translate:function(b,d){this.translateX=b;this.translateY=d},updateBounds:function(b,d){b&&(!this.bounds||b.x!=this.bounds.x||b.y!=this.bounds.y)&&this.trigger("move",this,b,this.bounds);this.bounds=b;if(d)this.xy=Ta.center(this.bounds)}};var Yb=1E3/60|0,Ya=[],vb=0;n.prototype=COG.extend(za.prototype,{constructor:n});r.prototype=COG.extend({},za.prototype,{constructor:r});x.prototype=COG.extend({},r.prototype);Q.prototype=COG.extend({},za.prototype,{constructor:Q});$.prototype=COG.extend({},Q.prototype,
{constructor:$});ia.prototype=COG.extend(za.prototype,{constructor:ia});W.prototype={constructor:W,clip:null,cycle:function(){},draw:function(){},hitGuess:null};Ua=function(b,d){function f(m,t){if(a&&i){g=a.search(i);i=null}var E=t&&t.getZoomLevel?t.getZoomLevel():0;c[E]||(c[E]=new hb);a=c[E]}d=COG.extend({imageLoadArgs:{}},d);var e=b?yb.init(b,d).run:null,a=null,c=[],g=[],i=null,h=COG.extend(new W(d),{draw:function(m,t){m.drawTiles(t,a.search(t))}});h.bind("refresh",function(m,t,E){var X=(new Date).getTime();
if(a){g=i?a.search(i):[];e(t,E,a,function(){t.invalidate();COG.info("GEN COMPLETED IN "+((new Date).getTime()-X)+" ms")});i=Ta.copy(E)}});h.bind("parentChange",f);h.bind("resync",f);return h};var Lb=function(b){function d(h){clearTimeout(g);g=setTimeout(function(){a.sort(function(m,t){if(t.xy&&m.xy){var E=t.xy.y-m.xy.y;return E!=0?E:t.xy.x-m.xy.x}});h&&h.invalidate()},50)}function f(h,m,t,E){E&&c.remove(E,m);c.insert(t,m)}function e(h,m){for(var t=a.length;t--;)a[t].resync(m)}b=COG.extend({zindex:10},
b);var a=[],c=new hb,g=0,i=COG.extend(new W(b),{itemCount:0,add:function(h,m){if(h){h.layer=i;if(m)a.unshift(h);else a[a.length]=h;var t=i.view;if(t){h.resync(t);c&&h.bounds&&c.insert(h.bounds,h);d(t)}h.bind("move",f)}i.itemCount=a.length},clear:function(){c=new hb;a=[];i.itemCount=0},draw:function(h,m,t,E,X,H){E={};X=c&&m?c.search(m):[];for(var F=X.length;F--;){var k=X[F],v=k.style||i.style,B,L=h.applyTransform(k),J=k.getProps?k.getProps(h,t):E;if(J=(B=h["prep"+k.typeName])?B.call(h,k,m,H,t,J):null){if(J.hit){H.elements.push(Hits.initHit(k.type,
k,k.draggable?k.drag:null));B=H.type+"Style";v=k[B]||i[B]||v}v=v?h.applyStyle(v,true):null;(B=k.draw||J.draw)&&B.call(k,J);v&&h.applyStyle(v)}L&&L.undo&&L.undo()}},find:function(){return[].concat(a)},hitGuess:function(h,m){return c&&c.search({x:h-10,y:m-10,w:20,h:20}).length>0}});i.bind("parentChange",e);i.bind("resync",e);return i},fb=function(b){return new Lb(b)};COG.extend(G,{ex:COG.extend,is:u,ticks:V,userMessage:function(b,d,f){G.trigger("userMessage",b,d,f)},indexOf:ga,Rect:j,XY:Oa,XYRect:Ta,
Vector:gb,Hits:Hits,Generator:yb,tweenValue:COG.tweenValue,easing:COG.easing,Tile:z,TileLayer:Ua,getImage:q,viewState:ha,View:Kb,ViewLayer:W,ImageLayer:Ua,Drawable:za,Marker:n,Poly:r,Line:x,Arc:ia,ImageDrawable:Q,ImageMarker:$,DrawLayer:Lb,ShapeLayer:fb});COG.observable(G);var sb=[1.406245461070741,1.321415085624082,1.077179995861952,0.703119412486786,0.488332580888611],Da=Math.PI/180,Ma=180/Math.PI,tb=R,Mb=-tb,Nb=N,lc=-Nb,cb=0.08181919084262157,ab=null,Ob={RD:"ROAD",ST:"STREET",CR:"CRESCENT",CRES:"CRESCENT",
CT:"COURT",LN:"LANE",HWY:"HIGHWAY",MWY:"MOTORWAY"},ea=function(){function b(a){return a?f(a.lat,a.lon):null}function d(a){return!a||a.lat===0&&a.lon===0}function f(a,c){return{lat:parseFloat(a?a:0),lon:parseFloat(c?c:0)}}function e(a){if(a)if(u(a.lat,xb))return b(a);else{if(a.split)for(var c=[" ",","],g=0;g<c.length;g++){var i=a.split(c[g]);if(i.length===2)return f(i[0],i[1])}}else return f();return null}return{calcDistance:function(a,c){if(d(a)||d(c))return 0;var g=(c.lat-a.lat)*Da>>1,i=(c.lon-a.lon)*
Da>>1;g=M(g)*M(g)+sa(a.lat*Da)*sa(c.lat*Da)*M(i)*M(i);return 6371*2*Xa(Aa(g),Aa(1-g))},copy:b,empty:d,equal:function(a,c){return a&&c&&a.lat==c.lat&&a.lon==c.lon},fromMercatorPixels:function(a,c){for(var g=ma(Math.E,-c),i=R-2*ya(g),h=Ca(i,g),m=0;m<12&&aa(i-h)>1.0E-7;){i=h;h=Ca(i,g);m++}return f(h*Ma,a%360*Ma)},generalize:function(a,c,g){var i=a.length,h=[],m=null;g||(g=250);g/=1E3;COG.info("generalizing positions, must include "+c.length+" positions");for(var t=i;t--;)if(t===0)h.unshift(a[t]);else{var E=
...
a[h]);if(c){if(c=="auto"){h=b(g.min,g.max);c=U(h.x,h.y)*0.3}g=d(g,c)}COG.trace("calculated bounds for "+a.length+" positions",i);return g},getCenter:f,getGeoHash:function(a){var c=T5.Geo.GeoHash.encode(a.min.lat,a.min.lon);a=T5.Geo.GeoHash.encode(a.max.lat,a.max.lon);COG.info("min hash = "+c+", max hash = "+a)},getZoomLevel:function(a,c){var g=f(a);g=K(Z(aa(g.lat)*0.05),sb.length);var i=sb[g],h=b(a.min,a.max);g=C(I(sb[3]*c.h/h.y)/I(2));i=C(I(i*c.w/h.x)/I(2));return K(isNaN(g)?1E3:g,isNaN(i)?1E3:i)},
...
f,e.poly);return 0}}),multipolygon:COG.extend({},Ra,{processor:function(b,d,f,e){d=d&&d.coordinates?d.coordinates:[];for(var a=0,c=0;c<d.length;c++)a+=Y(b,d[c][0],f,e.poly);return a}})},Qb=function(b,d,f,e){function a(k,v){var B=k.processor,L=m+k.group,J=COG.extend({},k,f,{properties:v.properties});if(B){var T=F[L];if(!T){T=new k.layerClass({id:L});F[L]=T}return B(T,v.data,J,e)}return 0}function c(k,v){var B=k&&k.type?k.type.toLowerCase():null;return B&&B===oc?c(k.geometry,k.properties):{type:B,isCollection:B?
B===nc:false,definition:pc[B],data:k,properties:v?v:k.properties}}function g(k){var v=0,B=COG.extend({},f),L=t;k=k?k:(new Date).getTime();if(!X){for(;L<E;L++){k=c(h?h(b[L]):b[L]);var J=null;if(k.isCollection){B.layerPrefix=m+H++ +"-";X=new Qb(k.data.features,function(T){X=null;for(var fa in T)F[fa]=T[fa];t>=E&&d&&d(F)},B);J+=1}else if(k.definition)J=a(k.definition,k);v+=J?J:1;if(v>=i)break}t=L+1;if(X||t<E)animFrame(g);else d&&d(F)}}f=COG.extend({rowPreParse:null,simplify:false,layerPrefix:"geojson-"},
f);e=COG.extend({marker:function(k){return new n({xy:k})},line:function(k,v){return new r(k,COG.extend({},f,v))},poly:function(k,v){return new r(k,COG.extend({fill:true},f,v))}},e);var i=500,h=f.rowPreParse,m=f.layerPrefix,t=0,E=0,X=null,H=0,F={};if(b){u(b,"array")||(b=[b]);E=b.length;animFrame(g)}};Ra=G.GeoJSON={parse:function(b,d,f){return new Qb(b,d,f)}};G.Map=function(b){function d(F){try{var k=ea.init(F.coords.latitude,F.coords.longitude),v=F.coords.accuracy/1E3;H.trigger("locationUpdate",F,
v);if(t){if(!h){h=new UI.LocationOverlay({pos:k,accuracy:v});h.update(H.getTileLayer());H.setLayer("location",h)}var B=Ia.createBoundsFromCenter(k,Math.max(v,1));H.gotoBounds(B)}else{h.pos=k;h.accuracy=v;h.update(H.getTileLayer());a(k,null,COG.easing("sine.out"))}t=false}catch(L){COG.exception(L)}}function f(F){COG.info("caught location tracking error:",F)}function e(F,k,v){H.setZoomLevel(k);a(F,v)}function a(F,k,v,B){F=Ja.init(F,ja(H.getZoomLevel()));H.updateOffset(F.x-(H.width>>1),F.y-(H.height>>
1),v,B,function(){k&&k(H)})}b=COG.extend({tapExtent:10,crosshair:false,zoomLevel:0,minZoom:1,maxZoom:18,pannable:true,scalable:true},b);var c={NONE:0,SINGLE:1,WATCH:2},g=new s,i=c.NONE,h=null,m=0,t=true,E=0,X=b.tapExtent;b.adjustScaleFactor=function(F){return Math.pow(2,(F<1?Math.floor:Math.ceil)(Math.log(F)))};var H=COG.extend(new Kb(b),{getBoundingBox:function(){var F=H.getViewport();return F?Ia.init(Ja.toPos(new s(F.x,F.y2),E),Ja.toPos(new s(F.x2,F.y),E)):null},getCenterPosition:function(){var F=
H.getViewport();if(F){F=new s(F.x+(F.w>>1),F.y+(F.h>>1));return Ja.toPos(F,E)}return null},gotoBounds:function(F,k){var v=Ia.getZoomLevel(F,H.getViewport());e(Ia.getCenter(F),v,k)},gotoPosition:e,panToPosition:a,syncXY:function(F,k){return(k?Ja.syncPos:Ja.sync)(F,E)},locate:function(){H.trackStart(c.SINGLE);setTimeout(H.trackCancel,1E4)},trackStart:function(F){if(navigator.geolocation&&!m){i=F?F:c.WATCH;t=true;m=navigator.geolocation.watchPosition(d,f,{enableHighAccuracy:true,timeout:1E4,maximumAge:5E3})}},
trackCancel:function(){m&&navigator.geolocation&&navigator.geolocation.clearWatch(m);H.removeLayer("location");h=null;i=c.NONE;m=0},animateRoute:function(F,k,v,B){var L=H.getLayer("route");if(L)(F=L.getAnimation(F,k,v,B))&&F.addToView(H)}});H.bind("pan",function(){i===c.SINGLE&&H.trackCancel()});H.bind("tap",function(F,k,v,B){F=Ja.toPos(B,E);var L=Ja.toPos(Oa.offset(B,-X,X),E);B=Ja.toPos(Oa.offset(B,X,-X),E);H.trigger("geotap",k,v,F,Ia.init(L,B))});H.bind("refresh",function(F,k,v){if(g.x!=v.x||g.y!=
v.y){H.trigger("boundsChange",H.getBoundingBox());g.x=v.x;g.y=v.y}});H.bind("zoomLevelChange",function(F,k){var v;E=ja(k);v=N/E|0;H.setMaxOffset(v,v,true,false);H.resetScale();H.triggerAll("resync",H)});return H};G.GeoShape=function(b,d){d=COG.extend({autoParse:true},d);for(var f=Array(b.length),e=d.autoParse,a=T5.Geo.Position.parse,c=b.length;c--;)f[c]=T5.GeoXY.init(e?a(b[c]):b[c]);return new T5.Poly(f,d)};G.LocationOverlay=function(b){b=COG.extend({pos:null,accuracy:null,zindex:90},b);var d=new Image,
