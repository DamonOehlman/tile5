...
"changedTouches"),M=$(Z,N.left,N.top);if(!ba){R=Y;r.triggerCustom("pointerDown",z("touch",I),Z,M)}ma&&r.triggerCustom("pointerDownMulti",z("touch",I),Z,M);ba=ia(I);if(ba.count>1)K=Q(ba);Aa=1;U=$(ba)}}function S(I){if(O(I,n)){I.preventDefault();C=ia(I);if(R==Y)R=Math.abs(ba.x-C.x)>ta||Math.abs(ba.y-C.y)>ta?Ca:Y;if(R!=Y){R=C.count>1?ga:da;if(R==ga)if(ba.count===1){ba=$(C);K=Q(ba)}else{var Z=Q(C);if(Math.abs(K-Z)<ja)R=da;else{var M;M=C.x;var ua=C.next.x,sa=C.y,ya=C.next.y,Xa=sa<ya?sa:ya;M={x:(M<ua?M:
ua)+(Math.abs(M-ua)>>1),y:Xa+(Math.abs(sa-ya)>>1)};Z=Z/K;ua=Z-Aa;r.triggerCustom("zoom",z("touch",I),M,ha(M,N),ua,"pinch");Aa=Z}}R==da&&r.triggerCustom("pointerMove",z("touch",I),C,$(C,N.left,N.top),j(C.x-U.x,C.y-U.y));ma&&r.triggerCustom("pointerMoveMulti",z("touch",I),C,$(C,N.left,N.top))}U=$(C)}}function pa(I){if(O(I,n)){var Z=ia(I,"changedTouches"),M=$(Z,N.left,N.top);C=ia(I);if(!C){r.triggerCustom("pointerUp",z("touch",I),Z,M);ba=null}ma&&r.triggerCustom("pointerUpMulti",z("touch",I),Z,M)}}x=
...
0;if(p!==0){na=ma(2,p);ra(Za+p,ec,fc)}redraw=true}function a(p,v){var A=renderer&&renderer.projectXY?renderer.projectXY(p,v):null;if(!A){A=aa.getViewport();var P=1/na;A=new s(A?A.x+p*P:p,A?A.y+v*P:v)}return A}function c(p,v,A){Ba("doubleTap",v,A,a(A.x,A.y));b.scalable&&fa(2,a(A.x,A.y),b.zoomEasing,null,b.zoomDuration)}function g(p,v,A){Ea=null;B("down",v,A)}function i(p,v,A){B("hover",v,A)}function h(p,v,A){k(v,A,false)}function m(p,v,A){k(v,A,true)}function t(){clearTimeout(Cb);Cb=setTimeout(function(){renderer.checkSize()},
250)}function E(p,v,A){B("tap",v,A);Ba("tap",v,A,a(A.x,A.y,true))}function X(){renderer=bc(b.renderer,aa,ib);Db=!renderer.preventPartialScale;F()}function H(p,v){v.id=p;v.added=V();v.view=aa;v.trigger("parentChange",aa,ib,gc);oa.push(v);oa.sort(function(A,P){var ca=P.zindex-A.zindex;if(ca===0)ca=P.added-A.added;return ca});Ga=oa.length;return v}function F(){wa&&wa.unbind();if(renderer){wa=INTERACT.watch(renderer.interactTarget);b.pannable&&wa.pannable().bind("pan",d);if(b.scalable){wa.bind("zoom",
f);wa.bind("doubleTap",c)}wa.bind("pointerDown",g);wa.bind("pointerMove",h);wa.bind("pointerUp",m);b.captureHover&&wa.bind("pointerHover",i);wa.bind("tap",E)}}function k(p,v,A){if(Ea){p=a(v.x,v.y);if(Ea.drag.call(Ea.target,Ea,p.x,p.y,A))redraw=true;if(A)Ea=null}}function w(p){var v,A,P=false;p=p||(new Date).getTime();if(P=p-Eb>hc){var ca=ba(ka-jb),va=ba(la-kb);v=ka!==lb||la!==mb;A=na!==Fb;if(v||A)Ka++;if(ca>=Gb||va>=Gb)T();A={index:ic++,draw:Ka};aa.trigger("enterFrame",p,A);Eb=p}if(P&&A.draw){La=
Hb|(na!==1?Ib:0)|(v?nb:0)|(ob?Jb:0);pb=(v=(La&(Ib|nb))!==0)&&(La&Jb)===0;v=J();if(renderer.prepare(oa,v,La,p,xa)){Ka=0;for(ii=Ga;ii--;){P=oa[ii];if(P.visible&&(La&P.validStates)!==0){A=P.style?renderer.applyStyle(P.style,true):null;P.draw(renderer,v,La,aa,p,xa);A&&renderer.applyStyle(A)}}renderer.render(v);aa.trigger("drawComplete",v,p);lb=ka;mb=la;Fb=na}if(xa){p=xa?xa.elements:[];if($a&&$a.type==="hover"){v=Hits.diffHits($a.elements,p);v.length>0&&Hits.triggerEvent($a,aa,"Out",v)}if(p.length>0){P=
xa.x;A=xa.y;for(v=p.length;v--;){ca=p[v];va=P;var Ha=A,Fa=ca&&ca.drag&&(!ca.canDrag||ca.canDrag(ca,va,Ha));if(Fa){Ea=ca;Ea.startX=va;Ea.startY=Ha}if(Fa)break}Hits.triggerEvent(xa,aa)}$a=p.length>0?COG.extend({},xa):null;xa=null}}animFrame(w)}function B(p,v,A){xa=Hits.init(p,v,A,a(A.x,A.y,true));for(p=Ga;p--;)qb=qb||(oa[p].hitGuess?oa[p].hitGuess(xa.x,xa.y,La,aa):false);qb&&Ka++}function L(){Ka++}function J(){var p=new j(ka,la,aa.width,aa.height);p.scaleFactor=na;if(na!==1)p.scaled=Ta.fromCenter(ka+
(aa.width>>1),la+(aa.height>>1),aa.width/na|0,aa.height/na|0);return p}function T(){var p=J();if(p){if(Va||Wa)if(p){var v=db?ka+(p.w>>1):ka,A=eb?la+(p.h>>1):la,P=p.w,ca=p.h;if(Va&&Va>P)if(v+P>Va)ka=db?ka:Va-P;else if(v<0)ka=db?ka:0;if(Wa&&Wa>ca)if(A+ca>Wa)la=eb?la:Wa-ca;else if(A<0)la=eb?la:0}jb=ka;kb=la;Ba("refresh",aa,p);Ka++}}function fa(p,v,A,P,ca){var va;if(!Db){A=false;va=Z(I(p)/Math.LN2);p=ma(2,va)}if(A)COG.tweenValue(na,p,A,ca,function(Ha,Fa){na=Ha;if(Fa){va=Z(I(na)/Math.LN2);na=ma(2,va);
P&&P()}e()});else{na=p;e()}return aa}function ra(p,v,A){p=U(b.minZoom,K(b.maxZoom,p));if(p!==Za){var P=ma(2,p-Za),ca=aa.width>>1,va=aa.height>>1,Ha=ca/P|0,Fa=va/P|0;Za=p;Qa(((v?v:ka+ca)-Ha)*P,((A?A:la+va)-Fa)*P);lb=ka;mb=la;kb=jb=0;Ba("zoomLevelChange",p);na=1;renderer.reset();T()}}function Ba(){for(var p=aa.trigger.apply(null,arguments).cancel,v=oa.length;v--;)p=oa[v].trigger.apply(null,arguments).cancel||p;return!p}function Qa(p,v,A,P,ca){function va(){Ha+=1;if(Ha>=2){ob=false;ca&&ca()}}var Ha=
0;if(A){if((La&nb)===0){COG.tweenValue(ka,p,A,P,function(Fa,rb){ka=Fa|0;(rb?va:L)();return!pb});COG.tweenValue(la,v,A,P,function(Fa,rb){la=Fa|0;(rb?va:L)();return!pb});ob=true}}else{ka=p|0;la=v|0;ca&&ca()}}b=COG.extend({id:COG.objId("view"),container:"",captureHover:true,captureDrag:false,inertia:true,refreshDistance:256,pannable:false,clipping:true,scalable:false,panAnimationEasing:COG.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,autoSize:true,tapExtent:10,guides:false,
...
La=Hb,aa={id:b.id,detach:function(){wa&&wa.unbind()},eachLayer:function(p){for(var v=Ga;v--;)p(oa[v])},getLayer:function(p){for(var v=0;v<Ga;v++)if(oa[v].id===p)return oa[v];return null},getZoomLevel:function(){return Za},setLayer:function(p,v){for(var A=0;A<Ga;A++)if(oa[A].id===p){oa.splice(A,1);break}if(v){H(p,v);v.trigger("refresh",aa,J());aa.trigger("layerChange",aa,v)}Ka++;return v},invalidate:L,refresh:T,resetScale:function(){na=1},scale:fa,setZoomLevel:ra,syncXY:function(){},triggerAll:Ba,
removeLayer:function(p){a:{for(var v=Ga;v--;)if(oa[v].id===p){p=v;break a}p=-1}if(p>=0&&p<Ga){aa.trigger("layerRemoved",oa[p]);oa.splice(p,1);Ka++}Ga=oa.length},getOffset:function(){return new s(ka,la)},getRenderer:function(){return renderer},getScaleFactor:function(){return na},setMaxOffset:function(p,v,A,P){Va=p;Wa=v;db=A;eb=P},getViewport:J,updateOffset:Qa,pan:function(p,v,A,P,ca){Qa(ka+p,la+v,A,P,ca)}};COG.observable(aa);aa.bind("resync",function(){});COG.configurable(aa,["container","captureHover",
"captureDrag","scalable","pannable","inertia","minZoom","maxZoom","zoom"],COG.paramTweaker(b,null,{container:function(p,v){ib=document.getElementById(v);X()},inertia:F,captureHover:F,scalable:F,pannable:F}),true);CANI.init(function(p){aa.markers=H("markers",new fb({zindex:20}));jc=p;X();kc?window.attachEvent("onresize",t):window.addEventListener("resize",t,false)});animFrame(w);return aa},za=function(b){b=COG.extend({style:null,xy:null,size:10,fill:false,stroke:true,draggable:false,observable:true,
...
b},scale:function(b){this.scaling=b},translate:function(b,d){this.translateX=b;this.translateY=d},updateBounds:function(b,d){b&&(!this.bounds||b.x!=this.bounds.x||b.y!=this.bounds.y)&&this.trigger("move",this,b,this.bounds);this.bounds=b;if(d)this.xy=Ta.center(this.bounds)}};var Yb=1E3/60|0,Ya=[],vb=0;n.prototype=COG.extend(za.prototype,{constructor:n});r.prototype=COG.extend({},za.prototype,{constructor:r});x.prototype=COG.extend({},r.prototype);Q.prototype=COG.extend({},za.prototype,{constructor:Q});
$.prototype=COG.extend({},Q.prototype,{constructor:$});ia.prototype=COG.extend(za.prototype,{constructor:ia});W.prototype={constructor:W,clip:null,cycle:function(){},draw:function(){},hitGuess:null};Ua=function(b,d){function f(m,t){if(a&&i){g=a.search(i);i=null}var E=t&&t.getZoomLevel?t.getZoomLevel():0;c[E]||(c[E]=new hb);a=c[E]}d=COG.extend({imageLoadArgs:{}},d);var e=b?yb.init(b,d).run:null,a=null,c=[],g=[],i=null,h=COG.extend(new W(d),{draw:function(m,t){m.drawTiles(t,a.search(t))}});h.bind("refresh",
function(m,t,E){var X=(new Date).getTime();if(a){g=i?a.search(i):[];e(t,E,a,function(){t.invalidate();COG.info("GEN COMPLETED IN "+((new Date).getTime()-X)+" ms")});i=Ta.copy(E)}});h.bind("parentChange",f);h.bind("resync",f);return h};var Lb=function(b){function d(h){clearTimeout(g);g=setTimeout(function(){a.sort(function(m,t){if(t.xy&&m.xy){var E=t.xy.y-m.xy.y;return E!=0?E:t.xy.x-m.xy.x}});h&&h.invalidate()},50)}function f(h,m,t,E){E&&c.remove(E,m);c.insert(t,m)}function e(h,m){for(var t=a.length;t--;)a[t].resync(m)}
b=COG.extend({zindex:10},b);var a=[],c=new hb,g=0,i=COG.extend(new W(b),{itemCount:0,add:function(h,m){if(h){h.layer=i;if(m)a.unshift(h);else a[a.length]=h;var t=i.view;if(t){h.resync(t);c&&h.bounds&&c.insert(h.bounds,h);d(t)}h.bind("move",f)}i.itemCount=a.length},clear:function(){c=new hb;a=[];i.itemCount=0},draw:function(h,m,t,E,X,H){E={};X=c&&m?c.search(m):[];for(var F=X.length;F--;){var k=X[F],w=k.style||i.style,B,L=h.applyTransform(k),J=k.getProps?k.getProps(h,t):E;if(J=(B=h["prep"+k.typeName])?
B.call(h,k,m,H,t,J):null){if(J.hit){H.elements.push(Hits.initHit(k.type,k,k.draggable?k.drag:null));B=H.type+"Style";w=k[B]||i[B]||w}w=w?h.applyStyle(w,true):null;(B=k.draw||J.draw)&&B.call(k,J);w&&h.applyStyle(w)}L&&L.undo&&L.undo()}},find:function(){return[].concat(a)},hitGuess:function(h,m){return c&&c.search({x:h-10,y:m-10,w:20,h:20}).length>0}});i.bind("parentChange",e);i.bind("resync",e);return i},fb=function(b){return new Lb(b)};COG.extend(G,{ex:COG.extend,is:u,ticks:V,userMessage:function(b,
d,f){G.trigger("userMessage",b,d,f)},indexOf:ga,Rect:j,XY:Oa,XYRect:Ta,Vector:gb,Hits:Hits,Generator:yb,tweenValue:COG.tweenValue,easing:COG.easing,Tile:z,TileLayer:Ua,getImage:q,viewState:ha,View:Kb,ViewLayer:W,ImageLayer:Ua,Drawable:za,Marker:n,Poly:r,Line:x,Arc:ia,ImageDrawable:Q,ImageMarker:$,DrawLayer:Lb,ShapeLayer:fb});COG.observable(G);var sb=[1.406245461070741,1.321415085624082,1.077179995861952,0.703119412486786,0.488332580888611],Da=Math.PI/180,Ma=180/Math.PI,tb=R,Mb=-tb,Nb=N,lc=-Nb,cb=
0.08181919084262157,ab=null,Ob={RD:"ROAD",ST:"STREET",CR:"CRESCENT",CRES:"CRESCENT",CT:"COURT",LN:"LANE",HWY:"HIGHWAY",MWY:"MOTORWAY"},ea=function(){function b(a){return a?f(a.lat,a.lon):null}function d(a){return!a||a.lat===0&&a.lon===0}function f(a,c){return{lat:parseFloat(a?a:0),lon:parseFloat(c?c:0)}}function e(a){if(a)if(u(a.lat,xb))return b(a);else{if(a.split)for(var c=[" ",","],g=0;g<c.length;g++){var i=a.split(c[g]);if(i.length===2)return f(i[0],i[1])}}else return f();return null}return{calcDistance:function(a,
c){if(d(a)||d(c))return 0;var g=(c.lat-a.lat)*Da>>1,i=(c.lon-a.lon)*Da>>1;g=M(g)*M(g)+sa(a.lat*Da)*sa(c.lat*Da)*M(i)*M(i);return 6371*2*Xa(Aa(g),Aa(1-g))},copy:b,empty:d,equal:function(a,c){return a&&c&&a.lat==c.lat&&a.lon==c.lon},fromMercatorPixels:function(a,c){for(var g=ma(Math.E,-c),i=R-2*ya(g),h=Ca(i,g),m=0;m<12&&ba(i-h)>1.0E-7;){i=h;h=Ca(i,g);m++}return f(h*Ma,a%360*Ma)},generalize:function(a,c,g){var i=a.length,h=[],m=null;g||(g=250);g/=1E3;COG.info("generalizing positions, must include "+
c.length+" positions");for(var t=i;t--;)if(t===0)h.unshift(a[t]);else{var E=!m||ea.inArray(a[t],c)?g:ea.calcDistance(m,a[t]);if(a[t]&&E>=g){h.unshift(a[t]);m=a[t]}}COG.info("generalized "+i+" positions into "+h.length+" positions");return h},inArray:function(a,c){for(var g=ea.equal,i=c.length;i--;)if(g(a,c[i]))return true;return false},inBounds:function(a,c){var g=!(ea.empty(a)||ea.empty(c));return g=(g=g&&a.lat>=c.min.lat&&a.lat<=c.max.lat)&&a.lon>=c.min.lon&&a.lon<=c.max.lon},init:f,offset:function(a,
c,g){var i=a.lat*Da;a=a.lon*Da;c=i+c/6371;g=ua(M(g/6371)/sa(i));g=a+g;c=(c+R)%Math.PI-R;g%=N;return f(c*Ma,g*Ma)},parse:e,parseArray:function(a){var c=a.length,g=Array(c);for(c=c;c--;)g[c]=e(a[c]);return g},toMercatorPixels:function(a){return T5.XY.init(parseFloat(a.lon)*Da,pa(a.lat))},toString:function(a){return a?a.lat+" "+a.lon:""},vectorize:function(a,c){function g(){for(var t=0,E=c.chunkSize,X=i;X--;){h[X]=T5.GeoXY.init(a[X]);t+=1;if(t>E)break}i=X;if(i<=0)c.callback&&c.callback(h);else animFrame(g)}
var i=a.length,h=Array(i);c=COG.extend({chunkSize:500,async:true,callback:null},c);if(!c.async){for(var m=i;m--;)h[m]=T5.GeoXY.init(a[m]);return h}animFrame(g);return null}}}(),Ia=function(){function b(a,c,g){var i=new s(0,c.lat-a.lat);i.x=(g||u(g,qa))&&a.lon>c.lon?360-a.lon+c.lon:c.lon-a.lon;return i}function d(a,c){return Ia.init(ea.init(a.min.lat-c,a.min.lon-c%360),ea.init(a.max.lat+c,a.max.lon+c%360))}function f(a){var c=b(a.min,a.max);return ea.init(a.min.lat+(c.y>>1),a.min.lon+(c.x>>1))}function e(a,
c){return{min:ea.parse(a),max:ea.parse(c)}}return{calcSize:b,createBoundsFromCenter:function(a,c){var g=c/6371,i=a.lat*Da,h=a.lon*Da,m=i-g,t=i+g;if(m>Mb&&t<tb){i=ua(M(g)/sa(i));g=(h-i)%N;h=(h+i)%N}else{m=U(m,Mb);t=K(t,tb);g=lc;h=Nb}return Ia.init(ea.init(m*Ma,g*Ma),ea.init(t*Ma,h*Ma))},expand:d,forPositions:function(a,c){var g=null,i=T5.ticks();c||(c="auto");for(var h=a.length;h--;)if(g){var m=b(g.min,a[h],false),t=b(a[h],g.max,false);if(m.x<0)g.min.lon=a[h].lon;if(m.y<0)g.min.lat=a[h].lat;if(t.x<
0)g.max.lon=a[h].lon;if(t.y<0)g.max.lat=a[h].lat}else g=e(a[h],a[h]);if(c){if(c=="auto"){h=b(g.min,g.max);c=U(h.x,h.y)*0.3}g=d(g,c)}COG.trace("calculated bounds for "+a.length+" positions",i);return g},getCenter:f,getGeoHash:function(a){var c=T5.Geo.GeoHash.encode(a.min.lat,a.min.lon);a=T5.Geo.GeoHash.encode(a.max.lat,a.max.lon);COG.info("min hash = "+c+", max hash = "+a)},getZoomLevel:function(a,c){var g=f(a);g=K(Z(ba(g.lat)*0.05),sb.length);var i=sb[g],h=b(a.min,a.max);g=C(I(sb[3]*c.h/h.y)/I(2));
i=C(I(i*c.w/h.x)/I(2));return K(isNaN(g)?1E3:g,isNaN(i)?1E3:i)},init:e,isEmpty:function(a){return!a||ea.empty(a.min)||ea.empty(a.max)},toString:function(a){return"min: "+ea.toString(a.min)+", max: "+ea.toString(a.max)}}}();gb=function(){var b=/^(\d+).*$/,d=/(\d+)\s?\-\s?(\d+)/;return{buildingMatch:function(f,e){b.lastIndex=-1;if(b.test(f))for(var a=f.replace(b,"$1"),c=e.split(","),g=0;g<c.length;g++){d.lastIndex=-1;if(d.test(c[g])){var i=d.exec(c[g]);if(a>=parseInt(i[1],10)&&a<=parseInt(i[2],10))return true}else if(a==
c[g])return true}return false},normalize:function(f){if(!f)return"";f=f.toUpperCase();if(!ab){var e=[];for(var a in Ob)e.push(a);ab=RegExp("(\\s)("+e.join("|")+")(\\s|$)","i")}ab.lastIndex=-1;if(e=ab.exec(f))f=f.replace(ab,"$1"+Ob[e[2]]);return f},toString:function(f){return f.streetDetails+" "+f.location}}}();var Ja=G.GeoXY=function(){function b(e,a){if(e.length){for(var c,g,i,h,m=e.length;m--;){b(e[m],a);c=u(c,qa)||e.x<c?e.x:c;g=u(g,qa)||e.y<g?e.y:g;i=u(i,qa)||e.x>i?e.x:i;h=u(h,qa)||e.y>h?e.y:h}return new j(c,
...
Math.PI-e.y*a)},updatePos:f}}(),Pa={};Ua=function(){return{bestResults:function(b,d){d||(d=20);for(var f=b.length>0?b[0]:null,e=[],a=0;a<b.length;a++)if(f&&b[a]&&f.matchWeight-b[a].matchWeight<=d)e.push(b[a]);else break;return e}}}();var Pb=function(b){b=COG.extend({id:null,caption:"",resultType:"",data:null,pos:null,matchWeight:0},b);return COG.extend(b,{toString:function(){return b.caption+(b.matchWeight?" ("+b.matchWeight+")":"")}})},mc=function(){function b(a,c){var g=new T5.ShapeLayer;c.geometry&&
...
waypoints:[],map:null,error:null,autoFit:true,success:null,generalize:false},a);var c=S("route");c&&c.route(a,function(g){if(a.generalize)g.geometry=ea.generalize(g.geometry,g.getInstructionPositions());if(a.map){b(a.map,g);a.autoFit&&a.map.gotoBounds(g.boundingBox)}a.success&&a.success(g)})},createMapOverlay:b,parseTurnType:d,TurnType:f,Instruction:function(a){var c=a=COG.extend({position:null,description:"",distance:0,distanceTotal:0,time:0,timeTotal:0,turnType:null},a),g=a.description;g=g.replace(/(\w)(\/)(\w)/g,
"$1 $2 $3");c.description=g;if(!a.turnType)a.turnType=d(a.description);return a},RouteData:function(a){a=COG.extend({geometry:[],instructions:[],boundingBox:null},a);if(!a.boundingBox)a.boundingBox=Ia.forPositions(a.geometry);return COG.extend({getInstructionPositions:function(){for(var c=[],g=0;g<a.instructions.length;g++)a.instructions[g].position&&c.push(a.instructions[g].position);return c}},a)}}}(),nc="featurecollection",oc="feature",wb={async:false},Ra={processor:null,group:"shapes",layerClass:fb},
...
d,f,e){d=d&&d.coordinates?d.coordinates:[];if(d.length>0)return Y(b,d[0],f,e.poly);return 0}}),multipolygon:COG.extend({},Ra,{processor:function(b,d,f,e){d=d&&d.coordinates?d.coordinates:[];for(var a=0,c=0;c<d.length;c++)a+=Y(b,d[c][0],f,e.poly);return a}})},Qb=function(b,d,f,e){function a(k,w){var B=k.processor,L=m+k.group,J=COG.extend({},k,f,{properties:w.properties});if(B){var T=F[L];if(!T){T=new k.layerClass({id:L});F[L]=T}return B(T,w.data,J,e)}return 0}function c(k,w){var B=k&&k.type?k.type.toLowerCase():
null;return B&&B===oc?c(k.geometry,k.properties):{type:B,isCollection:B?B===nc:false,definition:pc[B],data:k,properties:w?w:k.properties}}function g(k){var w=0,B=COG.extend({},f),L=t;k=k?k:(new Date).getTime();if(!X){for(;L<E;L++){k=c(h?h(b[L]):b[L]);var J=null;if(k.isCollection){B.layerPrefix=m+H++ +"-";X=new Qb(k.data.features,function(T){X=null;for(var fa in T)F[fa]=T[fa];t>=E&&d&&d(F)},B);J+=1}else if(k.definition)J=a(k.definition,k);w+=J?J:1;if(w>=i)break}t=L+1;if(X||t<E)animFrame(g);else d&&
d(F)}}f=COG.extend({rowPreParse:null,simplify:false,layerPrefix:"geojson-"},f);e=COG.extend({marker:function(k){return new n({xy:k})},line:function(k,w){return new r(k,COG.extend({},f,w))},poly:function(k,w){return new r(k,COG.extend({fill:true},f,w))}},e);var i=500,h=f.rowPreParse,m=f.layerPrefix,t=0,E=0,X=null,H=0,F={};if(b){u(b,"array")||(b=[b]);E=b.length;animFrame(g)}};Ra=G.GeoJSON={parse:function(b,d,f){return new Qb(b,d,f)}};G.Map=function(b){function d(F){try{var k=ea.init(F.coords.latitude,
F.coords.longitude),w=F.coords.accuracy/1E3;H.trigger("locationUpdate",F,w);if(t){if(!h){h=new UI.LocationOverlay({pos:k,accuracy:w});h.update(H.getTileLayer());H.setLayer("location",h)}var B=Ia.createBoundsFromCenter(k,Math.max(w,1));H.gotoBounds(B)}else{h.pos=k;h.accuracy=w;h.update(H.getTileLayer());a(k,null,COG.easing("sine.out"))}t=false}catch(L){COG.exception(L)}}function f(F){COG.info("caught location tracking error:",F)}function e(F,k,w){H.setZoomLevel(k);a(F,w)}function a(F,k,w,B){F=Ja.init(F,
ja(H.getZoomLevel()));H.updateOffset(F.x-(H.width>>1),F.y-(H.height>>1),w,B,function(){k&&k(H)})}b=COG.extend({tapExtent:10,crosshair:false,zoomLevel:0,minZoom:1,maxZoom:18,pannable:true,scalable:true},b);var c={NONE:0,SINGLE:1,WATCH:2},g=new s,i=c.NONE,h=null,m=0,t=true,E=0,X=b.tapExtent;b.adjustScaleFactor=function(F){return Math.pow(2,(F<1?Math.floor:Math.ceil)(Math.log(F)))};var H=COG.extend(new Kb(b),{getBoundingBox:function(){var F=H.getViewport();return F?Ia.init(Ja.toPos(new s(F.x,F.y2),E),
Ja.toPos(new s(F.x2,F.y),E)):null},getCenterPosition:function(){var F=H.getViewport();if(F){F=new s(F.x+(F.w>>1),F.y+(F.h>>1));return Ja.toPos(F,E)}return null},gotoBounds:function(F,k){var w=Ia.getZoomLevel(F,H.getViewport());e(Ia.getCenter(F),w,k)},gotoPosition:e,panToPosition:a,syncXY:function(F,k){return(k?Ja.syncPos:Ja.sync)(F,E)},locate:function(){H.trackStart(c.SINGLE);setTimeout(H.trackCancel,1E4)},trackStart:function(F){if(navigator.geolocation&&!m){i=F?F:c.WATCH;t=true;m=navigator.geolocation.watchPosition(d,
f,{enableHighAccuracy:true,timeout:1E4,maximumAge:5E3})}},trackCancel:function(){m&&navigator.geolocation&&navigator.geolocation.clearWatch(m);H.removeLayer("location");h=null;i=c.NONE;m=0},animateRoute:function(F,k,w,B){var L=H.getLayer("route");if(L)(F=L.getAnimation(F,k,w,B))&&F.addToView(H)}});H.bind("pan",function(){i===c.SINGLE&&H.trackCancel()});H.bind("tap",function(F,k,w,B){F=Ja.toPos(B,E);var L=Ja.toPos(Oa.offset(B,-X,X),E);B=Ja.toPos(Oa.offset(B,X,-X),E);H.trigger("geotap",k,w,F,Ia.init(L,
B))});H.bind("refresh",function(F,k,w){if(g.x!=w.x||g.y!=w.y){H.trigger("boundsChange",H.getBoundingBox());g.x=w.x;g.y=w.y}});H.bind("zoomLevelChange",function(F,k){var w;E=ja(k);w=N/E|0;H.setMaxOffset(w,w,true,false);H.resetScale();H.triggerAll("resync",H)});return H};G.GeoShape=function(b,d){d=COG.extend({autoParse:true},d);for(var f=Array(b.length),e=d.autoParse,a=T5.Geo.Position.parse,c=b.length;c--;)f[c]=T5.GeoXY.init(e?a(b[c]):b[c]);return new T5.Poly(f,d)};G.LocationOverlay=function(b){b=COG.extend({pos:null,
