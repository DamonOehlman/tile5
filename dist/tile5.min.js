...
z.y+fa.y):new T5.Vector(z.x-fa.x*ia,z.y-fa.y*ia))F=T5.V.offset(F,P.x,P.y)}B.save();try{da=N;if(y!==1)B.scale(y,y);else if(ca){B.translate(z.x,z.y);B.scale(N,N)}for(ea=t.length;ea--;)if(t[ea].shouldDraw(O)){var ka=t[ea].draw(B,F,A,O,J);K+=ka?ka:0}}finally{B.restore()}GT.Log.trace("draw complete",T);R=false;return K}function p(){var B=0,F=!I&&(S===ba||S===ga);X=T5.time();n.x=Math.floor(n.x);n.y=Math.floor(n.y);if(F){T5.cancelAnimation(function(O){return O.cancelOnInteract});H=false;if(M!==0){clearTimeout(M);
M=0}}for(F=t.length;F--;){var K=t[F].cycle(X,n,S);B+=K?K:0}B+=s(u,n);L=0;if(D+B>0)w();else if(!H&&M===0)M=setTimeout(C,500);GT.Log.trace("Completed draw cycle",X)}function w(){D++;if(!(v||L!==0)){D=0;L=setTimeout(p,0)}}e=T5.ex({id:GT.objId("view"),container:"",fastDraw:false,inertia:true,pannable:true,scalable:true,panAnimationEasing:T5.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,adjustScaleFactor:null,autoSize:false},e);var t=[],E=document.getElementById(e.container),
...
false;w()})}if(e.scalable){J.bind("pinchZoom",g);J.bind("pinchZoomEnd",k);J.bind("wheelZoom",d)}GT.configurable(J,["inertia","container"],GT.paramTweaker(e,null,{container:function(B,F){E=document.getElementById(F);h()}}),true);h();return J};
...
y,A){v=0;if(t.x!==0||t.y!==0){var D=t,z=e.shiftOrigin;if(!(D.x===0&&D.y===0)){var H=GT.Log.getTraceTicks(),I=Array(g.length);I.length=g.length;for(var L=0;L<e.gridSize;L++)for(var R=0;R<e.gridSize;R++)I[R*e.gridSize+L]=L+D.x>=0&&L+D.x<e.gridSize?g[(R+D.y)*e.gridSize+(L+D.x)]:null;g=I;l=z?z(l,D):T5.V.add(l,D);o.x+=-D.x*e.tileSize;o.y+=-D.y*e.tileSize;GT.Log.trace("tile storage shifted",H);b(h,q)}t=new T5.Vector;v++}A!==T5.viewState("PINCH")&&a(y,A);return v+x.dirty?1:0},deactivate:function(){s=false},
...
(u.matchWeight?" ("+u.matchWeight+")":"")}})},GeoSearchAgent:function(u){return new T5.Dispatcher.Agent(u)},GeocodingAgent:function(u){u=T5.ex({name:"Geocoding Search Agent",paramTranslator:null,execute:function(n,x){try{if(!n.reverse&&!n.freeform)address=new C(n);var v=E.getEngine("geocode");if(v)v.geocode({addresses:[n.freeform?n.freeform:address],complete:function(A,D){if(x){var z=D;if(n.freeform)z=E.rankGeocodeResponses(n.freeform,z,E.getEngine("geocode"));x(z,u)}}})}catch(y){GT.Log.exception(y)}}},
u);return new E.GeoSearchAgent(u)},PointOfInterest:function(u){u=T5.ex({id:0,title:"",pos:null,lat:"",lon:"",group:"",retrieved:0,isNew:true},u);if(!u.pos&&u.lat&&u.lon)u.pos=new T5.Geo.Position(u.lat,u.lon);return T5.ex({toString:function(){return u.id+": '"+u.title+"'"}},u)},POIStorage:function(u){function n(z){z=z?z:"default";y[z]||(y[z]=[]);return y[z]}function x(z){var H=[];for(var I in y)for(var L=0;L<y[I].length;L++)if(!z||z(y[I][L]))H.push(y[I][L]);return H}function v(){GT.say("geo.pois-updated",
{srcID:D.id,pois:D.getPOIs()})}u=T5.ex({visibilityChange:null,onPOIDeleted:null,onPOIAdded:null},u);var y={},A=true,D={id:GT.objId(),getPOIs:function(){return x()},getOldPOIs:function(z,H){return x(function(I){return I.group==z&&I.retrieved<H})},getVisible:function(){return A},setVisible:function(z){if(z!=A){A=z;u.visibilityChange&&u.visibilityChange()}},findById:function(z){var H=x(function(I){return I.id==z});return H.length>0?H[0]:null},findByBounds:function(z){return x(function(H){return T5.Geo.P.inBounds(H.pos,
...
true,a=null,g=[],k=0,d=[],l=[],h=T5.ex(new T5.ViewLayer(f),{getAnimation:function(o,q,r,C){if(b)return null;var s="routeAnimation"+e++;l.push(s);return new T5.AnimatedPathLayer({id:s,path:g,zindex:f.zindex+1,easing:o?o:T5.T5.easing("sine.inout"),duration:q?q:5E3,drawIndicator:r,autoCenter:C?C:false})},draw:function(o,q,r,C,s){r=0;C=f.data?f.data.geometry:null;if(b){b=false;a=null;g=[];k=0}if(C&&k<C.length){a:{s=s.getTileLayer();d=[];if(s){C=GT.Log.getTraceTicks();var p,w,t,E=f.data?f.data.geometry:
[],u=E.length,n=f.data?f.data.instructions:[],x=n.length,v=f.calculationsPerCycle,y=0;for(p=k;p<u;p++){w=s.getGridXYForPosition(E[p]);if(t=!a||p===0||Math.abs(w.x-a.x)+Math.abs(w.y-a.y)>f.pixelGeneralization){g.push(w);a=w}y++;if(y>=v){k=p;break a}}k=u;GT.Log.trace(u+" geometry points generalized to "+g.length+" coordinates",C);a=null;for(p=x;p--;)if(n[p].position){w=s.getGridXYForPosition(n[p].position);if(t=!a||p===0||Math.abs(w.x-a.x)+Math.abs(w.y-a.y)>f.pixelGeneralization){d.push(w);a=w}}}}r++}s=
g.length;if(s>0&&(!r||f.partialDraw)){o.strokeStyle=f.strokeStyle;o.lineWidth=f.lineWidth;o.beginPath();o.moveTo(g[s-1].x-q.x,g[s-1].y-q.y);for(s=s;s--;)o.lineTo(g[s].x-q.x,g[s].y-q.y);o.stroke();o.fillStyle=f.waypointFillStyle;for(s=d.length;s--;){o.beginPath();o.arc(d[s].x-q.x,d[s].y-q.y,2,0,Math.PI*2,false);o.stroke();o.fill()}}return r}});GT.listen("grid.updated",function(){for(var o=l.length;o--;)GT.say("layer.remove",{id:l[o]});l=[];b=true;h.wakeParent()});return h},Annotation:function(f){f=
...
p.getOffset()))>e.boundsChangeThreshold){a=p.getOffset();p.trigger("boundsChange",p.getBoundingBox())}});GT.configurable(p,["provider"],GT.paramTweaker(e,null,{provider:function(){p.cleanup();k=false}}),true);return p};
