...
l(2,-10*(n-=1))*r((n*m-N)*u/M)*0.5+j+C},quadin:function(n,C,j,m){return j*(n/=m)*n+C},quadout:function(n,C,j,m){return-j*(n/=m)*(n-2)+C},quadinout:function(n,C,j,m){if((n/=m/2)<1)return j/2*n*n+C;return-j/2*(--n*(n-2)-1)+C},sinein:function(n,C,j,m){return-j*Q(n/m*X)+j+C},sineout:function(n,C,j,m){return j*r(n/m*X)+C},sineinout:function(n,C,j,m){return-j/2*(Q(Math.PI*n/m)-1)+C}};COG.tweenValue=function(n,C,j,m,w){function M(O){O=O?O:(new Date).getTime();if(la+s<O){var ma=O-N,qa=j(ma,n,ka,m),ua=N+m<=
O;O=!ua;if(w){ma=w(qa,ua,ma);O=typeof ma!="undefined"?ma&&O:O}O&&animFrame(M)}}var N=(new Date).getTime(),la=0,ka=C-n;animFrame(M);return{}};COG.easing=function(n){return ta[n.replace(/[\-\_\s\.]/g,"").toLowerCase()]};COG.registerEasingType=function(n,C){ta[n.replace(/[\-\_\s\.]/g,"").toLowerCase()]=C}})();
...
ta=typeof FlashCanvas!="undefined",n=typeof G_vmlCanvasManager!="undefnined";Q.width=200;Q.height=200;l.pipTransformed=ta||n?false:y();r()})})(CANI);
...
m.target:m.srcElement}}function y(j,m){for(var w=j.target?j.target:j.srcElement;w&&w!==m;)w=w.parentNode;return w&&w===m}function Q(j,m){return{x:j.x-(m?m.left:0),y:j.y-(m?m.top:0)}}function ta(j,m){if(j.preventDefault){j.preventDefault();j.stopPropagation()}else if(typeof j.cancelBubble!="undefined")j.cancelBubble=true;m&&j.stopImmediatePropagation&&j.stopImmediatePropagation()}var n=[],C=function(j,m,w){function M(fa,ga,ia,L){var U=0,ca=0,A=0;COG.tweenValue(0,fa,ia,L,function(I){A=U;U=I});COG.tweenValue(0,
ga,ia,L,function(I){N.trigger("pan",U-A,I-ca);ca=I})}w=COG.extend({binder:null,unbinder:null,observable:null},w);for(var N=w.observable,la=null,ka=[],O=[],ma,qa,ua={bind:function(){return N.bind.apply(null,arguments)},pannable:function(fa){la=COG.extend({inertia:true},fa);return ua},unbind:function(){N.unbind();for(Ba=0;Ba<ka.length;Ba++)ka[Ba].unbind();return ua}},Ba=0;Ba<m.length;Ba++)ka.push(m[Ba](j,N,w));N.bind("pointerDown",function(){O=[];qa=ma=0});N.bind("pointerMove",function(fa,ga,ia,L){if(la){O[O.length]=
{ticks:(new Date).getTime(),x:L.x,y:L.y};N.trigger("pan",L.x,L.y)}ma+=L.x?L.x:0;qa+=L.y?L.y:0});N.bind("pointerUp",function(fa,ga,ia){if(Math.abs(ma)>20||Math.abs(qa)>20){if(la){ia=O.length;var L,U;ga=0;for(U=O.length;--U>1&&ga<50;)ga+=O[U].ticks-O[U-1].ticks;if(ia-U>1){fa=O[ia-1].x-O[U].x;L=O[ia-1].y-O[U].y;U=Math.sqrt(fa*fa+L*L)|0;if(U>15){fa=O[ia-1].x-O[0].x;L=O[ia-1].y-O[0].y;U=Math.sqrt(fa*fa+L*L)|0;ia=Math.asin(L/U);ga=U*500/ga|0;ga=ga>300?300:ga;M(Math.cos(fa>0?ia:Math.PI-ia)*ga,Math.sin(ia)*
ga,COG.easing("sine.out"),500)}}}}else N.trigger("tap",ga,ia)});return ua};k("pointer",{handler:function(j,m,w){function M(A){if(A.pageX&&A.pageY)return s(A.pageX,A.pageY);else{var I=document.documentElement,V=document.body;return s(A.clientX+(I&&I.scrollLeft||V&&V.scrollLeft||0)-(I&&I.clientLeft||V&&V.clientLeft||0),A.clientY+(I&&I.scrollTop||V&&V.scrollTop||0)-(I&&I.clientTop||V&&V.clientTop||0))}}function N(A){if(y(A,j))if(fa=ma(A)){var I=M(A);j.style.cursor="move";ta(A,true);U=I.x;ca=I.y;ga=s(U,
ca);m.triggerCustom("pointerDown",r("mouse",A),ga,Q(ga,l(j)))}}function la(A){var I=M(A);ia=I.x;L=I.y;if(y(A,j))qa(A,fa?"pointerMove":"pointerHover")}function ka(A){if(fa&&ma(A)){fa=false;if(y(A,j)){j.style.cursor="default";qa(A,"pointerUp")}}}function O(A){if(y(A,j)){var I;A=A||window.event;if(A.detail)I=A.axis===2?-A.detail*ua:0;else{I=A.wheelDeltaY?A.wheelDeltaY:A.wheelDelta;if(window.opera)I=-I}if(I!==0){var V=s(ia,L);m.triggerCustom("zoom",r("mouse",A),V,Q(V,l(j)),I/Ba,"wheel");ta(A);A.returnValue=
false}}}function ma(A){A=A||window.event;return(A.which||A.button)==1}function qa(A,I,V,ra,Ga){V=typeof V!="undefined"?V:ia;ra=typeof ra!="undefined"?ra:L;var H=V-U,$=ra-ca,K=s(V,ra);m.triggerCustom(I,r("mouse",A),K,Q(K,l(j)),s(H,$));if(typeof Ga=="undefined"||Ga){U=V;ca=ra}}w=COG.extend({},w);var ua=120,Ba=ua*8,fa=false,ga,ia,L,U,ca;w.binder("mousedown",N);w.binder("mousemove",la);w.binder("mouseup",ka);w.binder("dblclick",function(A){COG.info("captured double click");if(y(A,j)){var I=M(A);m.triggerCustom("doubleTap",
r("mouse",A),I,Q(I,l(j)))}});w.binder("mousewheel",O);w.binder("DOMMouseScroll",O);return{unbind:function(){w.unbinder("mousedown",N);w.unbinder("mousemove",la);w.unbinder("mouseup",ka);w.unbinder("mousewheel",O);w.unbinder("DOMMouseScroll",O)}}},checks:{touch:false}});k("pointer",{handler:function(j,m,w){function M(H){if(H.count<2)return 0;var $=H.x-H.next.x;H=H.y-H.next.y;return~~Math.sqrt($*$+H*H)}function N(H,$,K){$=$?$:0;K=K?K:0;for(var za={x:H.x-$,y:H.y-K,id:H.id,count:H.count},xa=za;H.next;){H=
H.next;xa=xa.next={x:H.x-$,y:H.y-K,id:H.id}}return za}function la(H,$){var K=H[$?$:"touches"],za,xa;if(K.length===0)return null;xa=za={x:K[0].pageX,y:K[0].pageY,id:K[0].identifier,count:K.length};for(var Ea=1,Ya=K.length;Ea<Ya;Ea++)xa=xa.next={x:K[Ea].pageX,y:K[Ea].pageY,id:K[Ea].identifier};return za}function ka(H){if(y(H,j)){L=l(j);var $=la(H,"changedTouches"),K=N($,L.left,L.top);if(!ca){U=fa;m.triggerCustom("pointerDown",r("touch",H),$,K)}ra&&m.triggerCustom("pointerDownMulti",r("touch",H),$,K);
ca=la(H);if(ca.count>1)I=M(ca);Ga=1;V=N(ca)}}function O(H){if(y(H,j)){ta(H);A=la(H);if(U==fa)U=Math.abs(ca.x-A.x)>ua||Math.abs(ca.y-A.y)>ua?Ba:fa;if(U!=fa){U=A.count>1?ia:ga;if(U==ia)if(ca.count===1){ca=N(A);I=M(ca)}else{var $=M(A);if(Math.abs(I-$)<qa)U=ga;else{var K;K=A.x;var za=A.next.x,xa=A.y,Ea=A.next.y,Ya=xa<Ea?xa:Ea;K={x:(K<za?K:za)+(Math.abs(K-za)>>1),y:Ya+(Math.abs(xa-Ea)>>1)};$=$/I;za=$-Ga;m.triggerCustom("zoom",r("touch",H),K,Q(K,L),za,"pinch");Ga=$}}U==ga&&m.triggerCustom("pointerMove",
r("touch",H),A,N(A,L.left,L.top),s(A.x-V.x,A.y-V.y));ra&&m.triggerCustom("pointerMoveMulti",r("touch",H),A,N(A,L.left,L.top))}V=N(A)}}function ma(H){if(y(H,j)){var $=la(H,"changedTouches"),K=N($,L.left,L.top);A=la(H);if(!A){m.triggerCustom("pointerUp",r("touch",H),$,K);ca=null}ra&&m.triggerCustom("pointerUpMulti",r("touch",H),$,K)}}w=COG.extend({detailed:false,inertia:false},w);var qa=20,ua=7,Ba=0,fa=1,ga=2,ia=3,L,U,ca,A,I,V,ra=w.detailed,Ga=1;w.binder("touchstart",ka);w.binder("touchmove",O);w.binder("touchend",
ma);COG.info("initialized touch handler");return{unbind:function(){w.unbinder("touchstart",ka);w.unbinder("touchmove",O);w.unbinder("touchend",ma)}}},checks:{touch:true}});return{register:k,watch:function(j,m,w){m=COG.extend({bindTarget:null,observable:null,isIE:typeof window.attachEvent!="undefined",types:null},m);capabilities=COG.extend({touch:"ontouchstart"in window},w);if(!m.observable){COG.info("creating observable");m.observable=COG.observable({});globalOpts=m}m.binder=(m.isIE?u:D)(m.bindTarget||
document);m.unbinder=(m.isIE?u:X)(m.bindTarget||document);w=m.types;for(var M=capabilities,N=[],la=n.length;la--;){var ka=n[la],O=!w||w.indexOf(ka.type)>=0,ma=true;for(var qa in ka.checks){var ua=ka.checks[qa];COG.info("checking "+qa+" capability. require: "+ua+", capability = "+M[qa]);ma=ma&&ua===M[qa]}if(O&&ma)N[N.length]=ka.handler}return new C(j,N,m)}}}();var T5={};
...
d,f,e,a,c){this.x=b;this.y=d;this.w=e||256;this.h=e||256;this.x2=this.x+this.w;this.y2=this.y+this.h;this.url=f;this.id=c||b+"_"+d;this.loaded=false;this.image=null}function Q(){for(var b=0,d=arguments.length;d--;){var f=Yb[arguments[d].toUpperCase()];if(f)b|=f}return b}function ta(b){b=b?b:(new Date).getTime();if(b-xb>Zb){for(var d=Za.splice(0),f=d.length;f--;)d[f](b);xb=b}Za.length&&animFrame(ta)}function n(b){var d=Za.length==0;Za[Za.length]=b;d&&animFrame(ta)}function C(b,d,f,e,a){a=COG.extend({easing:"sine.out",
duration:1E3,progress:null,complete:null,autoInvalidate:true},a);var c=(new Date).getTime(),g=b[d],h=d=="translate",i=a.autoInvalidate,o=f.length&&e.length&&f.length==e.length?f.length:0,p=Array(o),G=Array(o),P=COG.easing(a.easing),F=a.duration,v=a.progress,z=function(E){var J=E-c;E=c+F<=E;for(var ba=b.layer?b.layer.view:null,ja,va=o;va--;){ja=P(J,f[va],p[va],F);G[va]=h?ja|0:ja}g.apply(b,G);i&&ba&&ba.invalidate();if(v){J=[].concat(E?e:G);J.unshift(E);v.apply(b,J)}if(E){b.animations--;g.apply(b,e);
...
"ANIMATING","PAN","ZOOM"),style:null,minXY:null,maxXY:null},b);this.view=null;this.visible=true;COG.observable(COG.extend(this,b))}function O(b){for(var d=null,f=1;!d&&f<arguments.length;f++)d=ua(b,arguments[f]);d=d?d:ua(b);if(!d)throw Error("Unable to find GEO engine with "+b+" capability");return d}function ma(b){b=parseFloat(b)*Ha;b=K(b);var d=db*b;return H((1+b)/(1-b)*ra((1-d)/(1+d),db))/2}function qa(b){return L/(256<<b)}function ua(b,d){var f=null;for(var e in Ra)if(d){if(e==d&&Ra[e][b]){f=
...
reset:function(){}})},Bb={},Va=D.registerRenderer=function(b,d){Bb[b]=d},cc=D.attachRenderer=function(b,d,f,e){b=b.split("/");for(var a=new bc(d,f,e),c=0;c<b.length;c++){var g=Bb[b[c]];if(g)a=new g(d,f,e,a)}return a};Va("canvas",function(b,d,f,e){function a(B,aa,S){var da,Y;F[aa]=function(R){for(da=ja.length;da--;)if(Y=S[ja[da]])R[va[da]]=Y}}function c(B,aa,S,da){var Y=false;if(aa)Y=p.isPointInPath(z?aa.x-G:aa.relXY.x,z?aa.y-P:aa.relXY.y);return{draw:da||ba,viewport:B,state:S,hit:Y,vpX:G,vpY:P,context:p}}
f=COG.extend({},f);var g,h,i,o=false,p,G=0,P=0,F={},v=null,z=CANI.canvas.pipTransformed,E={},J=function(){},ba=function(){this.fill&&p.fill();this.stroke&&p.stroke()},ja=["fill","stroke","lineWidth","opacity"],va=["fillStyle","strokeStyle","lineWidth","globalAlpha"];(function(){if(d){var B=d.tagName=="CANVAS",aa=B?d.parentNode:d;g=b.width=aa.offsetWidth;h=b.height=aa.offsetHeight;if(B){i=d;i.width=g;i.height=h;cb&&FlashCanvas.initElement(i)}else{i=ac(g,h);i.style.cssText="position: absolute; z-index: 1;";
...
drawTiles:function(B,aa){for(var S,da,Y=G-256,R=P-256,W=B.x2,pa=B.y2,wa=aa.length;wa--;){S=aa[wa];if(da=S.x>=Y&&S.x<=W&&S.y>=R&&S.y<=pa)S.loaded?p.drawImage(S.image,S.x-G,S.y-P):S.load(b.invalidate)}},prepare:function(B,aa){var S,da=false,Y=aa.scaled||aa,R=aa.scaleFactor;if(p)p.restore();else p=i.getContext("2d");for(S=B.length;S--;)da=da||B[S].clip;G=Y.x;P=Y.y;if(p){da||p.clearRect(0,0,g,h);p.save();p.scale(R,R)}p.globalCompositeOperation="source-over";return p},prepArc:function(B,aa,S,da){p.beginPath();
...
J;p.rect(Y-(W>>1),R-(W>>1),W,W);if(B.reset&&B.image){B.image=null;B.reset=false}B.image?p.drawImage(B.image,Y-(W>>1),R-(W>>1),W,W):r(B.imageUrl,function(wa){B.image=wa;p.drawImage(B.image,Y-(W>>1),R-(W>>1),W,W)});break;default:p.moveTo(Y,R);p.lineTo(Y-(W>>1),R-W);p.lineTo(Y+(W>>1),R-W);p.lineTo(Y,R);break}return c(aa,S,da,pa)},prepPoly:function(B,aa,S,da,Y){var R=true;B=Y.points||B.points;Y=v?v.x:G;var W=v?v.y:P;p.beginPath();for(var pa=B.length;pa--;){var wa=B[pa].x-Y,eb=B[pa].y-W;if(R){p.moveTo(wa,
eb);R=false}else p.lineTo(wa,eb)}return c(aa,S,da)},getContext:function(){return p},getDimensions:function(){return{width:g,height:h}},getOffset:function(){return new s(G,P)}});(function(){for(var B in T5.styles)a(null,B,T5.styles[B]);T5.bind("styleDefined",a)})();f.bind("detach",function(){o&&d.removeChild(i)});return f});var kb=D.styles={},dc=D.defineStyle=function(b,d){kb[b]=d;D.trigger("styleDefined",b,kb[b]);return b},Cb=D.defineStyles=function(b){for(var d in b)dc(d,b[d])};D.getStyle=function(b){return kb[b]};
D.loadStyles=function(b){COG.jsonp(b,function(d){Cb(d)})};Cb({basic:{fill:"#ffffff"},highlight:{fill:"#ff0000"},waypoints:{lineWidth:4,stroke:"#003377",fill:"#ffffff"},waypointsHover:{lineWidth:4,stroke:"#ff0000",fill:"#ffffff"}});var Yb={NONE:0,ACTIVE:1,ANIMATING:4,PAN:8,ZOOM:16,FREEZE:128},Lb=function(b){function d(q,t,x){if(!wa){da(na-t,oa-x);Z.trigger("pan",na,oa)}}function f(q,t,x,T){B(I(V(sa+ra(2,T)-1,0.5),2))}function e(){var q=H(sa)/Math.LN2|0;if(q!==0){sa=ra(2,q);aa($a+q,ec,fc)}redraw=true}
function a(q,t){var x=renderer&&renderer.projectXY?renderer.projectXY(q,t):null;if(!x){x=Z.getViewport();var T=1/sa;x=new s(x?x.x+q*T:q,x?x.y+t*T:t)}return x}function c(q,t,x){S("doubleTap",t,x,a(x.x,x.y));b.scalable&&B(2,a(x.x,x.y),b.zoomEasing,null,b.zoomDuration)}function g(q,t,x){wa=null;J("down",t,x)}function h(q,t,x){J("hover",t,x)}function i(q,t,x){z(t,x,false)}function o(q,t,x){z(t,x,true)}function p(){clearTimeout(Db);Db=setTimeout(function(){renderer.checkSize()},250)}function G(q,t,x){J("tap",
...
g);Ca.bind("pointerMove",i);Ca.bind("pointerUp",o);b.captureHover&&Ca.bind("pointerHover",h);Ca.bind("tap",G)}}function z(q,t,x){if(wa){q=a(t.x,t.y);if(wa.drag.call(wa.target,wa,q.x,q.y,x))redraw=true;if(x)wa=null}}function E(q){var t,x,T=false;q=q||(new Date).getTime();if(T=q-Fb>hc){var ea=ca(na-lb),Aa=ca(oa-mb);t=na!==nb||oa!==ob;x=sa!==Gb;if(t||x)Ja++;if(ea>=Hb||Aa>=Hb)va();x={index:eb++,draw:Ja};Z.trigger("enterFrame",q,x);Fb=q}if(renderer&&T&&x.draw){Ka=Ib|(sa!==1?Jb:0)|(t?pb:0)|(qb?Kb:0);rb=
...
d,f,e){d=d&&d.coordinates?d.coordinates:[];if(d.length>0)return fa(b,d[0],f,e.poly);return 0}}),multipolygon:COG.extend({},Sa,{processor:function(b,d,f,e){d=d&&d.coordinates?d.coordinates:[];for(var a=0,c=0;c<d.length;c++)a+=fa(b,d[c][0],f,e.poly);return a}})},Rb=function(b,d,f,e){function a(z,E){var J=z.processor,ba=o+z.group,ja=COG.extend({},z,f,{properties:E.properties});if(J){var va=v[ba];if(!va){va=new z.layerClass({id:ba});v[ba]=va}return J(va,E.data,ja,e)}return 0}function c(z,E){var J=z&&
z.type?z.type.toLowerCase():null;return J&&J===mc?c(z.geometry,z.properties):{type:J,isCollection:J?J===lc:false,definition:nc[J],data:z,properties:E?E:z.properties}}function g(z){var E=0,J=COG.extend({},f),ba=p;z=z?z:(new Date).getTime();if(!P){for(;ba<G;ba++){z=c(i?i(b[ba]):b[ba]);var ja=null;if(z.isCollection){J.layerPrefix=o+F++ +"-";P=new Rb(z.data.features,function(va){P=null;for(var B in va)v[B]=va[B];p>=G&&d&&d(v)},J);ja+=1}else if(z.definition)ja=a(z.definition,z);E+=ja?ja:1;if(E>=h)break}p=
