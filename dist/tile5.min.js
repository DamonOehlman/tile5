COG=typeof COG!=="undefined"?COG:{};COG.extend=function(){for(var ca=arguments[0]||{},sa,L=1,J=arguments.length;L<J;L++)if((sa=arguments[L])!==null)for(var n in sa){var r=sa[n];if(ca!==r)if(r!==undefined)ca[n]=r}return ca};
...
(function(){function ca(r){return typeof console!=="undefined"?function(){console[r](Array.prototype.slice.call(arguments,0).join(" "));return true}:function(){return false}}var sa=window.console&&window.console.markTimeline,L=ca("error"),J=ca("info"),n=function(){return sa?function(r,x){console.markTimeline(r+(x?": "+((new Date).getTime()-x)+"ms":""))}:function(){}}();COG.extend(COG,{trace:n,debug:ca("debug"),info:J,warn:ca("warn"),error:L,exception:function(r){if(L)for(var x in r)J("ERROR DETAIL: "+
x+": "+r[x])}})})();
COG.Loopage=function(){function ca(Z){return COG.extend({id:r++,frequency:0,after:0,single:false,lastTick:0,execute:function(){}},Z)}function sa(Z){M.push(Z);L()}function L(){ta&&clearTimeout(ta);ta=setTimeout(J,0);za=true}function J(){for(var Z,G=(new Date).getTime(),H=x.length;M.length>0;){var I=M.shift();for(Z=H;Z--;)if(x[Z].id===I){x.splice(Z,1);break}za=true;H=x.length}if(za){ha=n;for(Z=H;Z--;)ha=x[Z].frequency<ha?x[Z].frequency:ha}for(Z=H;Z--;){I=G-x[Z].lastTick;if(x[Z].lastTick===0||I>=x[Z].frequency){x[Z].execute(G,
x[Z]);x[Z].lastTick=G;x[Z].single&&x[Z].trigger("complete")}}ta=H?setTimeout(J,ha):0}var n=6E4,r=0,x=[],M=[],ta=0,ha=n,za=true;return{join:function(Z){var G=new ca(Z);if(G.after>0)G.lastTick=(new Date).getTime()+G.after;COG.observable(G);G.bind("complete",function(){sa(G.id)});x.unshift(G);L();return G},leave:sa}}();
(function(){function ca(L,J){var n=L.obsHandlers;n[J]||(n[J]=[]);return n[J]}var sa=0;COG.observable=function(L){if(!L)return null;if(!L.obsHandlers)L.obsHandlers={};if(!(L.bind||L.trigger||L.unbind)){L.bind=function(J,n){var r="callback"+sa++;ca(L,J).unshift({fn:n,id:r});return r};L.trigger=function(J){var n=ca(L,J),r={cancel:false,name:J,tickCount:(new Date).getTime()},x;if(!n)return null;x=Array.prototype.slice.call(arguments,1);L.eventInterceptor&&L.eventInterceptor(J,r,x);x.unshift(r);for(var M=
...
(function(){function ca(n,r){n[r]||(n[r]=function(x){return n.configure(r,x)})}function sa(n){n.gtConfId="configurable"+J++;n.gtConfig={};n.gtConfigFns=[];return n.gtConfig}var L={},J=0;COG.paramTweaker=function(n,r,x){return function(M,ta){if(typeof ta!=="undefined"){if(M in n)n[M]=ta;x&&M in x&&x[M](M,ta)}else return r&&M in r?r[M](M):n[M]}};COG.configurable=function(n,r,x,M){if(n){n.gtConfId||sa(n);var ta=n.gtConfId,ha=n.gtConfig,za=n.gtConfigFns;L[ta]=n;za.push(x);for(x=r.length;x--;){ha[r[x]]=
true;M&&ca(n,r[x])}if(!n.configure)n.configure=function(Z,G){if(ha[Z]){for(var H=za.length;H--;){var I=za[H](Z,G);if(typeof I!=="undefined")return I}return L[ta]}return null}}}})();
(function(){function ca(n){var r=document.createElement("script"),x=false;r.src=n;r.async=true;r.onload=r.onreadystatechange=function(){if(!x&&(!this.readyState||this.readyState==="loaded"||this.readyState==="complete")){x=true;r.onload=r.onreadystatechange=null;r&&r.parentNode&&r.parentNode.removeChild(r)}};L||(L=document.getElementsByTagName("head")[0]);L.appendChild(r)}var sa=0,L,J=this;COG.jsonp=function(n,r,x){n+=n.indexOf("?")>=0?"&":"?";var M="json"+ ++sa;J[M]=function(ta){r(ta);J[M]=null;
try{delete J[M]}catch(ha){}};ca(n+(x?x:"callback")+"="+M);return M}})();
(function(){function ca(d){if(Z)return ha.length;Z=true;try{for(var t=0;t<ha.length;)if(ha[t].isComplete()){ha[t].triggerComplete(false);ha.splice(t,1)}else{ha[t].update(d);t++}}finally{Z=false}ha.length===0&&za.trigger("complete");return ha.length}function sa(){if(!za){za=COG.Loopage.join({execute:ca,frequency:20});za.bind("complete",function(){za=null})}}var L=1.70158,J=Math.PI/2,n=Math.abs,r=Math.pow,x=Math.sin,M=Math.asin,ta=Math.cos,ha=[],za=null,Z=false,G={linear:function(d,t,w,F){return w*
...
F){return-w*(d/=F)*(d-2)+t},quadinout:function(d,t,w,F){if((d/=F/2)<1)return w/2*d*d+t;return-w/2*(--d*(d-2)-1)+t},sinein:function(d,t,w,F){return-w*ta(d/F*J)+w+t},sineout:function(d,t,w,F){return w*x(d/F*J)+t},sineinout:function(d,t,w,F){return-w/2*(ta(Math.PI*d/F)-1)+t}},H=COG.Tween=function(d){d=COG.extend({target:null,property:null,startValue:0,endValue:null,duration:2E3,tweenFn:I("sine.out"),complete:null},d);var t=(new Date).getTime(),w=[],F=false,U=0,T=0,ma={isComplete:function(){return F},
...
0)F=true;sa();return ma};COG.tweenValue=function(d,t,w,F,U){d=new H({startValue:d,endValue:t,tweenFn:w,complete:F,duration:U});ha.push(d);return d};COG.tween=function(d,t,w,F,U,T){d=new H({target:d,property:t,endValue:w,tweenFn:F,duration:T,complete:U});ha.push(d);return d};COG.endTweens=function(d){if(!Z){Z=true;try{for(var t=0;t<ha.length;)if(!d||d(ha[t])){ha[t].triggerComplete(true);ha.splice(t,1)}else t++}finally{Z=false}}};COG.getTweens=function(){return[].concat(ha)};var I=COG.easing=function(d){return G[d.replace(/[\-\_\s\.]/g,
"").toLowerCase()]};COG.registerEasingType=function(d,t){G[d.replace(/[\-\_\s\.]/g,"").toLowerCase()]=t}})();
(function(){var ca=/^P(\d+Y)?(\d+M)?(\d+D)?$/,sa=/^(\d+H)?(\d+M)?(\d+S)?$/,L={8601:function(n){n=n.split("T");var r=null;r=null;var x=0,M=0;ca.lastIndex=-1;r=ca.exec(n[0]);x+=r[3]?parseInt(r[3].slice(0,-1),10):0;sa.lastIndex=-1;r=sa.exec(n[1]);M+=r[1]?parseInt(r[1].slice(0,-1),10)*3600:0;M+=r[2]?parseInt(r[2].slice(0,-1),10)*60:0;M+=r[3]?parseInt(r[3].slice(0,-1),10):0;return new J(x,M)}},J=COG.Duration=function(n,r){return{days:n?n:0,seconds:r?r:0}};COG.addDuration=function(){for(var n=new J,r=arguments.length;r--;){n.days+=
...
-b.y)},min:function(b,e){return g(b.x<e.x?b.x:e.x,b.y<e.y?b.y:e.y)},max:function(b,e){return g(b.x>e.x?b.x:e.x,b.y>e.y?b.y:e.y)},offset:function(b,e,k){return g(b.x+e,b.y+(k?k:e))},simplify:function(b,e){if(!b)return null;e=e?e:N.VECTOR_SIMPLIFICATION;for(var k=[],l=null,p=b.length;p--;){var u=b[p];if(include=!l||p===0||t(u.x-l.x)+t(u.y-l.y)>e){k.unshift(u);l=u}}return k},theta:function(b,e,k){k=Va((b.y-e.y)/k);return b.x>e.x?k:Math.PI-k},toString:function(b){return b.x+", "+b.y}}}(),D=function(){return{dotProduct:function(c,
...
qa){typeof FlashCanvas!="undefined"&&FlashCanvas.initElement(qa);qa.width=Ia;qa.height=Ha;qa.style.width=Ia+"px";qa.style.height=Ha+"px";qb=false}if(Ra||Ja)fa();f=Ma();if(oa){COG.endTweens(function(Ya){return Ya.cancelOnInteract});ja=false;if(Ga!==0){clearTimeout(Ga);Ga=0}}for(oa=Na;oa--;){if(ia[oa].animated)$a|=yb;ia[oa].cycle(B,f,$a,Ba);R=ia[oa].shouldDraw($a,f,Ba)||R}Ba=Ba||($a&yb)!==0;if(R=R||Ba||la!==1&&la!==zb){W($,f,Ba,B);zb=la;Oa=false}if(!R&&i===0&&!Gb){if(!ja&&Ga===0)Ga=setTimeout(v,nb);
z.trigger("complete")}B-aa>sb&&Sa();i=0;COG.trace("Completed draw cycle",B)}function Q(B){Oa=B?true:false;i+=1;if(!m){m=COG.Loopage.join({execute:Y,frequency:Hb});m.bind("complete",function(){m=null})}}function Ma(){return na.init(xa,ya,xa+Ia,ya+Ha)}function Sa(){aa=(new Date).getTime();ka("refresh",Fa);Q()}function eb(B,z,R,$,oa){if(R&&!ob){ob=COG.tweenValue(la,B,R,null,oa);ob.requestUpdates(function(Ba,Ya){la=Ba;if(Ya){var Za=Ca(pa(la)/Math.LN2);la=T(2,Za);$&&$();ob=null}k(z);e(Ya)})}else{la=B;
...
zoomDuration:300},c);var ia=[],Na=0,qa=document.getElementById(c.container),ga=null,tb=typeof window.attachEvent!="undefined",nb=c.idleDelay,sb=c.minRefresh,xa=0,ya=0,Ra=null,Ja=null,gb=false,hb=false,rb=c.clipping,f=null,m=null,j,E=c.guides,a=1,i=0,h=0,C=0,K=null,ba=null,ja=false,va=false,Ga=0,S=0,P=null,X=null,aa=0,ra=0,ua=null,Oa=false,Qa=0,mb=null,la=1,ob=null,zb=0,qb=false,ab=null,Ha,Ia,Gb=typeof FlashCanvas!=="undefined",Hb=~~(1E3/c.fps),kb,lb,pb=H("ACTIVE"),jb=H("PAN"),xb=H("ZOOM"),yb=H("ANIMATING"),
$a=pb,Fa={id:c.id,deviceScaling:a,fastDraw:c.fastDraw||Z().requireFastDraw,getDimensions:function(){return Wa.init(Ia,Ha)},getLayer:function(B){for(var z=0;z<Na;z++)if(ia[z].getId()==B)return ia[z];return null},setLayer:function(B,z){for(var R=0;R<Na;R++)if(ia[R].getId()===B){ia.splice(R,1);break}z&&y(B,z);Q();return z},eachLayer:function(B){for(var z=Na;z--;)B(ia[z])},invalidate:Q,refresh:Sa,resetScale:function(){la=1},resize:function(B,z){if(qa){c.autoSize=false;if(Ia!==B||Ha!==z)A(B,z)}},scale:eb,
...
Fa.bind("invalidate",function(B,z){Q(z)});Fa.bind("resync",function(){X=P=null});COG.configurable(Fa,["inertia","container","rotation","tapExtent","scalable","pannable"],COG.paramTweaker(c,null,{container:function(B,z){qa=document.getElementById(z);A()},rotation:function(B,z){ra=z}}),true);A();if(c.autoSize)tb?window.attachEvent("onresize",u):window.addEventListener("resize",u,false);return Fa},ib=function(c){c=COG.extend({id:"",zindex:0,supportFastDraw:false,animated:false,validStates:H("ACTIVE",
...
i)?h:ka.calcDistance(ba,a[ja]);if(a[ja]&&va>=h){K.unshift(a[ja]);ba=a[ja]}}COG.info("generalized "+C+" positions into "+K.length+" positions");return K},inArray:function(a,i){for(var h=ka.equal,C=i.length;C--;)if(h(a,i[C]))return true;return false},inBounds:function(a,i){var h=!(ka.empty(a)||ka.empty(i));return h=(h=h&&a.lat>=i.min.lat&&a.lat<=i.max.lat)&&a.lon>=i.min.lon&&a.lon<=i.max.lon},init:j,offset:function(a,i,h){var C=a.lat*y;a=a.lon*y;i=C+i/6371;h=Va(wa(h/6371)/Ka(C));h=a+h;i=(i+d)%Math.PI-
...
if(!Sa){var E=[];for(var a in eb)E.push(a);Sa=RegExp("(\\s)("+E.join("|")+")(\\s|$)","i")}Sa.lastIndex=-1;if(E=Sa.exec(j))j=j.replace(Sa,"$1"+eb[E[2]]);return j},toString:function(j){return j.streetDetails+" "+j.location}}}(),qa=Ta.GeoXY=function(){function f(j,E){if(j.length){for(var a,i,h,C,K=j.length;K--;){f(j[K],E);a=typeof a==="undefined"||j.x<a?j.x:a;i=typeof i==="undefined"||j.y<i?j.y:i;h=typeof h==="undefined"||j.x>h?j.x:h;C=typeof C==="undefined"||j.y>C?j.y:C}return na.init(a,i,C,C)}else if(j.mercXY){a=
j.mercXY;j.x=(a.x+Math.PI)/E|0;j.y=(Math.PI-a.y)/E|0;j.rpp=E}else COG.warn("Attempted to sync an XY composite, not a GeoXY");return j}function m(j,E,a){j.pos=E;j.mercXY=ka.toMercatorPixels(E);(a=typeof a!=="undefined"?a:j.rpp)&&f(j,a)}return{init:function(j,E){var a=N.init();m(a,j,E);return a},sync:f,toPos:function(j,E){E=E?E:self.rpp;return ka.fromMercatorPixels(j.x*E-Math.PI,Math.PI-j.y*E)},updatePos:m}}();Ta.GeoVector=function(f){COG.warn("The T5.Geo.GeoVector class has been deprecated, please use T5.GeoXY.init instead");
return qa.init(f)};var ga={},tb=function(){return{bestResults:function(f,m){m||(m=20);for(var j=f.length>0?f[0]:null,E=[],a=0;a<f.length;a++)if(j&&f[a]&&j.matchWeight-f[a].matchWeight<=m)E.push(f[a]);else break;return E}}}(),nb=function(f){f=COG.extend({id:null,caption:"",resultType:"",data:null,pos:null,matchWeight:0},f);return COG.extend(f,{toString:function(){return f.caption+(f.matchWeight?" ("+f.matchWeight+")":"")}})},sb=function(){function f(a,i){var h=a.getDimensions();h=new rb({data:i,width:h.width,
height:h.height});a.setLayer("route",h)}function m(a){for(var i=j.Unknown,h=E,C=0;C<h.length;C++){h[C].regex.lastIndex=-1;var K=h[C].regex.exec(a);if(K){i=h[C].customCheck?h[C].customCheck(a,K):h[C].turnType;break}}return i}var j={Unknown:"turn-unknown",Start:"turn-none-start",Continue:"turn-none",Arrive:"turn-none-arrive",TurnLeft:"turn-left",TurnLeftSlight:"turn-left-slight",TurnLeftSharp:"turn-left-sharp",TurnRight:"turn-right",TurnRightSlight:"turn-right-slight",TurnRightSharp:"turn-right-sharp",
Merge:"merge",UTurnLeft:"uturn-left",UTurnRight:"uturn-right",EnterRoundabout:"roundabout-enter",Ramp:"ramp",RampExit:"ramp-exit"},E=function(){var a=[];a.push({regex:/continue/i,turnType:j.Continue});a.push({regex:/(take|bear|turn)(.*?)left/i,customCheck:function(i,h){return/bear/i.test(h[1])?j.TurnLeftSlight:j.TurnLeft}});a.push({regex:/(take|bear|turn)(.*?)right/i,customCheck:function(i,h){return/bear/i.test(h[1])?j.TurnRightSlight:j.TurnRight}});a.push({regex:/enter\s(roundabout|rotaty)/i,turnType:j.EnterRoundabout});
a.push({regex:/take.*?ramp/i,turnType:j.Ramp});a.push({regex:/take.*?exit/i,turnType:j.RampExit});a.push({regex:/make(.*?)u\-turn/i,customCheck:function(i,h){return/right/i.test(h[1])?j.UTurnRight:j.UTurnLeft}});a.push({regex:/proceed/i,turnType:j.Start});a.push({regex:/arrive/i,turnType:j.Arrive});a.push({regex:/fell\sthrough/i,turnType:j.Merge});return a}();return{calculate:function(a){a=COG.extend({engineId:"",waypoints:[],map:null,error:null,autoFit:true,success:null,generalize:false},a);var i=
c("route");i&&i.route(a,function(h){if(a.generalize)h.geometry=ka.generalize(h.geometry,h.getInstructionPositions());if(a.map){f(a.map,h);a.autoFit&&a.map.gotoBounds(h.boundingBox)}a.success&&a.success(h)})},createMapOverlay:f,parseTurnType:m,TurnType:j,Instruction:function(a){var i=a=COG.extend({position:null,description:"",distance:0,distanceTotal:0,time:0,timeTotal:0,turnType:null},a),h=a.description;h=h.replace(/(\w)(\/)(\w)/g,"$1 $2 $3");i.description=h;if(!a.turnType)a.turnType=m(a.description);
return a},RouteData:function(a){a=COG.extend({geometry:[],instructions:[],boundingBox:null},a);if(!a.boundingBox)a.boundingBox=ia.forPositions(a.geometry);return COG.extend({getInstructionPositions:function(){for(var i=[],h=0;h<a.instructions.length;h++)a.instructions[h].position&&i.push(a.instructions[h].position);return i}},a)}}}(),xa="featurecollection",ya="feature",Ra={async:false},Ja={processor:null,group:"shapes",layerClass:Db},gb={point:COG.extend({},Ja,{processor:function(f,m,j,E){m=q([m.coordinates],
Ra);if(m.length>0)if(j=E.marker(m[0],j)){f.add(j);return m.length}},group:"markers",layerClass:ub}),linestring:COG.extend({},Ja,{processor:function(f,m,j,E){m=q(m&&m.coordinates?m.coordinates:[]);return p(f,m,j,E)}}),multilinestring:COG.extend({},Ja,{processor:function(f,m,j,E){m=m&&m.coordinates?m.coordinates:[];for(var a=0,i=m.length;i--;)a+=p(f,m[i],j,E);return a}}),polygon:COG.extend({},Ja,{processor:function(f,m,j,E){m=m&&m.coordinates?m.coordinates:[];if(m.length>0)return u(f,m[0],j,E);return 0}}),
multipolygon:COG.extend({},Ja,{processor:function(f,m,j,E){m=m&&m.coordinates?m.coordinates:[];for(var a=0,i=0;i<m.length;i++)a+=u(f,m[i][0],j,E);return a}})},hb=function(f,m,j,E){function a(X,aa){var ra=X.processor,ua=K+X.group,Oa=COG.extend({},X,j,{properties:aa.properties});if(ra){var Qa=S[ua];if(!Qa){Qa=new X.layerClass({id:ua});S[ua]=Qa}globalLayers=S;return ra(Qa,aa.data,Oa,E)}return 0}function i(X,aa){var ra=X&&X.type?X.type.toLowerCase():null;return ra&&ra===ya?i(X.geometry,X.properties):
{type:ra,isCollection:ra?ra===xa:false,definition:gb[ra],data:X,properties:aa?aa:X.properties}}j=COG.extend({vectorsPerCycle:Ua.VECTORIZE_PER_CYCLE,rowPreParse:null,simplify:false,layerPrefix:"geojson-"},j);E=COG.extend({marker:function(X){return new vb({xy:X})},line:function(X,aa){return new wb(X,aa)},poly:function(X,aa){return new wb(X,COG.extend({fill:true},aa))}},E);var h=j.vectorsPerCycle,C=j.rowPreParse,K=j.layerPrefix,ba=0,ja=0,va=null,Ga=0,S={},P;if(!f)return null;if(typeof f.length==="undefined")f=
[f];ja=f.length;P=COG.Loopage.join({frequency:10,execute:function(X,aa){var ra=0,ua=ba;if(!va){for(;ua<ja;ua++){var Oa=i(C?C(f[ua]):f[ua]),Qa=null;if(Oa.isCollection){Ga+=1;va=s(Oa.data.features,function(mb){va=null;for(var la in mb)S[la]=mb[la]},{layerPrefix:K+Ga+"-"});Qa+=1}else if(Oa.definition)Qa=a(Oa.definition,Oa);ra+=Qa?Qa:1;if(ra>=h)break}ba=ua+1;!va&&ba>=ja&&aa.trigger("complete")}}});P.bind("complete",function(){m&&m(S)});return P};Ta.GeoJSON={parse:s};Ta.Map=function(f){function m(P){try{var X=
Ua.Position.init(P.coords.latitude,P.coords.longitude),aa=P.coords.accuracy/1E3;S.trigger("locationUpdate",P,aa);if(ja){if(!K){K=new Ua.UI.LocationOverlay({pos:X,accuracy:aa});K.update(S.getTileLayer());S.setLayer("location",K)}var ra=Ua.BoundingBox.createBoundsFromCenter(X,Math.max(aa,1));S.gotoBounds(ra)}else{K.pos=X;K.accuracy=aa;K.update(S.getTileLayer());a(X,null,COG.easing("sine.out"))}ja=false}catch(ua){COG.exception(ua)}}function j(P){COG.info("caught location tracking error:",P)}function E(P,
X,aa){S.setZoomLevel(X);COG.endTweens();a(P,aa)}function a(P,X,aa,ra){P=qa.init(P,Ua.radsPerPixel(S.getZoomLevel()));var ua=S.getDimensions();S.updateOffset(P.x-(ua.width>>1),P.y-(ua.height>>1),aa,ra,X);S.trigger("wake");S.trigger("boundsChange",S.getBoundingBox());X&&typeof aa==="undefined"&&X(S)}f=COG.extend({tapExtent:10,crosshair:false,zoomLevel:0,boundsChangeThreshold:30},f);var i={NONE:0,SINGLE:1,WATCH:2},h=N.init(),C=i.NONE,K=null,ba=0,ja=true,va=0,Ga=f.tapExtent;f.adjustScaleFactor=function(P){return Math.pow(2,
(P<1?Math.floor:Math.ceil)(Math.log(P)))};var S=COG.extend(new Ab(f),{getBoundingBox:function(){var P=S.getViewRect();return Ua.BoundingBox.init(qa.toPos(N.init(P.x1,P.y2),va),qa.toPos(N.init(P.x2,P.y1),va))},getCenterPosition:function(){var P=S.getViewRect();if(P){P=N.init(P.x1+(P.width>>1),P.y1+(P.height>>1));return qa.toPos(P,va)}return null},gotoBounds:function(P,X){var aa=Ua.BoundingBox.getZoomLevel(P,S.getDimensions());E(Ua.BoundingBox.getCenter(P),aa,X)},gotoPosition:E,panToPosition:a,syncXY:function(P){return qa.sync(P,
va)},locate:function(){S.trackStart(i.SINGLE);setTimeout(S.trackCancel,1E4)},trackStart:function(P){if(navigator.geolocation&&!ba){C=P?P:i.WATCH;ja=true;ba=navigator.geolocation.watchPosition(m,j,{enableHighAccuracy:true,timeout:1E4,maximumAge:5E3})}},trackCancel:function(){ba&&navigator.geolocation&&navigator.geolocation.clearWatch(ba);S.removeLayer("location");K=null;C=i.NONE;ba=0},animateRoute:function(P,X,aa,ra){var ua=S.getLayer("route");if(ua)(P=ua.getAnimation(P,X,aa,ra))&&P.addToView(S)}});
S.bind("pan",function(){C===i.SINGLE&&S.trackCancel()});S.bind("tap",function(P,X,aa,ra){var ua=Ua.radsPerPixel(S.getZoomLevel());P=qa.toPos(ra,ua);var Oa=qa.toPos(N.offset(ra,-Ga,Ga),ua);ra=qa.toPos(N.offset(ra,Ga,-Ga),ua);S.trigger("geotap",X,aa,P,ia.init(Oa,ra))});S.bind("refresh",function(){if(N.absSize(N.diff(h,S.getOffset()))>f.boundsChangeThreshold){h=N.copy(S.getOffset());S.trigger("boundsChange",S.getBoundingBox())}});G(S,f);S.bind("zoomLevelChange",function(P,X){var aa;va=Ua.radsPerPixel(X);
aa=I/va|0;S.setMaxOffset(aa,aa,true,false);Ea.cancelLoad();S.resetScale();S.triggerAll("resync",S)});return S};Ta.MapTileGenerator=function(f){f=COG.extend({relative:true},f);var m=0,j=null;return COG.extend(new Eb(f),{minLevel:2,maxLevel:20,getTileCreatorArgs:function(E){j=E.getCenterPosition();m=E.getZoomLevel();return{zoomLevel:m,position:j}},requireRefresh:function(E,a){return!j||m!==a.getZoomLevel()}})};Ta.GeoPoly=function(f,m){m=COG.extend({autoParse:true},m);for(var j=Array(f.length),E=m.autoParse,
a=T5.Geo.Position.parse,i=f.length;i--;)j[i]=T5.GeoXY.init(E?a(f[i]):f[i]);return new T5.Poly(j,m)};var rb=Ta.RouteOverlay=function(f){f=COG.extend({data:null,pixelGeneralization:8,partialDraw:false,strokeStyle:"rgba(0, 51, 119, 0.9)",waypointFillStyle:"#FFFFFF",lineWidth:4,zindex:50},f);var m=[],j=[],E=new T5.PathLayer(f);(function(){if(f.data&&f.data.instructions){for(var a=f.data.instructions,i=Array(a.length),h=a.length;h--;)i[h]=a[h].position;ka.vectorize(i).bind("complete",function(C,K){j=K})}f.data&&
f.data.geometry&&ka.vectorize(f.data.geometry).bind("complete",function(C,K){m=K;E.updateCoordinates(m,j,true)})})();return E};Ta.LocationOverlay=function(f){f=COG.extend({pos:null,accuracy:null,zindex:90},f);var m=new Image,j=T5.XY.init(),E=T5.XY.init(),a=null;m.src="data:image/png;base64,%3D%3D";
...
setTimeout(function(){i&&m(i,C)},f.searchTimeout)}}},f))},Routing:sb}}();return Ta}();
