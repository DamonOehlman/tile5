...
k(z);e(true)}return Fa}function ka(){for(var B=Fa.trigger.apply(null,arguments).cancel,z=ia.length;z--;)B=ia[z].trigger.apply(null,arguments).cancel||B;return!B}c=COG.extend({id:COG.objId("view"),container:"",fastDraw:false,inertia:true,idleDelay:100,minRefresh:1E3,pannable:true,clipping:false,scalable:true,interactive:true,panAnimationEasing:COG.easing("sine.out"),panAnimationDuration:750,pinchZoomAnimateTrigger:400,adjustScaleFactor:null,autoSize:true,tapExtent:10,mask:true,guides:false,fps:25,
zoomAnimation:COG.easing("quad.out"),zoomDuration:300},c);var ia=[],Na=0,ta=document.getElementById(c.container),ga=null,tb=typeof window.attachEvent!="undefined",nb=c.idleDelay,sb=c.minRefresh,xa=0,ya=0,Ra=null,Ja=null,gb=false,hb=false,rb=c.clipping,f=null,m=null,j,E=c.guides,a=1,i=0,h=0,C=0,K=null,ba=null,ja=false,va=false,Ga=0,S=0,P=null,X=null,aa=0,qa=0,ua=null,Oa=false,Qa=0,mb=null,la=1,ob=null,zb=0,qb=false,ab=null,Ha,Ia,Gb=typeof FlashCanvas!=="undefined",Hb=~~(1E3/c.fps),kb,lb,pb=H("ACTIVE"),
jb=H("PAN"),xb=H("ZOOM"),yb=H("ANIMATING"),$a=pb,Fa={id:c.id,deviceScaling:a,fastDraw:c.fastDraw||Z().requireFastDraw,getDimensions:function(){return Va.init(Ia,Ha)},getLayer:function(B){for(var z=0;z<Na;z++)if(ia[z].getId()==B)return ia[z];return null},setLayer:function(B,z){for(var R=0;R<Na;R++)if(ia[R].getId()===B){ia.splice(R,1);break}z&&y(B,z);Q();return z},eachLayer:function(B){for(var z=Na;z--;)B(ia[z])},invalidate:Q,refresh:Sa,resetScale:function(){la=1},resize:function(B,z){if(ta){c.autoSize=
false;if(Ia!==B||Ha!==z)A(B,z)}},scale:eb,triggerAll:ka,removeLayer:function(B){a:{for(var z=Na;z--;)if(ia[z].getId()==B){B=z;break a}B=-1}if(B>=0&&B<Na){Fa.trigger("layerRemoved",ia[B]);ia.splice(B,1);Q(true)}Na=ia.length},stateOverride:function(B){ua=B},getOffset:function(){return N.init(xa,ya)},setMaxOffset:function(B,z,R,$){Ra=B;Ja=z;gb=typeof R!="undefined"?R:false;hb=typeof $!="undefined"?$:false},getViewRect:Ma,updateOffset:s,pan:function(B,z,R,$,oa){s(xa+B,ya+z,R,$,oa)}};a=Z().getScaling();
Fa.markers=y("markers",new ub);COG.observable(Fa);Fa.bind("invalidate",function(B,z){Q(z)});Fa.bind("resync",function(){X=P=null});COG.configurable(Fa,["inertia","container","rotation","tapExtent","scalable","pannable"],COG.paramTweaker(c,null,{container:function(B,z){ta=document.getElementById(z);A()},rotation:function(B,z){qa=z}}),true);A();if(c.autoSize)tb?window.attachEvent("onresize",u):window.addEventListener("resize",u,false);return Fa},ib=function(c){c=COG.extend({id:"",zindex:0,supportFastDraw:false,
animated:false,validStates:H("ACTIVE","ANIMATING","PAN","ZOOM"),style:null,minXY:null,maxXY:null},c);var o=null,g=false,b=false,e=c.supportFastDraw,k=c.id,l=H("ACTIVE"),p=c.validStates,u=0,q=0,s=COG.extend({addToView:function(A){A.setLayer(k,s)},shouldDraw:function(A,y,O){return(b||O||u!==y.x1||q!==y.y1)&&(A&p)!==0&&(g?e:true)},clip:null,cycle:function(){},draw:function(){},remove:function(){s.trigger("remove",s)},changed:function(A){b=true;s.trigger("changed",s);o&&o.trigger("invalidate",A)},getId:function(){return k},
setId:function(A){k=A},getParent:function(){return o},setParent:function(A){g=(o=A)?o.fastDraw&&displayState!==l:false;s.trigger("parentChange",o)}},c);COG.observable(s);s.bind("drawComplete",function(A,y){b=false;u=y.x1;q=y.y1});s.bind("resync",function(A,y){if(y.syncXY){s.minXY&&y.syncXY(s.minXY);s.maxXY&&y.syncXY(s.maxXY)}});return s},vb=function(c){function o(u,q,s,A){b=u;e=q;k=s;l=A}c=COG.extend({xy:N.init(),offset:true,tweenIn:null,animationSpeed:null,isNew:true},c);var g=false,b=0,e=0,k=0,
l=0,p=COG.extend(c,{isAnimating:function(){return g},draw:function(u,q,s,A,y){if(p.isNew&&c.tweenIn){var O=p.xy.y;p.xy.y=q.y1-20;g=true;COG.tween(p.xy,"y",O,c.tweenIn,function(){p.xy.y=O>>0;g=false},c.animationSpeed?c.animationSpeed:250+Math.random()*500)}p.drawMarker(u,q,p.xy.x,p.xy.y,s,A,y);p.isNew=false},drawMarker:function(u,q,s,A){u.beginPath();u.arc(s,A,4,0,Math.PI*2,false);u.fill();o(s-4,A-4,8,8)},hitTest:function(u,q){return u>=b&&u<=b+k&&q>=e&&q<=e+l},updateBounds:o});COG.observable(p);return p},
ub=function(c){function o(){e.changed(true);e.trigger("markerUpdate",b)}function g(){var k=e.getParent();if(k&&k.syncXY)for(var l=b.length;l--;)k.syncXY([b[l].xy])}c=COG.extend({zindex:100,style:"basic"},c);var b=[],e=COG.extend(new ib(c),{draw:function(k,l,p,u){for(var q=b.length;q--;)b[q].draw(k,l,p,e,u)},add:function(k){if(k&&typeof k.length!=="undefined")for(var l=k.length;l--;){if(k[l])b[b.length]=k[l]}else if(k)b[b.length]=k;o()},clear:function(k){if(k)for(var l=0;l<b.length;)if(k(b[l]))b.splice(l,
1);else l+=1;else b=[];o()},each:function(k){if(k)for(var l=b.length;l--;)k(b[l])},find:function(k){for(var l=[],p=b.length;p--;)if(!k||k(b[p]))l[l.length]=b[p];return l}});e.bind("tap",function(k,l,p,u){var q=[],s=u.x;u=u.y;for(var A=b.length;A--;)if(b[A].hitTest(s,u))q[q.length]=b[A];if(q.length>0)k.cancel=e.trigger("markerTap",l,p,q).cancel});e.bind("parentChange",g);e.bind("resync",g);e.bind("changed",g);return e},Bb=function(c){function o(p,u,q){p.fillStyle="#FFFFFF";p.strokeStyle="#222222";
p.beginPath();p.arc(q.x,q.y,4,0,Math.PI*2,false);p.stroke();p.fill()}c=COG.extend({path:[],id:COG.objId("pathAni"),easing:COG.easing("sine.inout"),validStates:H("ACTIVE","PAN","ZOOM"),drawIndicator:null,duration:2E3},c);var g=N.edges(c.path),b,e=null,k=0;COG.tweenValue(0,g.total,c.easing,function(){l.remove()},c.duration).requestUpdates(function(p,u){k=p;u&&l.remove()});var l=COG.extend(new ib(c),{cycle:function(){for(var p=0;p<g.accrued.length&&g.accrued[p]<k;)p++;e=null;if(p<c.path.length-1){var u=
k-(p>0?g.accrued[p-1]:0),q=c.path[p];b=N.theta(q,c.path[p+1],g.edges[p]);e=N.extendBy(q,b,u)}return e},draw:function(p,u){if(e)(c.drawIndicator?c.drawIndicator:o)(p,u,N.init(e.x,e.y),b)}});return l},Cb=function(c){c=COG.extend({style:null,properties:{}},c);return COG.extend(c,{rect:null,draw:function(){},resync:function(){}})},wb=function(c,o){o=COG.extend({fill:false,simplify:false},o);var g=false,b=o.fill,e=o.simplify,k=H("ZOOM"),l=[],p=COG.extend(new Cb(o),{draw:function(u,q,s,A,y,O){if(g){q=true;
s=(O&k)!==0;u.beginPath();for(O=l.length;O--;){s=l[O].x;A=l[O].y;if(q){u.moveTo(s,A);q=false}else u.lineTo(s,A);s=true}if(s){b&&u.fill();u.stroke()}}},resync:function(u){p.xy=u.syncXY(c);l=N.floor(e?N.simplify(c):c)}});g=c&&c.length>=2;return p},Db=function(c){function o(k){for(var l=b.length;l--;)b[l].resync(k);b.sort(function(p,u){var q=u.xy.y-p.xy.y;if(q===0)q=u.xy.x-p.xy.y;return q});e.changed()}function g(k,l){l.syncXY&&o(l)}c=COG.extend({zindex:80},c);var b=[],e=COG.extend(new ib(c),{add:function(k){b[b.length]=
k},each:function(k){for(var l=b.length;l--;)k(b[l])},draw:function(k,l,p){var u=l.x1,q=l.y1,s=l.width;l=l.height;for(var A=b.length;A--;){var y=b[A].style;y=y?bb.apply(k,y):null;b[A].draw(k,u,q,s,l,p);y&&bb.apply(k,y)}}});e.bind("parentChange",g);e.bind("resync",g);return e},Ib=function(){return{tileSize:256,init:function(c,o,g,b,e){return COG.extend({x:c,y:o,width:g,height:b},e)}}}(),Eb=function(c){function o(O,fa,v,W){function Y(){if(y.initTileCreator){l=true;y.initTileCreator(O,fa,v,W)}}y.prepTileCreator?
y.prepTileCreator(O,fa,v,Y):Y()}function g(O,fa){for(var v=~~((O.x1-e.x)/p),W=~~((O.y1-e.y)/u),Y=[],Q=-k;Q<s;Q++)for(var Ma=-k;Ma<A;Ma++){var Sa=q(v+Q,W+Ma);if(Sa)Y[Y.length]=Sa}fa&&fa(Y);b=na.copy(O)}c=COG.extend({tileWidth:256,tileHeight:256,relative:false,padding:2},c);var b=null,e=N.init(),k=c.padding,l=false,p=c.tileWidth,u=c.tileHeight,q=null,s=0,A=0,y={getTileCreatorArgs:null,initTileCreator:null,prepTileCreator:null,requireRefresh:function(){return false},reset:function(){q=null;l=false;b=
null},run:function(O,fa,v){var W=!b||Math.abs(O.x1-b.x1)>p||Math.abs(O.y1-b.y1)>u,Y=W?y.requireRefresh(O,fa):false;if(W){if(!q&&!l||Y){e=c.relative?N.init(O.x1,O.y1):N.init();s=Math.ceil(O.width/p)+k;A=Math.ceil(O.height/u)+k;o(p,u,y.getTileCreatorArgs?y.getTileCreatorArgs(fa):{},function(Q){q=Q;l=false;g(O,v)})}q&&g(O,v)}}};COG.observable(y);y.bind("update",function(){COG.info("captured generator update");b=null});return y},Ya={ex:COG.extend,ticks:ca,getConfig:Z,userMessage:function(c,o,g){Ya.trigger("userMessage",
c,o,g)},zoomable:G,XY:N,XYRect:na,Dimensions:Va,Vector:D,D:Va,Images:Ea,Generator:Wa,tween:COG.tween,tweenValue:COG.tweenValue,easing:COG.easing,Tween:COG.Tween,Style:bb,viewState:H,View:Ab,ViewLayer:ib,ImageLayer:function(c,o){function g(q,s,A){for(s=k.length;s--;){var y=k[s],O=y.x,fa=y.y,v=na.init(y.x,y.y,y.x+y.width,y.y+y.height);if(A&&na.intersect(q,v)){v=Ea.get(y.url,function(){u.changed()},p);A(v,O,fa,y.width,y.height)}}}function b(q){e&&e.run(q,u.getParent(),function(s){k=[].concat(s);u.changed()})}
o=COG.extend({imageLoadArgs:{}},o);var e=c?Wa.init(c,o):null,k=[],l=na.init(),p=o.imageLoadArgs,u=COG.extend(new ib(o),{changeGenerator:function(q,s){e=Wa.init(q,COG.extend({},o,s));k=null;b(l)},clip:function(q,s,A){g(s,A,function(y,O,fa,v,W){y&&q.rect(O,fa,v,W)})},cycle:function(q,s){b(s)},draw:function(q,s,A){g(s,A,function(y,O,fa,v,W){u.drawImage(q,y,O,fa,v,W,s,A)});l=na.copy(s)},drawImage:function(q,s,A,y){s&&q.drawImage(s,A,y,s.width,s.height)}});u.bind("refresh",function(){b(l)});u.bind("tap",
function(q,s,A,y){q=[];var O=y.x,fa=y.y,v,W;if(k)for(var Y=k.length;Y--;){v=k[Y];if(W=O>=v.x&&O<=v.x+v.width&&fa>=v.y&&fa<=v.y+v.height)q[q.length]=v}q.length>0&&u.trigger("tapImage",q,s,A,y)});return u},Marker:vb,ImageMarker:function(c){c=COG.extend({image:null,imageUrl:null,animatingImage:null,animatingImageUrl:null,imageAnchor:null,rotation:0,scale:1,opacity:1},c);var o=c.imageAnchor?T5.XY.invert(c.imageAnchor):null,g=COG.extend(new vb(c),{drawMarker:function(b,e,k,l){if((e=g.isAnimating()&&g.animatingImage?
g.animatingImage:g.image)&&e.width>0){o||(o=N.init(-e.width>>1,-e.height>>1));var p=g.scale,u=k+~~(o.x*p),q=l+~~(o.y*p),s=~~(e.width*p),A=~~(e.height*p);g.updateBounds(u,q,s,s);if(g.rotation||g.opacity!==1){b.save();try{b.globalAlpha=g.opacity;b.translate(k,l);b.rotate(g.rotation);b.drawImage(e,o.x*p,o.y*p,s,A)}finally{b.restore()}}else b.drawImage(e,u,q,s,A)}}});if(!g.image)g.image=Ea.get(c.imageUrl,function(b){g.image=b});if(!g.animatingImage)g.animatingImage=Ea.get(c.animatingImageUrl,function(b){g.animatingImage=
