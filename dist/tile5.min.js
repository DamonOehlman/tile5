...
e={avgImageSize:25,loadTimeout:10,Cache:function(){return{read:function(){return null},write:function(){},getUrlCacheKey:function(i,d){for(var f=(i?i.replace(/^.*\?/,""):"").split("&"),g=i?i.replace(/\?.*$/,"?"):"",h=0;h<f.length;h++){var c=f[h].split("=");if(!d||!d.test(c[0]))g+=f[h]+"&"}return g.replace(/\W/g,"")},isValidCacheKey:function(i){GRUNT.Log.info("cache key = "+i);return false}}}(),getPath:function(i){return/^(https?|\/)/.test(i)?i:s+i},setBasePath:function(i){s=i},getImage:function(i){return a.getImage(i)},
loadImage:function(i,d){a.loadImage(i,d)},resetImageLoadQueue:function(){a.resetLoadingQueue()},getStats:function(){return{imageLoadingCount:a.loadingImages.length,queuedImageCount:a.queuedImages.length,imageCacheFullness:a.getCacheFullness()}},loadResource:function(i){i=GRUNT.extend({filename:"",cacheable:true,dataType:null,callback:null},i);var d=function(f){i.callback&&GRUNT.Log.watch("CALLING RESOURCE CALLBACK",function(){i.callback(f)})};i.cacheable&&q[i.filename]?d(q[i.filename]):GRUNT.XHR.ajax({url:e.getPath(i.filename),
dataType:i.dataType,success:function(f){if(i.cacheable)q[i.filename]=f;d(f)},error:function(f,g,h){GRUNT.Log.error("error loading resource ["+i.filename+"], error = "+h)}})},loadSnippet:function(i,d){/\.\w+$/.test(i)||(i+=".html");e.loadResource({filename:"snippets/"+i,callback:d,dataType:"html"})}};return e}();
...
TILE5.Animation=function(){function s(){if(e===0)e=setInterval(function(){var d;d=TILE5.Clock.getTime();if(!a){a=true;try{for(var f=0;f<q.length;)if(q[f].isComplete()){q[f].triggerComplete(false);q.splice(f,1);GRUNT.WaterCooler.say("animation.complete")}else{q[f].update(d);f++}}finally{a=false}}d=q.length;if(d===0){clearInterval(e);e=0}},20)}var q=[],a=false,e=0,i={DURATION:2E3,tweenValue:function(d,f,g,h,c){d=new i.Tween({startValue:d,endValue:f,tweenFn:g,complete:h,duration:c});q.push(d);return d},
tween:function(d,f,g,h,c,b){d=new i.Tween({target:d,property:f,endValue:g,tweenFn:h,duration:b,complete:c});q.push(d);return d},tweenVector:function(d,f,g,h,c,b){var j=[];if(d){var k=d.x==f,n=d.y==g;k||j.push(i.tween(d,"x",f,h,function(){(k=true)&&n&&c()},b));n||j.push(i.tween(d,"y",g,h,function(){n=true;k&&n&&c()},b))}return j},cancel:function(d){if(!a){a=true;try{for(var f=0;f<q.length;)if(!d||d(q[f])){GRUNT.Log.info("CANCELLING ANIMATION");q[f].triggerComplete(true);q.splice(f,1)}else f++}finally{a=
false}}},isTweening:function(){return q.length>0},Tween:function(d){d=GRUNT.extend({target:null,property:null,startValue:0,endValue:null,duration:i.DURATION,tweenFn:i.DEFAULT,complete:null,cancelOnInteract:false},d);var f=TILE5.Clock.getTime(),g=[],h=false,c=0,b=0,j={cancelOnInteract:d.cancelOnInteract,isComplete:function(){return h},triggerComplete:function(k){d.complete&&d.complete(k)},update:function(k){try{var n=d.tweenFn(k-f,c,b,d.duration);if(d.target)d.target[d.property]=n;for(var l=g.length;l--;)g[l](n,
...
getGridXYForPosition:function(g){g=TILE5.Geo.P.toMercatorPixels(g,a.radsPerPixel);return new TILE5.Vector(g.x-i,f.gridDimensions.height-(g.y-d))},getGuideOffset:function(g){var h=f.tileSize;return new TILE5.Vector(g.x%h,g.y%h)},pixelsToPos:function(g){return TILE5.Geo.P.fromMercatorPixels(i+g.x,d+f.gridDimensions.height-g.y,a.radsPerPixel)}});return f},RadarOverlay:function(a){a=GRUNT.extend({radarFill:"rgba(0, 221, 238, 0.1)",radarStroke:"rgba(0, 102, 136, 0.3)",zindex:100},a);return GRUNT.extend(new TILE5.Graphics.ViewLayer(a),
{draw:function(e,i,d){i=d.width>>1;d=d.height>>1;e.fillStyle=a.radarFill;e.strokeStyle=a.radarStroke;e.beginPath();e.arc(i,d,50,0,Math.PI*2,false);e.fill();e.stroke()}})},CrosshairOverlay:function(a){a=GRUNT.extend({lineWidth:1.5,strokeStyle:"rgba(0, 0, 0, 0.5)",size:15,zindex:150,scalePosition:false,validStates:TILE5.Graphics.DisplayState.ACTIVE|TILE5.Graphics.DisplayState.ANIMATING|TILE5.Graphics.DisplayState.PAN},a);var e=null,i=function(){var d=document.createElement("canvas");d.width=a.size*
2;d.height=a.size*2;var f=d.getContext("2d"),g=new TILE5.Vector(a.size,a.size),h=a.size;f.lineWidth=a.lineWidth;f.strokeStyle=a.strokeStyle;f.beginPath();f.moveTo(g.x,g.y-h);f.lineTo(g.x,g.y+h);f.moveTo(g.x-h,g.y);f.lineTo(g.x+h,g.y);f.arc(g.x,g.y,h*0.6666,0,2*Math.PI,false);f.stroke();return d}();return GRUNT.extend(new TILE5.Graphics.ViewLayer(a),{draw:function(d,f,g){e||(e=TILE5.D.getCenter(g));d.drawImage(i,e.x-a.size,e.y-a.size)}})},RouteOverlay:function(a){a=GRUNT.extend({strokeStyle:"rgba(0, 51, 119, 0.9)",
waypointFillStyle:"#FFFFFF",lineWidth:4,data:null,pixelGeneralization:8,calculationsPerCycle:250,partialDraw:false,zindex:50},a);var e=true,i=null,d=[],f=0,g=[],h=[],c=GRUNT.extend(new TILE5.Graphics.ViewLayer(a),{getAnimation:function(b,j,k,n){if(e)return null;var l="routeAnimation"+s++;h.push(l);return new TILE5.Graphics.AnimatedPathLayer({id:l,path:d,zindex:a.zindex+1,easing:b?b:TILE5.Animation.Easing.Sine.InOut,duration:j?j:5E3,drawIndicator:k,autoCenter:n?n:false})},draw:function(b,j,k,n,l){k=
0;n=a.data?a.data.geometry:null;if(e){e=false;i=null;d=[];f=0}if(n&&f<n.length){a:{l=l.getTileLayer();g=[];n=GRUNT.Log.getTraceTicks();var o,m,p,r=a.data?a.data.geometry:[],t=r.length,v=a.data?a.data.instructions:[],x=v.length,z=a.calculationsPerCycle,B=0;for(o=f;o<t;o++){m=l.getGridXYForPosition(r[o]);if(p=!i||o===0||Math.abs(m.x-i.x)+Math.abs(m.y-i.y)>a.pixelGeneralization){d.push(m);i=m}B++;if(B>=z){f=o;break a}}f=t;GRUNT.Log.trace(t+" geometry points generalized to "+d.length+" coordinates",n);
i=null;for(o=x;o--;)if(v[o].position){m=l.getGridXYForPosition(v[o].position);if(p=!i||o===0||Math.abs(m.x-i.x)+Math.abs(m.y-i.y)>a.pixelGeneralization){g.push(m);i=m}}}k++}l=d.length;if(l>0&&(!k||a.partialDraw)){b.strokeStyle=a.strokeStyle;b.lineWidth=a.lineWidth;b.beginPath();b.moveTo(d[l-1].x-j.x,d[l-1].y-j.y);for(l=l;l--;)b.lineTo(d[l].x-j.x,d[l].y-j.y);b.stroke();b.fillStyle=a.waypointFillStyle;for(l=g.length;l--;){b.beginPath();b.arc(g[l].x-j.x,g[l].y-j.y,2,0,Math.PI*2,false);b.stroke();b.fill()}}return k}});
GRUNT.WaterCooler.listen("grid.updated",function(){for(var b=h.length;b--;)GRUNT.WaterCooler.say("layer.remove",{id:h[b]});h=[];e=true;c.wakeParent()});return c},Annotation:function(a){a=GRUNT.extend({xy:null,pos:null,draw:null,tweenIn:q.AnnotationTween,animationSpeed:null},a);var e=false,i={xy:a.xy,pos:a.pos,isNew:true,isAnimating:function(){return e},draw:function(d,f,g,h){if(i.xy){if(i.isNew&&a.tweenIn){var c=i.xy.y;i.xy.y=f.y-20;e=true;TILE5.Animation.tween(i.xy,"y",c,a.tweenIn,function(){i.xy.y=
c;e=false},a.animationSpeed?a.animationSpeed:250+Math.random()*500)}if(a.draw)a.draw(d,f,new TILE5.Vector(i.xy.x-f.x,i.xy.y-f.y),g,h);else{d.beginPath();d.arc(i.xy.x-f.x,i.xy.y-f.y,4,0,Math.PI*2,false);d.fill()}i.isNew=false}}};return i},ImageAnnotation:function(a){a=GRUNT.extend({imageUrl:null,animatingImageUrl:null,imageAnchor:null},a);var e=a.imageAnchor?TILE5.V.invert(a.imageAnchor):null;a.draw=function(d,f,g,h,c){if(a.animatingImageUrl&&i.isAnimating()){TILE5.Resources.loadImage(a.imageUrl);
h=a.animatingImageUrl}else h=a.imageUrl;if(f=TILE5.Resources.getImage(h)){if(f.complete&&f.width>0){e||(e=new TILE5.Vector(-f.width>>1,-f.height>>1));g=TILE5.V.offset(g,e.x,e.y);d.drawImage(f,g.x,g.y,f.width,f.height)}}else TILE5.Resources.loadImage(h,function(){c.wakeParent()})};var i=new q.Annotation(a);return i},AnnotationsOverlay:function(a){function e(h){for(var c=a.map?a.map.getTileLayer():null,b=0;c&&b<h.length;b++)h[b].xy=c.getGridXYForPosition(h[b].pos)}a=GRUNT.extend({pois:null,map:null,
createAnnotationForPOI:null,zindex:100},a);var i=[],d=false,f=[],g=GRUNT.extend(new TILE5.Graphics.ViewLayer(a),{draw:function(h,c,b,j){h.save();try{var k;d=false;h.fillStyle="rgba(255, 0, 0, 0.75)";h.globalCompositeOperation="source-over";for(k=i.length;k--;){i[k].draw(h,c,j,g);d=d||i[k].isAnimating()}for(k=f.length;k--;){f[k].draw(h,c,j,g);d=d||f[k].isAnimating()}}finally{h.restore()}return d?1:0},add:function(h){f.push(h);e(f);g.wakeParent()},clear:function(h){f=[];e(f);if(h){i=[];e(i)}g.wakeParent()}});
GRUNT.WaterCooler.listen("geo.pois-updated",function(h){if(a.pois&&a.pois.id==h.srcID){h=h.pois;try{i=[];for(var c=0;c<h.length;c++)if(h[c].pos){var b;var j=h[c];if(j&&j.pos){var k=null;if(k=a.createAnnotationForPOI?a.createAnnotationForPOI(j):new q.Annotation({pos:j.pos})){k.isNew=j.isNew;j.isNew=false}b=k}else b=void 0;b&&i.push(b)}e(i)}catch(n){GRUNT.Log.exception(n)}g.wakeParent()}});GRUNT.WaterCooler.listen("grid.updated",function(){e(i);e(f);g.wakeParent()});return g},Tiler:function(a){a=GRUNT.extend({tapExtent:10,
provider:null,crosshair:true,copyright:undefined,zoomLevel:0,boundsChange:null,tapPOI:null,tapHandler:null,boundsChangeThreshold:30,pois:new TILE5.Geo.POIStorage,createAnnotationForPOI:null,onTilesLoaded:null},a);if(typeof a.copyright==="undefined")a.copyright=a.provider?a.provider.getCopyright():"";var e=new TILE5.Vector,i=a.copyright,d=false,f=[],g=0,h=null,c=null,b=a.zoomLevel,j=a.tapHandler;if(!a.provider)a.provider=new TILE5.Geo.MapProvider;var k=new TILE5.Tiling.Tiler(GRUNT.extend({},a,{tapHandler:function(l,
o){var m=n.getTileLayer(),p=null;if(m){p=n.viewPixToGridPix(new TILE5.Vector(o.x,o.y));var r=m.pixelsToPos(p),t=m.pixelsToPos(TILE5.V.offset(p,-a.tapExtent,a.tapExtent)),v=m.pixelsToPos(TILE5.V.offset(p,a.tapExtent,-a.tapExtent));GRUNT.Log.info("grid tapped @ "+TILE5.Geo.P.toString(m.pixelsToPos(p)));p=new TILE5.Geo.BoundingBox(t,v);f=n.pois.findByBounds(p);a.tapPOI&&a.tapPOI(f)}j&&j(l,o,r,p)},doubleTapHandler:function(l,o){n.animate(2,TILE5.D.getCenter(n.getDimensions()),new TILE5.Vector(o.x,o.y),
TILE5.Animation.Easing.Sine.Out)},onScale:function(l,o){var m=0;l=Math.sqrt(l);if(l<1)m=-(0.5/l);else if(l>1)m=l;n.gotoPosition(n.getXYPosition(o),b+Math.floor(m))}})),n=GRUNT.extend({},k,{pois:a.pois,annotations:null,getBoundingBox:function(){var l=new TILE5.Geo.BoundingBox,o=n.getTileLayer(),m=n.getOffset(),p=n.getDimensions();if(o)l=o.getBoundingBox(m.x,m.y,p.width,p.height);return l},getCenterPosition:function(){return n.getXYPosition(TILE5.D.getCenter(n.gridDimensions))},getXYPosition:function(l){return n.getTileLayer().pixelsToPos(n.viewPixToGridPix(l))},
gotoBounds:function(l,o){var m=TILE5.Geo.B.getZoomLevel(l,n.getDimensions());n.gotoPosition(TILE5.Geo.B.getCenter(l),m,o)},gotoCurrentPosition:function(l){TILE5.Geo.Location.get({successCallback:function(o){n.clearBackground();n.gotoPosition(o,15,l)},errorCallback:function(){}})},gotoPosition:function(l,o,m){var p=b,r=(new Date).getTime();b=o?o:b;if(!b)throw"Zoom level required to goto a position.";if(a.provider)b=a.provider.checkZoomLevel(b);if(!d||b!==p){TILE5.Resources.resetImageLoadQueue();(o=
n.getTileLayer())&&o.deactivate();TILE5.Animation.cancel();d&&n.freeze();g=r;a.provider.zoomLevel=b;a.provider.getMapTiles(n,l,function(t){if(r===g){n.setTileLayer(t);c=t.getId();n.panToPosition(l,function(){n.unfreeze();m&&m()})}else GRUNT.Log.info("request time mismatch - ignoring update")});d=true}else n.panToPosition(l,m)},panToPosition:function(l,o,m){var p=n.getTileLayer();if(p){l=p.getGridXYForPosition(l);p=n.getDimensions();l.x-=p.width/2;l.y-=p.height/2;if(h)h=null;n.updateOffset(l.x,l.y,
m);GRUNT.WaterCooler.say("view.wake",{id:n.id});a.boundsChange&&a.boundsChange(n.getBoundingBox());o&&o(n)}},setZoomLevel:function(l){n.gotoPosition(n.getCenterPosition(),l)},zoomIn:function(){n.scale(2,TILE5.Animation.Easing.Sine.Out)||n.setZoomLevel(b+1)},zoomOut:function(){n.scale(0.5,TILE5.Animation.Easing.Sine.Out)||n.setZoomLevel(b-1)},animateRoute:function(l,o,m,p){var r=n.getLayer("route");if(r)(l=r.getAnimation(l,o,m,p))&&l.addToView(n)}},k);k=new TILE5.Geo.UI.AnnotationsOverlay({pois:n.pois,
map:n,createAnnotationForPOI:a.createAnnotationForPOI});n.annotations=k;n.setLayer("annotations",k);a.crosshair&&n.setLayer("crosshair",new TILE5.Geo.UI.CrosshairOverlay);i&&n.setLayer("copyright",new TILE5.Graphics.ViewLayer({zindex:999,draw:function(l,o,m){l.lineWidth=2.5;l.fillStyle="rgb(50, 50, 50)";l.strokeStyle="rgba(255, 255, 255, 0.8)";l.font="bold 10px sans";l.textBaseline="bottom";l.strokeText(i,10,m.height-10);l.fillText(i,10,m.height-10)}}));GRUNT.WaterCooler.listen("view.idle",function(l){if(l.id&&
l.id==n.id)if(TILE5.V.absSize(TILE5.V.diff(e,n.getOffset()))>a.boundsChangeThreshold&&a.boundsChange){e=n.getOffset();a.boundsChange(n.getBoundingBox())}});GRUNT.WaterCooler.listen("tiles.drawn",function(l){l.id&&l.id==c&&a.onTilesLoaded&&a.onTilesLoaded()});return n}};return q}();
